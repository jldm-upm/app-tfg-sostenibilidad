export const languages =
{
  'en:english': {
    wikidata: {
      en: 'Q1860'
    },
    name: {
      is: 'Enska',
      na: 'Dorerin Ingerand',
      uk: 'Англійська мова',
      ce: 'Ингалсан мотт',
      zu: 'IsiNgisi',
      az: 'İngilis dili',
      ps: 'انگليسي',
      lo: 'ພາສາອັງກິດ',
      et: 'Inglise keel',
      ky: 'Англис тили',
      av: 'Ингилис мацӀ',
      lv: 'Angļu valoda',
      gd: 'Beurla',
      ks: 'اَنٖگرَیزی زَبانَ',
      lb: 'Englesch',
      lt: 'Anglų kalba',
      da: 'Engelsk',
      an: 'Idioma anglés',
      iu: 'ᖃᓪᓗᓈᑎᑐᑦ',
      fa: 'زبان انگلیسی',
      si: 'ඉංග්‍රීසි භාෂාව',
      bo: 'དབྱིན་ཇིའི་སྐད།',
      tg: 'Забони англисӣ',
      rm: 'Lingua englaisa',
      mr: 'इंग्लिश भाषा',
      en: 'English',
      ln: 'Lingɛlɛ́sa',
      tl: 'Wikang Ingles',
      sm: "Fa'aperetania",
      tw: 'English',
      gv: 'Baarle',
      th: 'ภาษาอังกฤษ',
      pa: 'ਅੰਗਰੇਜ਼ੀ ਬੋਲੀ',
      ca: 'Anglès',
      so: 'Ingiriis',
      su: 'Basa Inggris',
      ty: 'Anglès',
      cr: 'ᐊᑲᔭᓯᒧᐃᐧᐣ',
      fo: 'Enskt mál',
      ur: 'انگریزی زبان',
      cu: 'Англїискъ ѩꙁꙑкъ',
      mg: 'Fiteny anglisy',
      ko: '영어',
      sn: 'English',
      li: 'Ingels',
      gu: 'અંગ્રેજી ભાષા',
      hy: 'Անգլերեն',
      nv: 'Bilagáana bizaad',
      ml: 'ഇംഗ്ലീഷ്',
      yi: 'ענגליש',
      ja: '英語',
      wa: 'Inglès',
      bh: 'अंग्रेज़ी भाषा',
      vi: 'Tiếng Anh',
      ms: 'Bahasa Inggeris',
      ia: 'Lingua anglese',
      xh: 'IsiNgesi',
      nl: 'Engels',
      sw: 'Kiingereza',
      mi: 'Reo Pākehā',
      cy: 'Saesneg',
      ka: 'ინგლისური ენა',
      cv: 'Акăлчан чĕлхи',
      ig: 'Asụsụ Inglish',
      ng: 'English',
      kg: 'Kingelezi',
      as: 'ইংৰাজী ভাষা',
      ar: 'الإنجليزية',
      sh: 'Engleski jezik',
      hi: 'अंग्रेज़ी भाषा',
      sv: 'Engelska',
      my: 'အင်္ဂလိပ်ဘာသာ',
      ba: 'Инглиз теле',
      sl: 'Angleščina',
      tt: 'Инглиз теле',
      oc: 'Anglés',
      be: 'Англійская мова',
      kk: 'Ағылшын тілі',
      ve: 'English',
      nn: 'Engelsk',
      tr: 'İngilizce',
      os: 'Англисаг æвзаг',
      ug: 'ئىنگىلىز تىلى',
      bn: 'ইংরেজি ভাষা',
      wo: 'Wu-angalteer',
      cs: 'Angličtina',
      se: 'Eaŋgalsgiella',
      af: 'Engels',
      hr: 'Engleski jezik',
      pl: 'Język angielski',
      ny: 'Chingerezi',
      sg: 'Anglëe',
      mk: 'Англиски јазик',
      sc: 'Limba inglesa',
      sq: 'Gjuha angleze',
      ss: 'SíNgísi',
      fr: 'Anglais',
      ay: 'Inlish aru',
      gl: 'Inglés',
      es: 'Inglés',
      ab: 'Англыз бызшәа',
      fy: 'Ingelsk',
      ak: 'Brɔfo',
      it: 'Inglese',
      qu: 'Inlish simi',
      ie: 'Angles',
      dv: 'އިނގިރޭސި',
      hu: 'Angol',
      bg: 'Английски език',
      sd: 'انگريزي ٻولي',
      am: 'እንግሊዝኛ',
      ht: 'Angle',
      de: 'Englisch',
      eu: 'Ingeles',
      nb: 'Engelsk',
      co: 'Lingua inglese',
      br: 'Saozneg',
      tn: 'Seisimane',
      kv: 'Англия кыв',
      sk: 'Angličtina',
      rw: 'Icyongereza',
      ro: 'Limba engleză',
      sr: 'Енглески језик',
      kw: 'Sowsnek',
      km: 'ភាសាអង់គ្លេស',
      gn: "Inglyesñe'ẽ",
      mn: 'Англи хэл',
      zh: '英语',
      ta: 'ஆங்கிலம்',
      kn: 'ಆಂಗ್ಲ',
      sa: 'आङ्ग्लभाषा',
      tk: 'Iňlis dili',
      ne: 'अङ्ग्रेजी भाषा',
      za: 'Vah Yinghgoz',
      ru: 'Английский язык',
      ku: 'Zimanê îngilîzî',
      yo: 'Èdè Gẹ̀ẹ́sì',
      bm: 'Angilɛkan',
      st: 'Senyesemane',
      pi: 'आंगलभाषा',
      la: 'Lingua Anglica',
      uz: 'Ingliz tili',
      eo: 'Angla lingvo',
      pt: 'Língua inglesa',
      bs: 'Engleski jezik',
      he: 'אנגלית',
      el: 'Αγγλική γλώσσα',
      kl: 'Tuluttut',
      ee: 'Eŋlisigbe',
      io: 'Angliana linguo',
      ga: 'An Béarla',
      vo: 'Linglänapük',
      id: 'Bahasa Inggris',
      or: 'ଇଂରାଜୀ ଭାଷା',
      te: 'ఆంగ్ల భాష',
      jv: 'Basa Inggris',
      fi: 'Englanti'
    },
    language_code_3: {
      en: 'eng'
    },
    language_code_2: {
      en: 'en'
    }
  },
  'en:old-church-slavonic': {
    name: {
      da: 'Oldkirkeslavisk',
      nl: 'Oudkerkslavisch',
      fa: 'زبان اسلاوی کلیسایی باستان',
      an: 'Antigo eslavo eclesiastico',
      ru: 'Старославянский язык',
      mk: 'Старословенски јазик',
      lv: 'Baznīcslāvu valoda',
      ta: 'பண்டைய மசிதோனிய மொழி',
      ia: 'Lingua slavonic ecclesiastic ancian',
      af: 'Ou Kerkslawies',
      hr: 'Staroslavenski jezik',
      lb: 'Alslawesch Sprooch',
      pl: 'Język staro-cerkiewno-słowiański',
      lt: 'Senoji bažnytinė slavų kalba',
      sk: 'Staroslovienčina',
      kv: 'Важ славян кыв',
      az: 'Qədim slavyan dili',
      cs: 'Staroslověnština',
      sr: 'Старословенски језик',
      ro: 'Limba slavă veche',
      zh: '古教會斯拉夫語',
      ja: '古代教会スラヴ語',
      uk: "Староцерковнослов'янська мова",
      cu: 'Словѣньскъ ѩꙁꙑкъ',
      is: 'Fornkirkjuslavneska',
      tr: 'Eski Kilise Slavcası',
      hy: 'Հին սլավոներեն',
      ko: '고대 교회 슬라브어',
      kk: 'Көне славян тілі',
      fi: 'Muinaiskirkkoslaavi',
      be: 'Стараславянская мова',
      hu: 'Óegyházi szláv nyelv',
      oc: 'Eslavon',
      bg: 'Старобългарски език',
      de: 'Altkirchenslawisch',
      nn: 'Gammalkyrkjeslavisk',
      nb: 'Gammelkirkeslavisk',
      eu: 'Antzinako eliza-eslaviera',
      sh: 'Staroslavenski jezik',
      ar: 'السلافونية الكنسية القديمة',
      sv: 'Fornkyrkoslaviska',
      it: 'Antico slavo ecclesiastico',
      sl: 'Stara cerkvena slovanščina',
      id: 'Bahasa Gereja Slavonia Lama',
      ba: 'Иҫке славян теле',
      he: 'סלאבית כנסייתית עתיקה',
      th: 'ภาษาสลาโวนิกคริสตจักรโบราณ',
      el: 'Αρχαία εκκλησιαστική σλαβονική γλώσσα',
      sq: 'Kisha e vjetër sllave',
      ca: 'Antic eslau',
      es: 'Antiguo eslavo eclesiástico',
      fr: 'Vieux-slave',
      eo: 'Malnovslava lingvo',
      pt: 'Antigo eslavo eclesiástico',
      ka: 'ძველი სლავური ენა',
      la: 'Lingua Slavonica antiqua',
      cy: 'Hen Slafoneg Eglwysig',
      en: 'Old Church Slavonic',
      cv: 'Ватă славян чĕлхи',
      bs: 'Staroslavenski jezik'
    },
    language_code_3: {
      en: 'chu'
    },
    wikidata: {
      en: 'Q35499'
    },
    language_code_2: {
      en: 'cu'
    }
  },
  'en:dzongkha-language': {
    name: {
      sv: 'Dzongkha',
      hi: 'जोंगखा',
      ar: 'لغة دزونكا',
      su: 'Basa Dzongkha',
      id: 'Bahasa Dzongkha',
      qu: 'Dzonkha simi',
      it: 'Dzongkha',
      bg: 'Дзонгкха',
      oc: 'Dzongkha',
      fi: 'Dzongkhan kieli',
      be: 'Дзонг-кэ',
      eu: 'Dzongkha',
      nb: 'Dzongkha',
      nn: 'Dzongkha',
      ur: 'زونگکھا',
      de: 'Dzongkha',
      mr: 'जोंगखा भाषा',
      en: 'Dzongkha language',
      la: 'Lingua Dzongkha',
      pt: 'Língua butanesa',
      eo: 'Dzonka lingvo',
      ka: 'ძონგკხა',
      tl: 'Wikang Dzongkha',
      gv: 'Butaanish',
      th: 'ภาษาซองคา',
      he: 'דזונגקה',
      fr: 'Dzongkha',
      es: 'Dzongkha',
      pa: 'ਜੌਙਖਾ ਬੋਲੀ',
      ca: 'Dzongkha',
      af: 'Dzongkha',
      ta: 'திஃசொங்கா மொழி',
      vi: 'Tiếng Dzongkha',
      lv: 'Dzongke',
      ms: 'Bahasa Jongkha',
      lt: 'Botijų kalba',
      pl: 'Dzongkha',
      ne: 'जोंगखा',
      hr: 'Dzongkha jezik',
      nl: 'Dzongkha',
      da: 'Dzongkha',
      sw: 'Kidzongkha',
      mk: 'Ѕонгка',
      bo: 'འབྲུག་པའི་སྐད།',
      dz: 'རྫོང་ཁ་',
      ru: 'Дзонг-кэ',
      fa: 'دزونگخا',
      tr: 'Dzongka',
      uk: 'Дзонг-ке',
      br: 'Boutaneg',
      bn: 'জংখা ভাষা',
      ko: '종카어',
      hy: 'Ձոնգքհա',
      et: 'Dzongkha keel',
      kv: 'Дзонг-кэ',
      az: 'Dzonqxa',
      sk: 'Dzongkha',
      ps: 'ژونگخا',
      gn: "Dzongkhañe'ẽ",
      ja: 'ゾンカ語',
      zh: '宗喀語',
      cs: 'Dzongkä',
      sr: 'Џонгка језик'
    },
    language_code_3: {
      en: 'dzo'
    },
    wikidata: {
      en: 'Q33081'
    },
    language_code_2: {
      en: 'dz'
    }
  },
  'en:japanese': {
    language_code_2: {
      en: 'ja'
    },
    wikidata: {
      en: 'Q5287'
    },
    name: {
      bg: 'Японски език',
      hu: 'Japán',
      fo: 'Japanskt mál',
      dv: 'ޖަޕާނީ',
      ty: 'Reo Tāpōnē',
      eu: 'Japoniera',
      nb: 'Japansk',
      ur: 'جاپانی',
      de: 'Japanisch',
      am: 'ጃፓንኛ',
      ab: 'Иапон бызшәа',
      fy: 'Japansk',
      su: 'Basa Jepang',
      qu: 'Nihun simi',
      it: 'Giapponese',
      gv: 'Shapaanish',
      sq: 'Gjuha japoneze',
      th: 'ภาษาญี่ปุ่น',
      sc: 'Limba giapponesa',
      fr: 'Japonais',
      es: 'Japonés',
      gl: 'Lingua xaponesa',
      pa: 'ਜਾਪਾਨੀ ਭਾਸ਼ਾ',
      ca: 'Japonès',
      mr: 'जपानी भाषा',
      en: 'Japanese',
      tl: 'Wikang Hapones',
      sm: "Fa'aiapani",
      ln: 'Lizapɔ́',
      da: 'Japansk',
      mk: 'Јапонски јазик',
      bo: 'ཉི་ཧོང་སྐད།',
      tg: 'Забони ҷопонӣ',
      an: 'Idioma chaponés',
      fa: 'زبان ژاپنی',
      af: 'Japannees',
      gd: 'Seapanais',
      lv: 'Japāņu valoda',
      lt: 'Japonų kalba',
      pl: 'Język japoński',
      hr: 'Japanski jezik',
      lb: 'Japanesch',
      lo: 'ພາສາຍີ່ປຸ່ນ',
      et: 'Jaapani keel',
      az: 'Yapon dili',
      ps: 'د ياماتو ژبه',
      cs: 'Japonština',
      se: 'Japánagiella',
      ky: 'Жапон тили',
      ce: 'Японийн мотт',
      tr: 'Japonca',
      na: 'Dorerin Djapan',
      is: 'Japanska',
      uk: 'Японська мова',
      bn: 'জাপানি ভাষা',
      ug: 'ياپون تىلى',
      zu: 'IsiJapani',
      oc: 'Japonés',
      kk: 'Жапон тілі',
      be: 'Японская мова',
      fi: 'Japani',
      nn: 'Japansk',
      sv: 'Japanska',
      vo: 'Yapänapük',
      hi: 'जापानी भाषा',
      my: 'ဂျပန်ဘာသာ',
      ar: 'لغة يابانية',
      sh: 'Japanski jezik',
      tt: 'Япон теле',
      jv: 'Basa Jepang',
      id: 'Bahasa Jepang',
      ba: 'Япон теле',
      or: 'ଜାପାନୀ ଭାଷା',
      sl: 'Japonščina',
      el: 'Ιαπωνική γλώσσα',
      he: 'יפנית',
      io: 'Japoniana linguo',
      ga: 'An tSeapáinis',
      kl: 'Japanimiusut',
      cv: 'Япон чĕлхи',
      la: 'Lingua Iaponica',
      cy: 'Japaneg',
      uz: 'Yapon tili',
      ka: 'იაპონური ენა',
      pt: 'Língua japonesa',
      eo: 'Japana lingvo',
      bs: 'Japanski jezik',
      nl: 'Japans',
      za: 'Vah Nditbonj',
      sw: 'Kijapani',
      mi: 'Reo Hapani',
      ru: 'Японский язык',
      yo: 'Èdè Japaní',
      ku: 'Zimanê japonî',
      ia: 'Lingua japonese',
      sa: 'जापानी भाषा',
      ta: 'ஜப்பானிய மொழி',
      vi: 'Tiếng Nhật',
      kn: 'ಜಪಾನಿ ಭಾಷೆ',
      ms: 'Bahasa Jepun',
      ne: 'जापानी भाषा',
      tk: 'Ýapon dili',
      ml: 'ജാപ്പനീസ് ഭാഷ',
      kv: 'Япон кыв',
      rw: 'Ikiyapani',
      sk: 'Japončina',
      km: 'ភាសាជប៉ុន',
      ja: '日本語',
      gn: "Hapõñe'ẽ",
      zh: '日语',
      mn: 'Япон хэл',
      wa: 'Djaponès',
      yi: 'יאפאניש',
      ro: 'Limba japoneză',
      sr: 'Јапански језик',
      kw: 'Nihonek',
      mg: 'Japonezy',
      cu: 'Ꙗпѡньскъ ѩꙁꙑкъ',
      br: 'Japaneg',
      ko: '일본어',
      li: 'Japans',
      hy: 'Ճապոներեն'
    },
    language_code_3: {
      en: 'jpn'
    }
  },
  'en:malay': {
    language_code_2: {
      en: 'ms'
    },
    wikidata: {
      en: 'Q9237'
    },
    language_code_3: {
      en: 'msa'
    },
    name: {
      fy: 'Maleisk',
      it: 'Malese',
      qu: 'Malaya simi',
      hu: 'Maláj',
      bg: 'Малайски език',
      sd: 'مالي ٻولي',
      eu: 'Malaysiera',
      nb: 'Malayisk',
      ur: 'ملایو زبان',
      de: 'Malaiisch',
      en: 'Malay',
      mr: 'मलाय भाषा',
      tl: 'Wikang Malay',
      th: 'ภาษามาเลย์',
      fr: 'Malais',
      gl: 'Lingua malaia',
      es: 'Malayo',
      ca: 'Malai',
      af: 'Maleis',
      lv: 'Malajiešu valoda',
      lt: 'Malajų kalba',
      pl: 'Język malajski',
      hr: 'Malajski jezik',
      da: 'Malajisk',
      mk: 'Малајски јазик',
      tg: 'Забони малайӣ',
      an: 'Idioma malayo',
      fa: 'زبان مالایی',
      ce: 'Малайн мотт',
      tr: 'Malayca',
      uk: 'Малайська мова',
      bn: 'মালয় ভাষা',
      ug: 'مالاي تىلى',
      et: 'Malai keel',
      az: 'Malay dili',
      cs: 'Malajština',
      hi: 'मलय भाषा',
      sv: 'Malajiska',
      my: 'ပသျှူးဘာသာစကား',
      sh: 'Malajski jezik',
      ar: 'لغة ملايو',
      jv: 'Basa Melayu',
      ba: 'Малай теле',
      id: 'Bahasa Melayu',
      oc: 'Malai',
      kk: 'Малай тілі',
      fi: 'Malaji',
      be: 'Малайская мова',
      nn: 'Malayisk',
      la: 'Lingua Malayana',
      ka: 'მალაიური ენა',
      pt: 'Língua malaia',
      eo: 'Malaja lingvo',
      bs: 'Malajski jezik',
      he: 'מלאית',
      io: 'Malaya linguo',
      ga: 'An Mhalaeis',
      ia: 'Lingua malay',
      ta: 'மலாய் மொழி',
      ms: 'Bahasa Melayu',
      vi: 'Tiếng Mã Lai',
      nl: 'Maleis',
      sw: 'Kimalay',
      mi: 'Reo Marēhia',
      ru: 'Малайский язык',
      mg: 'Fiteny Malay',
      br: 'Malayeg',
      ko: '말레이어',
      hy: 'Մալայերեն',
      kv: 'Малай кыв',
      ja: 'マレー語',
      km: 'ភាសាម៉ាឡេ',
      zh: '马来语',
      ro: 'Limba malaieză',
      sr: 'Малајски језик'
    }
  },
  'en:tagalog': {
    wikidata: {
      en: 'Q34057'
    },
    language_code_3: {
      en: 'tgl'
    },
    name: {
      pa: 'ਤਗਾਲੋਗ',
      ca: 'Tagal',
      fr: 'Tagalog',
      gl: 'Lingua tagala',
      es: 'Tagalo',
      th: 'ภาษาตากาล็อก',
      tl: 'Wikang Tagalog',
      mr: 'टागालोग भाषा',
      en: 'Tagalog',
      de: 'Tagalog',
      eu: 'Tagalo',
      nb: 'Tagalog',
      bg: 'Тагалог',
      hu: 'Tagalog',
      it: 'Tagalog',
      qu: 'Tagalu simi',
      cs: 'Tagalog',
      az: 'Taqal dili',
      et: 'Tagalogi keel',
      ug: 'تاگالوگ تىلى',
      bn: 'তাগালোগ ভাষা',
      tr: 'Tagalogca',
      is: 'Tagalog',
      uk: 'Тагальська мова',
      fa: 'زبان تاگالوگ',
      mk: 'Тагалошки јазик',
      da: 'Tagalog',
      hr: 'Tagaloški jezik',
      lt: 'Tagalų kalba',
      pl: 'Język tagalski',
      lv: 'Tagalogu valoda',
      af: 'Tagalog',
      ga: 'An Tagálaigis',
      he: 'טגלוג',
      cy: 'Tagalog',
      la: 'Lingua Tagalog',
      uz: 'Tagal tili',
      eo: 'Tagaloga lingvo',
      pt: 'Língua tagalo',
      ka: 'ტაგალური ენა',
      cv: 'Тагаль чĕлхи',
      nn: 'Tagalog',
      oc: 'Tagalòg',
      be: 'Тагальская мова',
      fi: 'Tagalog',
      id: 'Bahasa Tagalog',
      ar: 'لغة تاغالوغية',
      sh: 'Tagaloški jezik',
      hi: 'टागालोग भाषा',
      sv: 'Tagalog',
      yi: 'טאגאלאג',
      ro: 'Limba tagalog',
      sr: 'Тагалог',
      ja: 'タガログ語',
      zh: '他加祿語',
      kv: 'Тагалог кыв',
      ml: 'ടാഗലോഗ്',
      ko: '타갈로그어',
      hy: 'Թագալերեն',
      br: 'Tagalogeg',
      mg: 'Fiteny tagalaogy',
      ru: 'Тагальский язык',
      ku: 'Zimanê tagalogî',
      sw: 'Kitagalog',
      nl: 'Tagalog',
      ta: 'தகலாகு மொழி',
      vi: 'Tếng Tagalog',
      ms: 'Bahasa Tagalog',
      kn: 'ಟಾಗಲಾಗ್ ಭಾಷೆ',
      ia: 'Tagalog'
    },
    language_code_2: {
      en: 'tl'
    }
  },
  'en:moldovan': {
    language_code_2: {
      en: 'mo'
    },
    wikidata: {
      en: 'Q36392'
    },
    name: {
      ar: 'لغة مولدوفية',
      sh: 'Moldavski jezik',
      sv: 'Moldaviska',
      vo: 'Moldovänapük',
      it: 'Moldavo',
      sl: 'Moldavščina',
      tt: 'Молдав теле',
      be: 'Малдаўская мова',
      fi: 'Moldavian kieli',
      bg: 'Молдовски език',
      oc: 'Moldau',
      hu: 'Moldáv nyelv',
      am: 'ሞልዶቭኛ',
      de: 'Moldauische Sprache',
      nn: 'Moldovsk',
      nb: 'Moldovsk',
      eu: 'Moldaviera',
      ka: 'მოლდავური ენა',
      pt: 'Língua moldávia',
      eo: 'Moldava lingvo',
      mr: 'मोल्दोव्हन भाषा',
      en: 'Moldovan',
      he: 'מולדובנית',
      th: 'ภาษามอลโดวา',
      el: 'Μολδαβική γλώσσα',
      sq: 'Gjuha moldave',
      ca: 'Moldau',
      es: 'Moldavo',
      fr: 'Moldave',
      ms: 'Bahasa Moldova',
      lv: 'Moldāvu valoda',
      ta: 'மோல்தோவ மொழி',
      af: 'Moldawies',
      lb: 'Moldawesch',
      hr: 'Moldavski jezik',
      pl: 'Język mołdawski',
      lt: 'Moldavų kalba',
      nl: 'Moldavisch',
      fa: 'مولداویایی',
      ku: 'Zimanê moldawî',
      ru: 'Молдавский язык',
      mk: 'Молдавски јазик',
      uk: 'Молдавська мова',
      tr: 'Moldovaca',
      li: 'Moldavisch',
      hy: 'Մոլդովերեն',
      ko: '몰도바어',
      ug: 'مولداۋىيە تىلى',
      sk: 'Moldavčina',
      az: 'Moldav dili',
      mo: 'Лимба молдовеняскэ',
      cs: 'Moldavština',
      sr: 'Молдавски језик',
      ro: 'Limba moldovenească',
      zh: '摩爾多瓦語',
      ja: 'モルドバ語'
    },
    language_code_3: {
      en: 'rum'
    }
  },
  'en:korean': {
    language_code_2: {
      en: 'ko'
    },
    name: {
      uz: 'Koreys tili',
      pt: 'Coreano',
      ka: 'კორეული ენა',
      eo: 'Korea lingvo',
      cy: 'Corëeg',
      la: 'Lingua Coreana',
      bs: 'Korejski jezik',
      el: 'Κορεατική γλώσσα',
      he: 'קוריאנית',
      io: 'Koreana linguo',
      ga: 'An Chóiréis',
      kl: 'Koreamiusut',
      sv: 'Koreanska',
      vo: 'Koreyänapük',
      hi: 'कोरियाई भाषा',
      ar: 'لغة كورية',
      sh: 'Korejski jezik',
      jv: 'Basa Koréa',
      tt: 'Корей теле',
      sl: 'Korejščina',
      id: 'Bahasa Korea',
      be: 'Карэйская мова',
      kk: 'Корей тілі',
      fi: 'Korea',
      oc: 'Corean',
      nn: 'Koreansk',
      mg: 'Fiteny koreanina',
      br: 'Koreaneg',
      li: 'Koreaans',
      hy: 'Կորեերեն',
      ko: '한국어',
      ml: 'കൊറിയൻ ഭാഷ',
      nv: 'Kolíya Dineʼé bizaad',
      sk: 'Kórejčina',
      kv: 'Корея кыв',
      zh: '朝鲜语',
      mn: 'Солонгос хэл',
      ja: '朝鮮語',
      km: 'ភាសាកូរ៉េ',
      gn: "Koreañe'ẽ",
      sr: 'Корејски језик',
      kw: 'Koreek',
      ro: 'Limba coreeană',
      yi: 'קארעיש',
      sa: 'कोरियालि भाषा',
      ia: 'Lingua corean',
      kn: 'ಕೊರಿಯಾದ ಭಾಷೆ',
      vi: 'Tiếng Triều Tiên',
      ms: 'Bahasa Korea',
      ta: 'கொரிய மொழி',
      tk: 'Koreý dili',
      nl: 'Koreaans',
      sw: 'Kikorea',
      mi: 'Reo Kōrea',
      ku: 'Zimanê korêyî',
      ru: 'Корейский язык',
      mr: 'कोरियन भाषा',
      en: 'Korean',
      sm: "Fa'aKolea",
      tl: 'Wikang Koreano',
      th: 'ภาษาเกาหลี',
      sq: 'Gjuha koreane',
      sc: 'Limba coreana',
      es: 'Coreano',
      gl: 'Lingua coreana',
      fr: 'Coréen',
      ca: 'Coreà',
      pa: 'ਕੋਰੀਅਨ ਭਾਸ਼ਾ',
      fy: 'Koreaansk',
      su: 'Basa Koréa',
      it: 'Coreano',
      qu: 'Kuryu simi',
      fo: 'Koreanskt mál',
      bg: 'Корейски език',
      hu: 'Koreai',
      dv: 'ކޮރެޔާ ބަސް',
      nb: 'Koreansk',
      eu: 'Koreera',
      de: 'Koreanisch',
      am: 'ኮሪይኛ',
      ht: 'Lang kore',
      ce: 'Корейхойн мотт',
      uk: 'Корейська мова',
      is: 'Kóreska',
      tr: 'Korece',
      na: 'Dorerin Korea',
      bn: 'কোরীয় ভাষা',
      ug: 'كورىيەچە',
      et: 'Korea keel',
      lo: 'ພາສາເກົາຫຼີ',
      az: 'Koreya dili',
      cs: 'Korejština',
      se: 'Koreagiella',
      ky: 'Корей тили',
      af: 'Koreaans',
      lv: 'Korejiešu valoda',
      pl: 'Język koreański',
      lt: 'Korėjiečių kalba',
      hr: 'Korejski jezik',
      da: 'Koreansk',
      tg: 'Забони кореягӣ',
      mk: 'Корејски јазик',
      si: 'කොරියානු භාෂාව',
      fa: 'زبان کره‌ای',
      an: 'Idioma corián',
      iu: 'ᑯᕆᐊᑎᑐᑦ'
    },
    language_code_3: {
      en: 'kor'
    },
    wikidata: {
      en: 'Q9176'
    }
  },
  'en:luba-katanga-language': {
    language_code_3: {
      en: 'lub'
    },
    name: {
      ln: 'Kiluba',
      it: 'Lingua kiluba',
      en: 'Luba-Katanga language',
      ru: 'Луба-катанга',
      de: 'Kiluba',
      es: 'KiLuba',
      sw: 'Kiluba-Katanga',
      fr: 'Luba-katanga',
      ja: 'ルバ・カタンガ語',
      rw: 'Ikiluba',
      kg: 'Kiluba',
      fi: 'Katanganluba',
      nl: 'Luba-Katanga'
    },
    wikidata: {
      en: 'Q36157'
    },
    language_code_2: {
      en: 'lu'
    }
  },
  'en:kazakh': {
    language_code_2: {
      en: 'kk'
    },
    wikidata: {
      en: 'Q9252'
    },
    language_code_3: {
      en: 'kaz'
    },
    name: {
      th: 'ภาษาคาซัค',
      sq: 'Gjuha kazake',
      ca: 'Kazakh',
      pa: 'ਕਜ਼ਾਖ ਭਾਸ਼ਾ',
      gl: 'Lingua casaca',
      es: 'Kazajo',
      fr: 'Kazakh',
      mr: 'कझाक भाषा',
      en: 'Kazakh',
      bg: 'Казахски език',
      hu: 'Kazak',
      ur: 'قازق زبان',
      de: 'Kasachisch',
      nb: 'Kasakhisk',
      eu: 'Kazakhera',
      qu: 'Qasaq simi',
      it: 'Kazako',
      az: 'Qazax dili',
      et: 'Kasahhi keel',
      ky: 'Казак тили',
      cs: 'Kazaština',
      uk: 'Казахська мова',
      tr: 'Kazakça',
      is: 'Kasakska',
      ce: 'Казахийн мотт',
      ug: 'قازاق تىلى',
      bn: 'কাজাখ ভাষা',
      da: 'Kasakhisk',
      fa: 'زبان قزاقی',
      an: 'Idioma cazaco',
      tg: 'Забони қазоқӣ',
      mk: 'Казашки јазик',
      lv: 'Kazahu valoda',
      af: 'Kasaks',
      hr: 'Kazaški jezik',
      pl: 'Język kazachski',
      lt: 'Kazachų kalba',
      he: 'קזחית',
      ga: 'An Chasaicis',
      uz: 'Qozoq tili',
      eo: 'Kazaĥa lingvo',
      pt: 'Língua cazaque',
      ka: 'ყაზახური ენა',
      la: 'Lingua cazachica',
      cv: 'Казах чĕлхи',
      kk: 'Қазақ тілі',
      fi: 'Kazakin kieli',
      be: 'Казахская мова',
      oc: 'Cazac',
      nn: 'Kasakhisk',
      ar: 'اللغة القازاقية',
      sh: 'Kazaški jezik',
      hi: 'कज़ाख़ भाषा',
      vo: 'Kazakapük',
      sv: 'Kazakiska',
      sl: 'Kazaščina',
      id: 'Bahasa Kazak',
      ba: 'Ҡаҙаҡ теле',
      tt: 'Казакъ теле',
      sk: 'Kazaština',
      rw: 'Igikazaki',
      kv: 'Казах кыв',
      sr: 'Казашки језик',
      ro: 'Limba kazahă',
      mn: 'Kазах хэл',
      zh: '哈萨克语',
      ja: 'カザフ語',
      mg: 'Fiteny kazàka',
      hy: 'Ղազախերեն',
      li: 'Kazachs',
      ko: '카자흐어',
      br: 'Kazakeg',
      nl: 'Kazachs',
      yo: 'Èdè Kàsákhì',
      ru: 'Казахский язык',
      sw: 'Kikazakhi',
      vi: 'Tiếng Kazakh',
      ms: 'Bahasa Kazakh',
      ta: 'காசாக்கு மொழி'
    }
  },
  'en:quechua-languages': {
    wikidata: {
      en: 'Q5218'
    },
    name: {
      cv: 'Кечуа',
      pt: 'Quíchua',
      eo: 'Keĉua lingvaro',
      ka: 'კეჩუა',
      cy: 'Quechua',
      la: 'Linguae quechuae',
      bs: 'Kečuanski jezik',
      el: 'Κέτσουα γλώσσες',
      he: "קצ'ואה",
      io: 'Kechuana linguo',
      ga: 'An Cheatsuais',
      sv: 'Quechua',
      hi: 'केचुआ भाषाएँ',
      sh: 'Kečua',
      ar: 'كتشوا',
      sl: 'Kečuanščina',
      id: 'Bahasa Quechua',
      kk: 'Кечуа тілі',
      be: 'Мова кечуа',
      fi: 'Ketšua',
      oc: 'Quíchoa',
      nn: 'Quechua',
      br: 'Kechuaeg',
      hy: 'Կեչուա լեզուներ',
      li: 'Quechua',
      ko: '케추아어족',
      sk: 'Kečuánčina',
      nv: 'Kéchwa bizaad',
      kv: 'Кечуа',
      zh: '克丘亞語',
      wa: 'Ketchwa',
      gn: "Kechuañe'ẽ",
      ja: 'ケチュア語',
      sr: 'Кечуа језик',
      ro: 'Limbi quechua',
      ia: 'Quechuan linguas',
      ms: 'Bahasa Quechua',
      vi: 'Tiếng Quechua',
      ta: 'கெச்வா மொழிகள்',
      nl: 'Quechua',
      mi: 'Reo Runa Simi',
      ru: 'Кечуа',
      en: 'Quechua languages',
      mr: 'क्वेचुआ भाषा',
      th: 'ภาษาเกชัว',
      sq: 'Kueçua',
      gl: 'Linguas quechuas',
      es: 'Lenguas quechuas',
      ay: 'Qhichwa aru',
      fr: 'Quechua',
      ca: 'Quítxua',
      qu: 'Qhichwa simi',
      it: 'Quechua',
      hu: 'Kecsua nyelv',
      bg: 'Кечуа',
      nb: 'Quechua',
      eu: 'Kitxua',
      de: 'Quechua',
      am: 'ቀቿ',
      uk: 'Кечуа',
      is: 'Quechua',
      tr: 'Keçuva dilleri',
      bn: 'কেচুয়া ভাষা',
      ug: 'كۋېچۇئا تىلى',
      et: 'Ketšua keel',
      cs: 'Kečuánština',
      af: 'Quechua',
      gd: 'Quechua',
      lv: 'Kečvu valoda',
      pl: 'Język keczua',
      lt: 'Kečujų kalba',
      hr: 'Kečuanski jezik',
      da: 'Quechua',
      rm: 'Quechua',
      mk: 'Кечуански јазици',
      fa: 'کچوآ',
      iu: 'ᑮᓱᐊ'
    },
    language_code_3: {
      en: 'que'
    },
    language_code_2: {
      en: 'qu'
    }
  },
  'en:ukrainian': {
    language_code_2: {
      en: 'uk'
    },
    name: {
      ms: 'Bahasa Ukraine',
      vi: 'Tiếng Ukraina',
      ta: 'உக்குரேனிய மொழி',
      ne: 'यूक्रेनी भाषा',
      tk: 'Ukrain dili',
      nl: 'Oekraïens',
      sw: 'Kiukraine',
      yo: 'Èdè Ukraníà',
      ku: 'Zimanê ukraynî',
      ru: 'Украинский язык',
      co: 'Lingua ucraina',
      cu: 'Оукраиньскъ ѩꙁꙑкъ',
      br: 'Ukraineg',
      hy: 'Ուկրաիներեն',
      li: 'Oekraïens',
      ko: '우크라이나어',
      ml: 'ഉക്രൈനിയൻ ഭാഷ',
      sk: 'Ukrajinčina',
      kv: 'Украин кыв',
      zh: '乌克兰语',
      mn: 'Украйн хэл',
      ja: 'ウクライナ語',
      km: 'ភាសាអ៊ុយក្រែន',
      sr: 'Украјински језик',
      kw: 'Ukraynek',
      ro: 'Limba ucraineană',
      yi: 'אוקראיניש',
      hi: 'यूक्रेनी भाषा एवं साहित्य',
      vo: 'Lukrayänapük',
      sv: 'Ukrainska',
      ar: 'لغة أوكرانية',
      sh: 'Ukrajinski jezik',
      tt: 'Украин теле',
      sl: 'Ukrajinščina',
      ba: 'Украин теле',
      id: 'Bahasa Ukraina',
      kk: 'Украин тілі',
      be: 'Украінская мова',
      fi: 'Ukraina',
      oc: 'Ucraïnian',
      nn: 'Ukrainsk',
      cv: 'Украин чĕлхи',
      ka: 'უკრაინული ენა',
      uz: 'Ukrain tili',
      pt: 'Língua ucraniana',
      eo: 'Ukraina lingvo',
      cy: 'Wcreineg',
      la: 'Lingua Ucrainica',
      bs: 'Ukrajinski jezik',
      el: 'Ουκρανική γλώσσα',
      he: 'אוקראינית',
      ga: 'An Úcráinis',
      io: 'Ukrainiana linguo',
      kl: 'Ukrainimiusut',
      bi: 'Toktok Ukraine',
      af: 'Oekraïns',
      gd: 'Ucràinis',
      lv: 'Ukraiņu valoda',
      pl: 'Język ukraiński',
      lt: 'Ukrainiečių kalba',
      hr: 'Ukrajinski jezik',
      lb: 'Ukrainesch',
      da: 'Ukrainsk',
      bo: 'ཨུཀ་རཡི་ནི་སྐད།',
      tg: 'Забони украйинӣ',
      mk: 'Украински јазик',
      fa: 'زبان اوکراینی',
      an: 'Idioma ucrainés',
      ce: 'Украинийн мотт',
      uk: 'Українська мова',
      is: 'Úkraínska',
      tr: 'Ukraynaca',
      bn: 'ইউক্রেনীয় ভাষা',
      ug: 'ئۇكرائىن تىلى',
      os: 'Украинаг æвзаг',
      et: 'Ukraina keel',
      az: 'Ukrayna dili',
      av: 'Украиназул мацІ',
      ky: 'Украин тили',
      se: 'Ukrainagiella',
      cs: 'Ukrajinština',
      fy: 'Oekraynsk',
      ab: 'Украин бызшәа',
      qu: 'Ukranya simi',
      it: 'Ucraino',
      bg: 'Украински език',
      hu: 'Ukrán',
      nb: 'Ukrainsk',
      eu: 'Ukrainera',
      ur: 'یوکرینی زبان',
      am: 'ዩክሬንኛ',
      de: 'Ukrainisch',
      en: 'Ukrainian',
      mr: 'युक्रेनियन भाषा',
      sm: "Fa'aUkaraina",
      tl: 'Wikang Ukranyano',
      th: 'ภาษายูเครน',
      sq: 'Gjuha ukrainase',
      gv: 'Ookraanish',
      sc: 'Limba ucraina',
      tw: 'Ukrainian',
      gl: 'Lingua ucraína',
      es: 'Ucraniano',
      fr: 'Ukrainien',
      ca: 'Ucraïnès',
      pa: 'ਉਕਰੇਨੀ ਭਾਸ਼ਾ'
    },
    language_code_3: {
      en: 'ukr'
    },
    wikidata: {
      en: 'Q8798'
    }
  },
  'en:occitan': {
    language_code_2: {
      en: 'oc'
    },
    wikidata: {
      en: 'Q14185'
    },
    name: {
      vo: 'Loxitänapük',
      om: 'Afaan Occitan',
      sv: 'Occitanska',
      ar: 'لغة قسطانية',
      sh: 'Oksitanski jezik',
      jv: 'Basa Ositania',
      tt: 'Провансаль теле',
      sl: 'Okcitanščina',
      id: 'Bahasa Oksitan',
      ba: 'Окситан теле',
      fi: 'Oksitaani',
      be: 'Аксітанская мова',
      oc: 'Occitan',
      nn: 'Oksitansk',
      mt: 'Oċċitan',
      cv: 'Окситан чĕлхи',
      uz: 'Oksitan tili',
      pt: 'Língua occitana',
      eo: 'Okcitana lingvo',
      ka: 'ოქსიტანური ენა',
      la: 'Lingua Occitana',
      cy: 'Ocsitaneg',
      bs: 'Oksitanski jezik',
      el: 'Οξιτανικά',
      kg: 'Kiunsita',
      he: 'אוקסיטנית',
      io: 'Ocitaniana linguo',
      ga: 'An Ocsatáinis',
      ia: 'Lingua occitan',
      ms: 'Bahasa Occitan',
      vi: 'Tiếng Occitan',
      ta: 'ஆக்சிதம்',
      nl: 'Occitaans',
      sw: 'Kioksitania',
      ku: 'Zimanê oksîtanî',
      yo: 'Èdè Occitani',
      ru: 'Окситанский язык',
      co: 'Lingua occitanica',
      mg: 'Fiteny ôksitana',
      br: 'Okitaneg',
      li: 'Occitaans',
      hy: 'Օքսիտաներեն',
      ko: '오크어',
      sk: 'Okcitánčina',
      kv: 'Окситан кыв',
      wa: 'Occitan',
      zh: '奥克语',
      ja: 'オック語',
      gn: "Occitáno ñe'ẽ",
      sr: 'Окситански језик',
      kw: 'Oksitanek',
      ro: 'Limba occitană',
      fy: 'Oksitaansk',
      su: 'Basa Occitan',
      qu: 'Uqsitan simi',
      it: 'Occitano',
      ak: 'Occitan',
      hu: 'Okcitán',
      bg: 'Окситански език',
      ty: 'Reo Otitānia',
      nb: 'Oksitansk',
      eu: 'Okzitaniera',
      ur: 'اوسیٹین',
      am: 'ኦኪታንኛ',
      de: 'Okzitanische Sprache',
      en: 'Occitan',
      mr: 'ऑक्सितान भाषा',
      tl: 'Wikang Occittan',
      ln: 'Liosita',
      th: 'ภาษาอ็อกซิตัน',
      gv: 'Ocsitaanish',
      sq: 'Gjuha oksitane',
      sc: 'Limba otzitana',
      es: 'Occitano',
      gl: 'Lingua occitana',
      fr: 'Occitan',
      ca: 'Occità',
      af: 'Oksitaans',
      gd: 'Ocseadanais',
      lv: 'Oksitāņu valoda',
      pl: 'Język oksytański',
      lt: 'Oksitanų kalba',
      hr: 'Okcitanski jezik',
      da: 'Occitansk',
      rm: 'Lingua occitana',
      mk: 'Окситански јазик',
      fa: 'زبان اکسیتان',
      an: 'Idioma occitán',
      ce: 'Окситанхойн мотт',
      uk: 'Окситанська мова',
      tr: 'Oksitanca',
      is: 'Oksítanska',
      na: 'Dorerin Occitan',
      bn: 'অক্সিতঁ ভাষা',
      zu: 'IsiOsithani',
      os: 'Окситайнаг æвзаг',
      et: 'Oksitaani keel',
      lo: 'ພາສາອົກຊີຕັງ',
      az: 'Oksitan dili',
      cs: 'Okcitánština',
      se: 'Oksitánagiella'
    },
    language_code_3: {
      en: 'oci'
    }
  },
  'en:bihari-languages': {
    language_code_2: {
      en: 'bh'
    },
    wikidata: {
      en: 'Q135305'
    },
    language_code_3: {
      en: 'bih'
    },
    name: {
      nb: 'Bihari',
      nn: 'Biharispråk',
      de: 'Bihari',
      fi: 'Bihar-kielet',
      it: 'Lingue bihari',
      hi: 'बिहारी भाषाएँ',
      ar: 'قائمة اللغات البهارية',
      fr: 'Bihari',
      es: 'Lenguas bihari',
      ca: 'Bihari',
      el: 'Μπιχάρι γλώσσα',
      en: 'Bihari languages',
      mr: 'बिहारी भाषा',
      la: 'Lingua bihari',
      pt: 'Línguas biaris',
      eo: 'Bihara lingvo',
      ru: 'Бихарские языки',
      fa: 'زبان بیهاری',
      nl: 'Bihari',
      da: 'Bihari',
      lt: 'Biharų kalbos',
      pl: 'Bihari',
      hr: 'Biharski jezici',
      bh: 'बिहारी भाषा',
      ta: 'பீகாரி மொழிகள்',
      lv: 'Bihāri valodas',
      ja: 'ビハール語',
      zh: '比哈尔语',
      cs: 'Bihárské jazyky',
      ml: 'ബിഹാറി ഭാഷകൾ',
      et: 'Bihaari keel',
      bn: 'বিহারি ভাষাসমূহ',
      ug: 'بىھارى تىلى',
      ko: '비하르어',
      is: 'Bíharísk tungumál',
      uk: 'Біхарі'
    }
  },
  'en:southern-ndebele': {
    wikidata: {
      en: 'Q36785'
    },
    name: {
      br: 'Ndebeleg',
      ko: '남은데벨레어',
      zu: 'IsiNdebele',
      tr: 'Güney Ndebele dili',
      uk: 'Південна ндебеле',
      ja: '南ンデベレ語',
      kv: 'Лунвыв ндебеле',
      az: 'Cənubi Ndebele dili',
      pl: 'Język ndebele południowy',
      gd: 'Ndebele (cànan)',
      af: 'Suid-Ndebele',
      lv: 'Isindebele',
      vi: 'Tiếng Nam Ndebele',
      sw: 'Kindebele',
      mk: 'Јужен ндебеле',
      ru: 'Южный ндебеле',
      yo: 'Èdè Ndebele Apágúúsù',
      nl: 'Zuid-Ndebele',
      en: 'Southern Ndebele',
      pt: 'Língua ndebele',
      eo: 'Suda Ndebela lingvo',
      fr: 'Nrebele',
      es: 'Ndebele del Sur',
      io: 'Suda-Ndebele-linguo',
      gl: 'Lingua ndebele',
      sq: 'Gjuha ndebele',
      he: 'נדבלה דרומית',
      it: 'Ndebele del sud',
      sv: 'Sydndebele',
      ar: 'لغة نديبلي جنوبية',
      eu: 'Hegoaldeko ndebelera',
      nb: 'Sørndebele',
      nn: 'Sørndebele',
      de: 'Süd-Ndebele',
      ve: 'IsiNdebele',
      bg: 'Ндебеле',
      fi: 'Etelä-ndebelen kieli'
    },
    language_code_3: {
      en: 'nbl'
    },
    language_code_2: {
      en: 'nr'
    }
  },
  'en:bokmal': {
    name: {
      uk: 'Букмол',
      is: 'Bókmál',
      ce: 'Букмолхойн мотт',
      hy: 'Նորվեգերեն',
      li: 'Bokmål',
      ko: '보크몰',
      os: 'Букмол',
      br: 'Bokmål',
      ps: 'بوکمول',
      sk: 'Bokmål',
      az: 'Bukmol',
      et: 'Bokmål',
      cs: 'Bokmål',
      ro: 'Bokmål',
      zh: '書面挪威語',
      ja: 'ブークモール',
      lv: 'Būkmols',
      vi: 'Bokmål',
      ms: 'Bokmål',
      ta: 'பூக்மோல் மொழி',
      af: 'Bokmål',
      hr: 'Bokmål',
      lb: 'Bokmål',
      pl: 'Bokmål',
      lt: 'Bokmål',
      da: 'Bokmål',
      nl: 'Bokmål',
      fa: 'بوکمال',
      ku: 'Bokmål',
      ru: 'Букмол',
      ka: 'ბუკმოლი',
      pt: 'Bokmål',
      eo: 'Bokmål',
      cy: 'Bokmål',
      en: 'Bokmål',
      he: 'נורבגית ספרותית',
      ca: 'Bokmål',
      pa: 'ਬੂਕਮਾਲ ਨਾਰਵੇਜਿਅਨ',
      gl: 'Bokmål',
      es: 'Bokmål',
      fr: 'Bokmål',
      fy: 'Bokmål',
      ar: 'بوكمول',
      sv: 'Bokmål',
      sl: 'Bokmål',
      it: 'Bokmål',
      id: 'Bokmål',
      dv: 'ބޯކިމާލު',
      kk: 'Букмол тілі',
      be: 'Букмал',
      fi: 'Kirjanorja',
      oc: 'Bokmål',
      bg: 'Букмол',
      hu: 'Bokmål',
      de: 'Bokmål',
      nb: 'Bokmål',
      nn: 'Bokmål',
      eu: 'Bokmål'
    },
    language_code_3: {
      en: 'nob'
    },
    wikidata: {
      en: 'Q25167'
    },
    language_code_2: {
      en: 'nb'
    }
  },
  'en:komi': {
    language_code_2: {
      en: 'kv'
    },
    name: {
      cv: 'Коми чĕлхи',
      en: 'Komi',
      mr: 'कोमी भाषा',
      eo: 'Komia lingvo',
      pt: 'Língua komi',
      fr: 'Komi',
      gl: 'Lingua komi',
      es: 'Komi',
      io: 'Komi-linguo',
      ca: 'Komi',
      gv: 'Komish',
      th: 'ภาษาโคมิ',
      tt: 'Коми теле',
      ba: 'Коми-зырян теле',
      it: 'Comi',
      sv: 'Komi',
      ar: 'لغة كومية',
      eu: 'Komiera',
      nb: 'Syrjensk',
      nn: 'Komi',
      de: 'Komi',
      hu: 'Komi nyelv',
      oc: 'Komi',
      bg: 'Коми',
      fi: 'Komin kieli',
      kk: 'Коми тілі',
      br: 'Komieg-zirieg',
      os: 'Коми',
      ko: '코미어',
      ce: 'Комийн мотт',
      tr: 'Komice',
      uk: 'Комі',
      ja: 'コミ語',
      zh: '科米語',
      cs: 'Komi',
      se: 'Komigiella',
      et: 'Komi keel',
      az: 'Komi dili',
      kv: 'Коми кыв',
      sk: 'Komijčina',
      lt: 'Komių kalba',
      pl: 'Język komi',
      af: 'Komi',
      ta: 'கோமி மொழி',
      mk: 'Коми',
      ru: 'Коми',
      fa: 'زبان کومی',
      nl: 'Zurjeens'
    },
    language_code_3: {
      en: 'kom'
    },
    wikidata: {
      en: 'Q36126'
    }
  },
  'en:modern-greek': {
    language_code_2: {
      en: 'el'
    },
    wikidata: {
      en: 'Q36510'
    },
    language_code_3: {
      en: 'gre'
    },
    name: {
      ru: 'Новогреческий язык',
      mk: 'Новогрчки јазик',
      nl: 'Nieuwgrieks',
      da: 'Nygræsk',
      hr: 'Novogrčki jezik',
      pl: 'Język nowogrecki',
      lv: 'Jaungrieķu valoda',
      ro: 'Limba greacă modernă',
      cs: 'Novořečtina',
      zh: '现代希腊语',
      sk: 'Novogréčtina',
      ko: '현대 그리스어',
      hy: 'Նոր հունարեն',
      uk: 'Новогрецька мова',
      de: 'Neugriechisch',
      nb: 'Nygresk',
      fo: 'Nýgrikskt mál',
      be: 'Новагрэчаская мова',
      fi: 'Nykykreikka',
      it: 'Greco moderno',
      hi: 'आधुनिक यूनानी भाषा',
      om: 'Afaan Giriik',
      sv: 'Nygrekiska',
      ca: 'Grec demòtic',
      fr: 'Grec moderne',
      gl: 'Grego moderno',
      es: 'Griego moderno',
      he: 'יוונית מודרנית',
      el: 'Νέα ελληνική γλώσσα',
      ln: 'Ligreki lya lɛlɔ́',
      tl: 'Modernong Griyego',
      la: 'Lingua Neograeca',
      pt: 'Grego moderno',
      en: 'Modern Greek'
    }
  },
  'en:fijian-language': {
    wikidata: {
      en: 'Q33295'
    },
    name: {
      de: 'Fidschi',
      nn: 'Fijiansk',
      nb: 'Fijiansk',
      eu: 'Fijiera',
      be: 'Фіджыйская мова',
      fi: 'Fidžin kieli',
      bg: 'Фиджийски език',
      it: 'Figiano',
      qu: 'Phiyi simi',
      id: 'Bahasa Fiji',
      ar: 'لغة فيجية',
      sv: 'Fijianska',
      hi: 'फ़िजीयाई भाषा',
      ca: 'Fijià',
      es: 'Fiyiano',
      io: 'Fidjiana linguo',
      fr: 'Fidjien',
      he: "פיג'ית",
      th: 'ภาษาฟิจิ',
      eo: 'Fiĝia lingvo',
      pt: 'Língua fidjiana',
      cy: 'Ffijïeg',
      mr: 'फिजीयन भाषा',
      en: 'Fijian language',
      fa: 'زبان فیجیایی',
      ru: 'Фиджийский язык',
      mk: 'Фиџиски јазик',
      da: 'Rerevaka na Kalou ka Doka na Tui',
      nl: 'Fijisch',
      hr: 'Fidžijski jezik',
      pl: 'Język fidżyjski',
      lt: 'Fidžių kalba',
      vi: 'Tiếng Fiji',
      lv: 'Fidžiešu valoda',
      ta: 'விசிய மொழி',
      cs: 'Fidžijština',
      zh: '斐濟語',
      ja: 'フィジー語',
      rw: 'Igifijiyani',
      kv: 'Фиджи кыв',
      ko: '피지어',
      uk: 'Фіджійська мова',
      tr: 'Fiji dili',
      na: 'Dorerin Bidji',
      is: 'Fídjíska'
    },
    language_code_3: {
      en: 'fij'
    },
    language_code_2: {
      en: 'fj'
    }
  },
  'en:zulu': {
    wikidata: {
      en: 'Q10179'
    },
    name: {
      sq: 'Gjuha zulu',
      fr: 'Zoulou',
      es: 'Zulú',
      gl: 'Lingua zulú',
      ss: 'SíZulu',
      pa: 'ਜ਼ੁਲੂ ਭਾਸ਼ਾ',
      ca: 'Zulu',
      mr: 'इसिझुलू',
      en: 'Zulu',
      bg: 'Зулуски език',
      fo: 'Zulu mál',
      eu: 'Zuluera',
      nb: 'Zulu',
      de: 'IsiZulu',
      fy: 'Sûlû',
      so: 'Af-Zulu',
      it: 'Zulu',
      qu: 'Zulu simi',
      et: 'Suulu keel',
      az: 'Zulu dili',
      ps: 'زولو',
      cs: 'Zulština',
      se: 'Zulugiella',
      tr: 'Zuluca',
      uk: 'Зулу',
      bn: 'জুলু ভাষা',
      ug: 'زۇلۇچە',
      zu: 'IsiZulu',
      da: 'Zulu',
      mk: 'Зулу',
      bo: 'ཙོ་ལུའུའི་སྐད།',
      an: 'Idioma zulú',
      fa: 'زبان زولو',
      af: 'Zoeloe',
      gd: 'Zulu',
      lv: 'Zulu valoda',
      lt: 'Zulų kalba',
      pl: 'Język zulu',
      hr: 'Zulu jezik',
      el: 'Ζουλού γλώσσα',
      he: 'זולו',
      kg: 'Kizulu',
      ga: 'An tSúlúis',
      cv: 'Зулу',
      cy: 'Swlw',
      la: 'Lingua Zuluana',
      pt: 'Língua zulu',
      uz: 'Zulu',
      eo: 'Zulua lingvo',
      oc: 'Zulu',
      be: 'Зулу',
      fi: 'Zulu',
      nn: 'Språket zulu',
      ve: 'IsiZulu',
      sv: 'Zulu',
      hi: 'ज़ुलु भाषा',
      ar: 'لغة زولوية',
      sh: 'Zulu jezik',
      id: 'Bahasa Zulu',
      kv: 'Зулу',
      tn: 'Sezulu',
      ja: 'ズールー語',
      zh: '祖鲁语',
      ro: 'Limba zulu',
      sr: 'Зулу језик',
      kw: 'Zoulou',
      br: 'Zouloueg',
      ko: '줄루어',
      nl: 'Zoeloe',
      sw: 'Kizulu',
      ru: 'Зулу',
      yo: 'Èdè Zulu',
      xh: 'IsiZulu',
      ta: 'சுலு மொழி',
      vi: 'Tiếng Zulu',
      ms: 'Bahasa Zulu'
    },
    language_code_3: {
      en: 'zul'
    },
    language_code_2: {
      en: 'zu'
    }
  },
  'en:ido': {
    wikidata: {
      en: 'Q35224'
    },
    name: {
      li: 'Ido',
      hy: 'Իդո',
      ko: '이도',
      zh: '伊多語',
      ja: 'イド語',
      ff: 'Ido',
      sr: 'Идо',
      ro: 'Ido',
      sk: 'Ido',
      kv: 'Идо',
      ia: 'Ido',
      vi: 'Tiếng Ido',
      ms: 'Bahasa Ido',
      ta: 'இத்தோ மொழி',
      sw: 'Kiido',
      ku: 'Ido',
      ru: 'Идо',
      nl: 'Ido',
      pt: 'Ido',
      ka: 'იდო',
      eo: 'Ido',
      la: 'Ido',
      io: 'Ido',
      ga: 'Ido',
      el: 'Ίντο',
      he: 'אידו',
      sl: 'Ido',
      id: 'Bahasa Ido',
      vo: 'Ido',
      hi: 'ईदो',
      sv: 'Ido',
      ar: 'لغة إيدو',
      sh: 'Ido',
      nn: 'Ido',
      mt: 'Ido',
      kk: 'Идо',
      be: 'Іда',
      fi: 'Ido',
      oc: 'Ido',
      uk: 'Ідо',
      tr: 'İdo dili',
      is: 'Ido',
      cs: 'Ido',
      et: 'Ido',
      ha: 'Ido',
      az: 'İdo dili',
      pl: 'Ido',
      lt: 'Ido',
      lb: 'Ido',
      hr: 'Ido',
      af: 'Ido',
      lv: 'Ido',
      rm: 'Ido',
      mk: 'Идо',
      fa: 'زبان ایدو',
      an: 'Ido',
      da: 'Ido',
      en: 'Ido',
      mr: 'इदो',
      gl: 'Ido',
      es: 'Ido',
      fr: 'Ido',
      ay: 'Idu aru',
      ca: 'Ido',
      ss: 'Sí-Ido',
      pa: 'ਈਡੋ',
      th: 'ภาษาอิดอ',
      sq: 'Gjuha ido',
      sc: 'Ido',
      ie: 'Ido',
      qu: 'Ido',
      it: 'Ido',
      so: 'Ido',
      fy: 'Ido',
      ab: 'Идо',
      nb: 'Ido',
      eu: 'Ido',
      de: 'Ido',
      ur: 'ایڈو',
      bg: 'Идо',
      hu: 'Ido'
    },
    language_code_3: {
      en: 'ido'
    },
    language_code_2: {
      en: 'io'
    }
  },
  'en:khmer': {
    name: {
      bh: 'ख्मेर भाषा',
      ta: 'கெமர் மொழி',
      vi: 'Tiếng Khmer',
      lv: 'Khmeru valoda',
      ms: 'Bahasa Khmer',
      af: 'Khmer',
      lt: 'Khmerų kalba',
      pl: 'Język khmerski',
      nl: 'Khmer',
      da: 'Khmer',
      ru: 'Кхмерский язык',
      fa: 'زبان خمر',
      mk: 'Кмерски јазик',
      tr: 'Khmerce',
      uk: 'Кхмерська мова',
      mg: 'Fiteny kimera',
      ko: '크메르어',
      ug: 'كخمېر تىلى',
      br: 'Kmereg',
      bn: 'খ্‌মের ভাষা',
      kv: 'Кхмер кыв',
      az: 'Kxmer dili',
      et: 'Khmeeri keel',
      ro: 'Limba khmeră',
      sr: 'Кмерски језик',
      cs: 'Khmerština',
      km: 'ភាសាខ្មែរ',
      ja: 'クメール語',
      zh: '高棉语',
      wa: 'Xhmer',
      sh: 'Kmerski jezik',
      ar: 'لغة خميرية',
      sv: 'Khmer',
      hi: 'खमेर भाषा',
      id: 'Bahasa Khmer',
      qu: 'Khimir simi',
      it: 'Khmer',
      bg: 'Кхмерски език',
      oc: 'Khmer',
      hu: 'Khmer nyelv',
      fi: 'Khmerin kieli',
      be: 'Кхмерская мова',
      de: 'Khmer-Sprache',
      ur: 'خمیر زبان',
      eu: 'Khmerera',
      nb: 'Khmer',
      nn: 'Khmer',
      la: 'Lingua Chmerica',
      cy: 'Chmereg',
      eo: 'Kmera lingvo',
      pt: 'Língua khmer',
      en: 'Khmer',
      mr: 'ख्मेर भाषा',
      tl: 'Wikang Khmer',
      el: 'Χμερ γλώσσα',
      th: 'ภาษาเขมร',
      pa: 'ਖਮੇਰ ਭਾਸ਼ਾ',
      ca: 'Llengua khmer',
      fr: 'Khmer',
      ga: 'An Chiméiris',
      es: 'Camboyano',
      gl: 'Lingua khmer'
    },
    language_code_3: {
      en: 'khm'
    },
    wikidata: {
      en: 'Q9205'
    },
    language_code_2: {
      en: 'km'
    }
  },
  'en:sanskrit': {
    language_code_2: {
      en: 'sa'
    },
    wikidata: {
      en: 'Q11059'
    },
    name: {
      da: 'Sanskrit',
      an: 'Sanscrito',
      fa: 'زبان سانسکریت',
      mk: 'Санскрит',
      si: 'සංස්කෘත',
      bo: 'ལེགས་སྦྱར་སྐད།',
      lv: 'Sanskrits',
      af: 'Sanskrit',
      ks: 'سَنٛسکرِت زَبان',
      hr: 'Sanskrt',
      lt: 'Sanskritas',
      pl: 'Sanskryt',
      az: 'Sanskrit',
      ps: 'سانسګریت ژبه',
      et: 'Sanskriti keel',
      cs: 'Sanskrt',
      is: 'Sanskrít',
      tr: 'Sanskritçe',
      uk: 'Санскрит',
      ce: 'Санскрит',
      bn: 'সংস্কৃত ভাষা',
      dv: 'ސަންސްކްރިއްތް',
      hu: 'Szanszkrit',
      bg: 'Санскрит',
      am: 'ሳንስክሪት',
      ur: 'سنسکرت',
      de: 'Sanskrit',
      eu: 'Sanskrito',
      nb: 'Sanskrit',
      fy: 'Sanskryt',
      it: 'Sanscrito',
      qu: 'Sanskrit simi',
      su: 'Basa Sangsakerta',
      th: 'ภาษาสันสกฤต',
      pa: 'ਸੰਸਕ੍ਰਿਤ ਭਾਸ਼ਾ',
      ca: 'Sànscrit',
      fr: 'Sanskrit',
      gl: 'Lingua sánscrita',
      es: 'Sánscrito',
      mr: 'संस् कृत भाषा',
      en: 'Sanskrit',
      tl: 'Wikang Sanskrito',
      za: 'Vahfan',
      nl: 'Sanskriet',
      ru: 'Санскрит',
      ku: 'Sanskrît',
      sw: 'Kisanskrit',
      bh: 'संस्कृत',
      ta: 'சமசுகிருதம்',
      vi: 'Tiếng Phạn',
      ms: 'Sanskrit',
      kn: 'ಸಂಸ್ಕೃತ',
      ia: 'Sanscrito',
      sa: 'संस्कृतम्',
      ne: 'संस्कृत',
      kv: 'Санскрит',
      rw: 'Igisansikiriti',
      sk: 'Sanskrit',
      ml: 'സംസ്കൃതം',
      ro: 'Limba sanscrită',
      yi: 'סאנסקריט',
      kw: 'Sanskrytek',
      sr: 'Санскрит',
      km: 'សំស្ក្រឹត',
      ja: 'サンスクリット',
      gn: 'Sánskrito',
      mn: 'Санскрит хэл',
      zh: '梵语',
      mg: 'Sanskrity',
      ko: '산스크리트어',
      gu: 'સંસ્કૃત ભાષા',
      li: 'Sanskriet',
      hy: 'Սանսկրիտ',
      br: 'Sañskriteg',
      oc: 'Sanscrit',
      fi: 'Sanskrit',
      kk: 'Санскрит',
      be: 'Санскрыт',
      nn: 'Sanskrit',
      ar: 'لغة سنسكريتية',
      sh: 'Sanskrit',
      vo: 'Sanskrit',
      sv: 'Sanskrit',
      hi: 'संस्कृत भाषा',
      my: 'သင်္သကရိုက်ဘာသာ',
      ba: 'Санскрит',
      id: 'Bahasa Sanskerta',
      or: 'ସଂସ୍କୃତ ଭାଷା',
      sl: 'Sanskrt',
      tt: 'Санскрит',
      jv: 'Basa Sangskreta',
      te: 'సంస్కృతము',
      he: 'סנסקריט',
      as: 'সংস্কৃত',
      el: 'Σανσκριτική γλώσσα',
      io: 'Sanskrita linguo',
      ga: 'An tSanscrait',
      la: 'Lingua Sanscrita',
      cy: 'Sansgrit',
      uz: 'Sanskrit',
      pt: 'Sânscrito',
      ka: 'სანსკრიტი',
      eo: 'Sanskrito',
      bs: 'Sanskrit'
    },
    language_code_3: {
      en: 'san'
    }
  },
  'en:macedonian': {
    language_code_3: {
      en: 'mkd'
    },
    name: {
      en: 'Macedonian',
      mr: 'मॅसिडोनियन भाषा',
      fr: 'Macédonien',
      es: 'Macedonio',
      gl: 'Lingua macedonia',
      ca: 'Macedònic',
      gv: 'Massadoanish',
      sq: 'Gjuha sllavomaqedone',
      th: 'ภาษามาซิโดเนีย',
      qu: 'Makidunya simi',
      it: 'Macedone',
      eu: 'Mazedoniera',
      nb: 'Makedonsk',
      de: 'Mazedonisch',
      ur: 'میسیڈونیائی زبان',
      bg: 'Македонска литературна норма',
      hu: 'Macedón',
      ug: 'ماكېدون تىلى',
      ce: 'Македонийн мотт',
      tr: 'Makedonca',
      is: 'Makedónska',
      uk: 'Македонська мова',
      cs: 'Makedonština',
      se: 'Makedoniagiella',
      et: 'Makedoonia keel',
      az: 'Makedon dili',
      lt: 'Makedonų kalba',
      pl: 'Język macedoński',
      lb: 'Mazedonesch',
      hr: 'Makedonski jezik',
      af: 'Macedonies',
      lv: 'Maķedoniešu valoda',
      mk: 'Македонски јазик',
      tg: 'Забони мақдунӣ',
      an: 'Idioma macedonián',
      fa: 'زبان مقدونی',
      da: 'Makedonsk',
      bs: 'Makedonski jezik',
      cv: 'Македон чĕлхи',
      la: 'Lingua Macedonica',
      uz: 'Makedon tili',
      pt: 'Língua macedônia',
      ka: 'მაკედონური ენა',
      eo: 'Makedona lingvo',
      io: 'Macedoniana linguo',
      ga: 'An Mhacadóinis',
      el: 'Σλαβομακεδονική γλώσσα',
      he: 'מקדונית',
      id: 'Bahasa Makedonia',
      ba: 'Македон теле',
      sl: 'Makedonščina',
      sv: 'Makedonska',
      hi: 'मेसिडोनियन भाषा',
      sh: 'Makedonski jezik',
      ar: 'لغة مقدونية',
      nn: 'Makedonsk',
      oc: 'Macedonian',
      be: 'Македонская мова',
      fi: 'Makedonian kieli',
      br: 'Makedoneg',
      ko: '마케도니아어',
      li: 'Macedonisch',
      hy: 'Մակեդոներեն',
      cu: 'Макєдоньскъ ѩꙁꙑкъ',
      gn: "Masendoñañe'ẽ",
      ja: 'マケドニア語',
      zh: '马其顿语',
      mn: 'Македон хэл',
      ro: 'Limba macedoneană',
      sr: 'Македонски језик',
      kw: 'Makedonek',
      kv: 'Македон кыв',
      rw: 'Ikimasedoniyani',
      sk: 'Macedónčina',
      ta: 'மக்கதோனிய மொழி',
      vi: 'Tiếng Macedonia',
      ms: 'Bahasa Macedonia',
      ru: 'Македонский язык',
      ku: 'Zimanê makedonî',
      nl: 'Macedonisch'
    },
    wikidata: {
      en: 'Q9296'
    },
    language_code_2: {
      en: 'mk'
    }
  },
  'en:abkhaz': {
    name: {
      li: 'Abchazisch',
      hy: 'Աբխազերեն',
      ko: '압하지야어',
      br: "Abc'hazeg",
      sr: 'Абхаски језик',
      ro: 'Limba abhază',
      zh: '阿布哈茲語',
      ja: 'アブハズ語',
      rw: 'Icyabukaziyani',
      sk: 'Abcházština',
      kv: 'Абхаз кыв',
      tk: 'Abhaz dili',
      vi: 'Tiếng Abkhaz',
      ms: 'Bahasa Abkhaz',
      ta: 'அபுகாசிய மொழி',
      yo: 'Èdè Abkhaz',
      ru: 'Абхазский язык',
      nl: 'Abchazisch',
      uz: 'Abxaz tili',
      eo: 'Abĥaza lingvo',
      pt: 'Língua abecásia',
      ka: 'აფხაზური ენა',
      la: 'Lingua Abasca',
      cy: 'Abchaseg',
      cv: 'Абхаз чĕлхи',
      ga: 'An Abcáisis',
      he: 'אבחזית',
      el: 'Αμπχαζική γλώσσα',
      id: 'Bahasa Abkhaz',
      tt: 'Абхаз теле',
      sh: 'Abhaški jezik',
      ar: 'لغة أبخازية',
      sv: 'Abchaziska',
      nn: 'Abkhasisk',
      fi: 'Abhaasi',
      kk: 'Абхаз тілі',
      be: 'Абхазская мова',
      oc: 'Abkhaz',
      os: 'Абхазаг æвзаг',
      bn: 'আবখাজ ভাষা',
      uk: 'Абхазька мова',
      tr: 'Abhazca',
      is: 'Abkasíska',
      ce: 'Абхазхойн мотт',
      cs: 'Abcházština',
      ky: 'Абхаз тили',
      se: 'Abkhasagiella',
      az: 'Abxaz dili',
      et: 'Abhaasi keel',
      hr: 'Abhaski jezik',
      pl: 'Język abchaski',
      lt: 'Abchazų kalba',
      lv: 'Abhāzu valoda',
      af: 'Abchasies',
      fa: 'زبان آبخازی',
      an: 'Idioma abkhasio',
      mk: 'Апхаски јазик',
      en: 'Abkhaz',
      mr: 'अबखाझ भाषा',
      ca: 'Abkhaz',
      pa: 'ਆਬਖ਼ਾਜ਼ ਭਾਸ਼ਾ',
      gl: 'Lingua abkhaza',
      es: 'Abjasio',
      fr: 'Abkhaze',
      th: 'ภาษาอับฮาเซีย',
      it: 'Abcaso',
      ab: 'Аҧсуа бызшәа',
      de: 'Abchasisch',
      ur: 'ابخاز زبان',
      nb: 'Abkhasisk',
      eu: 'Abkhazera',
      hu: 'Abház',
      bg: 'Абхазки език'
    }
  },
  'en:sotho': {
    wikidata: {
      en: 'Q34340'
    },
    name: {
      su: 'Basa Sotho',
      id: 'Bahasa Sesotho',
      qu: 'Sesotho simi',
      it: 'Sotho del sud',
      sv: 'Sesotho',
      hi: 'सिसोथो भाषा',
      ar: 'لغة سوتية',
      eu: 'Sothoera',
      nb: 'Sesotho',
      nn: 'Sotho',
      de: 'Sesotho',
      ve: 'SeSotho',
      bg: 'Сото',
      fi: 'Eteläsothon kieli',
      mr: 'सोथो भाषा',
      en: 'Sotho',
      pt: 'Língua soto do sul',
      eo: 'Sota lingvo',
      fr: 'Sotho du Sud',
      gl: 'Lingua sotho',
      es: 'Sesotho',
      ca: 'Sesotho',
      sq: 'Gjuha sesote',
      he: 'ססוטו',
      lt: 'Sesoto kalba',
      pl: 'Język sotho',
      hr: 'Južni sotho jezik',
      af: 'Suid-Sotho',
      xh: 'IsiSotho',
      ta: 'சோத்தோ மொழி',
      vi: 'Tiếng Sotho',
      mk: 'Сото',
      sw: 'Kisotho-Kusini',
      st: 'Sesotho',
      ru: 'Сесото',
      yo: 'Èdè Sotho Apágúúsù',
      fa: 'زبان سوتو',
      nl: 'Zuid-Sotho',
      da: 'Sesotho',
      br: 'Sothoeg',
      ko: '소토어',
      ug: 'سېسوتوچە',
      zu: 'IsiSuthu',
      is: 'Sótó',
      tr: 'Sotho dili',
      uk: 'Сесото',
      ja: 'ソト語',
      zh: '塞索托语',
      ro: 'Limba sotho',
      se: 'Sothogiella',
      cs: 'Sotština',
      et: 'Sotho keel',
      kv: 'Лунвыв сото',
      az: 'Soto dili'
    },
    language_code_3: {
      en: 'sot'
    },
    language_code_2: {
      en: 'st'
    }
  },
  'en:scottish-gaelic': {
    language_code_2: {
      en: 'gd'
    },
    name: {
      oc: 'Gaelic escocés',
      be: 'Шатландская гэльская мова',
      fi: 'Gaeli',
      nn: 'Skotsk-gælisk',
      hi: 'स्कॉटिश गैलिक',
      sv: 'Skotsk gäliska',
      ar: 'لغة غيلية إسكتلندية',
      sh: 'Škotski gelski jezik',
      tt: 'Шотланд теле',
      jv: 'Basa Gaelik Skotlandia',
      id: 'Bahasa Gaelik Skotlandia',
      sl: 'Škotska gelščina',
      el: 'Σκωτική Γαελική γλώσσα',
      he: 'גאלית סקוטית',
      io: 'Skota-Gaelana linguo',
      ga: 'Gaeilge na hAlban',
      cv: 'Шотланд чĕлхи',
      la: 'Lingua Scotica',
      cy: 'Gaeleg yr Alban',
      pt: 'Língua gaélica escocesa',
      ka: 'შოტლანდიური ენა',
      eo: 'Skotgaela lingvo',
      nl: 'Schots-Gaelisch',
      ru: 'Шотландский язык',
      ia: 'Lingua gaelic scote',
      ta: 'சுகாத்திசு கேலிக்கு',
      vi: 'Tiếng Gael Scotland',
      kv: 'Шотландса кыв',
      sk: 'Škótska gaelčina',
      ja: 'スコットランド・ゲール語',
      wa: 'Escôswès',
      zh: '苏格兰盖尔语',
      ro: 'Limba scoțiană',
      sr: 'Шкотски гелски језик',
      kw: 'Albanek',
      br: 'Gouezeleg Skos',
      ko: '스코틀랜드 게일어',
      li: 'Sjots Gaelic',
      hy: 'Գելական շոտլանդերեն',
      bg: 'Шотландски келтски език',
      hu: 'Skót gael nyelv',
      fo: 'Skotskt gæliskt',
      eu: 'Eskoziako gaelera',
      nb: 'Skotsk-gælisk',
      am: 'የስኮትላንድ ጋይሊክኛ',
      de: 'Schottisch-Gälisch',
      fy: 'Skotsk-Gaelysk',
      it: 'Gaelico scozzese',
      qu: 'Iskut kilta simi',
      gv: 'Gaelg Albinagh',
      sc: 'Limba gaelica scotzesa',
      fr: 'Gaélique écossais',
      es: 'Gaélico escocés',
      gl: 'Lingua gaélica escocesa',
      ca: 'Gaèlic escocès',
      en: 'Scottish Gaelic',
      mr: 'स्कॉटिश गेलिक भाषा',
      da: 'Skotsk gælisk',
      mk: 'Шкотски јазик',
      an: 'Idioma gaelico escocés',
      fa: 'گیلیک اسکاتلندی',
      af: 'Skots-Gaelies',
      gd: 'Gàidhlig na h-Alba',
      lv: 'Skotu gēlu valoda',
      lt: 'Škotų gėlų kalba',
      pl: 'Język gaelicki szkocki',
      hr: 'Škotski gaelski jezik',
      et: 'Gaeli keel',
      az: 'Şotland dili',
      cs: 'Skotská gaelština',
      se: 'Gaelagiella',
      ce: 'Гэлийн мотт',
      is: 'Gelíska',
      tr: 'İskoçça',
      uk: 'Шотландська гельська мова',
      bn: 'স্কট্‌স গ্যালিক ভাষা',
      ug: 'شوتلاندىيە گائېل تىلى',
      os: 'Шотландиаг æвзаг'
    },
    language_code_3: {
      en: 'gla'
    },
    wikidata: {
      en: 'Q9314'
    }
  },
  'en:marathi': {
    name: {
      th: 'ภาษามราฐี',
      gl: 'Lingua marata',
      es: 'Maratí',
      fr: 'Marathi',
      ca: 'Marathi',
      pa: 'ਮਰਾਠੀ ਭਾਸ਼ਾ',
      mr: 'मराठी भाषा',
      en: 'Marathi',
      hu: 'Maráthi',
      bg: 'Маратхи',
      dv: 'މަރާޓީ',
      nb: 'Marathi',
      eu: 'Marathera',
      ur: 'مراٹھی زبان',
      de: 'Marathi',
      it: 'Marathi',
      qu: 'Marathi simi',
      et: 'Marathi keel',
      az: 'Marathi dili',
      cs: 'Maráthština',
      uk: 'Маратхі',
      tr: 'Marathi',
      is: 'Marathí',
      bn: 'মারাঠি ভাষা',
      ug: 'ماراتىچە',
      da: 'Marathi',
      mk: 'Марати',
      fa: 'زبان مراتی',
      af: 'Marathi',
      lv: 'Marathu valoda',
      pl: 'Język marathi',
      lt: 'Maračių kalba',
      hr: 'Marathi jezik',
      el: 'Μαράτι',
      he: 'מראטהי',
      io: 'Marata linguo',
      eo: 'Marata lingvo',
      pt: 'Marata',
      ka: 'მარათჰი',
      la: 'Lingua marathica',
      cy: 'Marathi',
      fi: 'Marathin kieli',
      be: 'Маратхі',
      oc: 'Marathi',
      nn: 'Marathi',
      hi: 'मराठी भाषा',
      sv: 'Marathi',
      sh: 'Marati jezik',
      ar: 'لغة مراثية',
      te: 'మరాఠీ భాష',
      or: 'ମରାଠୀ ଭାଷା',
      id: 'Bahasa Marathi',
      ml: 'മറാഠി',
      sk: 'Máráthčina',
      kv: 'Маратхи',
      zh: '马拉地语',
      ja: 'マラーティー語',
      sr: 'Маратхи језик',
      ro: 'Limba marathi',
      br: 'Marateg',
      gu: 'મરાઠી ભાષા',
      hy: 'Մարաթհի',
      ko: '마라티어',
      nl: 'Marathi',
      sw: 'Kimarathi',
      ku: 'Zimanê marathî',
      ru: 'Маратхи',
      sa: 'मराठी भाषा',
      vi: 'Tiếng Marathi',
      ms: 'Bahasa Marathi',
      kn: 'ಮರಾಠಿ',
      ta: 'மராத்தி',
      ne: 'मराठी भाषा'
    },
    language_code_3: {
      en: 'mar'
    },
    wikidata: {
      en: 'Q1571'
    },
    language_code_2: {
      en: 'mr'
    }
  },
  'en:nauruan': {
    name: {
      az: 'Nauru dili',
      kv: 'Науру кыв',
      et: 'Nauru keel',
      cs: 'Nauruština',
      ja: 'ナウル語',
      zh: '瑙鲁语',
      tr: 'Nauruca',
      na: 'Dorerin Naoero',
      uk: 'Науруанська мова',
      ko: '나우루어',
      hy: 'Նաուրուերեն',
      br: 'Naoeroeg',
      nl: 'Nauruaans',
      ru: 'Науруанский язык',
      fa: 'زبان نائورویی',
      sw: 'Kinauru',
      mk: 'Науруански јазик',
      rm: 'Lingua nauruaisa',
      ta: 'நவூரு மொழி',
      ms: 'Bahasa Nauru',
      vi: 'Tiếng Nauru',
      lv: 'Nauruiešu valoda',
      ia: 'Lingua nauruan',
      lt: 'Nauriečių kalba',
      pl: 'Język naurański',
      he: 'נאורית',
      th: 'ภาษานาอูรู',
      el: 'Ναουρού',
      ca: 'Nauruà',
      fr: 'Nauruan',
      es: 'Nauruano',
      gl: 'Lingua nauruana',
      pt: 'Língua nauruana',
      eo: 'Naura lingvo',
      ka: 'ნაურუული ენა',
      en: 'Nauruan',
      oc: 'Nauruan',
      bg: 'Науруански език',
      fi: 'Naurun kieli',
      am: 'ናውሩኛ',
      de: 'Nauruische Sprache',
      eu: 'Nauruera',
      nb: 'Naurisk',
      nn: 'Naurisk',
      ar: 'لغة ناورونية',
      sv: 'Nauruanska',
      id: 'Bahasa Nauru',
      it: 'Nauruano'
    },
    language_code_3: {
      en: 'nau'
    },
    wikidata: {
      en: 'Q13307'
    },
    language_code_2: {
      en: 'na'
    }
  },
  'en:oromo': {
    name: {
      oc: 'Oromo',
      hu: 'Oromo nyelv',
      fi: 'Oromon kieli',
      am: 'ኦሮምኛ',
      de: 'Oromo',
      eu: 'Oromoera',
      nn: 'Oromo',
      nb: 'Oromo',
      ar: 'لغة الأورومو',
      sv: 'Oromo',
      om: 'Afaan Oromoo',
      so: 'Af-Oromo',
      it: 'Oromo',
      qu: 'Oromo simi',
      he: 'אורומו',
      pa: 'ਓਰੋਮ ਭਾਸ਼ਾ',
      ca: 'Oromo',
      fr: 'Oromo',
      io: 'Oromo linguo',
      es: 'Oromo',
      ka: 'ორომო',
      uz: 'Oromo tili',
      pt: 'Língua oromo',
      eo: 'Oroma lingvo',
      mr: 'ओरोमो भाषा',
      en: 'Oromo',
      nl: 'Afaan Oromo',
      da: 'Oromo',
      ru: 'Оромо',
      fa: 'زبان اورومو',
      ta: 'ஒரோமோ மொழி',
      af: 'Oromo',
      hr: 'Oromo jezici',
      lt: 'Oromų kalba',
      pl: 'Język oromo',
      kv: 'Оромо',
      rw: 'Icyoromo',
      se: 'Oromogiella',
      cs: 'Oromština',
      ja: 'オロモ語',
      zh: '奧羅莫語',
      uk: 'Оромо',
      ko: '오로모어',
      br: 'Yezhoù oromoek',
      bn: 'অরমো ভাষা'
    },
    language_code_3: {
      en: 'orm'
    },
    wikidata: {
      en: 'Q33864'
    },
    language_code_2: {
      en: 'om'
    }
  },
  'en:welsh': {
    language_code_2: {
      en: 'cy'
    },
    wikidata: {
      en: 'Q9309'
    },
    language_code_3: {
      en: 'wel'
    },
    name: {
      ia: 'Lingua gallese',
      ta: 'வேல்சு மொழி',
      ms: 'Bahasa Wales',
      vi: 'Tiếng Wales',
      nl: 'Welsh',
      sw: 'Kiwelisi',
      ru: 'Валлийский язык',
      br: 'Kembraeg',
      ko: '웨일스어',
      li: 'Welsh',
      hy: 'Վալլիերեն',
      kv: 'Кӧмри кыв',
      sk: 'Waleština',
      ja: 'ウェールズ語',
      zh: '威尔士语',
      wa: 'Walès',
      ro: 'Limba galeză',
      sr: 'Велшки језик',
      kw: 'Kembrek',
      hi: 'वेल्श भाषा',
      vo: 'Velsänapük',
      sv: 'Kymriska',
      ar: 'لغة ويلزية',
      sh: 'Velški jezik',
      id: 'Bahasa Wales',
      sl: 'Valižanščina',
      oc: 'Galés',
      fi: 'Kymri',
      be: 'Валійская мова',
      nn: 'Walisisk',
      cv: 'Валли чĕлхи',
      cy: 'Cymraeg',
      la: 'Lingua Cambrica',
      pt: 'Língua galesa',
      uz: 'Valliy tili',
      eo: 'Kimra lingvo',
      ka: 'ვალიური ენა',
      bs: 'Velški jezik',
      el: 'Ουαλική γλώσσα',
      he: 'ולשית',
      io: 'Walsana linguo',
      ga: 'An Bhreatnais',
      af: 'Wallies',
      gd: 'Cuimris',
      lv: 'Velsiešu valoda',
      lt: 'Valų kalba',
      pl: 'Język walijski',
      lb: 'Walisesch',
      hr: 'Velški jezik',
      da: 'Walisisk',
      mk: 'Велшки јазик',
      tg: 'Забони уелсӣ',
      rm: 'Lingua valisica',
      an: 'Idioma galés',
      fa: 'زبان ویلزی',
      is: 'Velska',
      tr: 'Galce',
      na: 'Dorerin Wales',
      uk: 'Валлійська мова',
      bn: 'ওয়েলশ ভাষা',
      ug: 'ۋېلش تىلى',
      os: 'Валлийаг æвзаг',
      lo: 'ພາສາເວລ',
      et: 'Kõmri keel',
      az: 'Vall dili',
      se: 'Kymragiella',
      cs: 'Velština',
      fy: 'Welsk',
      it: 'Gallese',
      qu: 'Kamri simi',
      bg: 'Уелски език',
      hu: 'Walesi',
      fo: 'Walisiskt mál',
      eu: 'Gales',
      nb: 'Walisisk',
      am: 'ዌልስኛ',
      de: 'Walisische Sprache',
      en: 'Welsh',
      mr: 'वेल्श भाषा',
      tl: 'Wikang Gales',
      sq: 'Gjuha Uelshe',
      gv: 'Bretnish',
      th: 'ภาษาเวลส์',
      sc: 'Limba gallesa',
      fr: 'Gallois',
      gl: 'Lingua galesa',
      es: 'Galés',
      ca: 'Gal·lès'
    }
  },
  'en:vietnamese': {
    language_code_2: {
      en: 'vi'
    },
    wikidata: {
      en: 'Q9199'
    },
    language_code_3: {
      en: 'vie'
    },
    name: {
      hu: 'Vietnami',
      bg: 'Виетнамски език',
      de: 'Vietnamesische Sprache',
      nb: 'Vietnamesisk',
      eu: 'Vietnamera',
      fy: 'Fjetnameesk',
      qu: 'Witnam simi',
      it: 'Vietnamita',
      th: 'ภาษาเวียดนาม',
      sq: 'Gjuha vietnameze',
      ca: 'Vietnamita',
      pa: 'ਵਿਅਤਨਾਮੀ ਭਾਸ਼ਾ',
      es: 'Vietnamita',
      gl: 'Lingua vietnamita',
      fr: 'Vietnamien',
      mr: 'व्हियेतनामी भाषा',
      en: 'Vietnamese',
      sm: "Fa'aViatename",
      tl: 'Wikang Biyetnames',
      da: 'Vietnamesisk',
      fa: 'زبان ویتنامی',
      tg: 'Забони ветнамӣ',
      mk: 'Виетнамски јазик',
      lv: 'Vjetnamiešu valoda',
      af: 'Viëtnamees',
      hr: 'Vijetnamski jezik',
      pl: 'Język wietnamski',
      lt: 'Vietnamiečių kalba',
      az: 'Vyetnam dili',
      et: 'Vietnami keel',
      cs: 'Vietnamština',
      uk: "В'єтнамська мова",
      is: 'Víetnamska',
      tr: 'Vietnamca',
      ce: 'Вьетнамхой мотт',
      ug: 'ۋيېتنام تىلى',
      bn: 'ভিয়েতনামীয় ভাষা',
      kk: 'Вьетнам тілі',
      fi: 'Vietnamin kieli',
      be: "В'етнамская мова",
      oc: 'Việt',
      nn: 'Vietnamesisk',
      sh: 'Vijetnamski jezik',
      ar: 'لغة فيتنامية',
      vo: 'Vietnamänapük',
      sv: 'Vietnamesiska',
      hi: 'वियतनामी भाषा',
      ba: 'Вьетнам теле',
      id: 'Bahasa Vietnam',
      jv: 'Basa Vietnam',
      he: 'וייטנאמית',
      el: 'Βιετναμέζικη γλώσσα',
      kl: 'Vietnamimiusut',
      io: 'Vietnamana linguo',
      ga: 'An Vítneaimis',
      ka: 'ვიეტნამური ენა',
      uz: 'Vyetnam tili',
      pt: 'Língua vietnamita',
      eo: 'Vjetnama lingvo',
      cy: 'Fietnameg',
      la: 'Lingua Vietnamica',
      za: 'Vah Yiednamz',
      nl: 'Vietnamees',
      yo: 'Èdè Fiẹtnám',
      ru: 'Вьетнамский язык',
      mi: 'Reo Whitinamu',
      vi: 'Tiếng Việt',
      ms: 'Bahasa Vietnam',
      kn: 'ವಿಯೆಟ್ನಾಮಿನ ಭಾಷೆ',
      ta: 'வியட்நாமிய மொழி',
      sa: 'वियतनामी भाषा',
      ne: 'भियतनामी भाषा',
      kv: 'Въет кыв',
      ml: 'വിയറ്റ്നാമീസ്',
      sr: 'Вијетнамски језик',
      yi: 'וויעטנאמעזיש',
      ro: 'Limba vietnameză',
      zh: '越南语',
      mn: 'Вьетнам хэл',
      ja: 'ベトナム語',
      hy: 'Վիետնամերեն',
      ko: '베트남어',
      br: 'Vietnameg'
    }
  },
  'en:bislama': {
    language_code_2: {
      en: 'bi'
    },
    language_code_3: {
      en: 'bis'
    },
    name: {
      nb: 'Bislama',
      nn: 'Bislama',
      eu: 'Bislama',
      de: 'Bislama',
      ht: 'Bichlamar',
      fo: 'Bislama mál',
      fi: 'Bislama',
      be: 'Біслама',
      bg: 'Бислама',
      oc: 'Bislama',
      it: 'Bislama',
      id: 'Bahasa Bislama',
      sv: 'Bislama',
      ar: 'لغة بسلاما',
      sh: 'Bislama jezik',
      es: 'Bislama',
      gl: 'Bislama',
      io: 'Bislama linguo',
      fr: 'Bichelamar',
      ca: 'Bislama',
      th: 'ภาษาบิสลามา',
      el: 'Μπισλάμα',
      he: 'ביסלמה',
      sm: 'Gagana Bislama',
      en: 'Bislama',
      eo: 'Bislama lingvo',
      pt: 'Língua bislamá',
      cy: 'Bislama',
      la: 'Bislama',
      mk: 'Бислама',
      fa: 'زبان بیسلاما',
      ru: 'Бислама',
      nl: 'Bislama',
      pl: 'Język bislama',
      lt: 'Bislama',
      hr: 'Bislama jezik',
      bi: 'Bislama',
      vi: 'Bislama',
      lv: 'Bislama',
      ms: 'Bahasa Bislama',
      ta: 'பிசுலாமா மொழி',
      zh: '比斯拉马语',
      ja: 'ビスラマ語',
      cs: 'Bislamština',
      sr: 'Бислама',
      et: 'Bislama keel',
      sk: 'Bislamčina',
      kv: 'Бислама',
      br: 'Bislama',
      ko: '비슬라마',
      uk: 'Біслама'
    },
    wikidata: {
      en: 'Q35452'
    }
  },
  'en:somali': {
    name: {
      yo: 'Èdè Somalí',
      ru: 'Сомалийский язык',
      sw: 'Kisomali',
      nl: 'Somalisch',
      vi: 'Tiếng Somali',
      ms: 'Bahasa Somali',
      ta: 'சோமாலி மொழி',
      sr: 'Сомалски језик',
      ro: 'Limba somaleză',
      zh: '索馬里語',
      ja: 'ソマリ語',
      kv: 'Сомали кыв',
      ml: 'സൊമാലി ഭാഷ',
      ko: '소말리어',
      br: 'Somalieg',
      nn: 'Somali',
      fi: 'Somalin kieli',
      be: 'Самалійская мова',
      oc: 'Somali',
      id: 'Bahasa Somali',
      jv: 'Basa Somali',
      sh: 'Somalski jezik',
      ar: 'لغة صومالية',
      sv: 'Somaliska',
      hi: 'सोमाली भाषा',
      io: 'Somaliana linguo',
      he: 'סומלית',
      pt: 'Língua somali',
      ka: 'სომალი ენა',
      eo: 'Somala lingvo',
      la: 'Lingua Somalica',
      fa: 'زبان سومالیایی',
      mk: 'Сомалиски јазик',
      da: 'Somali',
      hr: 'Somalski jezik',
      pl: 'Język somalijski',
      lt: 'Somalių kalba',
      lv: 'Somāļu valoda',
      af: 'Somalies',
      cs: 'Somálština',
      ky: 'Сомали тили',
      az: 'Somali dili',
      et: 'Somaali keel',
      bn: 'সোমালি ভাষা',
      uk: 'Сомалійська мова',
      tr: 'Somalice',
      ce: 'Сомалийн мотт',
      de: 'Somali',
      ur: 'صومالی زبان',
      am: 'ሶማልኛ',
      nb: 'Somali',
      eu: 'Somaliera',
      bg: 'Сомалийски език',
      hu: 'Szomáli nyelv',
      it: 'Somalo',
      qu: 'Sumali simi',
      so: 'Af-Soomaali',
      ca: 'Somali',
      es: 'Somalí',
      gl: 'Lingua somalí',
      fr: 'Somali',
      th: 'ภาษาโซมาลี',
      en: 'Somali',
      mr: 'सोमाली भाषा'
    },
    language_code_3: {
      en: 'som'
    },
    wikidata: {
      en: 'Q13275'
    },
    language_code_2: {
      en: 'so'
    }
  },
  'en:lithuanian': {
    language_code_2: {
      en: 'lt'
    },
    wikidata: {
      en: 'Q9083'
    },
    name: {
      ta: 'இலித்துவானிய மொழி',
      vi: 'Tiếng Litva',
      ms: 'Bahasa Lithuania',
      sa: 'लेतुवाभाषा',
      ru: 'Литовский язык',
      yo: 'Èdè Lituéníà',
      ku: 'Zimanê lîtvanî',
      sw: 'Kilituanya',
      mi: 'Reo Rituānia',
      nl: 'Litouws',
      ko: '리투아니아어',
      hy: 'Լիտվերեն',
      li: 'Litouws',
      br: 'Lituaneg',
      mg: 'Fiteny litoanianina',
      ro: 'Limba lituaniană',
      yi: 'ליטוויש',
      sr: 'Литвански језик',
      kw: 'Lithywanek',
      ja: 'リトアニア語',
      gn: "Lituañañe'ẽ",
      zh: '立陶宛语',
      mn: 'Литва хэл',
      kv: 'Литва кыв',
      rw: 'Ikilituwaniya',
      sk: 'Litovčina',
      ml: 'ലിത്വാനിയൻ ഭാഷ',
      id: 'Bahasa Lituavi',
      sl: 'Litovščina',
      tt: 'Литвалылар теле',
      ar: 'لغة لتوانية',
      sh: 'Litvanski jezik',
      vo: 'Lietuvänapük',
      sv: 'Litauiska',
      hi: 'लिथुआनियाई भाषा',
      nn: 'Litauisk',
      oc: 'Lituanian',
      be: 'Літоўская мова',
      kk: 'Литва тілі',
      fi: 'Liettua',
      bs: 'Litvanski jezik',
      la: 'Lingua Lithuanica',
      cy: 'Lithwaneg',
      pt: 'Língua lituana',
      uz: 'Litva tili',
      eo: 'Litova lingvo',
      ka: 'ლიტვური ენა',
      cv: 'Литва чĕлхи',
      kl: 'Litauenimiutut',
      ga: 'An Liotuáinis',
      io: 'Lituaniana linguo',
      he: 'ליטאית',
      el: 'Λιθουανική γλώσσα',
      lb: 'Litauesch',
      hr: 'Litavski jezik',
      lt: 'Lietuvių kalba',
      pl: 'Język litewski',
      lv: 'Lietuviešu valoda',
      gd: 'Liotuànais',
      af: 'Litaus',
      an: 'Idioma lituán',
      fa: 'زبان لیتوانیایی',
      mk: 'Литвански јазик',
      tg: 'Забони литвонӣ',
      da: 'Litauisk',
      ug: 'لىتۋا تىلى',
      os: 'Литоваг æвзаг',
      zu: 'IsiLithuwaniya',
      bn: 'লিথুয়ানীয় ভাষা',
      na: 'Dorerin Rituainiya',
      tr: 'Litvanca',
      is: 'Litháíska',
      uk: 'Литовська мова',
      ce: 'Литвахойн мотт',
      ky: 'Литва тили',
      se: 'Liettuvagiella',
      cs: 'Litevština',
      az: 'Litva dili',
      et: 'Leedu keel',
      it: 'Lituano',
      qu: 'Lituwa simi',
      fy: 'Litousk',
      am: 'ሊትዌንኛ',
      de: 'Litauische Sprache',
      eu: 'Lituaniera',
      nb: 'Litauisk',
      bg: 'Литовски език',
      hu: 'Litván',
      fo: 'Litaviskt mál',
      tl: 'Wikang Litwano',
      sm: 'Gagana Lufiana',
      en: 'Lithuanian',
      mr: 'लिथुएनियन भाषा',
      ca: 'Lituà',
      fr: 'Lituanien',
      ay: 'Lituaña aru',
      gl: 'Lingua lituana',
      es: 'Lituano',
      sc: 'Limba lituana',
      sq: 'Gjuha lituane',
      gv: 'Litaanish',
      th: 'ภาษาลิทัวเนีย'
    },
    language_code_3: {
      en: 'lit'
    }
  },
  'en:haitian-creole': {
    language_code_2: {
      en: 'ht'
    },
    wikidata: {
      en: 'Q33491'
    },
    name: {
      vo: 'Haitiyänapük',
      hi: 'हैतियाई क्रियोल',
      sv: 'Haitisk kreol',
      ar: 'لغة كريولية هايتية',
      id: 'Bahasa Kreol Haiti',
      it: 'Creolo haitiano',
      bg: 'Хаитянски креолски език',
      hu: 'Haiti kreol nyelv',
      fi: 'Haitin kieli',
      eu: 'Haitiko kreolera',
      nn: 'Haitisk',
      nb: 'Haitisk',
      ht: 'Kreyòl',
      de: 'Haitianische Sprache',
      en: 'Haitian Creole',
      mr: 'हैतीयन क्रियोल',
      cy: 'Creol',
      la: 'Creolus Haitianus',
      pt: 'Crioulo haitiano',
      ka: 'ჰაიტიური კრეოლი',
      eo: 'Haitia kreola lingvo',
      bs: 'Haićanski kreolski jezik',
      he: 'קריאולית האיטית',
      fr: 'Haïtien',
      io: 'Haitiana kreolo',
      es: 'Criollo haitiano',
      gl: 'Crioulo haitiano',
      ca: 'Crioll haitià',
      ia: 'Lingua creol haitian',
      ta: 'ஐத்தி கிரியோல் மொழி',
      vi: 'Tiếng Haiti',
      lt: 'Haičio kreolų kalba',
      pl: 'Język kreolski haitański',
      hr: 'Haićanski kreolski jezik',
      nl: 'Haïtiaans Creools',
      da: 'Haitisk',
      ru: 'Гаитянский креольский язык',
      fa: 'زبان کریول آییسینی',
      mg: 'Kreôly haisianina',
      tr: 'Haiti Kreyolu',
      uk: 'Гаїтянська креольська мова',
      br: 'Kreoleg Haiti',
      bn: 'হাইতীয় ক্রেওল ভাষা',
      ko: '아이티어',
      ug: 'ھايتى كرېئول تىلى',
      kv: 'Гаити креол кыв',
      az: 'Haiti kreol dili',
      ja: 'ハイチ語',
      zh: '海地克里奧爾語',
      ro: 'Limba creolă haitiană',
      cs: 'Haitská kreolština'
    },
    language_code_3: {
      en: 'hat'
    }
  },
  'en:malagasy': {
    language_code_2: {
      en: 'mg'
    },
    language_code_3: {
      en: 'mlg'
    },
    name: {
      ia: 'Lingua malgache',
      ta: 'மலகசி மொழி',
      ms: 'Bahasa Malagasi',
      vi: 'Tiếng Malagasy',
      mi: 'Reo Malagasy',
      ru: 'Малагасийский язык',
      nl: 'Plateaumalagasi',
      br: 'Malagasieg',
      ko: '말라가시어',
      mg: 'Fiteny malagasy',
      ja: 'マダガスカル語',
      zh: '马拉加斯语',
      ro: 'Limba malgașă',
      sr: 'Малгашки језик',
      kv: 'Малагаси кыв',
      jv: 'Basa Malagasy',
      id: 'Bahasa Malagasi',
      vo: 'Malagaseänapük',
      sv: 'Malagassiska',
      hi: 'मालागासी भाषा',
      sh: 'Malagaški jezik',
      ar: 'لغة ملغاشية',
      nn: 'Gassisk',
      fi: 'Malagassi',
      be: 'Малагасійская мова',
      la: 'Lingua Malagasiana',
      pt: 'Língua malgaxe',
      uz: 'Malagasi tili',
      eo: 'Malagasa lingvo',
      ka: 'მალაგასიური ენა',
      io: 'Malagasi-linguo',
      ga: 'An Mhalagáisis',
      kl: 'Malagassiskisut',
      he: 'מלגשית',
      lt: 'Malagasių kalba',
      pl: 'Język malgaski',
      hr: 'Malgaški jezik',
      af: 'Malagasy',
      lv: 'Malgašu valoda',
      mk: 'Малгашки јазик',
      fa: 'زبان مالاگاسی',
      da: 'Malagassisk',
      bn: 'মালাগাসি ভাষা',
      is: 'Malagasíska',
      tr: 'Malgaşça',
      uk: 'Малагасійська мова',
      cs: 'Malgaština',
      ky: 'Малагаси тили',
      et: 'Malagassi keel',
      az: 'Malaqasi dili',
      qu: 'Malagasi simi',
      it: 'Malgascio',
      eu: 'Malgaxe',
      nb: 'Gassisk',
      am: 'መለጋሲ',
      ur: 'مالاگاسی',
      de: 'Malagasy',
      bg: 'Малгашки език',
      en: 'Malagasy',
      mr: 'मालागासी भाषा',
      fr: 'Malgache',
      es: 'Malgache',
      gl: 'Lingua malgaxe',
      ca: 'Malgaix',
      gv: 'Malagasish',
      sq: 'Gjuha madagaskare',
      th: 'ภาษามาลากาซี'
    },
    wikidata: {
      en: 'Q7930'
    }
  },
  'en:spanish': {
    language_code_2: {
      en: 'es'
    },
    wikidata: {
      en: 'Q1321'
    },
    name: {
      es: 'Español',
      gl: 'Lingua castelá',
      fr: 'Espagnol',
      ay: 'Kastilla aru',
      ss: 'Sipanishi',
      sq: 'Gjuha spanjolle',
      sc: 'Limba ispagnola',
      qu: 'Kastilla simi',
      it: 'Spagnolo',
      fy: 'Spaansk',
      nb: 'Spansk',
      eu: 'Gaztelania',
      de: 'Spanisch',
      am: 'እስፓንኛ',
      ht: 'Panyòl',
      bg: 'Испански език',
      hu: 'Spanyol',
      dv: 'އިސްޕެނިޝް',
      bn: 'স্পেনীয় ভাষা',
      os: 'Испайнаг æвзаг',
      ug: 'ئىسپان تىلى',
      tr: 'İspanyolca',
      se: 'Spánskagiella',
      cs: 'Španělština',
      ts: 'Xipaniya',
      pl: 'Język hiszpański',
      hr: 'Španjolski jezik',
      af: 'Spaans',
      mk: 'Шпански јазик',
      bs: 'Španski jezik',
      eo: 'Hispana lingvo',
      uz: 'Ispan tili',
      pt: 'Língua castelhana',
      la: 'Lingua Hispanica',
      io: 'Hispaniana linguo',
      ga: 'An Spáinnis',
      ee: 'Spangbe',
      kl: 'Spanskisut',
      el: 'Ισπανική γλώσσα',
      he: 'ספרדית',
      te: 'స్పానిష్ భాష',
      jv: 'Basa Spanyol',
      id: 'Bahasa Spanyol',
      vo: 'Spanyänapük',
      mt: 'Spanjol',
      fi: 'Espanja',
      br: 'Spagnoleg',
      co: 'Lingua spagnola',
      zh: '西班牙语',
      mn: 'Испани хэл',
      km: 'ភាសាអេស្ប៉ាញ',
      gn: "Karaiñe'ẽ",
      sr: 'Шпански језик',
      kw: 'Spaynek',
      ro: 'Limba spaniolă',
      sk: 'Španielčina',
      rw: 'Icyesipanyole',
      kv: 'Испан кыв',
      no: 'Spansk',
      ne: 'स्पेनी भाषा',
      tk: 'Ispan dili',
      sa: 'स्पैनिश भाषा',
      kn: 'ಸ್ಪ್ಯಾನಿಷ್ ಭಾಷೆ',
      ta: 'எசுப்பானியம்',
      yo: 'Èdè Spéìn',
      bm: 'Espankan',
      ku: 'Zimanê spanî',
      ru: 'Испанский язык',
      tl: 'Wikang Kastila',
      sm: 'Gagana spaniolo',
      ln: 'Lispanyoli',
      en: 'Spanish',
      mr: 'स्पॅनिश भाषा',
      ca: 'Castellà',
      pa: 'ਸਪੈਨਿਸ਼ ਭਾਸ਼ਾ',
      th: 'ภาษาสเปน',
      gv: 'Spaainish',
      su: 'Basa Spanyol',
      ur: 'ہسپانوی زبان',
      fo: 'Spanskt mál',
      ty: 'Reo Paniora',
      zu: 'IsiSpanish',
      ce: 'Испанхойн мотт',
      uk: 'Іспанська мова',
      na: 'Dorerin Pain',
      is: 'Spænska',
      ky: 'Испан тили',
      et: 'Hispaania keel',
      lo: 'ພາສາເອັສປາໂຍນ',
      ps: 'هسپاني',
      az: 'İspan dili',
      lt: 'Ispanų kalba',
      lb: 'Spuenesch',
      gd: 'Spàinntis',
      lv: 'Spāņu valoda',
      rm: 'Lingua spagnola',
      tg: 'Забони испанӣ',
      bo: 'སེ་པན་སྐད།',
      fa: 'زبان اسپانیایی',
      an: 'Idioma castellán',
      iu: 'ᓯᐸᐃᓂᑎᑐᑦ',
      da: 'Spansk',
      cv: 'Испан чĕлхи',
      ka: 'ესპანური ენა',
      cy: 'Sbaeneg',
      kg: 'Kispanya',
      tt: 'Испан теле',
      sl: 'Španščina',
      ba: 'Испан теле',
      sv: 'Spanska',
      hi: 'स्पेनिश भाषा',
      sh: 'Španski jezik',
      ar: 'لغة إسبانية',
      nn: 'Spansk',
      kk: 'Испан тілі',
      be: 'Іспанская мова',
      oc: 'Espanhòu',
      hy: 'Իսպաներեն',
      li: 'Castiliaans',
      ko: '스페인어',
      mg: 'Fiteny espaniola',
      wa: 'Espagnol',
      ja: 'スペイン語',
      yi: 'שפאניש',
      ml: 'സ്പാനിഷ്‌ ഭാഷ',
      nv: 'Naakaii bizaad',
      ch: 'Españót',
      ia: 'Lingua espaniol',
      ms: 'Bahasa Sepanyol',
      vi: 'Tiếng Tây Ban Nha',
      mi: 'Reo Pāniora',
      sw: 'Kihispania',
      nl: 'Spaans'
    },
    language_code_3: {
      en: 'spa'
    }
  },
  'en:danish': {
    wikidata: {
      en: 'Q9035'
    },
    name: {
      uk: 'Данська мова',
      is: 'Danska',
      tr: 'Danca',
      ce: 'Датхойн мотт',
      ug: 'دانىش تىلى',
      os: 'Даниаг æвзаг',
      bn: 'ডেনীয় ভাষা',
      az: 'Danimarka dili',
      et: 'Taani keel',
      ky: 'Дания тили',
      se: 'Dánskkagiella',
      cs: 'Dánština',
      lv: 'Dāņu valoda',
      gd: 'Danmhairgis',
      af: 'Deens',
      lb: 'Dänesch',
      hr: 'Danski jezik',
      pl: 'Język duński',
      lt: 'Danų kalba',
      da: 'Dansk',
      fa: 'زبان دانمارکی',
      an: 'Idioma danés',
      tg: 'Забони даниягӣ',
      mk: 'Дански јазик',
      en: 'Danish',
      mr: 'डॅनिश भाषा',
      tl: 'Wikang Danes',
      sm: "Fa'atenimaka",
      sc: 'Limba danesa',
      th: 'ภาษาเดนมาร์ก',
      sq: 'Gjuha daneze',
      gv: 'Danvargish',
      ca: 'Danès',
      pa: 'ਡੈਨਿਸ਼ ਭਾਸ਼ਾ',
      gl: 'Lingua dinamarquesa',
      es: 'Danés',
      fr: 'Danois',
      fy: 'Deensk',
      it: 'Danese',
      qu: 'Dan simi',
      fo: 'Danskt mál',
      hu: 'Dán',
      bg: 'Датски език',
      de: 'Dänisch',
      am: 'ዳንኛ',
      nb: 'Dansk',
      eu: 'Daniera',
      co: 'Lingua danese',
      hy: 'Դանիերեն',
      li: 'Deens',
      ko: '덴마크어',
      br: 'Daneg',
      sk: 'Dánčina',
      kv: 'Данск кыв',
      kw: 'Danek',
      sr: 'Дански језик',
      yi: 'דעניש',
      ro: 'Limba daneză',
      zh: '丹麦语',
      wa: 'Daenwès',
      ja: 'デンマーク語',
      km: 'ភាសាដាណឺម៉ាក',
      vi: 'Tiếng Đan Mạch',
      ms: 'Bahasa Denmark',
      ta: 'டேனிய மொழி',
      ia: 'Lingua danese',
      nl: 'Deens',
      ku: 'Zimanê danîmarkî',
      ru: 'Датский язык',
      sw: 'Kidenmark',
      pt: 'Língua dinamarquesa',
      eo: 'Dana lingvo',
      ka: 'დანიური ენა',
      la: 'Lingua Danica',
      cy: 'Daneg',
      bs: 'Danski jezik',
      he: 'דנית',
      el: 'Δανική γλώσσα',
      kl: 'Qallunaatut',
      io: 'Daniana linguo',
      ga: 'An Danmhairgis',
      sh: 'Danski jezik',
      ar: 'لغة دنماركية',
      sv: 'Danska',
      vo: 'Danänapük',
      hi: 'डेनिश भाषा',
      sl: 'Danščina',
      ba: 'Дат теле',
      id: 'Bahasa Denmark',
      jv: 'Basa Denmark',
      tt: 'Дания теле',
      fi: 'Tanska',
      kk: 'Дат тілі',
      be: 'Дацкая мова',
      oc: 'Danés',
      nn: 'Dansk'
    },
    language_code_3: {
      en: 'dan'
    },
    language_code_2: {
      en: 'da'
    }
  },
  'en:slovene': {
    language_code_3: {
      en: 'slv'
    },
    name: {
      he: 'סלובנית',
      el: 'Σλοβενική γλώσσα',
      io: 'Sloveniana linguo',
      ga: 'An tSlóivéinis',
      eo: 'Slovena lingvo',
      uz: 'Sloven tili',
      pt: 'Língua eslovena',
      ka: 'სლოვენური ენა',
      la: 'Lingua Slovena',
      cy: 'Slofeneg',
      cv: 'Словен чĕлхи',
      bs: 'Slovenski jezik',
      fi: 'Sloveeni',
      be: 'Славенская мова',
      oc: 'Eslovèn',
      mt: 'Sloven',
      nn: 'Slovensk',
      sh: 'Slovenski jezik',
      ar: 'لغة سلوفينية',
      sv: 'Slovenska',
      vo: 'Sloveniyänapük',
      hi: 'स्लोवेनियाई भाषा',
      sl: 'Slovenščina',
      id: 'Bahasa Slovenia',
      sk: 'Slovinčina',
      kv: 'Словен кыв',
      kw: 'Slovenek',
      sr: 'Словеначки језик',
      ro: 'Limba slovenă',
      yi: 'סלאוועניש',
      zh: '斯洛文尼亚语',
      ja: 'スロベニア語',
      gn: "Eloveñañe'ẽ",
      cu: 'Їллѷрїискословѣньскъ ѩꙁꙑкъ',
      co: 'Lingua sluvena',
      li: 'Sloveens',
      hy: 'Սլովեներեն',
      ko: '슬로베니아어',
      br: 'Sloveneg',
      nl: 'Sloveens',
      ku: 'Zimanê slovenî',
      ru: 'Словенский язык',
      vi: 'Tiếng Slovene',
      ms: 'Bahasa Slovene',
      ta: 'சுலோவேனிய மொழி',
      ia: 'Lingua sloven',
      sc: 'Limba islovena',
      gv: 'Sloveanish',
      sq: 'Gjuha sllovene',
      ca: 'Eslovè',
      es: 'Esloveno',
      gl: 'Lingua eslovena',
      fr: 'Slovène',
      en: 'Slovene',
      mr: 'स्लोव्हेन भाषा',
      sm: 'Gagana Slovene',
      fo: 'Slovenskt mál',
      bg: 'Словенски език',
      hu: 'Szlovén',
      de: 'Slowenisch',
      am: 'ስሎቬንኛ',
      nb: 'Slovensk',
      eu: 'Esloveniera',
      fy: 'Sloveensk',
      qu: 'Isluwinya simi',
      it: 'Sloveno',
      az: 'Sloven dili',
      et: 'Sloveeni keel',
      cs: 'Slovinština',
      se: 'Slovenagiella',
      uk: 'Словенська мова',
      is: 'Slóvenska',
      tr: 'Slovence',
      ce: 'Словенийн мотт',
      ug: 'سلوۋېن تىلى',
      bn: 'স্লোভেনীয় ভাষা',
      da: 'Slovensk',
      fa: 'زبان اسلونیایی',
      an: 'Idioma esloveno',
      tg: 'Забони словенӣ',
      mk: 'Словенечки јазик',
      lv: 'Slovēņu valoda',
      gd: 'Slòbhainis',
      af: 'Sloweens',
      hr: 'Slovenski jezik',
      lb: 'Slowenesch',
      pl: 'Język słoweński',
      lt: 'Slovėnų kalba'
    },
    wikidata: {
      en: 'Q9063'
    },
    language_code_2: {
      en: 'sl'
    }
  },
  'en:icelandic': {
    wikidata: {
      en: 'Q294'
    },
    language_code_3: {
      en: 'ice'
    },
    name: {
      ce: 'Исландхойн мотт',
      tr: 'İzlandaca',
      is: 'Íslenska',
      uk: 'Ісландська мова',
      bn: 'আইসল্যান্ডীয় ভাষা',
      os: 'Исландиаг æвзаг',
      ug: 'ئىسلاندىيە تىلى',
      zu: 'Isi-Icelandic',
      et: 'Islandi keel',
      az: 'İsland dili',
      cs: 'Islandština',
      se: 'Islánddagiella',
      af: 'Yslands',
      lv: 'Islandiešu valoda',
      lt: 'Islandų kalba',
      pl: 'Język islandzki',
      hr: 'Islandski jezik',
      lb: 'Islännesch',
      da: 'Islandsk',
      mk: 'Исландски јазик',
      tg: 'Забони исландӣ',
      an: 'Idioma islandés',
      fa: 'زبان ایسلندی',
      mr: 'इस्लेन्स्का भाषा',
      en: 'Icelandic',
      sq: 'Gjuha islandeze',
      gv: 'Eeslynnish',
      th: 'ภาษาไอซ์แลนด์',
      sc: 'Limba islandesa',
      fr: 'Islandais',
      es: 'Islandés',
      gl: 'Lingua islandesa',
      ca: 'Islandès',
      fy: 'Yslânsk',
      it: 'Islandese',
      qu: 'Islandya simi',
      bg: 'Исландски език',
      hu: 'Izlandi',
      fo: 'Íslendskt mál',
      eu: 'Islandiera',
      nb: 'Islandsk',
      de: 'Isländisch',
      co: 'Lingua islandesa',
      br: 'Islandeg',
      ko: '아이슬란드어',
      li: 'Ieslandjs',
      hy: 'Իսլանդերեն',
      ml: 'ഐസ്‌ലാന്റിക് ഭാഷ',
      kv: 'Исленска кыв',
      sk: 'Islandčina',
      ja: 'アイスランド語',
      km: 'ភាសាអាយឡែន',
      zh: '冰岛语',
      ro: 'Limba islandeză',
      kw: 'Islandek',
      sr: 'Исландски језик',
      ta: 'இசுலேன்சுக மொழி',
      ms: 'Bahasa Iceland',
      vi: 'Tiếng Iceland',
      ne: 'अइसल्यान्डेली',
      nl: 'IJslands',
      ru: 'Исландский язык',
      cv: 'Исланд чĕлхи',
      cy: 'Islandeg',
      la: 'Lingua Islandica',
      ka: 'ისლანდიური ენა',
      pt: 'Língua islandesa',
      eo: 'Islanda lingvo',
      bs: 'Islandski jezik',
      el: 'Ισλανδική γλώσσα',
      he: 'איסלנדית',
      io: 'Islandana linguo',
      ga: 'An Íoslainnis',
      kl: 'Islandimiusut',
      hi: 'आइसलैण्डिक भाषा',
      vo: 'Lisladeänapük',
      sv: 'Isländska',
      sh: 'Islandski jezik',
      ar: 'لغة آيسلندية',
      tt: 'Исланд теле',
      id: 'Bahasa Islan',
      sl: 'Islandščina',
      oc: 'Islandés',
      be: 'Ісландская мова',
      kk: 'Исланд тілі',
      fi: 'Islanti',
      nn: 'Islandsk'
    },
    language_code_2: {
      en: 'is'
    }
  },
  'en:estonian': {
    wikidata: {
      en: 'Q9072'
    },
    language_code_3: {
      en: 'est'
    },
    name: {
      sl: 'Estonščina',
      id: 'Bahasa Esti',
      ba: 'Эстон теле',
      tt: 'Эстон теле',
      ar: 'لغة إستونية',
      sh: 'Estonski jezik',
      vo: 'Lestiyänapük',
      hi: 'एस्टोनियाई भाषा',
      sv: 'Estniska',
      nn: 'Estisk',
      kk: 'Эстон тілі',
      fi: 'Viro',
      be: 'Эстонская мова',
      oc: 'Estonian',
      bs: 'Estonski jezik',
      ka: 'ესტონური ენა',
      uz: 'Eston tili',
      pt: 'Estónio',
      eo: 'Estona lingvo',
      la: 'Lingua Estonica',
      cy: 'Estoneg',
      cv: 'Эстон чĕлхи',
      kl: 'Estlandimiutut',
      io: 'Estoniana linguo',
      ga: 'An Eastóinis',
      he: 'אסטונית',
      el: 'Εσθονική γλώσσα',
      ms: 'Bahasa Estonia',
      vi: 'Tiếng Estonia',
      ta: 'எசுத்தோனிய மொழி',
      ku: 'Zimanê estonî',
      ru: 'Эстонский язык',
      nl: 'Estisch',
      li: 'Estisch',
      hy: 'Էստոներեն',
      ko: '에스토니아어',
      br: 'Estoneg',
      sr: 'Естонски језик',
      kw: 'Estonek',
      ro: 'Limba estonă',
      yi: 'עסטיש',
      wa: 'Estonyin',
      zh: '爱沙尼亚语',
      mn: 'Эстон хэл',
      km: 'ភាសាអេស្តូនី',
      ja: 'エストニア語',
      rw: 'Icyesitoniya',
      sk: 'Estónčina',
      kv: 'Эст кыв',
      mo: 'Лимбa ecтoнэ',
      ml: 'എസ്തോണിയൻ ഭാഷ',
      it: 'Estone',
      qu: 'Istunya simi',
      fy: 'Estlânsk',
      de: 'Estnische Sprache',
      am: 'ኤስቶንኛ',
      nb: 'Estisk',
      eu: 'Estoniera',
      fo: 'Estiskt mál',
      bg: 'Естонски език',
      hu: 'Észt',
      en: 'Estonian',
      mr: 'एस्टोनियन भाषा',
      ca: 'Estonià',
      pa: 'ਏਸਟੋਨਿਆਈ ਭਾਸ਼ਾ',
      gl: 'Lingua estoniana',
      es: 'Estonio',
      fr: 'Estonien',
      sc: 'Limba èstone',
      th: 'ภาษาเอสโตเนีย',
      sq: 'Gjuha estoneze',
      gv: 'Estoinish',
      hr: 'Estonski jezik',
      pl: 'Język estoński',
      lt: 'Estų kalba',
      lv: 'Igauņu valoda',
      af: 'Estnies',
      gd: 'Eastoinis',
      fa: 'زبان استونیایی',
      an: 'Idioma estón',
      tg: 'Забони эстонӣ',
      mk: 'Естонски јазик',
      da: 'Estisk',
      os: 'Эстойнаг æвзаг',
      ug: 'ئېستون تىلى',
      bn: 'এস্তোনীয় ভাষা',
      uk: 'Естонська мова',
      is: 'Eistneska',
      tr: 'Estonca',
      na: 'Dorerit Etoniya',
      ce: 'Эстонийн мотт',
      se: 'Esttegiella',
      ky: 'Эстон тили',
      cs: 'Estonština',
      az: 'Eston dili',
      et: 'Eesti keel'
    },
    language_code_2: {
      en: 'et'
    }
  },
  'en:wolof': {
    name: {
      kv: 'Волоф',
      et: 'Volofi keel',
      wo: 'Wolof',
      ja: 'ウォロフ語',
      wa: 'Wolof',
      zh: '沃洛夫語',
      uk: 'Волоф',
      mg: 'Fiteny wolof',
      ko: '월로프어',
      li: 'Wolof',
      br: 'Wolofeg',
      nl: 'Wolof',
      da: 'Wolof',
      ru: 'Волоф',
      fa: 'ولوف',
      yo: 'Èdè Wolof',
      mk: 'Волофски јазик',
      sw: 'Kiwolofu',
      ta: 'வோலோஃப் மொழி',
      vi: 'Tiếng Wolof',
      hr: 'Wolof jezik',
      lt: 'Volofų kalba',
      pl: 'Język wolof',
      sq: 'Gjuha volof',
      th: 'ภาษาโวลอฟ',
      ca: 'Wòlof',
      fr: 'Wolof',
      es: 'Wólof',
      gl: 'Lingua wólof',
      ga: 'An Volaifis',
      cy: 'Woloffeg',
      pt: 'Língua wolof',
      eo: 'Volofa lingvo',
      en: 'Wolof',
      oc: 'Wolòf',
      hu: 'Volof nyelv',
      bg: 'Волоф',
      fo: 'Wolof mál',
      fi: 'Wolofin kieli',
      ur: 'وولوف زبان',
      de: 'Wolof',
      eu: 'Wolofera',
      nb: 'Wolof',
      nn: 'Wolof',
      ar: 'لغة ولوفية',
      sv: 'Wolof',
      id: 'Bahasa Wolof',
      qu: 'Wolof simi',
      it: 'Wolof'
    },
    language_code_3: {
      en: 'wol'
    },
    wikidata: {
      en: 'Q34257'
    },
    language_code_2: {
      en: 'wo'
    }
  },
  'en:hiri-motu': {
    wikidata: {
      en: 'Q33617'
    },
    language_code_3: {
      en: 'hmo'
    },
    name: {
      en: 'Hiri Motu',
      pt: 'Hiri Motu',
      eo: 'Hirimotua lingvo',
      th: 'ภาษาฮีรีโมตู',
      he: 'הארי מוטו',
      ga: 'Hiri Motu',
      es: 'Hiri motu',
      fr: 'Hiri motu',
      sv: 'Hiri motu',
      ar: 'هيري موتو',
      su: 'Basa Hiri Motu',
      it: 'Hiri motu',
      id: 'Bahasa Hiri Motu',
      fi: 'Hiri motu',
      bg: 'Хири моту',
      hu: 'Hiri motu nyelv',
      nb: 'Hiri motu',
      nn: 'Hiri motu',
      eu: 'Hiri Motu',
      de: 'Hiri Motu',
      uk: 'Гірі-моту',
      br: 'Hirimotoueg',
      ko: '히리 모투',
      et: 'Hirimotu keel',
      az: 'Hiri-motu dili',
      kv: 'Хири-моту',
      zh: '希里摩圖語',
      ja: 'ヒリモツ語',
      cs: 'Hiri motu',
      sr: 'Хири моту',
      ro: 'Hiri motu',
      ms: 'Bahasa Hiri Motu',
      ta: 'இரிமோட்டு மொழி',
      pl: 'Hiri motu',
      lt: 'Hiri motu kalba',
      hr: 'Hiri Motu jezik',
      nl: 'Hiri Motu',
      mk: 'Хири моту',
      fa: 'زبان هیری موتو',
      ru: 'Хири-моту'
    },
    language_code_2: {
      en: 'ho'
    }
  },
  'en:tamil': {
    wikidata: {
      en: 'Q5885'
    },
    language_code_3: {
      en: 'tam'
    },
    name: {
      nn: 'Tamil',
      fi: 'Tamil',
      be: 'Тамільская мова',
      oc: 'Tamol',
      jv: 'Basa Tamil',
      te: 'తమిళ భాష',
      sl: 'Tamilščina',
      or: 'ତାମିଲ ଭାଷା',
      id: 'Bahasa Tamil',
      hi: 'तमिल भाषा',
      vo: 'Tamilänapük',
      sv: 'Tamil',
      ar: 'لغة تاملية',
      sh: 'Tamilski jezik',
      io: 'Tamil linguo',
      ga: 'An Tamailis',
      el: 'Ταμίλ γλώσσα',
      he: 'טמילית',
      bs: 'Tamilski jezik',
      uz: 'Tamil tili',
      eo: 'Tamila lingvo',
      pt: 'Língua tâmil',
      ka: 'ტამილური ენა',
      cy: 'Tamileg',
      la: 'Lingua Tamulica',
      sw: 'Kitamil',
      ku: 'Zimanê tamîlî',
      yo: 'Èdè Tàmil',
      ru: 'Тамильский язык',
      nl: 'Tamil',
      ne: 'तमिल भाषा',
      sa: 'तमिऴ्',
      vi: 'Tiếng Tamil',
      kn: 'ತಮಿಳು',
      ms: 'Bahasa Tamil',
      bh: 'तमिल',
      ta: 'தமிழ்',
      zh: '泰米尔语',
      gn: "Támil ñe'ẽ",
      ja: 'タミル語',
      sr: 'Тамилски језик',
      kw: 'Tamilek',
      yi: 'טאמיל',
      ro: 'Limba tamilă',
      ml: 'തമിഴ്',
      rw: 'Igitamili',
      kv: 'Тамил кыв',
      br: 'Tamileg',
      hy: 'Թամիլերեն',
      gu: 'તમિલ ભાષા',
      ko: '타밀어',
      mg: 'Fiteny tamily',
      nb: 'Tamilsk',
      eu: 'Tamilera',
      ur: 'تامل زبان',
      de: 'Tamil',
      hu: 'Tamil',
      bg: 'Тамилски език',
      dv: 'ތަމަޅަ',
      qu: 'Tamil simi',
      it: 'Tamil',
      so: 'Luqada Tamil-ka',
      gl: 'Lingua támil',
      es: 'Tamil',
      fr: 'Tamoul',
      ca: 'Tàmil',
      pa: 'ਤਾਮਿਲ ਭਾਸ਼ਾ',
      th: 'ภาษาทมิฬ',
      sq: 'Gjuha tamile',
      tl: 'Wikang Tamil',
      mr: 'तमिळ भाषा',
      en: 'Tamil',
      tg: 'Забони томилӣ',
      si: 'දෙමළ',
      mk: 'Тамилски јазик',
      fa: 'زبان تامیلی',
      an: 'Idioma tamil',
      da: 'Tamil',
      pl: 'Język tamilski',
      lt: 'Tamilų kalba',
      lb: 'Tamil',
      hr: 'Tamilski jezik',
      af: 'Tamil',
      gd: 'Tamailis',
      lv: 'Tamilu valoda',
      cs: 'Tamilština',
      et: 'Tamili keel',
      az: 'Tamil dili',
      bn: 'তামিল ভাষা',
      ug: 'تامىل تىلى',
      uk: 'Тамільська мова',
      tr: 'Tamilce',
      is: 'Tamílska'
    },
    language_code_2: {
      en: 'ta'
    }
  },
  'en:slovak': {
    wikidata: {
      en: 'Q9058'
    },
    name: {
      de: 'Slowakisch',
      nb: 'Slovakisk',
      eu: 'Eslovakiera',
      fo: 'Slovakiskt mál',
      hu: 'Szlovák',
      bg: 'Словашки език',
      it: 'Slovacco',
      qu: 'Isluwakya simi',
      fy: 'Slowaaksk',
      ca: 'Eslovac',
      pa: 'ਸਲੋਵਾਕ ਭਾਸ਼ਾ',
      gl: 'Lingua eslovaca',
      es: 'Eslovaco',
      fr: 'Slovaque',
      sc: 'Limba Islovaca',
      th: 'ภาษาสโลวัก',
      gv: 'Slovackish',
      sq: 'Gjuha sllovake',
      sm: "Fa'aSolovak",
      en: 'Slovak',
      mr: 'स्लोव्हाक भाषा',
      fa: 'زبان اسلواکیایی',
      an: 'Idioma eslovaco',
      tg: 'Забони словакӣ',
      mk: 'Словачки јазик',
      da: 'Slovakisk',
      lb: 'Slowakesch',
      hr: 'Slovački jezik',
      pl: 'Język słowacki',
      lt: 'Slovakų kalba',
      lv: 'Slovāku valoda',
      gd: 'Slòbhacais',
      af: 'Slowaaks',
      se: 'Slovákiagiella',
      cs: 'Slovenština',
      az: 'Slovak dili',
      et: 'Slovaki keel',
      ug: 'سلوۋاك تىلى',
      os: 'Словакаг æвзаг',
      bn: 'স্লোভাক ভাষা',
      uk: 'Словацька мова',
      tr: 'Slovakça',
      is: 'Slóvakíska',
      ce: 'Словакийн мотт',
      nn: 'Slovakisk',
      be: 'Славацкая мова',
      fi: 'Slovakki',
      oc: 'Eslovac',
      sl: 'Slovaščina',
      id: 'Bahasa Slowakia',
      jv: 'Basa Slowakia',
      sh: 'Slovački jezik',
      ar: 'لغة سلوفاكية',
      hi: 'स्लोवाक भाषा',
      vo: 'Slovakiyänapük',
      sv: 'Slovakiska',
      kl: 'Slovakimiusut',
      io: 'Slovakiana linguo',
      ga: 'An tSlóvaicis',
      he: 'סלובקית',
      el: 'Σλοβακική γλώσσα',
      bs: 'Slovački jezik',
      uz: 'Slovak tili',
      eo: 'Slovaka lingvo',
      pt: 'Língua eslovaca',
      ka: 'სლოვაკური ენა',
      la: 'Lingua Slovaca',
      cv: 'Словак чĕлхи',
      yo: 'Èdè Slofákíà',
      ku: 'Zimanê slovakî',
      ru: 'Словацкий язык',
      sw: 'Kislovakia',
      nl: 'Slowaaks',
      tk: 'Slowak dili',
      vi: 'Tiếng Slovak',
      ms: 'Bahasa Slovak',
      ta: 'சுலோவாக்கிய மொழி',
      sr: 'Словачки језик',
      kw: 'Slovakek',
      yi: 'סלאוואקיש',
      ro: 'Limba slovacă',
      zh: '斯洛伐克语',
      mn: 'Словак хэл',
      ja: 'スロバキア語',
      sk: 'Slovenčina',
      rw: 'Igisilovaki',
      kv: 'Словак кыв',
      li: 'Slowaaks',
      hy: 'Սլովակերեն',
      ko: '슬로바키아어',
      br: 'Slovakeg',
      cu: 'Карпатьскословѣньскъ ѩꙁꙑкъ',
      co: 'Lingua sluvacca'
    },
    language_code_3: {
      en: 'slk'
    },
    language_code_2: {
      en: 'sk'
    }
  },
  'en:herero': {
    language_code_2: {
      en: 'hz'
    },
    language_code_3: {
      en: 'her'
    },
    name: {
      pt: 'Hereró',
      eo: 'Herera lingvo',
      en: 'Herero',
      ca: 'Herero',
      fr: 'Héréro',
      es: 'Herero',
      th: 'ภาษาเฮเรโร',
      id: 'Bahasa Herero',
      it: 'Herero',
      ar: 'هيريرو',
      sv: 'Herero',
      hi: 'हीरीरो भाषा',
      de: 'Otjiherero',
      eu: 'Hereroera',
      nn: 'Herero',
      nb: 'Herero',
      fo: 'Herero mál',
      fi: 'Hereron kieli',
      hy: 'Հերերո լեզու',
      br: 'Herereg',
      tr: 'Herero dili',
      uk: 'Гереро',
      ro: 'Limba herero',
      cs: 'Herero',
      ja: 'ヘレロ語',
      az: 'Herero dili',
      kv: 'Гереро',
      rw: 'Igiherero',
      et: 'Hereero keel',
      hr: 'Herero jezik',
      lt: 'Hererų kalba',
      pl: 'Język herero',
      ta: 'எரேரோ மொழி',
      ms: 'Bahasa Herero',
      ia: 'Lingua herero',
      ru: 'Гереро',
      fa: 'زبان هررو',
      sw: 'Kiherero',
      nl: 'Herero'
    },
    wikidata: {
      en: 'Q33315'
    }
  },
  'en:italian': {
    wikidata: {
      en: 'Q652'
    },
    name: {
      fy: 'Italjaansk',
      so: 'Af-Taliyaani',
      qu: 'Italya simi',
      it: 'Italiano',
      su: 'Basa Italia',
      ty: "Reo 'Itāria",
      dv: 'އިޓަލީ',
      bg: 'Италиански език',
      hu: 'Olasz',
      fo: 'Italskt mál',
      ur: 'اطالوی زبان',
      am: 'ጣልያንኛ',
      de: 'Italienisch',
      eu: 'Italiera',
      nb: 'Italiensk',
      mr: 'इटालियन भाषा',
      en: 'Italian',
      tl: 'Wikang Italyano',
      sm: "Fa'aitaliani",
      tw: 'Italian',
      sc: 'Limba italiana',
      gv: 'Iddaalish',
      sq: 'Gjuha italiane',
      th: 'ภาษาอิตาลี',
      pa: 'ਇਤਾਲਵੀ ਭਾਸ਼ਾ',
      ca: 'Italià',
      fr: 'Italien',
      es: 'Italiano',
      gl: 'Lingua italiana',
      lv: 'Itāļu valoda',
      gd: 'Eadailtis',
      af: 'Italiaans',
      lb: 'Italienesch',
      hr: 'Talijanski jezik',
      lt: 'Italų kalba',
      pl: 'Język włoski',
      da: 'Italiensk',
      an: 'Idioma italián',
      fa: 'زبان ایتالیایی',
      mk: 'Италијански јазик',
      tg: 'Забони итолиявӣ',
      bo: 'ཨི་ཏ་ལིའི་སྐད།',
      rm: 'Lingua taliana',
      tr: 'İtalyanca',
      is: 'Ítalska',
      uk: 'Італійська мова',
      ce: 'Итальянийн мотт',
      os: 'Италиаг æвзаг',
      ug: 'ئىتاليان تىلى',
      zu: 'IsiTaliya',
      bn: 'ইতালীয় ভাষা',
      az: 'İtalyan dili',
      lo: 'ພາສາອິຕາລີ',
      et: 'Itaalia keel',
      se: 'Itáliagiella',
      ky: 'Италян тили',
      cs: 'Italština',
      sh: 'Italijanski jezik',
      ar: 'لغة إيطالية',
      hi: 'इतालवी भाषा',
      vo: 'Litaliyänapük',
      sv: 'Italienska',
      my: 'အီတာလျံ ဘာသာ',
      id: 'Bahasa Italia',
      sl: 'Italijanščina',
      tt: 'Итальян теле',
      jv: 'Basa Italia',
      oc: 'Italian',
      fi: 'Italia',
      kk: 'Италиян тілі',
      be: 'Італьянская мова',
      mt: 'Taljan',
      nn: 'Italiensk',
      la: 'Lingua Italiana',
      cy: 'Eidaleg',
      uz: 'Italyan tili',
      eo: 'Itala lingvo',
      pt: 'Italiano',
      ka: 'იტალიური ენა',
      cv: 'Итал чĕлхи',
      bs: 'Italijanski jezik',
      he: 'איטלקית',
      el: 'Ιταλική γλώσσα',
      ga: 'An Iodáilis',
      io: 'Italiana linguo',
      ta: 'இத்தாலிய மொழி',
      vi: 'Tiếng Ý',
      ms: 'Bahasa Itali',
      kn: 'ಇಟಲಿಯ ಭಾಷೆ',
      ia: 'Lingua italian',
      sa: 'इतालवी भाषा',
      ne: 'इटालेली',
      nl: 'Italiaans',
      ru: 'Итальянский язык',
      ku: 'Zimanê îtalî',
      sw: 'Kiitalia',
      mg: 'Fiteny italiana',
      co: 'Lingua taliana',
      ko: '이탈리아어',
      li: 'Italiaans',
      hy: 'Իտալերեն',
      br: 'Italianeg',
      kv: 'Итальян кыв',
      no: 'Italiensk',
      sk: 'Taliančina',
      ml: 'ഇറ്റാലിയൻ ഭാഷ',
      ro: 'Limba italiană',
      yi: 'איטאליעניש',
      kw: 'Italek',
      sr: 'Италијански језик',
      km: 'ភាសាអ៊ីតាលី',
      gn: "Italiañe'ẽ",
      ja: 'イタリア語',
      zh: '意大利语',
      mn: 'Итали хэл'
    },
    language_code_3: {
      en: 'ita'
    },
    language_code_2: {
      en: 'it'
    }
  },
  'en:irish': {
    language_code_2: {
      en: 'ga'
    },
    name: {
      fo: 'Írskt mál',
      hu: 'Ír',
      bg: 'Ирландски език',
      de: 'Irisch',
      am: 'አየርላንድኛ',
      nb: 'Irsk',
      eu: 'Irlandako gaelera',
      fy: 'Iersk',
      qu: 'Ilanda simi',
      it: 'Irlandese',
      sc: 'Gaelicu Irlandiesu',
      th: 'ภาษาไอริช',
      sq: 'Gjuha irlandeze',
      gv: 'Yernish',
      ca: 'Gaèlic irlandès',
      gl: 'Lingua irlandesa',
      es: 'Irlandés',
      fr: 'Irlandais',
      mr: 'आयरिश भाषा',
      en: 'Irish',
      tl: 'Wikang Irlandes',
      da: 'Irsk',
      fa: 'زبان ایرلندی',
      an: 'Idioma irlandés',
      rm: 'Lingua irlandaisa',
      tg: 'Забони ирландӣ',
      mk: 'Ирски јазик',
      lv: 'Īru valoda',
      gd: 'Gàidhlig na h-Èireann',
      af: 'Iers-Gaelies',
      hr: 'Irski jezik',
      pl: 'Język irlandzki',
      lt: 'Airių kalba',
      az: 'İrland dili',
      et: 'Iiri keel',
      se: 'Iirragiella',
      ky: 'Ирланд тили',
      cs: 'Irština',
      uk: 'Ірландська мова',
      tr: 'İrlandaca',
      na: 'Dorerin Airerand',
      is: 'Írska',
      ce: 'Ирландхойн мотт',
      os: 'Ирландиаг æвзаг',
      ug: 'ئرېلاندىيە تىلى',
      bn: 'আইরিশ ভাষা',
      fi: 'Iiri',
      be: 'Ірландская мова',
      oc: 'Irlandés',
      nn: 'Irsk',
      ar: 'لغة أيرلندية',
      sh: 'Irski jezik',
      vo: 'Lireyänapük',
      hi: 'आयरिश भाषा',
      sv: 'Iriska',
      sl: 'Irska gelščina',
      id: 'Bahasa Irlandia',
      tt: 'Ирланд теле',
      he: 'אירית',
      el: 'Ιρλανδική γλώσσα',
      ga: 'An Ghaeilge',
      io: 'Gaelana linguo',
      ka: 'ირლანდიური ენა',
      pt: 'Língua irlandesa',
      eo: 'Irlanda lingvo',
      la: 'Lingua Hibernica',
      cy: 'Gwyddeleg',
      cv: 'Ирланд чĕлхи',
      bs: 'Irski jezik',
      nl: 'Iers',
      yo: 'Irishi',
      ku: 'Zimanê îrlandî',
      ru: 'Ирландский язык',
      ms: 'Bahasa Ireland',
      vi: 'Tiếng Ireland',
      ta: 'ஐரிய மொழி',
      ia: 'Lingua irlandese',
      sk: 'Írčina',
      kv: 'Ирландса кыв',
      no: 'Irsk',
      ml: 'ഐറിഷ് ഭാഷ',
      kw: 'Wordhonek',
      sr: 'Ирски језик',
      yi: 'איריש',
      ro: 'Limba irlandeză',
      zh: '愛爾蘭語',
      ja: 'アイルランド語',
      li: 'Iers',
      hy: 'Իռլանդերեն',
      ko: '아일랜드어',
      br: 'Iwerzhoneg'
    },
    language_code_3: {
      en: 'gle'
    },
    wikidata: {
      en: 'Q9142'
    }
  },
  'en:shona': {
    language_code_2: {
      en: 'sn'
    },
    name: {
      oc: 'Shona',
      fi: 'Šonan kieli',
      be: 'Шона',
      de: 'Shona',
      nb: 'Shona',
      nn: 'Shona',
      ar: 'لغة الشونا',
      sv: 'Shona',
      id: 'Bahasa Shona',
      qu: 'Shona simi',
      it: 'Shona',
      he: 'שונה',
      kg: 'Kishona',
      el: 'Σόνα γλώσσα',
      ca: 'Xona',
      fr: 'Shona',
      es: 'Shona',
      eo: 'Ŝona lingvo',
      pt: 'Língua chona',
      mr: 'शोना भाषा',
      en: 'Shona',
      nl: 'Shona',
      ru: 'Шона',
      fa: 'زبان شونا',
      sw: 'Kishona',
      mk: 'Шона',
      ta: 'சோனா மொழி',
      af: 'Sjona',
      hr: 'Shona jezik',
      lt: 'Šonų kalba',
      pl: 'Język shona',
      kv: 'Шона',
      az: 'Şona dili',
      et: 'Šona keel',
      sr: 'Шона',
      ja: 'ショナ語',
      zh: '修納語',
      uk: 'Шонська мова',
      ug: 'شونا تىلى',
      sn: 'ChiShona',
      br: 'Choneg',
      bn: 'শোনা ভাষা'
    },
    language_code_3: {
      en: 'sna'
    },
    wikidata: {
      en: 'Q34004'
    }
  },
  'en:marshallese': {
    wikidata: {
      en: 'Q36280'
    },
    name: {
      hr: 'Maršalski jezik',
      pl: 'Język marszalski',
      lt: 'Maršaliečių kalba',
      vi: 'Tiếng Marshall',
      lv: 'Māršaliešu valoda',
      ta: 'கயின மொழி',
      fa: 'زبان مارشالی',
      ru: 'Маршалльский язык',
      mk: 'Маршалски јазик',
      nl: 'Marshallees',
      hy: 'Մարշալերեն',
      ko: '마셜어',
      br: 'Marchalleg',
      uk: 'Маршальська мова',
      cs: 'Maršálština',
      zh: '马绍尔语',
      ja: 'マーシャル語',
      rw: 'Ikimarishali',
      kv: 'Маршалл кыв',
      az: 'Marşall dili',
      et: 'Maršalli keel',
      it: 'Marshallese',
      ar: 'لغة مارشالية',
      sh: 'Maršalski jezik',
      sv: 'Marshallesiska',
      de: 'Marshallesische Sprache',
      nn: 'Marshallesisk',
      nb: 'Marshallesisk',
      eu: 'Marshallera',
      fi: 'Marshallin kieli',
      be: 'Маршальская мова',
      mh: 'Kajin M̧ajeļ',
      bg: 'Маршалски език',
      sm: 'Gagana Marshallese',
      pt: 'Língua marshalesa',
      eo: 'Marŝala lingvo',
      en: 'Marshallese',
      ca: 'Marshallès',
      gl: 'Lingua marshalesa',
      es: 'Marshalés',
      fr: 'Marshallais',
      he: 'מרשלית',
      sq: 'Gjuha marshalleze'
    },
    language_code_3: {
      en: 'mah'
    },
    language_code_2: {
      en: 'mh'
    }
  },
  'en:french': {
    language_code_2: {
      en: 'fr'
    },
    name: {
      ps: 'فرانسوي',
      az: 'Fransız dili',
      et: 'Prantsuse keel',
      lo: 'ພາສາຝະລັ່ງ',
      ky: 'Француз тили',
      av: 'Француз мацӀ',
      uk: 'Французька мова',
      na: 'Dorerin Prant',
      is: 'Franska',
      ce: 'Французийн мотт',
      zu: 'IsiFulentshi',
      da: 'Fransk',
      fa: 'زبان فرانسوی',
      iu: 'ᐅᐃᒍᐃᕐᒥᐅᖅ',
      an: 'Idioma francés',
      rm: 'Lingua franzosa',
      tg: 'Забони фаронсавӣ',
      bo: 'ཧྥ་རན་སིའི་སྐད།',
      si: 'ප් රංශ භාෂාව',
      lv: 'Franču valoda',
      gd: 'Fraingis',
      lb: 'Franséisch',
      lt: 'Prancūzų kalba',
      th: 'ภาษาฝรั่งเศส',
      gv: 'Frangish',
      ca: 'Francès',
      pa: 'ਫ਼ਰਾਂਸਿਸੀ ਭਾਸ਼ਾ',
      en: 'French',
      mr: 'फ्रेंच भाषा',
      ln: 'Falansé',
      tl: 'Wikang Pranses',
      sm: "Fa'afarani",
      ty: 'Reo farāni',
      fo: 'Franskt mál',
      cr: 'ᐅᐱᔥᑎᑯᔮᐅᐊᔨᒧᐎᓐ',
      ur: 'فرانسیسی زبان',
      so: 'Af-Faransiis',
      su: 'Basa Perancis',
      nv: 'Dáághahii bizaad',
      ml: 'ഫ്രഞ്ച് ഭാഷ',
      yi: 'פראנצויזיש',
      wa: 'Francès',
      ja: 'フランス語',
      ff: 'Faransinkoore',
      cu: 'Франкїискъ ѩꙁꙑкъ',
      mg: 'Fiteny frantsay',
      li: 'Frans',
      hy: 'Ֆրանսերեն',
      ko: '프랑스어',
      nl: 'Frans',
      sw: 'Kifaransa',
      mi: 'Reo Wīwī',
      ms: 'Bahasa Perancis',
      vi: 'Tiếng Pháp',
      ia: 'Lingua francese',
      kg: 'Kifalanse',
      ka: 'ფრანგული ენა',
      cy: 'Ffrangeg',
      cv: 'Франци чĕлхи',
      kk: 'Француз тілі',
      be: 'Французская мова',
      oc: 'Francés',
      nn: 'Fransk',
      ar: 'لغة فرنسية',
      sh: 'Francuski jezik',
      my: 'ပြင်သစ် ဘာသာ',
      sv: 'Franska',
      hi: 'फ़्रान्सीसी भाषा',
      sl: 'Francoščina',
      ba: 'Француз теле',
      tt: 'Француз теле',
      cs: 'Francouzština',
      se: 'Fránskkagiella',
      wo: 'Wu-faraas',
      tr: 'Fransızca',
      os: 'Францаг æвзаг',
      ug: 'فرانسۇز تىلى',
      bn: 'ফরাসি ভাষা',
      mk: 'Француски јазик',
      af: 'Frans',
      hr: 'Francuski jezik',
      pl: 'Język francuski',
      sc: 'Limba frantzesa',
      sq: 'Gjuha frënge',
      ss: 'SíFulentji',
      es: 'Francés',
      gl: 'Lingua francesa',
      fr: 'Français',
      ay: 'Phransiya aru',
      dv: 'ފަރަންސޭސި',
      bg: 'Френски език',
      hu: 'Francia',
      ht: 'Lang franse',
      de: 'Französisch',
      am: 'ፈረንሳይኛ',
      nb: 'Fransk',
      eu: 'Frantses',
      fy: 'Frânsk',
      qu: 'Ransis simi',
      it: 'Francese',
      rw: 'Igifaransa',
      sk: 'Francúzština',
      no: 'Fransk',
      kv: 'Франс кыв',
      kw: 'Frenkek',
      sr: 'Француски језик',
      ro: 'Limba franceză',
      mn: 'Франц хэл',
      zh: '法语',
      km: 'ភាសាបារាំង',
      gn: "Hyãsiañe'ẽ",
      co: 'Lingua francese',
      br: 'Galleg',
      za: 'Vah Fazgoz',
      ku: 'Zimanê fransî',
      yo: 'Èdè Faransé',
      bm: 'Faransekan',
      ru: 'Французский язык',
      st: 'Se-french',
      kn: 'ಫ್ರೆಂಚ್ ಭಾಷೆ',
      ta: 'பிரான்சிய மொழி',
      sa: 'फ्रांसीसी भाषा',
      tk: 'Fransuz dili',
      ne: 'फ्रान्सेली भाषा',
      he: 'צרפתית',
      el: 'Γαλλική γλώσσα',
      kl: 'Franskisut',
      ga: 'An Fhraincis',
      io: 'Franciana linguo',
      ee: 'Fransegbe',
      uz: 'Fransuz tili',
      pt: 'Língua francesa',
      eo: 'Franca lingvo',
      la: 'Lingua Francogallica',
      bs: 'Francuski jezik',
      fi: 'Ranska',
      vo: 'Fransänapük',
      or: 'ଫରାସୀ ଭାଷା',
      id: 'Bahasa Perancis',
      te: 'ఫ్రెంచి భాష',
      jv: 'Basa Prancis'
    },
    language_code_3: {
      en: 'fra'
    },
    wikidata: {
      en: 'Q150'
    }
  },
  'en:aymara': {
    language_code_2: {
      en: 'ay'
    },
    language_code_3: {
      en: 'aym'
    },
    name: {
      bn: 'আইমারা ভাষা',
      os: 'Аймараг æвзаг',
      ce: 'Аймара',
      uk: 'Аймара',
      is: 'Aímaríska',
      tr: 'Aymaraca',
      cs: 'Ajmarština',
      se: 'Aymaragiella',
      et: 'Aimara keel',
      az: 'Aymara dili',
      pl: 'Język ajmara',
      lt: 'Aimarų kalba',
      hr: 'Ajmarski jezik',
      af: 'Aymara',
      lv: 'Aimaru valoda',
      mk: 'Ајмарски јазик',
      fa: 'زبان آیمارا',
      an: 'Idioma aimara',
      da: 'Aymara',
      mr: 'आयमारा भाषा',
      en: 'Aymara',
      es: 'Aimara',
      gl: 'Lingua aimará',
      ay: 'Aymar aru',
      fr: 'Aymara',
      ca: 'Aimara',
      gv: 'Aymarish',
      sq: 'Gjuha aimare',
      qu: 'Aymara simi',
      it: 'Aymara',
      nb: 'Aymara',
      eu: 'Aimara',
      de: 'Aymara',
      bg: 'Аймара',
      br: 'Aymareg',
      ko: '아이마라어',
      wa: 'Aymara',
      zh: '艾馬拉語',
      ja: 'アイマラ語',
      gn: "Aimarañe'ẽ",
      sr: 'Ајмара',
      ro: 'Limba aimara',
      nv: 'Aymara bizaad',
      kv: 'Аймара',
      ne: 'आएमारा',
      ia: 'Aymara',
      vi: 'Tiếng Aymara',
      ms: 'Bahasa Aymara',
      ta: 'ஐமர மொழி',
      mi: 'Reo Aymara',
      ru: 'Аймара',
      nl: 'Aymara',
      eo: 'Ajmara lingvo',
      uz: 'Aymara tili',
      pt: 'Língua aimará',
      ka: 'აიმარა',
      cy: 'Aymara',
      la: 'Lingua Aymara',
      io: 'Aymarana linguo',
      sl: 'Ajmarščina',
      id: 'Bahasa Aymara',
      sv: 'Aymara',
      ar: 'لغة أيمارا',
      nn: 'Aymara',
      kk: 'Аймара тілі',
      fi: 'Aimaran kieli',
      be: 'Аймара',
      oc: 'Aymara'
    },
    wikidata: {
      en: 'Q4627'
    }
  },
  'en:hebrew': {
    wikidata: {
      en: 'Q9288'
    },
    name: {
      ug: 'ئىبرانى تىلى',
      zu: 'IsiHebheru',
      bn: 'হিব্রু ভাষা',
      tr: 'İbranice',
      is: 'Hebreska',
      uk: 'Іврит',
      ce: 'Иврит',
      ky: 'Жөөт тили',
      cs: 'Hebrejština',
      az: 'İvrit dili',
      et: 'Heebrea keel',
      hr: 'Hebrejski jezik',
      lt: 'Hebrajų kalba',
      pl: 'Język hebrajski',
      lv: 'Ivrits',
      gd: 'Eabhra',
      af: 'Hebreeus',
      an: 'Idioma hebreu',
      fa: 'زبان عبری',
      si: 'හීබෲ භාෂාව',
      mk: 'Хебрејски јазик',
      tg: 'Забони ибрӣ',
      da: 'Hebraisk',
      ln: 'Liébeleo',
      tl: 'Wikang Ebreo',
      en: 'Hebrew',
      mr: 'हिब्रू भाषा',
      pa: 'ਹਿਬਰੂ ਭਾਸ਼ਾ',
      ca: 'Hebreu',
      fr: 'Hébreu',
      es: 'Hebreo',
      gl: 'Lingua hebrea',
      sc: 'Limba ebrea',
      sq: 'Gjuha hebraike',
      gv: 'Ewnish',
      th: 'ภาษาฮีบรู',
      so: 'Af-Hebrow',
      qu: 'Iwriyu simi',
      it: 'Ebraico',
      ab: 'Иврит',
      fy: 'Hebrieusk',
      ur: 'عبرانی زبان',
      de: 'Hebräisch',
      am: 'ዕብራይስጥ',
      eu: 'Hebreera',
      nb: 'Hebraisk',
      bg: 'Иврит',
      hu: 'Héber',
      fo: 'Hebraiskt mál',
      ko: '히브리어',
      hy: 'Եբրայերեն',
      li: 'Hebreeuws',
      br: 'Hebraeg',
      cu: 'Єврєискъ ѩꙁꙑкъ',
      mg: 'Fiteny hebreo',
      co: 'Lingua ebraica',
      ro: 'Limba ebraică',
      yi: 'העברעיש',
      kw: 'Ebrow',
      sr: 'Хебрејски језик',
      km: 'ភាសាហេប្រ៊ូ',
      ja: 'ヘブライ語',
      gn: "Evréo ñe'ẽ",
      zh: '希伯来语',
      mn: 'Еврей хэл',
      kv: 'Иврит',
      sk: 'Hebrejčina',
      ml: 'ഹീബ്രു',
      tk: 'Iwrit dili',
      ta: 'எபிரேயம்',
      kn: 'ಇವ್ರಿತ್',
      vi: 'Tiếng Hebrew',
      ms: 'Bahasa Ibrani',
      ia: 'Lingua hebree',
      ru: 'Иврит',
      ku: 'Zimanê îbranî',
      yo: 'Èdè Hébérù',
      mi: 'Hiperu te reo',
      sw: 'Kiebrania',
      ti: 'ዕብራይስጥ',
      nl: 'Hebreeuws',
      bs: 'Hebrejski jezik',
      cy: 'Hebraeg',
      la: 'Lingua Hebraica',
      uz: 'Ivrit',
      pt: 'Língua hebraica',
      eo: 'Hebrea lingvo',
      ka: 'ებრაული ენა',
      cv: 'Иврит',
      kl: 'Hebraimiutut',
      io: 'Hebrea linguo',
      ga: 'An Eabhrais',
      he: 'עברית',
      el: 'Εβραϊκή γλώσσα',
      ba: 'Йәһүд теле',
      id: 'Bahasa Ibrani',
      sl: 'Hebrejščina',
      jv: 'Basa Ibrani',
      te: 'హీబ్రూ భాష',
      ar: 'لغة عبرية',
      sh: 'Hebrejski jezik',
      hi: 'इब्रानी भाषा',
      vo: 'Hebreyapük',
      sv: 'Hebreiska',
      my: 'ဟီဘရူးဘာသာ',
      nn: 'Hebraisk',
      oc: 'Ebrieu',
      be: 'Іўрыт',
      kk: 'Иврит тілі',
      fi: 'Heprea'
    },
    language_code_3: {
      en: 'heb'
    },
    language_code_2: {
      en: 'he'
    }
  },
  'en:northern-sami': {
    language_code_2: {
      en: 'se'
    },
    language_code_3: {
      en: 'sme'
    },
    name: {
      vi: 'Tiếng Bắc Sami',
      lv: 'Ziemeļsāmu valoda',
      ta: 'வட சமி மொழி',
      hr: 'Sjevernolaponski jezik',
      pl: 'Język północnolapoński',
      lt: 'Šiaurės samių kalba',
      da: 'Nordsamisk',
      nl: 'Noord-samisch',
      fa: 'زبان سامی شمالی',
      ru: 'Северносаамский язык',
      mk: 'Севернолапонски јазик',
      uk: 'Північносаамська мова',
      tr: 'Kuzeylapça',
      is: 'Norðursamíska',
      ce: 'Къилбаседа-Саамийн мотт',
      ko: '북부 사미어',
      br: 'Samieg an Norzh',
      sk: 'Severná saamčina',
      kv: 'Войвыв саами кыв',
      et: 'Põhjasaami keel',
      se: 'Davvisámegiella',
      sr: 'Севернолапонски језик',
      cs: 'Severní sámština',
      zh: '北方萨米语',
      ja: '北部サーミ語',
      sh: 'Sjevernolaponski jezik',
      ar: 'لغة سامي الشمالية',
      sv: 'Nordsamiska',
      it: 'Sami settentrionale',
      fo: 'Norðursámiskt mál',
      fi: 'Pohjoissaame',
      be: 'Паўночнасаамская мова',
      hu: 'Északi számi nyelv',
      bg: 'Оеверносаамски език',
      am: 'ስሜን ሳሚኛ',
      de: 'Nordsamische Sprache',
      nb: 'Nordsamisk',
      nn: 'Nordsamisk',
      eu: 'Ipar samiera',
      pt: 'Língua sami setentrional',
      eo: 'Nord-samea lingvo',
      la: 'Lingua samica septentrionalis',
      cy: 'Saameg gogleddol',
      en: 'Northern Sami',
      gv: 'Saamish Hwoaie',
      ca: 'Sami septentrional',
      gl: 'Lingua saami setentrional',
      es: 'Sami septentrional',
      fr: 'Same du Nord'
    },
    wikidata: {
      en: 'Q33947'
    }
  },
  'en:bengali': {
    language_code_2: {
      en: 'bn'
    },
    name: {
      mg: 'Fiteny bengali',
      li: 'Bengaals',
      gu: 'બંગાળી ભાષા',
      hy: 'Բենգալերեն',
      ko: '벵골어',
      br: 'Banglaeg',
      sk: 'Bengálčina',
      kv: 'Бенгал кыв',
      ml: 'ബംഗാളി',
      sr: 'Бенгалски језик',
      kw: 'Bengalek',
      ro: 'Limba bengaleză',
      yi: 'בענגאליש',
      zh: '孟加拉语',
      mn: 'Бенгал хэл',
      ja: 'ベンガル語',
      gn: "Vengali ñe'ẽ",
      ms: 'Bahasa Benggali',
      vi: 'Tiếng Bengal',
      kn: 'ಬಂಗಾಳಿ ಭಾಷೆ',
      ta: 'வங்காள மொழி',
      bh: 'बांग्ला',
      sa: 'वङ्ग भाषा',
      ne: 'बांग्ला भाषा',
      za: 'Vahmanggyalah',
      nl: 'Bengaals',
      yo: 'Èdè Bẹ̀ngálì',
      ru: 'Бенгальский язык',
      sw: 'Kibengali',
      mi: 'Reo Bengali',
      eo: 'Bengala lingvo',
      uz: 'Bengal tili',
      pt: 'Língua bengali',
      ka: 'ბენგალური ენა',
      la: 'Lingua bengalica',
      cy: 'Bengaleg',
      bs: 'Bangla',
      kg: "Kibang'la",
      he: 'בנגלית',
      el: 'Μπενγκάλι γλώσσα',
      as: 'বাংলা ভাষা',
      kl: 'Bengalimiutut',
      io: 'Bengali linguo',
      sh: 'Bengalski jezik',
      ar: 'لغة بنغالية',
      my: 'ဘင်္ဂါလီဘာသာ',
      vo: 'Bängalänapük',
      sv: 'Bengali',
      hi: 'बांग्ला भाषा',
      or: 'ବଙ୍ଗଳା ଭାଷା',
      sl: 'Bengalščina',
      ba: 'Бенгали',
      id: 'Bahasa Bengali',
      te: 'బంగ్లా భాష',
      tt: 'Бенгаль теле',
      kk: 'Бенгали тілі',
      be: 'Бенгальская мова',
      fi: 'Bengali',
      oc: 'Bengalin',
      nn: 'Bengali',
      uk: 'Бенгальська мова',
      is: 'Bengalska',
      tr: 'Bengalce',
      ce: 'Бенгалхойн мотт',
      os: 'Бенгайлаг æвзаг',
      ug: 'بېنگال تىلى',
      bn: 'বাংলা ভাষা',
      az: 'Benqal dili',
      et: 'Bengali keel',
      se: 'Bengalgiella',
      cs: 'Bengálština',
      lv: 'Bengāļu valoda',
      af: 'Bengaals',
      gd: 'Bangla',
      hr: 'Bengalski jezik',
      pl: 'Język bengalski',
      lt: 'Bengalų kalba',
      da: 'Bengali',
      fa: 'زبان بنگالی',
      an: 'Idioma bengalí',
      tg: 'Забони банголӣ',
      mk: 'Бенгалски јазик',
      mr: 'बंगाली भाषा',
      en: 'Bengali',
      tl: 'Wikang Bengali',
      th: 'ภาษาเบงกาลี',
      sq: 'Gjuha bengali',
      ca: 'Bengalí',
      pa: 'ਬੰਗਾਲੀ ਭਾਸ਼ਾ',
      es: 'Bengalí',
      gl: 'Lingua bengalí',
      fr: 'Bengali',
      qu: 'Banla simi',
      it: 'Bengali',
      su: 'Basa Benggala',
      dv: 'ބެންގާލީ',
      fo: 'Bengalskt mál',
      bg: 'Бенгалски език',
      hu: 'Bengáli',
      am: 'በንጋልኛ',
      ur: 'بنگلہ',
      de: 'Bengalische Sprache',
      nb: 'Bengali',
      eu: 'Bengalera'
    },
    language_code_3: {
      en: 'ben'
    },
    wikidata: {
      en: 'Q9610'
    }
  },
  'en:odia': {
    language_code_2: {
      en: 'or'
    },
    language_code_3: {
      en: 'ori'
    },
    name: {
      da: 'Oriya',
      nl: 'Odia',
      fa: 'زبان اوریه',
      ru: 'Ория',
      sw: 'Kioriya',
      lv: 'Oriju valoda',
      vi: 'Tiếng Oriya',
      bh: 'ओडिया',
      ta: 'ஒடியா மொழி',
      sa: 'ओडिया',
      af: 'Oriya',
      hr: 'Oriya jezici',
      pl: 'Język orija',
      lt: 'Orijų kalba',
      rw: 'Icyoriya',
      kv: 'Ория',
      az: 'Oriya dili',
      ml: 'ഒഡിയ',
      et: 'Oria keel',
      sr: 'Орија језик',
      ro: 'Limba oriya',
      zh: '奥里亚语',
      ja: 'オリヤー語',
      uk: 'Орія',
      tr: 'Oriya dili',
      gu: 'ઉડિયા ભાષા',
      ug: 'ئورىياچە',
      ko: '오리야어',
      bn: 'ওড়িয়া ভাষা',
      br: 'Oriyeg',
      be: 'Орыя',
      fi: 'Orija',
      oc: 'Oriya',
      de: 'Oriya',
      ur: 'اڑیہ زبان',
      nn: 'Oriya',
      nb: 'Oriya',
      eu: 'Oriya',
      ar: 'لغة الأوريا',
      sh: 'Oriya',
      hi: 'ओड़िआ भाषा',
      sv: 'Oriya',
      qu: 'Oriya simi',
      or: 'ଓଡ଼ିଆ',
      it: 'Oriya',
      id: 'Bahasa Oriya',
      te: 'ఒరియా భాష',
      he: 'אורייה',
      th: 'ภาษาโอริยา',
      as: 'ওড়িয়া ভাষা',
      ca: 'Oriya',
      pa: 'ਉੜੀਆ ਭਾਸ਼ਾ',
      es: 'Idioma oriya',
      fr: 'Oriya',
      pt: 'Língua oriá',
      uz: 'Oriya',
      ka: 'ორია',
      eo: 'Orija lingvo',
      la: 'Lingua orissensis',
      en: 'Odia',
      mr: 'ओडिया भाषा',
      cv: 'Ори'
    },
    wikidata: {
      en: 'Q33810'
    }
  },
  'en:malayalam': {
    wikidata: {
      en: 'Q36236'
    },
    language_code_3: {
      en: 'mal'
    },
    name: {
      th: 'ภาษามาลายาลัม',
      pa: 'ਮਲਿਆਲਮ ਭਾਸ਼ਾ',
      ca: 'Malaiàlam',
      fr: 'Malayalam',
      gl: 'Lingua malaialam',
      es: 'Malayalam',
      en: 'Malayalam',
      mr: 'मलयाळम',
      dv: 'މަލަޔާޅަމް',
      hu: 'Malajálam',
      bg: 'Малаялам',
      de: 'Malayalam',
      ur: 'ملیالم',
      am: 'ማላያላም',
      eu: 'Malabarera',
      nb: 'Malayalam',
      it: 'Malayalam',
      qu: 'Malayalam simi',
      az: 'Malayalam',
      et: 'Malajalami keel',
      cs: 'Malajálamština',
      uk: 'Малаялам',
      ug: 'مالايالامچە',
      bn: 'মালয়ালম ভাষা',
      da: 'Malayalam',
      fa: 'زبان مالایالم',
      mk: 'Малајалам',
      lv: 'Malajālama',
      af: 'Malayalam',
      lt: 'Malajalių kalba',
      pl: 'Język malajalam',
      he: 'מלאיאלאם',
      ga: 'An Mhailéalaimis',
      io: 'Malayala linguo',
      la: 'Lingua Malabarica',
      uz: 'Malayalam',
      ka: 'მალაიალამური ენა',
      pt: 'Malaiala',
      eo: 'Malajala lingvo',
      cv: 'Малаялам',
      oc: 'Malayalam',
      fi: 'Malajalam',
      be: 'Малаялам',
      nn: 'Malayalam',
      ar: 'لغة ماليالامية',
      sh: 'Malajalam jezik',
      sv: 'Malayalam',
      hi: 'मलयालम भाषा',
      id: 'Bahasa Malayalam',
      or: 'ମଲୟାଲମ',
      sl: 'Malajalščina',
      te: 'మలయాళ భాష',
      kv: 'Малаялам',
      sk: 'Malajálamčina',
      ml: 'മലയാളം',
      ro: 'Limba malayalam',
      yi: 'מאַלאַיאַלאַם',
      sr: 'Малајалам језик',
      ja: 'マラヤーラム語',
      zh: '马拉雅拉姆语',
      mg: 'Fiteny malayalam',
      ko: '말라얄람어',
      gu: 'મલયાલમ ભાષા',
      br: 'Malayalameg',
      nl: 'Malayalam',
      ru: 'Малаялам',
      sw: 'Kimalayalam',
      ta: 'மலையாளம்',
      ms: 'Bahasa Malayalam',
      vi: 'Tiếng Malayalam',
      kn: 'ಮಲಯಾಳಂ',
      sa: 'मलयाळम्‌',
      ne: 'मलयालम'
    },
    language_code_2: {
      en: 'ml'
    }
  },
  'en:dutch': {
    wikidata: {
      en: 'Q7411'
    },
    language_code_3: {
      en: 'dut'
    },
    name: {
      th: 'ภาษาดัตช์',
      gv: 'Ollanish',
      sq: 'Gjuha holandeze',
      sc: 'Limba olandesa',
      gl: 'Lingua neerlandesa',
      es: 'Neerlandés',
      fr: 'Néerlandais',
      ca: 'Neerlandès',
      pa: 'ਡਚ ਬੋਲੀ',
      en: 'Dutch',
      mr: 'डच भाषा',
      sm: "Fa'aholani",
      tl: 'Wikang Olandes',
      ln: 'Lifalamá',
      fo: 'Niðurlendskt mál',
      hu: 'Holland',
      bg: 'Нидерландски език',
      dv: 'ޑަޗު',
      nb: 'Nederlandsk',
      eu: 'Nederlandera',
      am: 'ሆላንድኛ',
      ur: 'ولندیزی زبان',
      de: 'Niederländisch',
      fy: 'Nederlânsk',
      su: 'Basa Walanda',
      qu: 'Urasuyu simi',
      it: 'Olandese',
      so: 'Af-Holandees',
      et: 'Hollandi keel',
      ps: 'نېدرلنډي',
      az: 'Niderland dili',
      cs: 'Nizozemština',
      se: 'Hollánddagiella',
      ce: 'Недерландхойн мотт',
      uk: 'Нідерландська мова',
      tr: 'Felemenkçe',
      is: 'Hollenska',
      bn: 'ওলন্দাজ ভাষা',
      zu: 'IsiHolandi',
      ug: 'گوللاندىيە تىلى',
      os: 'Нидерландаг æвзаг',
      da: 'Nederlandsk',
      rm: 'Lingua neerlandaisa',
      tg: 'Забони нидерландӣ',
      mk: 'Холандски јазик',
      fa: 'زبان هلندی',
      an: 'Idioma neerlandés',
      af: 'Nederlands',
      gd: 'Duitsis',
      lv: 'Nīderlandiešu valoda',
      pl: 'Język niderlandzki',
      lt: 'Olandų kalba',
      hr: 'Nizozemski jezik',
      lb: 'Hollännesch',
      el: 'Ολλανδική γλώσσα',
      he: 'הולנדית',
      ga: 'An Ollainnis',
      io: 'Nederlandana linguo',
      cv: 'Голланд чĕлхи',
      eo: 'Nederlanda lingvo',
      pt: 'Língua neerlandesa',
      ka: 'ნიდერლანდური ენა',
      cy: 'Iseldireg',
      la: 'Lingua Batava',
      bs: 'Holandski jezik',
      be: 'Нідэрландская мова',
      kk: 'Нидерланд тілі',
      fi: 'Hollanti',
      oc: 'Neerlandés',
      nn: 'Nederlandsk',
      hi: 'डच भाषा',
      vo: 'Nedänapük',
      sv: 'Nederländska',
      ar: 'لغة هولندية',
      sh: 'Holandski jezik',
      jv: 'Basa Walanda',
      tt: 'Нидерланд теле',
      sl: 'Nizozemščina',
      id: 'Bahasa Belanda',
      ba: 'Нидерланд теле',
      ml: 'ഡച്ച് ഭാഷ',
      sk: 'Holandčina',
      kv: 'Нидерланд кыв',
      wa: 'Neyerlandès',
      zh: '荷蘭語',
      gn: "Holandañe'ẽ",
      ja: 'オランダ語',
      kw: 'Iseldiryek',
      sr: 'Холандски језик',
      ro: 'Limba neerlandeză',
      yi: 'האלענדיש',
      co: 'Lingua neerlandese',
      br: 'Nederlandeg',
      li: 'Nederlands',
      hy: 'Հոլանդերեն',
      ko: '네덜란드어',
      nl: 'Nederlands',
      st: 'Se-dutch',
      sw: 'Kiholanzi',
      ku: 'Zimanê holandî',
      ru: 'Нидерландский язык',
      sa: 'डच भाषा',
      xh: 'IsiHolani',
      ia: 'Lingua nederlandese',
      ms: 'Bahasa Belanda',
      vi: 'Tiếng Hà Lan',
      ta: 'டச்சு மொழி'
    },
    language_code_2: {
      en: 'nl'
    }
  },
  'en:uyghur': {
    wikidata: {
      en: 'Q13263'
    },
    language_code_3: {
      en: 'uig'
    },
    name: {
      tt: 'Уйгур теле',
      id: 'Bahasa Uighur',
      sv: 'Uiguriska',
      hi: 'उइगुर भाषा',
      ar: 'لغة أويغورية',
      nn: 'Uigurisk',
      be: 'Уйгурская мова',
      kk: 'Ұйғыр тілі',
      fi: 'Uiguurin kieli',
      oc: 'Oigor',
      cv: 'Уйгур чĕлхи',
      pt: 'Língua uigur',
      uz: 'Uygʻur tili',
      ka: 'უიგურული ენა',
      eo: 'Ujgura lingvo',
      el: 'Ουιγκούρ γλώσσα',
      he: 'אויגור',
      vi: 'Tiếng Uyghur',
      ms: 'Bahasa Uyghur',
      ta: 'உய்குர் மொழி',
      ru: 'Уйгурский язык',
      nl: 'Oeigoers',
      br: 'Ouigoureg',
      ko: '위구르어',
      zh: '维吾尔语',
      mn: 'Уйгур хэл',
      ja: 'ウイグル語',
      sr: 'Ујгурски језик',
      sk: 'Ujgurčina',
      kv: 'Уйгур кыв',
      it: 'Uiguro',
      qu: "Uyq'ur simi",
      nb: 'Uigurisk',
      eu: 'Uigurrera',
      de: 'Uigurisch',
      ur: 'اویغور زبان',
      am: 'ኡይጉርኛ',
      bg: 'Уйгурски език',
      tl: 'Wikang Uighur',
      en: 'Uyghur',
      mr: 'उय्गुर भाषा',
      es: 'Uigur',
      fr: 'Ouïghour',
      ca: 'Uigur',
      pa: 'ਉਇਗੁਰ ਭਾਸ਼ਾ',
      th: 'ภาษาอุยกูร์',
      pl: 'Język ujgurski',
      lt: 'Uigūrų kalba',
      hr: 'Ujgurski jezik',
      lb: 'Uiguresch',
      af: 'Oeigoers',
      lv: 'Uiguru valoda',
      tg: 'Забони уйғурӣ',
      bo: 'ཡུལ་གོར་སྐད།',
      mk: 'Ујгурски јазик',
      fa: 'زبان اویغوری',
      bn: 'উইগুর ভাষা',
      ug: 'ئۇيغۇر تىلى',
      ce: 'Уйгурийн мотт',
      uk: 'Уйгурська мова',
      tr: 'Uygurca',
      is: 'Úýgúríska',
      se: 'Uiguragiella',
      cs: 'Ujgurština',
      et: 'Uiguuri keel',
      az: 'Uyğur dili'
    },
    language_code_2: {
      en: 'ug'
    }
  },
  'en:serbian': {
    language_code_2: {
      en: 'sr'
    },
    language_code_3: {
      en: 'srp'
    },
    name: {
      ru: 'Сербский язык',
      ku: 'Zimanê serbî',
      yo: 'Èdè Sérbíà',
      nl: 'Servisch',
      tk: 'Serb dili',
      ta: 'செருபிய மொழி',
      vi: 'Tiếng Serbia',
      ms: 'Bahasa Serbia',
      ia: 'Lingua serbe',
      ro: 'Limba sârbă',
      yi: 'סערביש',
      kw: 'Serbek',
      sr: 'Српски језик',
      ja: 'セルビア語',
      gn: "Seviañe'ẽ",
      mn: 'Серб хэл',
      zh: '塞尔维亚语',
      kv: 'Серб кыв',
      sk: 'Srbčina',
      ko: '세르비아어',
      hy: 'Սերբերեն',
      br: 'Serbeg',
      cu: 'Срьбьскъ ѩꙁꙑкъ',
      co: 'Lingua serba',
      nn: 'Serbisk',
      oc: 'Sèrbe',
      be: 'Сербская мова',
      kk: 'Серб тілі',
      fi: 'Serbia',
      id: 'Bahasa Serbia',
      sl: 'Srbščina',
      jv: 'Basa Serbia',
      sh: 'Srpski jezik',
      ar: 'لغة صربية',
      om: 'Afaan Sarbiyaa',
      sv: 'Serbiska',
      hi: 'सर्बियाई भाषा',
      io: 'Serbiana linguo',
      ga: 'An tSeirbis',
      he: 'סרבית',
      el: 'Σερβική γλώσσα',
      bs: 'Srpski jezik',
      cy: 'Serbeg',
      la: 'Lingua Serbica',
      pt: 'Língua sérvia',
      eo: 'Serba lingvo',
      ka: 'სერბული ენა',
      cv: 'Серб чĕлхи',
      an: 'Idioma serbio',
      fa: 'زبان صربی',
      mk: 'Српски јазик',
      tg: 'Забони сербӣ',
      da: 'Serbisk',
      hr: 'Srpski jezik',
      lb: 'Serbesch',
      lt: 'Serbų kalba',
      pl: 'Język serbski',
      lv: 'Serbu valoda',
      af: 'Serwies',
      gd: 'Searbais',
      ky: 'Серб тили',
      cs: 'Srbština',
      av: 'Сербиязул мацІ',
      az: 'Serb dili',
      et: 'Serbia keel',
      os: 'Сербаг æвзаг',
      ug: 'سېرب تىلى',
      bn: 'সার্বীয় ভাষা',
      is: 'Serbneska',
      tr: 'Sırpça',
      uk: 'Сербська мова',
      ce: 'Сербийн мотт',
      de: 'Serbisch',
      eu: 'Serbiera',
      nb: 'Serbisk',
      ty: 'Reo Terepia',
      hu: 'Szerb',
      bg: 'Сръбски език',
      qu: 'Sirbya simi',
      it: 'Serbo',
      ab: 'Асерб бызшәа',
      fy: 'Servysk',
      pa: 'ਸਰਬੀਆਈ ਭਾਸ਼ਾ',
      ca: 'Serbi',
      fr: 'Serbe',
      gl: 'Lingua serbia',
      es: 'Serbio',
      sc: 'Limba serba',
      sq: 'Gjuha serbe',
      gv: 'Serbish',
      th: 'ภาษาเซอร์เบีย',
      tl: 'Wikang Serbiyo',
      sm: 'Gagana Serbia',
      mr: 'सर्बियन भाषा',
      en: 'Serbian'
    },
    wikidata: {
      en: 'Q9299'
    }
  },
  'en:tibetan-language': {
    language_code_2: {
      en: 'bo'
    },
    name: {
      nn: 'Tibetansk',
      oc: 'Tibetan',
      fi: 'Tiibetin kieli',
      kk: 'Тибет тілдері',
      be: 'Тыбецкая мова',
      id: 'Bahasa Tibet',
      ar: 'تبتية',
      hi: 'तिब्बती भाषा',
      sv: 'Tibetanska',
      he: 'טיבטית',
      cy: 'Tibeteg',
      la: 'Lingua Tibetana',
      uz: 'Tibet tili',
      pt: 'Língua tibetana',
      eo: 'Tibeta lingvo',
      ka: 'ტიბეტური ენა',
      ru: 'Тибетский язык',
      dz: 'བོད་སྐད',
      sw: 'Kitibeti',
      nl: 'Tibetaans',
      ne: 'तिब्बती भाषा',
      ta: 'திபெத்திய மொழி',
      ms: 'Bahasa Tibet',
      vi: 'Tiếng Tạng',
      ro: 'Limba tibetană',
      sr: 'Тибетански језик',
      ja: 'チベット語',
      gn: "Tivetano ñe'ẽ",
      zh: '藏语',
      wa: 'Tibetin',
      kv: 'Тибет кыв',
      sk: 'Tibetčina',
      ml: 'തിബത്തൻ ഭാഷ',
      ko: '티베트어',
      hy: 'Տիբեթերեն',
      br: 'Tibeteg',
      de: 'Tibetische Sprache',
      eu: 'Tibetera',
      nb: 'Tibetansk',
      bg: 'Тибетски език',
      hu: 'Tibeti nyelv',
      qu: 'Tibet simi',
      it: 'Tibetano',
      pa: 'ਤਿੱਬਤੀ ਭਾਸ਼ਾ',
      ca: 'Tibetà',
      fr: 'Tibétain',
      es: 'Tibetano',
      gv: 'Tibetish Veanagh',
      th: 'ภาษาทิเบต',
      en: 'Tibetan language',
      mr: 'तिबेटी भाषा',
      fa: 'زبان تبتی',
      mk: 'Тибетски јазик',
      tg: 'Забони тибетӣ',
      bo: 'བོད་སྐད།',
      da: 'Tibetansk',
      hr: 'Centralnotibetanski jezik',
      lt: 'Tibetiečių kalba',
      pl: 'Język tybetański',
      lv: 'Tibetiešu valoda',
      af: 'Tibettaans',
      cs: 'Tibetština',
      az: 'Tibet dili',
      et: 'Tiibeti keel',
      ug: 'زاڭزۇ تىلى',
      tr: 'Tibetçe',
      uk: 'Тибетська мова'
    },
    language_code_3: {
      en: 'bod'
    },
    wikidata: {
      en: 'Q34271'
    }
  },
  'en:belarusian': {
    wikidata: {
      en: 'Q9091'
    },
    language_code_3: {
      en: 'bel'
    },
    name: {
      bs: 'Bjeloruski jezik',
      cy: 'Belarwseg',
      la: 'Lingua Ruthenica Alba',
      pt: 'Língua bielorrussa',
      uz: 'Belorus tili',
      eo: 'Belorusa lingvo',
      ka: 'ბელარუსული ენა',
      cv: 'Беларус чĕлхи',
      io: 'Bielorusiana linguo',
      ga: 'An Bhealarúisis',
      he: 'בלארוסית',
      el: 'Λευκορωσική γλώσσα',
      id: 'Bahasa Belarus',
      ba: 'Белорус теле',
      sl: 'Beloruščina',
      jv: 'Basa Bélarus',
      sh: 'Bjeloruski jezik',
      ar: 'لغة بيلاروسية',
      vo: 'Belarusänapük',
      sv: 'Vitryska',
      nn: 'Kviterussisk',
      oc: 'Bielorús',
      kk: 'Беларус тілі',
      be: 'Беларуская мова',
      fi: 'Valkovenäjä',
      ko: '벨라루스어',
      hy: 'Բելառուսերեն',
      li: 'Witrussisch',
      br: 'Belaruseg',
      cu: 'Бѣлороусьскъ ѩꙁꙑкъ',
      co: 'Lingua bielorussa',
      yi: 'בעלארוסיש',
      ro: 'Limba bielorusă',
      sr: 'Белоруски језик',
      kw: 'Belarussek',
      ja: 'ベラルーシ語',
      gn: "Vielorrusiañe'ẽ",
      zh: '白俄罗斯语',
      mn: 'Беларусь хэл',
      kv: 'Беларус кыв',
      sk: 'Bieloruština',
      ta: 'பெலருசிய மொழி',
      vi: 'Tiếng Belarus',
      kn: 'ಬೆಲಾರೂಸ್ ನ ಭಾಷೆ',
      ms: 'Bahasa Belarus',
      ru: 'Белорусский язык',
      ku: 'Zimanê belarûsî',
      sw: 'Kibelarus',
      nl: 'Wit-Russisch',
      tl: 'Wikang Belarusyano',
      en: 'Belarusian',
      mr: 'बेलारूशियन भाषा',
      pa: 'ਬੇਲਾਰੂਸੀ ਭਾਸ਼ਾ',
      ca: 'Bielorús',
      fr: 'Biélorusse',
      es: 'Bielorruso',
      gl: 'Lingua bielorrusa',
      sq: 'Gjuha bjelloruse',
      gv: 'Belarooshish',
      th: 'ภาษาเบลารุส',
      it: 'Bielorusso',
      qu: 'Bilurusu simi',
      ab: 'Абеларус бызшәа',
      fy: 'Wyt-Russysk',
      ur: 'بیلاروسی زبان',
      de: 'Weißrussisch',
      am: 'ቤላሩስኛ',
      eu: 'Bielorrusiera',
      nb: 'Hviterussisk',
      ty: 'Reo Belarus',
      bg: 'Беларуски език',
      hu: 'Belarusz',
      os: 'Белоруссаг æвзаг',
      ug: 'بېلورۇس تىل',
      bn: 'বেলারুশীয় ভাষা',
      tr: 'Beyaz Rusça',
      na: 'Dorerin Berarut',
      is: 'Hvítrússneska',
      uk: 'Білоруська мова',
      ce: 'Белорусийн мотт',
      cs: 'Běloruština',
      se: 'Vilges-ruoššagiella',
      az: 'Belarus dili',
      et: 'Valgevene keel',
      hr: 'Bjeloruski jezik',
      lt: 'Baltarusių kalba',
      pl: 'Język białoruski',
      lv: 'Baltkrievu valoda',
      gd: 'Bealaruisis',
      af: 'Wit-Russies',
      an: 'Idioma belorruso',
      fa: 'زبان بلاروسی',
      mk: 'Белоруски јазик',
      tg: 'Забони белорусӣ',
      da: 'Hviderussisk'
    },
    language_code_2: {
      en: 'be'
    }
  },
  'en:samoan': {
    name: {
      fr: 'Samoan',
      es: 'Samoano',
      gl: 'Lingua samoana',
      ca: 'Samoà',
      el: 'Σαμόα',
      he: 'סמואית',
      sm: "Gagana fa'a Sāmoa",
      en: 'Samoan',
      mr: 'सामोअन भाषा',
      cy: 'Samöeg',
      la: 'Lingua Samoana',
      uz: 'Samoa',
      eo: 'Samoa lingvo',
      pt: 'Língua samoana',
      eu: 'Samoera',
      nb: 'Samoansk',
      nn: 'Samoansk',
      de: 'Samoanische Sprache',
      oc: 'Samoan',
      hu: 'Szamoai nyelv',
      bg: 'Самоански език',
      be: 'Самаанская мова',
      fi: 'Samoan kieli',
      id: 'Bahasa Samoa',
      it: 'Samoano',
      qu: 'Samwa simi',
      sv: 'Samoanska',
      ar: 'لغة ساموية',
      sh: 'Samoanski jezik',
      ja: 'サモア語',
      zh: '薩摩亞語',
      ro: 'Limba samoană',
      et: 'Samoa keel',
      kv: 'Самоа кыв',
      sk: 'Samojčina',
      br: 'Samoaeg',
      ko: '사모아어',
      na: 'Dorerin Tamoa',
      uk: 'Самоанська мова',
      mk: 'Самоански јазик',
      ru: 'Самоанский язык',
      fa: 'زبان ساموآیی',
      nl: 'Samoaans',
      to: 'Lea fakahaʻamoa',
      lt: 'Samoa kalba',
      pl: 'Język samoański',
      hr: 'Samoanski jezik',
      af: 'Samoaans',
      ta: 'சமோவ மொழி',
      ms: 'Bahasa Samoa',
      vi: 'Tiếng Samoa',
      lv: 'Samoāņu valoda'
    },
    language_code_3: {
      en: 'smo'
    },
    wikidata: {
      en: 'Q34011'
    },
    language_code_2: {
      en: 'sm'
    }
  },
  'en:punjabi': {
    wikidata: {
      en: 'Q58635'
    },
    name: {
      uk: 'Пенджабська мова',
      is: 'Púndjabí',
      tr: 'Pencapça',
      ug: 'پەنجاب تىلى',
      bn: 'পাঞ্জাবি ভাষা',
      ps: 'پنجابي',
      az: 'Pəncab dili',
      et: 'Pandžabi keel',
      cs: 'Paňdžábština',
      lv: 'Pandžabu valoda',
      ks: 'پنجابی زَبان',
      gd: 'Punjabi',
      af: 'Panjabi',
      hr: 'Istočnopandžapski jezik',
      pl: 'Język pendżabski',
      lt: 'Pandžabų kalba',
      da: 'Punjabi',
      fa: 'زبان پنجابی',
      tg: 'Забони панҷобӣ',
      mk: 'Пенџапски јазик',
      en: 'Punjabi',
      mr: 'पंजाबी भाषा',
      th: 'ภาษาปัญจาบ',
      ca: 'Panjabi',
      pa: 'ਪੰਜਾਬੀ ਭਾਸ਼ਾ',
      gl: 'Lingua punjabi',
      es: 'Idioma punjabí',
      fr: 'Panjābī',
      fy: 'Pûnjabysk',
      it: 'Punjabi',
      qu: 'Panyabi simi',
      bg: 'Панджабски език',
      de: 'Panjabi',
      ur: 'پنجابی زبان',
      nb: 'Panjabi',
      eu: 'Punjabera',
      gu: 'પંજાબી ભાષા',
      ko: '펀자브어',
      br: 'Pandjabeg',
      sk: 'Pandžábčina',
      kv: 'Панджаби',
      ml: 'പഞ്ചാബി ഭാഷ',
      sr: 'Панџаби',
      ro: 'Limba punjabă',
      mn: 'Панжаб хэл',
      zh: '旁遮普語',
      ja: 'パンジャーブ語',
      vi: 'Tiếng Punjab',
      ms: 'Bahasa Punjabi',
      kn: 'ಪಂಜಾಬಿ',
      ta: 'பஞ்சாபி மொழி',
      sa: 'पञ्जाबी',
      ia: 'Lingua panjabi',
      ne: 'पञ्जाबी भाषा',
      nl: 'Punjabi',
      ru: 'Панджаби',
      sw: 'Kipunjabi ya Mashariki',
      eo: 'Panĝaba lingvo',
      uz: 'Panjobi',
      pt: 'Língua panjabi',
      ka: 'პენჯაბური ენა',
      la: 'Lingua Panjabica',
      cy: 'Punjabi',
      cv: 'Панджаби',
      he: "פנג'אבי",
      el: 'Παντζάμπι γλώσσα',
      io: 'Punjabana linguo',
      ga: 'An Phuinseáibis',
      ar: 'لغة بنجابية',
      sh: 'Punjabi jezik',
      sv: 'Punjabi',
      hi: 'पंजाबी भाषा',
      or: 'ପଞ୍ଜାବୀ ଭାଷା',
      id: 'Bahasa Punjab',
      te: 'పంజాబీ భాష',
      fi: 'Punjabin kieli',
      be: 'Панджабі',
      nn: 'Panjabi'
    },
    language_code_3: {
      en: 'pan'
    },
    language_code_2: {
      en: 'pa'
    }
  },
  'en:russian': {
    language_code_2: {
      en: 'ru'
    },
    name: {
      mk: 'Руски јазик',
      af: 'Russies',
      pl: 'Język rosyjski',
      hr: 'Ruski jezik',
      se: 'Ruoššagiella',
      cs: 'Ruština',
      tr: 'Rusça',
      bn: 'রুশ ভাষা',
      ug: 'رۇس تىلى',
      os: 'Уырыссаг æвзаг',
      hu: 'Orosz',
      bg: 'Руски език',
      dv: 'ރޫސީ',
      nb: 'Russisk',
      eu: 'Errusiera',
      ht: 'Lang ris',
      am: 'መስኮብኛ',
      de: 'Russisch',
      fy: 'Russysk',
      ab: 'Урыс бызшәа',
      qu: 'Rusu simi',
      it: 'Russo',
      ak: 'Russian kasa',
      sq: 'Gjuha ruse',
      sc: 'Limba russa',
      es: 'Ruso',
      gl: 'Lingua rusa',
      ay: 'Rusi aru',
      fr: 'Russe',
      ku: 'Zimanê rûsî',
      yo: 'Èdè Rọ́síà',
      ru: 'Русский язык',
      sa: 'रूसी भाषा',
      kn: 'ರಷ್ಯಾದ ಭಾಷೆ',
      ta: 'உருசிய மொழி',
      ne: 'रूसी भाषा',
      tk: 'Rus dili',
      rw: 'Kirusiya',
      sk: 'Ruština',
      kv: 'Роч кыв',
      zh: '俄语',
      mn: 'Орос хэл',
      km: 'ភាសារុស្ស៊ី',
      gn: "Rrusiañe'ẽ",
      sr: 'Руски језик',
      kw: 'Russek',
      ro: 'Limba rusă',
      co: 'Lingua russia',
      br: 'Rusianeg',
      fi: 'Venäjä',
      vo: 'Rusänapük',
      jv: 'Basa Rusia',
      te: 'రష్యన్ భాష',
      id: 'Bahasa Rusia',
      el: 'Ρωσική γλώσσα',
      he: 'רוסית',
      ga: 'An Rúisis',
      io: 'Rusiana linguo',
      uz: 'Rus tili',
      eo: 'Rusa lingvo',
      pt: 'Língua russa',
      la: 'Russica',
      ik: 'Uruusiq uqautchit',
      bs: 'Ruski jezik',
      da: 'Russisk',
      bo: 'ཨུ་རུ་སུའི་སྐད།',
      tg: 'Забони русӣ',
      rm: 'Lingua russa',
      fa: 'زبان روسی',
      an: 'Idioma ruso',
      ks: 'روٗسک زَبانَ',
      bi: 'Toktok Rusia',
      gd: 'Ruisis',
      lv: 'Krievu valoda',
      lt: 'Rusų kalba',
      lb: 'Russesch',
      et: 'Vene keel',
      lo: 'ພາສາລັດເຊຍ',
      ps: 'روسي',
      az: 'Rus dili',
      av: 'ГІурус мацI',
      ky: 'Орус тили',
      ce: 'Оьрсийн мотт',
      uk: 'Російська мова',
      na: 'Dorerin Ratsiya',
      is: 'Rússneska',
      zu: 'IsiRashiya',
      fo: 'Russiskt mál',
      ty: 'Reo Rūtia',
      ur: 'روسی زبان',
      su: 'Basa Rusia',
      so: 'Af-Ruush',
      th: 'ภาษารัสเซีย',
      gv: 'Rooshish',
      ca: 'Rus',
      pa: 'ਰੂਸੀ ਬੋਲੀ',
      en: 'Russian',
      mr: 'रशियन भाषा',
      sm: "Gagana fa'a Lusia",
      tl: 'Wikang Ruso',
      nl: 'Russisch',
      sw: 'Kirusi',
      ia: 'Lingua russe',
      vi: 'Tiếng Nga',
      ms: 'Bahasa Rusia',
      bh: 'रूसी भाषा',
      ml: 'റഷ്യൻ ഭാഷ',
      mo: 'Лимба русэ',
      nv: 'Biʼééʼ Łichííʼí bizaad',
      wa: 'Rûsse',
      ja: 'ロシア語',
      ff: 'Riisinkoore',
      yi: 'רוסיש',
      mg: 'Fiteny rosy',
      cu: 'Роусьскъ ѩꙁꙑкъ',
      hy: 'Ռուսերեն',
      li: 'Russisch',
      ko: '러시아어',
      kk: 'Орыс тілі',
      be: 'Руская мова',
      oc: 'Rus',
      nn: 'Russisk',
      my: 'ရုရှားဘာသာ',
      hi: 'रूसी भाषा',
      sv: 'Ryska',
      ar: 'لغة روسية',
      sh: 'Ruski jezik',
      tt: 'Урыс теле',
      sl: 'Ruščina',
      ba: 'Урыҫ теле',
      cv: 'Вырăс чĕлхи',
      ka: 'რუსული ენა',
      cy: 'Rwseg'
    },
    language_code_3: {
      en: 'rus'
    },
    wikidata: {
      en: 'Q7737'
    }
  },
  'en:tahitian': {
    language_code_3: {
      en: 'tah'
    },
    name: {
      fr: 'Tahitien',
      es: 'Idioma tahitiano',
      ca: 'Tahitià',
      en: 'Tahitian',
      cy: 'Tahitïeg',
      pt: 'Língua taitiana',
      eo: 'Tahitia lingvo',
      nn: 'Tahitisk',
      nb: 'Tahitisk',
      de: 'Tahitianische Sprache',
      bg: 'Таитянски език',
      oc: 'Tahitian',
      hu: 'Tahiti nyelv',
      be: 'Таіцянская мова',
      fi: 'Tahitin kieli',
      ty: 'Reo tahiti',
      jv: 'Basa Tahiti',
      id: 'Bahasa Tahiti',
      it: 'Tahitiano',
      qu: 'Tahiti simi',
      sv: 'Tahitiska',
      ar: 'لغة تاهيتية',
      ja: 'タヒチ語',
      zh: '大溪地語',
      sr: 'Тахићански језик',
      et: 'Tahiti keel',
      kv: 'Таити кыв',
      br: 'Tahitieg',
      ko: '타히티어',
      mg: 'Fiteny tahisiana',
      is: 'Tahítíska',
      uk: 'Таїтянська мова',
      sw: 'Kitahiti',
      mk: 'Тахитски јазик',
      mi: 'Reo Tahiti',
      ru: 'Таитянский язык',
      fa: 'زبان تاهیتیایی',
      nl: 'Tahitiaans',
      lt: 'Taitiečių kalba',
      pl: 'Język tahitański',
      hr: 'Tahićanski jezik',
      ta: 'தயீத்திய மொழி',
      vi: 'Tiếng Tahiti',
      lv: 'Taitiešu valoda'
    },
    wikidata: {
      en: 'Q34128'
    },
    language_code_2: {
      en: 'ty'
    }
  },
  'en:interlingua': {
    name: {
      he: 'אינטרלינגואה',
      el: 'Ιντερλίνγκουα',
      io: 'Interlingua',
      ga: 'An Interlingua',
      cy: 'Interlingua',
      la: 'Interlingua',
      eo: 'Interlingvao',
      pt: 'Interlíngua',
      ka: 'ინტერლინგუა',
      cv: 'Интерлингва',
      oc: 'Interlingua',
      kk: 'Интерлингва',
      fi: 'Interlingua',
      be: 'Інтэрлінгва',
      ar: 'إنترلنغوا',
      sh: 'Interlingva',
      sv: 'Interlingua',
      vo: 'Interlingua',
      om: 'Iinterliinguwaa',
      hi: 'अन्तरभाषा',
      id: 'Interlingua',
      sl: 'Interlingua',
      kv: 'Интерлингва',
      sk: 'Interlingua',
      ro: 'Interlingua',
      sr: 'Интерлингва',
      ja: 'インターリングア',
      zh: '国际语',
      co: 'Interlingua',
      ko: '인테르링구아',
      nl: 'Interlingua',
      ru: 'Интерлингва',
      ta: 'நடுநிலை மொழி',
      vi: 'Interlingua',
      ms: 'Interlingua',
      ia: 'Interlingua',
      sc: 'Interlingua',
      sq: 'Gjuha interlingua',
      th: 'ภาษาอินเทอร์ลิงกวา',
      ss: 'Sí-Nterlingua',
      pa: 'ਇੰਟਰ ਲਿੰਗੂਆ',
      ca: 'Interlingua',
      fr: 'Interlingua',
      gl: 'Interlingua',
      es: 'Interlingua',
      en: 'Interlingua',
      mr: 'इंटरलिंग्वा',
      tl: 'Interlingua',
      hu: 'Interlingva',
      ht: 'Interlingua',
      am: 'ኢንተርሊንጉዋ',
      de: 'Interlingua',
      eu: 'Interlingua',
      nb: 'Interlingua',
      it: 'Interlingua',
      qu: 'Interlingua',
      ie: 'Interlingua',
      az: 'İnterlingua',
      et: 'Interlingua',
      cs: 'Interlingua',
      tr: 'İnterlingua',
      uk: 'Інтерлінгва',
      an: 'Interlingua',
      fa: 'زبان اینترلینگوا',
      rm: 'Interlingua',
      bo: 'རྒྱལ་སྤྱིའི་སྐད།',
      lv: 'Interlingva',
      af: 'Interlingua',
      hr: 'Interlingua',
      lt: 'Interlingua',
      pl: 'Interlingua'
    },
    language_code_3: {
      en: 'ina'
    },
    wikidata: {
      en: 'Q35934'
    },
    language_code_2: {
      en: 'ia'
    }
  },
  'en:afar': {
    name: {
      ur: 'عفار زبان',
      de: 'Afar',
      am: 'አፋርኛ',
      eu: 'Afarrera',
      nn: 'Afar',
      nb: 'Afar',
      bg: 'Афарски език',
      fi: 'Afarin kieli',
      so: 'Af-Cafari',
      id: 'Bahasa Afar',
      it: 'Afar',
      sh: 'Afarski jezik',
      ar: 'لغة عفارية',
      hi: 'अफ़ार भाषा',
      sv: 'Afar',
      ca: 'Llengua àfar',
      fr: 'Afar',
      es: 'Idioma afar',
      gl: 'Lingua afar',
      io: 'Afar-linguo',
      he: 'עפרית',
      sq: 'Gjuha afare',
      la: 'Lingua Afarica',
      eo: 'Afara lingvo',
      pt: 'Língua afar',
      en: 'Afar',
      mr: 'अफार भाषा',
      ru: 'Афарский язык',
      fa: 'زبان عفار',
      mk: 'Афарски јазик',
      sw: 'Kiafar',
      nl: 'Afar',
      hr: 'Afarski jezik',
      lt: 'Afarų kalba',
      pl: 'Język afar',
      ta: 'அபர மொழி',
      ms: 'Bahasa Afar',
      lv: 'Afāru valoda',
      cs: 'Afarština',
      ja: 'アファル語',
      zh: '阿法爾語',
      kv: 'Афар кыв',
      az: 'Afar dili',
      sk: 'Afarčina',
      et: 'Afari keel',
      li: 'Afar',
      br: 'Afareg',
      bn: 'আফার ভাষা',
      tr: 'Afarca',
      uk: 'Афар'
    },
    language_code_3: {
      en: 'aar'
    },
    wikidata: {
      en: 'Q27811'
    },
    language_code_2: {
      en: 'aa'
    }
  },
  'en:greenlandic': {
    wikidata: {
      en: 'Q25355'
    },
    name: {
      sh: 'Grenlandski jezik',
      ar: 'لغة جرينلاندية',
      fy: 'Grienlânsk',
      sv: 'Grönländska',
      hi: 'ग्रीनलैंडिक भाषा',
      id: 'Bahasa Kalaallisut',
      it: 'Groenlandese',
      sl: 'Grenlandščina',
      qu: 'Kalalit simi',
      hu: 'Grönlandi nyelv',
      bg: 'Гренландски език',
      fo: 'Grønlendskt mál',
      be: 'Грэнландская мова',
      fi: 'Grönlannin kieli',
      de: 'Grönländisch',
      eu: 'Groenlandiera',
      nb: 'Grønlandsk',
      nn: 'Grønlandsk',
      la: 'Lingua Groenlandica',
      pt: 'Groenlandês',
      eo: 'Gronlanda lingvo',
      ka: 'გრენლანდიური ენა',
      en: 'Greenlandic',
      he: 'גרינלנדית',
      gv: 'Greenlynnish',
      el: 'Γροιλανδική γλώσσα',
      th: 'ภาษากรีนแลนด์',
      kl: 'Kalaallisut',
      ca: 'Grenlandès',
      fr: 'Groenlandais',
      gl: 'Lingua grenlandesa',
      es: 'Groenlandés',
      ga: 'Graonlainnis',
      ta: 'கிரீன்லாந்து மொழி',
      lv: 'Grenlandiešu valoda',
      vi: 'Tiếng Greenland',
      hr: 'Grenlandski jezik',
      lt: 'Grenlandų kalba',
      pl: 'Język grenlandzki',
      nl: 'Groenlands',
      da: 'Grønlandsk',
      ru: 'Гренландский язык',
      ku: 'Kalaallisut',
      fa: 'زبان گرینلندی',
      mk: 'Гренландски јазик',
      tr: 'Grönlandca',
      is: 'Grænlenska',
      uk: 'Ґренландська мова',
      ko: '그린란드어',
      hy: 'Գրենլանդերեն',
      br: 'Greunlandeg',
      kv: 'Гренланд кыв',
      sk: 'Grónčina',
      et: 'Grööni keel',
      cs: 'Grónština',
      sr: 'Гренландски језик',
      ja: 'グリーンランド語',
      zh: '格陵兰语'
    },
    language_code_3: {
      en: 'kal'
    },
    language_code_2: {
      en: 'kl'
    }
  },
  'en:latin': {
    language_code_2: {
      en: 'la'
    },
    wikidata: {
      en: 'Q397'
    },
    language_code_3: {
      en: 'lat'
    },
    name: {
      ca: 'Llatí',
      pa: 'ਲਾਤੀਨੀ ਭਾਸ਼ਾ',
      gl: 'Lingua latina',
      es: 'Latín',
      fr: 'Latin',
      sc: 'Limba latina',
      th: 'ภาษาละติน',
      sq: 'Gjuha latine',
      gv: 'Ladjyn',
      ln: 'Latina',
      tl: 'Wikang Latin',
      mr: 'लॅटिन भाषा',
      en: 'Latin',
      am: 'ሮማይስጥ',
      ur: 'لاطینی زبان',
      de: 'Latein',
      nb: 'Latin',
      eu: 'Latin',
      fo: 'Latín',
      sd: 'لاطيني ٻولي',
      hu: 'Latin',
      bg: 'Латински език',
      it: 'Latino',
      qu: 'Latin simi',
      so: 'Af-Laatiin',
      ie: 'Latin',
      su: 'Basa Latin',
      fy: 'Latyn',
      ab: 'Алаҭын бызшәа',
      se: 'Láhtengiella',
      ky: 'Латын тили',
      cs: 'Latina',
      ps: 'لاتين ژبه',
      az: 'Latın dili',
      et: 'Ladina keel',
      lo: 'ພາສາລາແຕັງ',
      zu: 'ILatin',
      ug: 'لاتىن تىلى',
      os: 'Латинаг æвзаг',
      bn: 'লাতিন ভাষা',
      uk: 'Латинська мова',
      tr: 'Latince',
      is: 'Latína',
      na: 'Dorerin Latin',
      ce: 'Латинан мотт',
      fa: 'زبان لاتین',
      an: 'Latín',
      tg: 'Забони лотинӣ',
      bo: 'ལ་ཏིན​་སྐད།',
      rm: 'Latin',
      si: 'ලතින්',
      mk: 'Латински јазик',
      da: 'Latin',
      ny: 'Chilatini',
      hr: 'Latinski jezik',
      lb: 'Latäin',
      pl: 'Łacina',
      lt: 'Lotynų kalba',
      lv: 'Latīņu valoda',
      gd: 'Laideann',
      af: 'Latyn',
      kl: 'Latiinerisut',
      io: 'Latina linguo',
      ga: 'An Laidin',
      kg: 'Kilatini',
      he: 'לטינית',
      el: 'Λατινική γλώσσα',
      bs: 'Latinski jezik',
      uz: 'Lotin tili',
      eo: 'Latina lingvo',
      pt: 'Latim',
      ka: 'ლათინური ენა',
      la: 'Lingua Latina',
      cy: 'Lladin',
      cv: 'Латин чĕлхи',
      mt: 'Latin',
      nn: 'Latin',
      kk: 'Латын тілі',
      fi: 'Latina',
      be: 'Лацінская мова',
      oc: 'Latin',
      sl: 'Latinščina',
      or: 'ଲାଟିନ ଭାଷା',
      id: 'Bahasa Latin',
      ba: 'Латин теле',
      te: 'లాటిన్',
      jv: 'Basa Latin',
      tt: 'Латин теле',
      ar: 'لغة لاتينية',
      sh: 'Latinski jezik',
      hi: 'लातिन भाषा',
      vo: 'Latänapük',
      sv: 'Latin',
      kw: 'Latin',
      sr: 'Латински језик',
      ro: 'Limba latină',
      yi: 'לאטיין',
      wa: 'Latén',
      zh: '拉丁语',
      mn: 'Латин хэл',
      ja: 'ラテン語',
      gn: "Latinañe'ẽ",
      sk: 'Latinčina',
      rw: 'Ikilatini',
      kv: 'Латин кыв',
      ml: 'ലാറ്റിൻ',
      li: 'Latien',
      hy: 'Լատիներեն',
      ko: '라틴어',
      br: 'Latin',
      cu: 'Латиньскъ ѩꙁꙑкъ',
      co: 'Lingua latina',
      mg: 'Fiteny latina',
      yo: 'Èdè Látìnì',
      ku: 'Zimanê latînî',
      bm: 'Latin',
      ru: 'Латинский язык',
      sw: 'Kilatini',
      nl: 'Latijn',
      tk: 'Latyn dili',
      ne: 'ल्याटिन',
      ms: 'Bahasa Latin',
      vi: 'Latinh',
      kn: 'ಲ್ಯಾಟಿನ್',
      ta: 'இலத்தீன்',
      sa: 'लातिनी',
      ia: 'Lingua latin'
    }
  },
  'en:chinese': {
    wikidata: {
      en: 'Q7850'
    },
    language_code_3: {
      en: 'chi'
    },
    name: {
      se: 'Kiinnágiella',
      ky: 'Кытай тили',
      cs: 'Čínština',
      lo: 'ພາສາຈີນ',
      et: 'Hiina keel',
      az: 'Çin dili',
      ps: 'چيني',
      bn: 'চীনা ভাষা',
      ug: 'خەنزۇ تىلى',
      zu: 'IsiShayina',
      ce: 'Китайн мотт',
      tr: 'Çince',
      na: 'Dorerin Tsiene',
      is: 'Kínverska',
      uk: 'Китайська мова',
      mk: 'Кинески јазик',
      bo: 'རྒྱ་སྐད།',
      tg: 'Забони чинӣ',
      an: 'Idioma chinés',
      fa: 'زبان‌های چینی',
      da: 'Kinesisk',
      lt: 'Kinų kalba',
      pl: 'Język chiński',
      hr: 'Kineski jezik',
      gd: 'Sìonais',
      af: 'Sjinees',
      lv: 'Ķīniešu valoda',
      fr: 'Langues chinoises',
      gl: 'Lingua chinesa',
      es: 'Chino',
      pa: 'ਚੀਨੀ ਭਾਸ਼ਾ',
      ca: 'Xinès',
      gv: 'Çhengaghyn Sheenagh',
      sq: 'Gjuha kineze',
      th: 'กลุ่มภาษาจีน',
      tw: 'Chinese',
      tl: 'Wikang Tsino',
      sm: "Fa'asaina",
      ln: 'Lisinwa',
      en: 'Chinese',
      mr: 'चिनी भाषा',
      eu: 'Txinera',
      nb: 'Kinesisk',
      ur: 'چینی زبان',
      am: 'ቻይንኛ',
      de: 'Chinesische Sprachen',
      bg: 'Китайски език',
      hu: 'Kínai',
      sd: 'چيني ٻولي',
      fo: 'Kinesiskt mál',
      ty: 'Reo Tinitō',
      dv: 'ސީނީ',
      qu: 'Chun simi',
      it: 'Cinese',
      fy: 'Sineesk',
      km: 'ភាសាចិន',
      ja: '中国語',
      gn: "Chinañe'ẽ",
      mn: 'Хятад хэл',
      zh: '汉语',
      wa: 'Chinwès',
      yi: 'כינעזיש',
      ro: 'Limba chineză',
      sr: 'Кинески језик',
      kw: 'Chinek',
      ml: 'ചൈനീസ് ഭാഷ',
      kv: 'Китай кыв',
      sk: 'Čínština',
      br: 'Yezhoù sinaek',
      ko: '중국어',
      hy: 'Չինարեն',
      li: 'Chinees',
      gu: 'ચાઇનીઝ ભાષા',
      mg: 'Fiteny sinoa',
      cu: 'Срѣдиньскъ ѩꙁꙑкъ',
      sw: 'Kichina',
      st: 'Se-china',
      ru: 'Китайский язык',
      yo: 'Èdè Ṣáínà',
      nl: 'Chinese talen',
      za: 'Vahgun',
      ne: 'चिनियाँ भाषा',
      tk: 'Hytaý dili',
      sa: 'चीनी भाषा',
      ta: 'சீன மொழி',
      bh: 'चीनी भाषा',
      ms: 'Bahasa Cina',
      vi: 'Tiếng Hoa',
      kn: 'ಚೀನಿ ಭಾಷೆ',
      io: 'Chiniana linguo',
      ga: 'An tSínis',
      kl: 'Kineserisut',
      el: 'Κινεζική γλώσσα',
      he: 'שפות סיניות',
      kg: 'Kitsunkwo',
      bs: 'Kineski jezik',
      cv: 'Китай чĕлхи',
      cy: 'Tsieinëeg',
      la: 'Lingua Sinica',
      uz: 'Xitoy tili',
      ka: 'ჩინური ენა',
      pt: 'Língua chinesa',
      eo: 'Ĉina lingvo',
      nn: 'Kinesisk',
      oc: 'Chinés',
      kk: 'Қытай тілі',
      be: 'Кітайская мова',
      fi: 'Kiina',
      tt: 'Кытай теле',
      te: 'చైనీస్ భాష',
      jv: 'Basa Cina',
      id: 'Rumpun bahasa Tionghoa',
      ba: 'Ҡытай теле',
      sl: 'Kitajščina',
      vo: 'Tsyinänapük',
      sv: 'Kinesiska',
      hi: 'चीनी भाषा',
      my: 'တရုတ်ဘာသာ',
      ar: 'لغة صينية',
      sh: 'Kineski jezik'
    },
    language_code_2: {
      en: 'zh'
    }
  },
  'en:turkmen': {
    wikidata: {
      en: 'Q9267'
    },
    name: {
      he: 'טורקמנית',
      ga: 'An Tuircméinis',
      cv: 'Турккăлла чĕлхи',
      la: 'Lingua Turcomannica',
      pt: 'Língua turcomena',
      uz: 'Turkman tili',
      ka: 'თურქმენული ენა',
      eo: 'Turkmena lingvo',
      oc: 'Turcmèn',
      kk: 'Түрікмен тілі',
      be: 'Туркменская мова',
      fi: 'Turkmeenin kieli',
      nn: 'Turkmensk',
      vo: 'Turkmenänapük',
      sv: 'Turkmeniska',
      hi: 'तुर्कमेन भाषा',
      sh: 'Turkmenski jezik',
      ar: 'لغة تركمانية',
      tt: 'Төрекмән теле',
      id: 'Bahasa Turkmen',
      ba: 'Төрөкмән теле',
      kv: 'Туркмен кыв',
      sk: 'Turkménčina',
      rw: 'Igiturukimeni',
      ja: 'トルクメン語',
      mn: 'Түркмен хэл',
      zh: '土库曼语',
      ro: 'Limba turkmenă',
      sr: 'Туркменски језик',
      br: 'Turkmeneg',
      ko: '투르크멘어',
      hy: 'Թուրքմեներեն',
      nl: 'Turkmeens',
      ru: 'Туркменский язык',
      yo: 'Èdè Turkmẹ́nì',
      ku: 'Zimanê tirkmenî',
      ta: 'துருக்குமேனிய மொழி',
      vi: 'Tiếng Turkmen',
      ms: 'Bahasa Turkmen',
      kn: 'ತುರ್ಕ್‌ಮೇನಿಸ್ತಾನ್‌ನ ಭಾಷೆ',
      tk: 'Türkmen dili',
      th: 'ภาษาเติร์กเมน',
      fr: 'Turkmène',
      gl: 'Lingua turcomá',
      es: 'Turcomano',
      ca: 'Turcman',
      mr: 'तुर्कमेन भाषा',
      en: 'Turkmen',
      tl: 'Wikang Turkmen',
      bg: 'Туркменски език',
      eu: 'Turkmenera',
      nb: 'Turkmensk',
      de: 'Turkmenische Sprache',
      am: 'ቱርክመንኛ',
      it: 'Turkmeno',
      qu: 'Turkmin simi',
      et: 'Türkmeeni keel',
      az: 'Türkmən dili',
      ps: 'ترکمني ژبه',
      cs: 'Turkmenština',
      ce: 'Туркменийн мотт',
      tr: 'Türkmence',
      uk: 'Туркменська мова',
      bn: 'তুর্কমেনীয় ভাষা',
      ug: 'تۈركمەن تىلى',
      da: 'Turkmensk',
      mk: 'Туркменски јазик',
      tg: 'Забони туркменӣ',
      fa: 'زبان ترکمنی',
      af: 'Turkmeens',
      lv: 'Turkmēņu valoda',
      lt: 'Turkmėnų kalba',
      pl: 'Język turkmeński',
      hr: 'Turkmenski jezik'
    },
    language_code_3: {
      en: 'tuk'
    },
    language_code_2: {
      en: 'tk'
    }
  },
  'en:west-frisian': {
    wikidata: {
      en: 'Q27175'
    },
    language_code_3: {
      en: 'fry'
    },
    name: {
      id: 'Bahasa Frisia Barat',
      it: 'Frisone occidentale',
      sv: 'Västfrisiska',
      vo: 'Vesüda-Frisänapük',
      ar: 'اللغة الفريزية الغربية',
      sh: 'Zapadnofrizijski jezik',
      fy: 'Frysk',
      eu: 'Mendebaldeko frisiera',
      nb: 'Vestfrisisk',
      nn: 'Vestfrisisk',
      de: 'Westfriesische Sprache',
      fo: 'Vesturfrísiskt mál',
      be: 'Заходнефрызская мова',
      fi: 'Länsifriisin kieli',
      en: 'West Frisian',
      eo: 'Okcidentfrisa lingvo',
      fr: 'Frison occidental',
      es: 'Frisón occidental',
      gl: 'Lingua frisoa occidental',
      ca: 'Frisó occidental',
      ne: 'पश्चिमी फ्रिजी',
      pl: 'Język zachodniofryzyjski',
      hr: 'Zapadnofrizijski jezik',
      af: 'Wes-Fries',
      ta: 'மேல் விரிசிய மொழி',
      vi: 'Tiếng Tây Frisia',
      lv: 'Rietumfrīzu valoda',
      mk: 'Западнофризиски јазик',
      ru: 'Западнофризский язык',
      an: 'Idioma frisón occidental',
      fa: 'زبان فریسکی',
      nl: 'Westerlauwers Fries',
      da: 'Westerlauwersfrisisk',
      br: "Frizeg ar c'hornôg",
      li: 'Westerlauwers Fries',
      hy: 'Արևմտաֆրիզերեն',
      uk: 'Західнофризька мова',
      ja: '西フリジア語',
      zh: '西弗里斯語',
      sr: 'Западнофризијски језик',
      cs: 'Západofríština',
      az: 'Qərbi friz dili'
    },
    language_code_2: {
      en: 'fy'
    }
  },
  'en:tsonga': {
    wikidata: {
      en: 'Q34327'
    },
    language_code_3: {
      en: 'tso'
    },
    name: {
      uk: 'Тсонга',
      tr: 'Tsonga dili',
      is: 'Tsonga',
      ko: '총가어',
      br: 'Tsongeg',
      az: 'Tsonqa dili',
      kv: 'Тсонга',
      ts: 'Xitsonga',
      ja: 'ツォンガ語',
      vi: 'Tiếng Tsonga',
      ta: 'சோங்க மொழி',
      af: 'Tsonga',
      hr: 'Tsonga jezik',
      pl: 'Język tsonga',
      nl: 'Tsonga',
      fa: 'زبان شنگانی',
      yo: 'Èdè Tsonga',
      ru: 'Тсонга',
      sw: 'Kitsonga',
      st: 'Setsonga',
      mk: 'Цонга',
      pt: 'Língua tsonga',
      eo: 'Conga lingvo',
      en: 'Tsonga',
      he: 'צונגה',
      sq: 'Gjuha tsonga',
      ca: 'Tsonga',
      io: 'Conga-linguo',
      es: 'Tsonga',
      fr: 'Tsonga',
      ar: 'لغة تسونجا',
      sv: 'Tsonga',
      it: 'Tsonga',
      id: 'Bahasa Tsonga',
      fi: 'Tsongan kieli',
      fo: 'Tsonga mál',
      oc: 'Tsonga',
      bg: 'Цонга',
      ve: 'XiTsonga',
      de: 'Xitsonga',
      nn: 'Tsonga',
      nb: 'Tsonga',
      eu: 'Tsongera'
    },
    language_code_2: {
      en: 'ts'
    }
  },
  'en:romansh': {
    wikidata: {
      en: 'Q13199'
    },
    name: {
      bs: 'Retoromanski jezik',
      eo: 'Romanĉa lingvo',
      pt: 'Língua romanche',
      ka: 'რეტორომანული ენა',
      la: 'Lingua Rhaetica',
      cy: 'Romaunsch',
      io: 'Romancha linguo',
      ga: 'An Rómainis',
      el: 'Ρομανσική γλώσσα',
      he: 'רומאנש',
      jv: 'Basa Romansh',
      sl: 'Retoromanščina',
      id: 'Bahasa Romansh',
      hi: 'रूमान्त्च भाषा',
      sv: 'Rätoromanska',
      ar: 'لغة رومانشية',
      sh: 'Romanš',
      nn: 'Retoromansk',
      kk: 'Романш тілі',
      be: 'Рэтараманская мова',
      fi: 'Retoromaani',
      oc: 'Romanch',
      br: 'Romañcheg',
      li: 'Reto-Romaans',
      ko: '로만슈어',
      mg: 'Fiteny romantsa',
      wa: 'Roumantche',
      zh: '罗曼什语',
      ja: 'ロマンシュ語',
      kw: 'Roumantsh',
      sr: 'Романш језик',
      yi: 'רומאנטש',
      ro: 'Limba retoromană',
      sk: 'Švajčiarska rétorománčina',
      kv: 'Романш кыв',
      ia: 'Romancio',
      ms: 'Bahasa Romansh',
      vi: 'Tiếng Romansh',
      ta: 'உரோமாஞ்சு மொழி',
      sw: 'Kirumanj',
      ru: 'Романшский язык',
      nl: 'Reto-Romaans',
      tl: 'Wikang Romansh',
      en: 'Romansh',
      mr: 'रोमान्श भाषा',
      gl: 'Lingua retorrománica',
      es: 'Romanche',
      fr: 'Romanche',
      ca: 'Romanx',
      sq: 'Gjuha romançe',
      gv: 'Romansh',
      sc: 'Limba romancia',
      it: 'Romancio',
      nb: 'Retoromansk',
      eu: 'Erretorromaniera',
      am: 'ሮማንሽ',
      de: 'Rätoromanisch',
      hu: 'Romans',
      bg: 'Романш',
      ug: 'دۇنيا تىلى',
      os: 'Романшаг æвзаг',
      uk: 'Ретороманська мова',
      tr: 'Romanşça',
      cs: 'Rétorománština',
      et: 'Romanši keel',
      pl: 'Język romansz',
      lt: 'Retoromanų kalba',
      hr: 'Romanš',
      lb: 'Rätoromanesch',
      gd: 'Rumantsch',
      af: 'Romansch',
      lv: 'Retoromāņu valoda',
      rm: 'Rumantsch dal Grischun',
      mk: 'Реторомански јазик',
      fa: 'زبان رومانش',
      an: 'Idioma rumanch',
      da: 'Rætoromansk'
    },
    language_code_3: {
      en: 'roh'
    },
    language_code_2: {
      en: 'rm'
    }
  },
  'en:inupiat-language': {
    wikidata: {
      en: 'Q27183'
    },
    name: {
      ja: 'イヌピアック語',
      fr: 'Inupiak',
      es: 'Idioma iñupiaq',
      cs: 'Inupiaq',
      ca: 'Inupiaq',
      et: 'Injupiaki keel',
      az: 'İnyupik dili',
      ik: 'Iñupiatun',
      en: 'Inupiat language',
      tr: 'İnyupikçe',
      pt: 'Língua inupiat',
      uk: 'Аляскинсько-інуїтська мова',
      eo: 'Inupiaka lingvo',
      nn: 'Iñupiaq',
      nb: 'Inupiak',
      ru: 'Аляскинско-инуитские языки',
      de: 'Inupiaq',
      fa: 'زبان اینوپیات',
      nl: 'Inupiak',
      fi: 'Inupiatun',
      lt: 'Inupiakų kalba',
      pl: 'Język inupiak',
      id: 'Bahasa Inupiaq',
      it: 'Inupiaq',
      hi: 'इनुपियाक भाषा',
      sv: 'Iñupiaq',
      ta: 'இனுபிக்கு மொழி'
    },
    language_code_3: {
      en: 'ipk'
    },
    language_code_2: {
      en: 'ik'
    }
  },
  'en:tajik': {
    language_code_2: {
      en: 'tg'
    },
    wikidata: {
      en: 'Q9260'
    },
    name: {
      nl: 'Tadzjieks',
      sw: 'Kitajiki',
      ru: 'Таджикский язык',
      ku: 'Zimanî tacîkî',
      ta: 'தாஜிக் மொழி',
      vi: 'Tiếng Tajik',
      kv: 'Таджик кыв',
      rw: 'Igitajiki',
      sk: 'Tadžičtina',
      ja: 'タジク語',
      zh: '塔吉克语',
      ro: 'Limba tadjică',
      sr: 'Таџички језик',
      br: 'Tadjikeg',
      ko: '타지크어',
      hy: 'Տաջիկերեն',
      oc: 'Tadjik',
      kk: 'Тәжік тілі',
      fi: 'Tadžikin kieli',
      be: 'Таджыкская мова',
      nn: 'Tadsjikisk',
      hi: 'ताजिकी भाषा',
      vo: 'Tacikistänapük',
      sv: 'Tadzjikiska',
      sh: 'Tadžički jezik',
      ar: 'لغة طاجيكية',
      id: 'Bahasa Tajik',
      el: 'Τατζίκ γλώσσα',
      he: "טג'יקית",
      ga: 'An Táidsícis',
      cv: 'Таджик чĕлхи',
      la: 'Lingua Tadzikica',
      uz: 'Tojik tili',
      pt: 'Língua tadjique',
      ka: 'ტაჯიკური ენა',
      eo: 'Taĝika lingvo',
      bs: 'Tadžički jezik',
      da: 'Tadsjikisk',
      mk: 'Таџички јазик',
      tg: 'Забони тоҷикӣ',
      fa: 'فارسی تاجیکی',
      af: 'Tadjiks',
      lv: 'Tadžiku valoda',
      lt: 'Tadžikų kalba',
      pl: 'Język tadżycki',
      hr: 'Tadžički jezik',
      et: 'Tadžiki keel',
      az: 'Tacik dili',
      ps: 'تاجکي ژبه',
      cs: 'Tádžičtina',
      ce: 'Таджикийн мотт',
      tr: 'Tacikçe',
      uk: 'Таджицька мова',
      ug: 'تاجىك تىلى',
      os: 'Таджикаг æвзаг',
      hu: 'Tádzsik nyelv',
      bg: 'Таджикски език',
      eu: 'Tajikera',
      nb: 'Tadsjikisk',
      de: 'Tadschikische Sprache',
      ur: 'تاجک زبان',
      am: 'ታጂክኛ',
      qu: 'Tayik simi',
      it: 'Tagico',
      th: 'ภาษาทาจิก',
      fr: 'Tadjik',
      gl: 'Lingua taxica',
      es: 'Tayiko',
      pa: 'ਤਾਜਿਕ ਭਾਸ਼ਾ',
      ca: 'Tadjik',
      mr: 'ताजिक भाषा',
      en: 'Tajik'
    },
    language_code_3: {
      en: 'tgk'
    }
  },
  'en:burmese': {
    language_code_2: {
      en: 'my'
    },
    language_code_3: {
      en: 'mya'
    },
    name: {
      uk: 'Бірманська мова',
      is: 'Búrmíska',
      tr: 'Burmaca',
      ug: 'بىرما تىلى',
      ko: '버마어',
      bn: 'বর্মী ভাষা',
      br: 'Birmaneg',
      kv: 'Бирма кыв',
      az: 'Birman dili',
      et: 'Birma keel',
      cs: 'Barmština',
      ky: 'Бирма тили',
      ro: 'Limba birmană',
      zh: '缅甸语',
      ja: 'ビルマ語',
      vi: 'Tiếng Miến Điện',
      lv: 'Birmiešu valoda',
      ms: 'Bahasa Burma',
      ta: 'பருமிய மொழி',
      af: 'Burmees',
      pl: 'Język birmański',
      lt: 'Mjanmų kalba',
      nl: 'Birmaans',
      fa: 'زبان برمه‌ای',
      ru: 'Бирманский язык',
      bo: 'འབར་མའི་སྐད།',
      mk: 'Мјанмарски јазик',
      ka: 'ბირმული ენა',
      pt: 'Língua birmanesa',
      eo: 'Birma lingvo',
      cy: 'Byrmaneg',
      la: 'Lingua Birmanica',
      en: 'Burmese',
      mr: 'बर्मी भाषा',
      tl: 'Wikang Burmes',
      th: 'ภาษาพม่า',
      ca: 'Birmà',
      pa: 'ਬਰਮੀ ਭਾਸ਼ਾ',
      es: 'Birmano',
      gl: 'Lingua birmana',
      fr: 'Birman',
      ar: 'لغة بورمية',
      my: 'မြန်မာဘာသာစကား',
      sv: 'Burmesiska',
      hi: 'बर्मी भाषा',
      qu: 'Birmanu simi',
      or: 'ବର୍ମୀଜ ଭାଷା',
      it: 'Birmano',
      id: 'Bahasa Myanmar',
      jv: 'Basa Myanmar',
      be: 'Бірманская мова',
      fi: 'Burman kieli',
      oc: 'Lenga birmana',
      hu: 'Burmai',
      bg: 'Бирмански език',
      de: 'Birmanisch',
      nn: 'Burmesisk',
      nb: 'Burmesisk',
      eu: 'Birmaniera'
    },
    wikidata: {
      en: 'Q9228'
    }
  },
  'en:javanese': {
    language_code_2: {
      en: 'jv'
    },
    wikidata: {
      en: 'Q33549'
    },
    language_code_3: {
      en: 'jav'
    },
    name: {
      qu: 'Yawa simi',
      it: 'Giavanese',
      su: 'Basa Jawa',
      bg: 'Явански език',
      de: 'Javanische Sprache',
      eu: 'Javera',
      nb: 'Javanesisk',
      en: 'Javanese',
      mr: 'बासा जावा',
      th: 'ภาษาชวา',
      pa: 'ਜਾਵਾਨੀ ਬੋਲੀ',
      ca: 'Javanès',
      fr: 'Javanais',
      gl: 'Lingua xavanesa',
      es: 'Idioma javanés',
      lv: 'Javiešu valoda',
      af: 'Javaans',
      hr: 'Javanski jezik',
      lt: 'Javiečių kalba',
      pl: 'Język jawajski',
      fa: 'زبان جاوه‌ای',
      mk: 'Јавански јазик',
      tg: 'Забони явагӣ',
      tr: 'Cava dili',
      uk: 'Яванська мова',
      ug: 'ياۋا تىلى',
      os: 'Явæйаг æвзаг',
      bn: 'জাভানীয় ভাষা',
      az: 'Yavan dili',
      et: 'Jaava keel',
      cs: 'Javánština',
      sh: 'Javanski jezik',
      ar: 'لغة جاوية',
      hi: 'जावा भाषा',
      vo: 'Yafeänapük',
      sv: 'Javanesiska',
      id: 'Bahasa Jawa',
      tt: 'Яван теле',
      jv: 'Basa Jawa',
      fi: 'Jaavan kieli',
      be: 'Яванская мова',
      nn: 'Javanesisk',
      la: 'Lingua Iavensis',
      cy: 'Jafaneg',
      pt: 'Língua javanesa',
      uz: 'Yava tili',
      eo: 'Java lingvo',
      ka: 'იავური ენა',
      he: "ג'אווה",
      el: 'Ιαβαϊκή γλώσσα',
      ga: 'An Iáivis',
      ta: 'சாவகம்',
      kn: 'ಜಾವಾದ ಭಾಷೆ',
      vi: 'Tiếng Java',
      ms: 'Bahasa Jawa',
      nl: 'Javaans',
      ru: 'Яванский язык',
      yo: 'Èdè Java',
      sw: 'Kijava',
      mg: 'Fiteny javaney',
      ko: '자와어',
      br: 'Javaeg',
      kv: 'Ява кыв',
      ro: 'Limba javaneză',
      sr: 'Јавански језик',
      ja: 'ジャワ語',
      zh: '爪哇語'
    }
  },
  'en:chechen': {
    language_code_2: {
      en: 'ce'
    },
    wikidata: {
      en: 'Q33350'
    },
    name: {
      he: "צ'צ'נית",
      th: 'ภาษาเชเชน',
      ca: 'Txetxè',
      es: 'Checheno',
      gl: 'Lingua chechena',
      fr: 'Tchétchène',
      eo: 'Ĉeĉena lingvo',
      uz: 'Chechen tili',
      pt: 'Checheno',
      ka: 'ჩეჩნური ენა',
      cy: 'Tsietsnieg',
      en: 'Chechen',
      mr: 'चेचन भाषा',
      cv: 'Чечен чĕлхи',
      be: 'Чачэнская мова',
      fi: 'Tšetšeenin kieli',
      oc: 'Chechèn',
      hu: 'Csecsen',
      bg: 'Чеченски език',
      ur: 'چیچنی زبان',
      de: 'Tschetschenisch',
      nb: 'Tsjetsjensk',
      nn: 'Tsjetsjensk',
      eu: 'Txetxenera',
      sh: 'Čečenski jezik',
      ar: 'لغة شيشانية',
      sv: 'Tjetjenska',
      it: 'Ceceno',
      sl: 'Čečenščina',
      id: 'Bahasa Chechnya',
      rw: 'Igiceceni',
      kv: 'Нохчи кыв',
      az: 'Çeçen dili',
      et: 'Tšetšeeni keel',
      cs: 'Čečenština',
      sr: 'Чеченски језик',
      ro: 'Limba cecenă',
      av: 'Чачан мацІ',
      zh: '車臣語',
      ja: 'チェチェン語',
      uk: 'Чеченська мова',
      tr: 'Çeçence',
      is: 'Téténska',
      ce: 'Нохчий мотт',
      hy: 'Չեչեներեն',
      os: 'Цæцæйнаг æвзаг',
      ko: '체첸어',
      br: 'Tchetcheneg',
      da: 'Tjetjensk',
      nl: 'Tsjetsjeens',
      fa: 'زبان چچنی',
      ku: 'Zimanê çeçenî',
      ru: 'Чеченский язык',
      mk: 'Чеченски јазик',
      ms: 'Bahasa Chechen',
      vi: 'Tiếng Chechnya',
      lv: 'Čečenu valoda',
      ta: 'செச்சன மொழி',
      af: 'Tsjetsjnies',
      hr: 'Čečenski jezik',
      pl: 'Język czeczeński',
      lt: 'Čečėnų kalba'
    },
    language_code_3: {
      en: 'che'
    }
  },
  'en:assamese': {
    language_code_2: {
      en: 'as'
    },
    language_code_3: {
      en: 'asm'
    },
    name: {
      nn: 'Assamesisk',
      be: 'Асамская мова',
      fi: 'Assami',
      id: 'Bahasa Assam',
      or: 'ଅସମୀୟା ଭାଷା',
      te: 'అస్సామీ భాష',
      ar: 'لغة أسامية',
      sh: 'Asamski jezik',
      sv: 'Assamesiska',
      hi: 'असमिया',
      he: 'אסאמית',
      as: 'অসমীয়া ভাষা',
      el: 'Ασσαμέζικη γλώσσα',
      cy: 'Assameg',
      la: 'Lingua assamica',
      pt: 'Língua assamesa',
      ka: 'ასამური ენა',
      eo: 'Asama lingvo',
      ru: 'Ассамский язык',
      ku: 'Zimanê assamî',
      yo: 'Asamiisi',
      sw: 'Kiassam',
      nl: 'Assamees',
      ne: 'असमिया',
      ta: 'அசாமிய மொழி',
      vi: 'Tiếng Assam',
      ms: 'Bahasa Assam',
      kn: 'ಅಸ್ಸಾಮಿ',
      sa: 'असमिया',
      ro: 'Limba assameză',
      sr: 'Асамски језик',
      ja: 'アッサム語',
      zh: '阿萨姆语',
      kv: 'Ассам кыв',
      sk: 'Ásamčina',
      ml: 'ആസ്സാമീസ്',
      ko: '아삼어',
      hy: 'Ասսամերեն',
      gu: 'આસામીઝ ભાષા',
      br: 'Asameg',
      mg: 'Fiteny assamey',
      de: 'Assamesische Sprache',
      ur: 'آسامی زبان',
      eu: 'Assamera',
      nb: 'Assamesisk',
      hu: 'Asszámi nyelv',
      qu: 'Asam simi',
      it: 'Assamese',
      pa: 'ਆਸਾਮੀ ਭਾਸ਼ਾ',
      ca: 'Assamès',
      fr: 'Assamais',
      gl: 'Lingua asamesa',
      es: 'Asamés',
      th: 'ภาษาอัสสัม',
      tl: 'Wikang Asames',
      en: 'Assamese',
      mr: 'आसामी भाषा',
      fa: 'زبان آسامی',
      mk: 'Асамски јазик',
      da: 'Assamesisk',
      hr: 'Asamski jezik',
      lt: 'Asamų kalba',
      pl: 'Język asamski',
      lv: 'Asamiešu valoda',
      af: 'Assamees',
      cs: 'Ásámština',
      az: 'Assam dili',
      et: 'Assami keel',
      bn: 'অসমীয়া ভাষা',
      is: 'Assameíska',
      tr: 'Assam dili',
      uk: 'Асамська мова'
    },
    wikidata: {
      en: 'Q29401'
    }
  },
  'en:unknown-language': {
    language_code_2: {
      en: 'xx'
    },
    name: {
      fr: 'Langue inconnue',
      xx: 'Unknown language',
      en: 'Unknown language'
    },
    language_code_3: {
      en: 'abk'
    },
    wikidata: {
      en: 'Q5111'
    }
  },
  'en:arabic': {
    wikidata: {
      en: 'Q13955'
    },
    name: {
      pa: 'ਅਰਬੀ ਭਾਸ਼ਾ',
      ca: 'Àrab',
      gv: 'Arabish',
      th: 'ภาษาอาหรับ',
      ln: 'Liarabi',
      tl: 'Wikang Arabe',
      mr: 'अरबी भाषा',
      en: 'Arabic',
      ur: 'عربی زبان',
      fo: 'Arábiskt mál',
      so: 'Carabi',
      su: 'Basa Arab',
      ky: 'Араб тили',
      av: 'ГІараб мацІ',
      az: 'Ərəb dili',
      ps: 'عربي',
      lo: 'ພາສາອາຣັບ',
      et: 'Araabia keel',
      is: 'Arabíska',
      uk: 'Арабська мова',
      ce: 'Ӏаьрбийн мотт',
      an: 'Idioma arabe',
      iu: 'ᐊᕋᕕ',
      fa: 'زبان عربی',
      bo: 'ཨ་རབ་སྐད།',
      tg: 'Забони арабӣ',
      da: 'Arabisk',
      lb: 'Arabesch',
      lt: 'Arabų kalba',
      lv: 'Arābu valoda',
      gd: 'Arabais',
      ks: 'اَربی زَبانَ',
      kg: 'Kilabu',
      cy: 'Arabeg',
      ka: 'არაბული ენა',
      cv: 'Арап чĕлхи',
      nn: 'Arabisk',
      oc: 'Arabi',
      kk: 'Араб тілі',
      be: 'Арабская мова',
      ba: 'Ғәрәп теле',
      sl: 'Arabščina',
      tt: 'Ğäräp tele',
      ar: 'العربية',
      sh: 'Arapski jezik',
      sv: 'Arabiska',
      hi: 'अरबी भाषा',
      my: 'အာရပ်ဘာသာ',
      yi: 'אראביש',
      ja: 'アラビア語',
      wa: 'Arabe',
      nv: 'Ásáí Bizaad',
      ml: 'അറബി ഭാഷ',
      ko: '아랍어',
      li: 'Arabisch',
      hy: 'Արաբերեն',
      cu: 'Аравьскъ ѩꙁꙑкъ',
      mg: 'Fiteny arabo',
      mi: 'Reo Ārapi',
      sw: 'Kiarabu',
      nl: 'Arabisch',
      vi: 'Tiếng Ả Rập',
      ms: 'Bahasa Arab',
      ia: 'Lingua arabe',
      fr: 'Arabe',
      es: 'Árabe',
      gl: 'Lingua árabe',
      sc: 'Limba araba',
      sq: 'Gjuha arabe',
      de: 'Arabisch',
      am: 'ዓረብኛ',
      eu: 'Arabiera',
      nb: 'Arabisk',
      dv: 'ޢަރަބި',
      hu: 'Arab',
      bg: 'Арабски език',
      sd: 'عربي ٻولي',
      qu: 'Arabya simi',
      it: 'Arabo',
      fy: 'Arabysk',
      cs: 'Arabština',
      se: 'Arábagiella',
      ug: 'ئەرەب تىلى',
      os: 'Араббаг æвзаг',
      bn: 'আরবি ভাষা',
      tr: 'Arapça',
      mk: 'Арапски јазик',
      ny: 'Chiarabu',
      hr: 'Arapski jezik',
      pl: 'Język arabski',
      af: 'Arabies',
      kl: 'Arabiamiutut',
      io: 'Arabiana linguo',
      ga: 'An Araibis',
      he: 'ערבית',
      el: 'Αραβική γλώσσα',
      bs: 'Arapski jezik',
      la: 'Lingua Arabica',
      uz: 'Arab tili',
      eo: 'Araba lingvo',
      pt: 'Língua árabe',
      mt: 'Għarbi',
      fi: 'Arabia',
      id: 'Bahasa Arab',
      jv: 'Basa Arab',
      te: 'అరబ్బీ భాష',
      vo: 'Larabänapük',
      om: 'Afaan Arabaa',
      ro: 'Limba arabă',
      sr: 'Арапски језик',
      kw: 'Arabek',
      gn: "Áraveñe'ẽ",
      km: 'ភាសាអារ៉ាប់',
      mn: 'Араб хэл',
      zh: '阿拉伯语',
      kv: 'Араб кыв',
      rw: 'Icyarabu',
      sk: 'Arabčina',
      br: 'Arabeg',
      co: 'Lingua àraba',
      ru: 'Арабский язык',
      yo: 'Èdè Lárúbáwá',
      ku: 'Zimanê erebî',
      tk: 'Arap dili',
      ne: 'अरबी भाषा',
      ta: 'அரபு மொழி',
      kn: 'ಅರಬ್ಬೀ ಭಾಷೆ',
      sa: 'अरबी'
    },
    language_code_3: {
      en: 'ara'
    },
    language_code_2: {
      en: 'ar'
    }
  },
  'en:kinyarwanda': {
    wikidata: {
      en: 'Q33573'
    },
    language_code_3: {
      en: 'kin'
    },
    name: {
      sw: 'Kinyarwanda',
      mk: 'Руандски јазик',
      ru: 'Руанда',
      fa: 'زبان رواندایی',
      nl: 'Kinyarwanda',
      da: 'Kinyarwanda',
      pl: 'Język ruanda-rundi',
      hr: 'Rwanda jezik',
      ta: 'கின்யருவாண்டா மொழி',
      rn: 'Ikinyarwanda',
      lv: 'Kiņaruandas valoda',
      ms: 'Bahasa Kinyarwanda',
      ja: 'ルワンダ語',
      zh: '卢旺达语',
      se: 'Kinyarwandagiella',
      cs: 'Rwandština',
      et: 'Ruanda keel',
      kv: 'Киняргванда',
      rw: 'Ikinyarwanda',
      sk: 'Rwandčina',
      bn: 'কিনিয়ারোয়ান্ডা ভাষা',
      ko: '키냐르완다어',
      hy: 'Ռուանդերեն',
      tr: 'Ruandaca',
      is: 'Rúandíska',
      uk: 'Руандійська мова',
      eu: 'Kinyaruanda',
      nn: 'Kinyarwanda',
      nb: 'Kinyarwanda',
      de: 'Kinyarwanda',
      oc: 'Kinyarwanda',
      hu: 'Ruandai nyelv',
      bg: 'Руанда',
      fi: 'Ruandan kieli',
      id: 'Bahasa Kinyarwanda',
      qu: 'Rwanda simi',
      it: 'Kinyarwanda',
      sv: 'Rwanda',
      hi: 'किन्यारुआण्डा भाषा',
      ar: 'كينيارواندا',
      fr: 'Kinyarwanda',
      es: 'Kiñaruanda',
      ca: 'Kinyarwanda',
      cv: 'Руанда чĕлхи',
      en: 'Kinyarwanda',
      eo: 'Ruanda lingvo',
      pt: 'Língua kinyarwanda'
    },
    language_code_2: {
      en: 'rw'
    }
  },
  'en:tongan-language': {
    wikidata: {
      en: 'Q34094'
    },
    language_code_3: {
      en: 'ton'
    },
    name: {
      nn: 'Tongansk',
      nb: 'Tongansk',
      eu: 'Tongera',
      de: 'Tongaische Sprache',
      fi: 'Tongan kieli',
      hu: 'Tongai nyelv',
      bg: 'Тонгански език',
      jv: 'Basa Tonga',
      it: 'Tongano',
      qu: 'Tonga simi',
      id: 'Bahasa Tonga',
      sv: 'Tonganska',
      ar: 'التونجية',
      sh: 'Tonganski jezik',
      io: 'Tongana linguo',
      es: 'Tongano',
      fr: 'Tongien',
      ca: 'Tongalès',
      th: 'ภาษาตองกา',
      sm: "Gagana fa'a Tonga",
      en: 'Tongan language',
      pt: 'Língua tonganesa',
      eo: 'Tonga lingvo de Tongo',
      la: 'Lingua Tongana',
      sw: 'Kitonga',
      mk: 'Тонгански јазик',
      fa: 'زبان تونگایی',
      ru: 'Тонганский язык',
      to: 'Lea fakatonga',
      nl: 'Tongaans',
      pl: 'Język tonga',
      lt: 'Tongos kalba',
      hr: 'Tonganski jezik',
      ia: 'Lingua tongan',
      ms: 'Bahasa Tonga',
      vi: 'Tiếng Tonga',
      ta: 'தோங்க மொழி',
      zh: '湯加語',
      wa: 'Tongyin',
      mn: 'Тонга хэл',
      ja: 'トンガ語',
      cs: 'Tongánština',
      et: 'Tonga keel',
      sk: 'Tongčina',
      kv: 'Тонга кыв',
      az: 'Tonqa dili',
      br: 'Tongaeg',
      ug: 'تونگاچە',
      ko: '통가어',
      uk: 'Тонганська мова',
      na: 'Dorerin Tonga'
    },
    language_code_2: {
      en: 'to'
    }
  },
  'en:church-slavonic': {
    language_code_2: {
      en: 'cu'
    },
    wikidata: {
      en: 'Q33251'
    },
    language_code_3: {
      en: 'chu'
    },
    name: {
      sv: 'Kyrkslaviska',
      sh: 'Crkvenoslovenski jezik',
      ar: 'سلافونية كنسية',
      sl: 'Cerkvenoslovanščina',
      it: 'Slavo ecclesiastico',
      bg: 'Църковнославянски език',
      kk: 'Шіркеу славян тілі',
      fi: 'Kirkkoslaavi',
      be: 'Царкоўнаславянская мова',
      nb: 'Kirkeslavisk',
      nn: 'Kyrkjeslavisk',
      de: 'Kirchenslawisch',
      en: 'Church Slavonic',
      la: 'Lingua Slavica Ecclesiastica',
      eo: 'Slavono',
      pt: 'Eslavo eclesiástico',
      bs: 'Crkvenoslavenski jezik',
      fr: 'Slavon liturgique',
      es: 'Eslavo eclesiástico',
      ca: 'Eslau eclesiàstic',
      af: 'Kerkslawies',
      pl: 'Język cerkiewnosłowiański',
      lb: 'Kiercheslawesch Sprooch',
      hr: 'Crkvenoslavenski jezik',
      nl: 'Kerkslavisch',
      mk: 'Црковнословенски јазик',
      ru: 'Церковнославянский язык',
      tr: 'Eski Kilise Slavcası',
      uk: "Церковнослов'янська мова",
      cu: 'Црькъвьнословѣньскъ ѩꙁꙑкъ',
      ko: '교회 슬라브어',
      et: 'Kirikuslaavi keel',
      az: 'Kilsə slavyan dili',
      sk: 'Cirkevná slovančina',
      ja: '教会スラヴ語',
      zh: '教會斯拉夫語',
      ro: 'Limba slavonă',
      cs: 'Církevní slovanština',
      sr: 'Црквенословенски језик'
    }
  },
  'en:sinhala': {
    language_code_2: {
      en: 'si'
    },
    language_code_3: {
      en: 'sin'
    },
    name: {
      da: 'Singalesisk',
      nl: 'Singalees',
      fa: 'زبان سینهالی',
      ru: 'Сингальский язык',
      si: 'සිංහල භාෂාව',
      mk: 'Синхалски јазик',
      vi: 'Tiếng Sinhala',
      lv: 'Singāļu valoda',
      ta: 'சிங்களம்',
      af: 'Singalees',
      pl: 'Język syngaleski',
      ne: 'सिन्हाला भाषा',
      lt: 'Sinhalų kalba',
      sk: 'Sinhalčina',
      az: 'Sinhala dili',
      kv: 'Сингал кыв',
      ml: 'സിംഹള ഭാഷ',
      et: 'Singali keel',
      sr: 'Сингалски језик',
      cs: 'Sinhálština',
      se: 'Singalesagiella',
      ro: 'Limba singaleză',
      zh: '僧伽罗语',
      ja: 'シンハラ語',
      uk: 'Сингальська мова',
      is: 'Singalíska',
      tr: 'Seylanca',
      mg: 'Fiteny singalesa',
      hy: 'Սինհալերեն',
      ug: 'سىنگال تىلى',
      ko: '싱할라어',
      bn: 'সিংহলি ভাষা',
      br: 'Sinhaleg',
      be: 'Сінгальская мова',
      fi: 'Sinhali',
      bg: 'Синхалски език',
      oc: 'Singalés',
      hu: 'Szingaléz nyelv',
      de: 'Singhalesische Sprache',
      ur: 'سنہالی زبان',
      nb: 'Singalesisk',
      nn: 'Singalesisk',
      eu: 'Sinhala',
      ar: 'لغة سنهالية',
      sv: 'Singalesiska',
      hi: 'सिंहली भाषा',
      sl: 'Singalščina',
      qu: 'Sinhala simi',
      it: 'Singalese',
      id: 'Bahasa Sinhala',
      th: 'ภาษาสิงหล',
      ca: 'Singalès',
      pa: 'ਸਿੰਹਾਲਾ ਭਾਸ਼ਾ',
      es: 'Cingalés',
      fr: 'Cingalais',
      uz: 'Singal tili',
      pt: 'Língua cingalesa',
      eo: 'Sinhala lingvo',
      ka: 'სინჰალური ენა',
      la: 'Lingua Singhalensis',
      mr: 'सिंहला भाषा',
      en: 'Sinhala'
    },
    wikidata: {
      en: 'Q13267'
    }
  },
  'en:armenian': {
    language_code_3: {
      en: 'arm'
    },
    name: {
      su: 'Basa Arménia',
      ak: 'Armenian kasa',
      it: 'Armeno',
      qu: 'Arminya simi',
      fy: 'Armeensk',
      eu: 'Armeniera',
      nb: 'Armensk',
      ur: 'آرمینیائی زبان',
      de: 'Armenische Sprache',
      am: 'አርሜንኛ',
      hu: 'Örmény',
      bg: 'Арменски език',
      mr: 'आर्मेनियन भाषा',
      en: 'Armenian',
      fr: 'Arménien',
      es: 'Armenio',
      gl: 'Lingua armenia',
      pa: 'ਆਰਮੇਨੀਆਈ ਭਾਸ਼ਾ',
      ca: 'Armeni',
      gv: 'Armeainish',
      sq: 'Gjuha armene',
      th: 'ภาษาอาร์มีเนีย',
      lt: 'Armėnų kalba',
      pl: 'Język ormiański',
      hr: 'Armenski jezik',
      af: 'Armeens',
      lv: 'Armēņu valoda',
      mk: 'Ерменски јазик',
      tg: 'Забони арманӣ',
      an: 'Idioma armenio',
      fa: 'زبان ارمنی',
      da: 'Armensk',
      bn: 'আর্মেনীয় ভাষা',
      os: 'Сомихаг æвзаг',
      ug: 'ئەرمەن تىلى',
      ce: 'Эрмалойн мотт',
      is: 'Armenska',
      tr: 'Ermenice',
      uk: 'Вірменська мова',
      cs: 'Arménština',
      se: 'Armeenalaš gielat',
      et: 'Armeenia keel',
      az: 'Erməni dili',
      tt: 'Әрмән теле',
      jv: 'Basa Arménia',
      id: 'Bahasa Armenia',
      sl: 'Armenščina',
      hi: 'आर्मीनियाई भाषा',
      vo: 'Larmeniyänapük',
      sv: 'Armeniska',
      sh: 'Jermenski jezik',
      ar: 'لغة أرمنية',
      nn: 'Armensk',
      oc: 'Armèni',
      kk: 'Армян тілі',
      be: 'Армянская мова',
      fi: 'Armenian kieli',
      cv: 'Эрмен чĕлхи',
      cy: 'Armeneg',
      la: 'Lingua armenica',
      uz: 'Arman tili',
      pt: 'Língua arménia',
      ka: 'სომხური ენა',
      eo: 'Armena lingvo',
      io: 'Armeniana linguo',
      ga: 'An Airméinis',
      el: 'Αρμενική γλώσσα',
      he: 'ארמנית',
      ne: 'अर्मेनियाली भाषा',
      ta: 'அருமேனிய மொழி',
      vi: 'Tiếng Armenia',
      ms: 'Bahasa Armenia',
      sw: 'Kiarmenia',
      ru: 'Армянский язык',
      ku: 'Zimanê ermenî',
      yo: 'Èdè Arméníà',
      nl: 'Armeens',
      br: 'Armenieg',
      ko: '아르메니아어',
      li: 'Armeens',
      hy: 'Հայերեն',
      ja: 'アルメニア語',
      gn: "Armeniañe'ẽ",
      wa: 'Årmenyin',
      zh: '亚美尼亚语',
      ro: 'Limba armeană',
      sr: 'Јерменски језик',
      ml: 'അർമേനിയൻ ഭാഷ',
      kv: 'Армен кыв',
      rw: 'Icyarumeniya',
      sk: 'Arménčina'
    },
    wikidata: {
      en: 'Q8785'
    },
    language_code_2: {
      en: 'hy'
    }
  },
  'en:kurdish': {
    language_code_2: {
      en: 'ku'
    },
    wikidata: {
      en: 'Q36368'
    },
    language_code_3: {
      en: 'kur'
    },
    name: {
      la: 'Lingua Curdica',
      cy: 'Cyrdeg',
      uz: 'Kurd tili',
      eo: 'Kurda lingvo',
      pt: 'Língua curda',
      ka: 'ქურთული ენა',
      cv: 'Курт чĕлхи',
      he: 'כורדית',
      el: 'Κουρδική γλώσσα',
      kl: 'Kurdiskisut',
      io: 'Kurda linguo',
      sh: 'Kurdski jezik',
      ar: 'لغة كردية',
      sv: 'Kurdiska',
      hi: 'कुर्दी भाषा',
      id: 'Bahasa Kurdi',
      tt: 'Көрд теле',
      oc: 'Curd',
      fi: 'Kurdi',
      kk: 'Күрд тілі',
      be: 'Курдская мова',
      nn: 'Kurdiske språk',
      ko: '쿠르드어',
      li: 'Koerdisch',
      hy: 'Քրդերեն',
      br: 'Kurdeg',
      kv: 'Курд кывъяс',
      sk: 'Kurdčina',
      ml: 'കുർദിഷ് ഭാഷകൾ',
      ro: 'Limba kurdă',
      sr: 'Курдски језик',
      ja: 'クルド語',
      zh: '庫爾德語',
      ta: 'குர்தி மொழி',
      vi: 'Tiếng Kurd',
      ms: 'Bahasa Kurdi',
      nl: 'Koerdisch',
      ru: 'Курдские языки',
      ku: 'Zimanê kurdî',
      mi: 'Reo Kuruti',
      sw: 'Kikurdi',
      mr: 'कुर्दी भाषा',
      en: 'Kurdish',
      sq: 'Gjuha kurde',
      th: 'ภาษาเคิร์ด',
      pa: 'ਕੁਰਦਿਸ਼ ਭਾਸ਼ਾ',
      ca: 'Kurd',
      fr: 'Kurde',
      es: 'Kurdo',
      gl: 'Lingua curda',
      it: 'Curdo',
      qu: 'Kurdi simi',
      bg: 'Кюрдски език',
      hu: 'Kurd nyelv',
      ur: 'کردی زبان',
      de: 'Kurdische Sprachen',
      eu: 'Kurduera',
      nb: 'Kurdisk',
      is: 'Kúrdíska',
      tr: 'Kürtçe',
      uk: 'Курдська мова',
      ug: 'كۇرد تىلى',
      bn: 'কুর্দি ভাষা',
      az: 'Kürd dili',
      ps: 'کوردي ژبه',
      et: 'Kurdi keel',
      cs: 'Kurdština',
      lv: 'Kurdu valoda',
      af: 'Koerdies',
      hr: 'Kurdski jezik',
      lt: 'Kurdų kalba',
      pl: 'Język kurdyjski',
      da: 'Kurdisk',
      an: 'Idioma kurdo',
      fa: 'زبان کردی',
      mk: 'Курдски јазик',
      tg: 'Забони курдӣ'
    }
  },
  'en:thai': {
    language_code_2: {
      en: 'th'
    },
    wikidata: {
      en: 'Q9217'
    },
    language_code_3: {
      en: 'tha'
    },
    name: {
      eu: 'Thailandiera',
      nb: 'Thai',
      ur: 'تھائی زبان',
      de: 'Thailändisch',
      bg: 'Тайски език',
      hu: 'Thai',
      it: 'Thailandese',
      qu: 'Thay simi',
      fr: 'Thaï',
      gl: 'Lingua tailandesa',
      es: 'Tailandés',
      pa: 'ਥਾਈ ਭਾਸ਼ਾ',
      ca: 'Tailandès',
      gv: 'Thaish',
      th: 'ภาษาไทย',
      tl: 'Wikang Thai',
      mr: 'थाई भाषा',
      en: 'Thai',
      mk: 'Тајландски јазик',
      tg: 'Забони таиландӣ',
      fa: 'زبان تایلندی',
      da: 'Thai',
      lt: 'Tajų kalba',
      pl: 'Język tajski',
      hr: 'Tajski jezik',
      af: 'Thai',
      lv: 'Taju valoda',
      cs: 'Thajština',
      se: 'Thaigiella',
      lo: 'ພາສາໄທ',
      et: 'Tai keel',
      az: 'Tay dili',
      bn: 'থাই ভাষা',
      ug: 'تايلاندچە',
      is: 'Taílenska',
      tr: 'Tayca',
      uk: 'Тайська мова',
      nn: 'Thai',
      oc: 'Tai',
      fi: 'Thain kieli',
      kk: 'Тай тілі',
      be: 'Тайская мова',
      jv: 'Basa Thai',
      id: 'Bahasa Thai',
      sl: 'Tajščina',
      vo: 'Tayänapük',
      sv: 'Thai',
      hi: 'थाई भाषा',
      sh: 'Tajlandski jezik',
      ar: 'لغة تايلندية',
      ga: 'An Téalainnis',
      io: 'Tai linguo',
      kl: 'Thailandimiutut',
      he: 'תאית',
      la: 'Lingua Thai',
      eo: 'Siama lingvo',
      uz: 'Tay tili',
      pt: 'Língua tailandesa',
      ka: 'ტაი ენა',
      sw: 'Kithai',
      ru: 'Тайский язык',
      yo: 'Èdè Tháí',
      nl: 'Thai',
      za: 'Vahdai',
      sa: 'थाई भाषा',
      ta: 'தாய்',
      vi: 'Tiếng Thái',
      ms: 'Bahasa Thai',
      ja: 'タイ語',
      km: 'ភាសាថៃ',
      zh: '泰语',
      ro: 'Limba thailandeză',
      yi: 'טיילאנדיש',
      sr: 'Тајландски језик',
      ml: 'തായ്ഭാഷ',
      kv: 'Таи кыв',
      sk: 'Thajčina',
      br: 'Taieg',
      ko: '태국어',
      hy: 'Թայերեն',
      mg: 'Fiteny thai'
    }
  },
  'en:cree': {
    language_code_3: {
      en: 'cre'
    },
    name: {
      en: 'Cree',
      eo: 'Kria lingvo',
      pt: 'Língua cree',
      gl: 'Lingua cree',
      es: 'Idioma cree',
      fr: 'Cri',
      ca: 'Cree',
      th: 'ภาษาครี',
      gv: 'Creeish',
      he: 'קרי',
      qu: 'Kri simi',
      it: 'Cree',
      sv: 'Cree',
      nn: 'Cree-montagnais-naskapi-språket',
      nb: 'Cree',
      eu: 'Cree hizkuntza',
      de: 'Cree',
      fi: 'Cree',
      be: 'Кры',
      cr: 'ᐃᔨᔫ ᐊᔨᒧᐎᓐ',
      oc: 'Cree',
      br: 'Kreeg',
      ko: '크리어',
      uk: 'Мова крі',
      tr: 'Krice',
      is: 'Cree',
      zh: '克里语',
      ja: 'クリー語',
      cs: 'Kríjština',
      kv: 'Кри',
      pl: 'Język kri',
      lt: 'Kri kalba',
      hr: 'Cree jezik',
      lv: 'Krī valoda',
      ta: 'கிறீ மொழி',
      mk: 'Криски јазик',
      fa: 'زبان کری',
      ru: 'Кри',
      nl: 'Cree'
    },
    wikidata: {
      en: 'Q33390'
    },
    language_code_2: {
      en: 'cr'
    }
  },
  'en:swahili': {
    language_code_2: {
      en: 'sw'
    },
    wikidata: {
      en: 'Q7838'
    },
    language_code_3: {
      en: 'swa'
    },
    name: {
      ia: 'Swaghili',
      vi: 'Tiếng Swahili',
      kn: 'ಸ್ವಾಹಿಲಿ ಭಾಷೆ',
      ms: 'Bahasa Swahili',
      ta: 'சுவாகிலி மொழி',
      mi: 'Reo Swahili',
      sw: 'Kiswahili',
      ku: 'Zimanê swahîlî',
      yo: 'Èdè Swàhílì',
      ru: 'Cуахили',
      nl: 'Swahili',
      za: 'Vah Swahili',
      br: 'Swahileg',
      li: 'Swahili',
      hy: 'Սվահիլի',
      ko: '스와힐리어',
      mg: 'Fiteny soahily',
      mn: 'Свахили хэл',
      zh: '斯瓦希里语',
      wa: 'Suwahili',
      ja: 'スワヒリ語',
      sr: 'Свахили',
      ro: 'Limba swahili',
      yi: 'סוואהילי',
      ml: 'സ്വാഹിലി ഭാഷ',
      rw: 'Igiswahili',
      sk: 'Swahilčina',
      kv: 'Суахили',
      jv: 'Basa Swahili',
      te: 'స్వాహిలి భాష',
      sl: 'Svahili',
      id: 'Bahasa Swahili',
      ba: 'Суахили теле',
      sv: 'Swahili',
      om: 'Swahili',
      hi: 'स्वाहिली भाषा',
      sh: 'Svahili',
      ar: 'لغة سواحلية',
      nn: 'Swahili',
      kk: 'Суахили',
      fi: 'Swahilin kieli',
      be: 'Суахілі',
      oc: 'Swahili',
      bs: 'Svahili jezik',
      cv: 'Суахили чĕлхи',
      uz: 'Suaxili tili',
      pt: 'Língua suaíli',
      ka: 'სუაჰილი ენა',
      eo: 'Svahila lingvo',
      cy: 'Swahili',
      la: 'Lingua Suahili',
      io: 'Swahilia linguo',
      ga: 'An Svahaílis',
      kl: 'Swahilimiutut',
      el: 'Σουαχίλι γλώσσα',
      kg: 'Kiswahili',
      he: 'סווהילי',
      pl: 'Suahili',
      lt: 'Svahilių kalba',
      hr: 'Svahili',
      af: 'Swahili',
      lv: 'Svahili',
      tg: 'Забони своҳилӣ',
      mk: 'Свахилски јазик',
      fa: 'زبان سواحلی',
      an: 'Idioma swahili',
      iu: 'ᔅᕙᐦᐃᓕᑐᑦ',
      da: 'Swahili',
      bn: 'সোয়াহিলি ভাষা',
      ug: 'سۋاھىلىچە',
      os: 'Суахили',
      ce: 'Суахили',
      uk: 'Суахілі',
      is: 'Svahílí',
      tr: 'Svahili',
      cs: 'Svahilština',
      wo: 'Kiswahili',
      et: 'Suahiili keel',
      lo: 'ພາສາສະວາຮີລີ',
      az: 'Suahili',
      su: 'Basa Swahili',
      qu: 'Swahili simi',
      it: 'Swahili',
      so: 'Sawaaxili',
      nb: 'Swahili',
      eu: 'Swahili',
      de: 'Swahili',
      ur: 'سواحلی زبان',
      am: 'ስዋሂሊ',
      fo: 'Swahiliskt mál',
      bg: 'Суахили',
      hu: 'Szuahéli',
      ln: 'Kiswahíli',
      en: 'Swahili',
      mr: 'स्वाहिली भाषा',
      es: 'Suajili',
      gl: 'Lingua suahili',
      fr: 'Swahili',
      ca: 'Suahili',
      pa: 'ਸਵਾਹਿਲੀ',
      th: 'ภาษาสวาฮีลี',
      gv: 'Swahilish'
    }
  },
  'en:gujarati': {
    name: {
      tr: 'Gucaratça',
      is: 'Gújaratí',
      uk: 'Гуджараті',
      bn: 'গুজরাটি ভাষা',
      os: 'Гуджаратаг æвзаг',
      ug: 'گۇجارات تىل',
      et: 'Gudžarati keel',
      az: 'Qucarat dili',
      cs: 'Gudžarátština',
      se: 'Gujaratagiella',
      af: 'Gujarati',
      lv: 'Gudžaratu valoda',
      lt: 'Gudžaratų kalba',
      pl: 'Język gudźarati',
      hr: 'Gudžaratski jezik',
      da: 'Gujarati',
      mk: 'Гуџаратски јазик',
      tg: 'Забони гуҷаратӣ',
      an: 'Gucharatí',
      fa: 'زبان گجراتی',
      en: 'Gujarati',
      mr: 'गुजराती भाषा',
      th: 'ภาษาคุชราต',
      fr: 'Gujarati',
      gl: 'Lingua guxaratí',
      es: 'Guyaratí',
      pa: 'ਗੁਜਰਾਤੀ ਭਾਸ਼ਾ',
      ca: 'Gujarati',
      it: 'Gujarati',
      qu: 'Gujarati simi',
      bg: 'Гуджаратски език',
      hu: 'Gudzsaráti',
      eu: 'Gujaratera',
      nb: 'Gujarati',
      de: 'Gujarati',
      ur: 'گجراتی زبان',
      am: 'ጉጃራቲ',
      mg: 'Fiteny Gojaratia',
      br: 'Goudjarateg',
      ko: '구자라트어',
      hy: 'Գուջարատի',
      gu: 'ગુજરાતી ભાષા',
      ml: 'ഗുജറാത്തി',
      kv: 'Гуджарати',
      ja: 'グジャラート語',
      zh: '古吉拉特语',
      ro: 'Limba gujarati',
      sr: 'Гуџарати језик',
      sa: 'गुजरातीभाषा',
      bh: 'गुजराती भाषा',
      ta: 'குஜராத்தி',
      ms: 'Bahasa Gujarat',
      vi: 'Tiếng Gujarat',
      ne: 'गुजराती भाषा',
      nl: 'Gujarati',
      sw: 'Kigujarati',
      ru: 'Гуджарати',
      la: 'Lingua Gujaratensis',
      cy: 'Gwjarati',
      ka: 'გუჯარათული ენა',
      pt: 'Língua gujarati',
      eo: 'Guĝarata lingvo',
      el: 'Γκουτζαράτι',
      he: "גוג'ראטית",
      sv: 'Gujarati',
      hi: 'गुजराती',
      sh: 'Gudžarati jezik',
      ar: 'اللغة الغواجاراتية',
      te: 'గుజరాతి భాష',
      id: 'Bahasa Gujarati',
      sl: 'Gudžaratščina',
      or: 'ଗୁଜୁରାଟି ଭାଷା',
      oc: 'Gujarati',
      fi: 'Gudžarati',
      be: 'Гуджараці',
      nn: 'Gujarati'
    },
    language_code_3: {
      en: 'guj'
    },
    wikidata: {
      en: 'Q5137'
    },
    language_code_2: {
      en: 'gu'
    }
  },
  'en:persian': {
    wikidata: {
      en: 'Q9168'
    },
    name: {
      kv: 'Фарси',
      sk: 'Perzské jazyky',
      ml: 'പേർഷ്യൻ ഭാഷ',
      ro: 'Limba persană',
      yi: 'פערסיש',
      kw: 'Persek',
      sr: 'Персијски језик',
      ja: 'ペルシア語',
      km: 'ភាសាពែក',
      mn: 'Перс хэл',
      zh: '波斯语',
      mg: 'Fiteny persana',
      ko: '페르시아어',
      li: 'Perzisch',
      hy: 'Պարսկերեն',
      gu: 'ફારસી',
      br: 'Perseg',
      nl: 'Perzisch',
      ru: 'Персидский язык',
      yo: 'Èdè Pẹ́rsíà',
      ku: 'Zimanê farisî',
      mi: 'Reo Farsi',
      sw: 'Kiajemi',
      ta: 'பாரசீக மொழி',
      ms: 'Bahasa Parsi',
      vi: 'Tiếng Ba Tư',
      ne: 'फारसी भाषा',
      he: 'פרסית',
      el: 'Περσική γλώσσα',
      kl: 'Farsimiutut',
      io: 'Persiana linguo',
      ga: 'An Pheirsis',
      la: 'Lingua Persica',
      cy: 'Perseg',
      eo: 'Persa lingvo',
      uz: 'Fors tili',
      pt: 'Língua persa',
      ka: 'სპარსული ენა',
      cv: 'Перс чĕлхи',
      bs: 'Perzijski jezik',
      oc: 'Persan',
      fi: 'Persian kieli',
      kk: 'Парсы тілі',
      be: 'Персідская мова',
      nn: 'Persisk',
      ar: 'لغة فارسية',
      sh: 'Perzijski jezik',
      hi: 'फ़ारसी भाषा',
      vo: 'Pärsänapük',
      sv: 'Persiska',
      my: 'ပါးရှန်းဘာသာစကား',
      id: 'Bahasa Persia',
      ba: 'Фарсы теле',
      sl: 'Perzijščina',
      tt: 'Фарсы теле',
      jv: 'Basa Parsi',
      te: 'పర్షియన్ భాష',
      az: 'Fars dili',
      ps: 'پارسي',
      et: 'Pärsia keel',
      cs: 'Perština',
      se: 'Persiagiella',
      is: 'Persneska',
      tr: 'Farsça',
      uk: 'Перська мова',
      ce: 'ГӀажарийн мотт',
      ug: 'پارس تىلى',
      os: 'Персайнаг æвзаг',
      bn: 'ফার্সি ভাষা',
      da: 'Persisk',
      an: 'Idioma persa',
      fa: 'زبان فارسی',
      mk: 'Персиски јазик',
      tg: 'Забони Форсӣ',
      bo: 'ཏ་ཟིག་གི་སྐད།',
      lv: 'Persiešu valoda',
      af: 'Persies',
      hr: 'Perzijski jezik',
      lt: 'Persų kalba',
      pl: 'Język perski',
      sq: 'Gjuha perse',
      th: 'ภาษาเปอร์เซีย',
      pa: 'ਫ਼ਾਰਸੀ ਬੋਲੀ',
      ca: 'Persa',
      fr: 'Persan',
      gl: 'Lingua iraniana',
      es: 'Persa',
      en: 'Persian',
      mr: 'फारसी भाषा',
      tl: 'Wikang Persiyano',
      dv: 'ފާރިސީ',
      hu: 'Perzsa nyelv',
      bg: 'Персийски език',
      sd: 'فارسي ٻولي',
      ur: 'فارسی زبان',
      de: 'Persisch',
      am: 'ፋርስኛ',
      eu: 'Persiera',
      nb: 'Persisk',
      fy: 'Perzysk',
      so: 'Af-Faarsi',
      it: 'Persiano',
      qu: 'Pharsi simi',
      su: 'Basa Pérsia'
    },
    language_code_3: {
      en: 'per'
    },
    language_code_2: {
      en: 'fa'
    }
  },
  'en:bosnian': {
    wikidata: {
      en: 'Q9303'
    },
    name: {
      tl: 'Wikang Bosniyo',
      sm: 'Gagana Bosnia',
      en: 'Bosnian',
      mr: 'बॉस्नियन भाषा',
      fr: 'Bosnien',
      es: 'Bosnio',
      gl: 'Lingua bosníaca',
      ca: 'Bosnià',
      sq: 'Gjuha boshnjake',
      gv: 'Bosnish',
      qu: 'Busna simi',
      it: 'Bosniaco',
      eu: 'Bosniera',
      nb: 'Bosnisk',
      de: 'Bosnisch',
      ur: 'بوسنیائی زبان',
      bg: 'Бошняшки език',
      hu: 'Bosnyák nyelv',
      ug: 'بوسنىيە تىلى',
      ce: 'Боснийн мотт',
      tr: 'Boşnakça',
      uk: 'Боснійська мова',
      av: 'Босниязул мацІ',
      cs: 'Bosenština',
      se: 'Bosniagiella',
      et: 'Bosnia keel',
      az: 'Bosniya dili',
      lt: 'Bosnių kalba',
      pl: 'Język bośniacki',
      hr: 'Bošnjački jezik',
      af: 'Bosnies',
      lv: 'Bosniešu valoda',
      mk: 'Босански јазик',
      tg: 'Забони бoсанскӣ',
      an: 'Idioma bosniaco',
      fa: 'زبان بوسنیایی',
      da: 'Bosnisk',
      bs: 'Bosanski jezik',
      la: 'Lingua Bosnica',
      cy: 'Bosnieg',
      eo: 'Bosnia lingvo',
      pt: 'Língua bósnia',
      ga: 'An Bhoisnis',
      el: 'Βοσνιακή γλώσσα',
      he: 'בוסנית',
      jv: 'Basa Bosnia',
      id: 'Bahasa Bosnia',
      sl: 'Bosanščina',
      sv: 'Bosniska',
      sh: 'Bosanski jezik',
      ar: 'لغة بوسنية',
      nn: 'Bosnisk',
      oc: 'Bosnian',
      fi: 'Bosnian kieli',
      kk: 'Боснақ тілі',
      be: 'Баснійская мова',
      br: 'Bosnieg',
      ko: '보스니아어',
      hy: 'Բոսնիերեն',
      co: 'Lingua busniaca',
      cu: 'Босньскъ ѩꙁꙑкъ',
      gn: "Voniañe'ẽ",
      ja: 'ボスニア語',
      zh: '波斯尼亚语',
      yi: 'באסניש',
      ro: 'Limba bosniacă',
      sr: 'Бошњачки језик',
      kv: 'Босняк кыв',
      sk: 'Bosniančina',
      ta: 'போசாங்கி மொழி',
      vi: 'Tiếng Bosnia',
      ms: 'Bahasa Bosnia',
      ru: 'Боснийский язык',
      ku: 'Zimanê bosnî',
      nl: 'Bosnisch'
    },
    language_code_3: {
      en: 'bos'
    },
    language_code_2: {
      en: 'bs'
    }
  },
  'en:amharic': {
    language_code_2: {
      en: 'am'
    },
    wikidata: {
      en: 'Q28244'
    },
    language_code_3: {
      en: 'amh'
    },
    name: {
      ml: 'അംഹാറിക്ക്',
      sk: 'Amharčina',
      kv: 'Амхара кыв',
      zh: '阿姆哈拉语',
      ja: 'アムハラ語',
      kw: 'Amharek',
      sr: 'Амхарски језик',
      ro: 'Limba amharică',
      mg: 'Fiteny amharika',
      cu: 'Амхарьскъ ѩꙁꙑкъ',
      br: 'Amhareg',
      hy: 'Ամհարերեն',
      gu: 'અમ્હારિક ભાષા',
      ko: '암하라어',
      nl: 'Amhaars',
      ti: 'ኣምሓረኛ ፊደል',
      sw: 'Kiamhari',
      yo: 'Èdè Àmháríkì',
      ru: 'Амхарский язык',
      ms: 'Bahasa Amhara',
      vi: 'Tiếng Amhara',
      ta: 'அம்காரியம்',
      el: 'Αμχαρική γλώσσα',
      he: 'אמהרית',
      io: 'Amhara linguo',
      ga: 'An Amairis',
      ka: 'ამჰარული ენა',
      uz: 'Amhara tili',
      pt: 'Língua amárica',
      eo: 'Amhara lingvo',
      cy: 'Amhareg',
      la: 'Lingua Amharica',
      bs: 'Amharski jezik',
      be: 'Амхарская мова',
      fi: 'Amharan kieli',
      nn: 'Amharisk',
      sv: 'Amhariska',
      hi: 'अम्हारिक',
      sh: 'Amharski jezik',
      ar: 'لغة أمهرية',
      sl: 'Amharščina',
      id: 'Bahasa Amhar',
      et: 'Amhara keel',
      az: 'Amhar dili',
      cs: 'Amharština',
      se: 'Amharagiella',
      ce: 'Амхарийн мотт',
      uk: 'Амхарська мова',
      is: 'Amharíska',
      tr: 'Amharca',
      bn: 'আমহারীয় ভাষা',
      ug: 'ئامخارا تىلى',
      da: 'Amharisk',
      mk: 'Амхарски јазик',
      fa: 'زبان امهری',
      an: 'Idioma amarico',
      gd: 'Amarais',
      af: 'Amharies',
      lv: 'Amharu valoda',
      pl: 'Język amharski',
      lt: 'Amharų kalba',
      hr: 'Amharski jezik',
      gv: 'Amharish',
      sc: 'Limba amarica',
      gl: 'Lingua amhárica',
      es: 'Amhárico',
      fr: 'Amharique',
      ca: 'Amhàric',
      pa: 'ਅਮਹਾਰੀ ਭਾਸ਼ਾ',
      mr: 'अम्हारिक भाषा',
      en: 'Amharic',
      bg: 'Амхарски език',
      hu: 'Amhara nyelv',
      nb: 'Amharisk',
      eu: 'Amharera',
      ur: 'امہری زبان',
      de: 'Amharisch',
      am: 'አማርኛ',
      qu: 'Amhara simi',
      it: 'Amarico',
      so: 'Amxaari'
    }
  },
  'en:aragonese': {
    name: {
      en: 'Aragonese',
      th: 'ภาษาอารากอน',
      gv: 'Aragonish',
      sc: 'Limba aragonesa',
      es: 'Aragonés',
      gl: 'Lingua aragonesa',
      fr: 'Aragonais',
      ca: 'Aragonès',
      it: 'Aragonese',
      hu: 'Aragóniai',
      bg: 'Арагонски език',
      nb: 'Aragonesisk',
      eu: 'Aragoiera',
      de: 'Aragonesisch',
      ce: 'Арагонхойн мотт',
      uk: 'Арагонська мова',
      is: 'Aragónska',
      tr: 'Aragonca',
      ug: 'ئاراگون تىلى',
      az: 'Araqon dili',
      se: 'Aragoniagiella',
      cs: 'Aragonština',
      lv: 'Aragoniešu valoda',
      pl: 'Język aragoński',
      hr: 'Aragonski jezik',
      da: 'Aragonisk',
      mk: 'Арагонски јазик',
      fa: 'زبان آراگونی',
      an: 'Idioma aragonés',
      cv: 'Арагон чĕлхи',
      eo: 'Aragona lingvo',
      pt: 'Língua aragonesa',
      cy: 'Aragoneg',
      la: 'Lingua Aragonensis',
      bs: 'Aragonski jezik',
      he: 'אראגונית',
      ga: 'An Aragóinis',
      io: 'Aragonana linguo',
      sv: 'Aragonska',
      vo: 'Laragonänapük',
      ar: 'لغة أراغونية',
      sh: 'Aragonski jezik',
      id: 'Bahasa Aragon',
      fi: 'Aragonian kieli',
      be: 'Арагонская мова',
      oc: 'Aragonés',
      nn: 'Aragonesisk',
      co: 'Lingua aragunesa',
      br: 'Aragoneg',
      li: 'Aragonees',
      ko: '아라곤어',
      rw: 'Icyaragoneze',
      zh: '阿拉贡语',
      ja: 'アラゴン語',
      kw: 'Aragonek',
      sr: 'Арагонски језик',
      ro: 'Limba aragoneză',
      ia: 'Lingua aragonese',
      vi: 'Tiếng Aragon',
      ms: 'Bahasa Aragones',
      ta: 'ஆராகோனீசிய மொழி',
      nl: 'Aragonees',
      mi: 'Reo Aragon',
      ku: 'Zimanê aragonî',
      ru: 'Арагонский язык'
    },
    language_code_3: {
      en: 'arg'
    },
    wikidata: {
      en: 'Q8765'
    },
    language_code_2: {
      en: 'an'
    }
  },
  'en:croatian': {
    name: {
      da: 'Kroatisk',
      tg: 'Забони хорватӣ',
      mk: 'Хрватски јазик',
      fa: 'زبان کرواتی',
      an: 'Idioma crovate',
      gd: 'Croatais',
      af: 'Kroaties',
      lv: 'Horvātu valoda',
      pl: 'Język chorwacki',
      lt: 'Kroatų kalba',
      lb: 'Kroatesch',
      hr: 'Hrvatski',
      et: 'Horvaadi keel',
      az: 'Xorvat dili',
      av: 'Хорватазул мацІ',
      cs: 'Chorvatština',
      se: 'Kroatiagiella',
      ce: 'Хорватийн мотт',
      uk: 'Хорватська мова',
      is: 'Króatíska',
      tr: 'Hırvatça',
      na: 'Dorerin Kroaitsiya',
      bn: 'ক্রোয়েশীয় ভাষা',
      zu: 'IsiKrowati',
      ug: 'كرودىيە تىلى',
      os: 'Хорватаг æвзаг',
      fo: 'Kroatiskt mál',
      hu: 'Horvát',
      bg: 'Хърватски език',
      nb: 'Kroatisk',
      eu: 'Kroaziera',
      am: 'ክሮኤሽኛ',
      ur: 'کروشیائی زبان',
      de: 'Kroatisch',
      fy: 'Kroatysk',
      ab: 'Ахорват бызшәа',
      ie: 'Lingue croates',
      it: 'Croato',
      qu: 'Hurwat simi',
      ak: 'Croatian kasa',
      th: 'ภาษาโครเอเชีย',
      gv: 'Croitish',
      sq: 'Gjuha kroate',
      sc: 'Limba croata',
      gl: 'Lingua croata',
      es: 'Croata',
      fr: 'Croate',
      ca: 'Croat',
      en: 'Croatian',
      mr: 'क्रोएशियन भाषा',
      sm: 'Gagana Croatian',
      tl: 'Wikang Kroato',
      ln: 'Likroasi',
      nl: 'Kroatisch',
      sw: 'Kikroatia',
      yo: 'Èdè Kroatíà',
      ku: 'Zimanê kroatî',
      ru: 'Хорватский язык',
      ia: 'Lingua croate',
      vi: 'Tiếng Croatia',
      ms: 'Bahasa Croatia',
      ta: 'குரோவாசிய மொழி',
      tk: 'Horwatça',
      sk: 'Chorvátčina',
      rw: 'Kinyakorowasi',
      kv: 'Хорват кыв',
      zh: '克罗地亚语',
      mn: 'Хорват хэл',
      wa: 'Crowate',
      ja: 'クロアチア語',
      gn: "Kyoasiañe'ẽ",
      sr: 'Хрватски језик',
      kw: 'Kroatek',
      yi: 'קראאטיש',
      ro: 'Limba croată',
      co: 'Lingua cruata',
      cu: 'Хръватьскъ ѩꙁꙑкъ',
      br: 'Kroateg',
      hy: 'Խորվաթերեն',
      li: 'Kroatisch',
      gu: 'ક્રોએશિયન ભાષા',
      ko: '크로아티아어',
      kk: 'Хорват тілі',
      be: 'Харвацкая мова',
      fi: 'Kroatia',
      oc: 'Croat',
      nn: 'Kroatisk',
      mt: 'Kroat',
      sv: 'Kroatiska',
      vo: 'Kroasänapük',
      hi: 'क्रोएशियन भाषा',
      sh: 'Hrvatska varijanta',
      ar: 'لغة كرواتية',
      jv: 'Basa Kroasia',
      tt: 'Хорват теле',
      sl: 'Hrvaščina',
      id: 'Bahasa Kroasia',
      ba: 'Хорват теле',
      el: 'Κροατική γλώσσα',
      he: 'קרואטית',
      io: 'Kroatiana linguo',
      ga: 'An Chróitis',
      cv: 'Хорват чĕлхи',
      uz: 'Xorvat tili',
      eo: 'Kroata lingvo',
      pt: 'Língua croata',
      ka: 'ხორვატული ენა',
      la: 'Lingua Croatica',
      cy: 'Croateg',
      bs: 'Hrvatski jezik'
    },
    language_code_3: {
      en: 'hrv'
    },
    wikidata: {
      en: 'Q6654'
    },
    language_code_2: {
      en: 'hr'
    }
  },
  'en:chewa': {
    language_code_2: {
      en: 'ny'
    },
    language_code_3: {
      en: 'nya'
    },
    name: {
      uk: 'Ньянджа',
      tr: 'Chewa dili',
      is: 'Chichewa',
      bn: 'চিচেওয়া ভাষা',
      br: 'Tchitcheweg',
      ko: '치체와어',
      et: 'Njandža keel',
      rw: 'Igicewa',
      sk: 'Čeva',
      kv: 'Ньянджа',
      az: 'Nıyanca',
      zh: '齐切瓦语',
      ja: 'チェワ語',
      sr: 'Нијанџа',
      cs: 'Čičevština',
      ms: 'Bahasa Chichewa',
      ta: 'நியாஞ்ச மொழி',
      pl: 'Język cziczewa',
      lt: 'Čevų kalba',
      da: 'Nyanja',
      ny: 'Chilankhulo cha Chichewa',
      nl: 'Nyanja',
      sw: 'Kinyanja',
      mk: 'Њанџа',
      fa: 'زبان چوایی',
      ru: 'Ньянджа',
      en: 'Chewa',
      pt: 'Língua nianja',
      eo: 'Njanĝa lingvo',
      cy: 'Chichewa',
      sq: 'Chichewa',
      es: 'Idioma chichewa',
      gl: 'Lingua chichewa',
      fr: 'Chewa',
      ca: 'Nyanja',
      sv: 'Chichewa',
      ar: 'لغة الشيشيوا',
      it: 'Chewa',
      qu: 'Chichewa simi',
      id: 'Bahasa Chichewa',
      fi: 'Njandžan kieli',
      bg: 'Нянджа',
      nn: 'Chinyanja',
      nb: 'Chewa',
      eu: 'Txewera',
      de: 'Chichewa'
    },
    wikidata: {
      en: 'Q33273'
    }
  },
  'en:zhuang-languages': {
    language_code_2: {
      en: 'za'
    },
    language_code_3: {
      en: 'zha'
    },
    name: {
      zh: '壮语',
      ja: 'チワン語',
      cs: 'Čuangština',
      lo: 'ພາສາຈ້ວງ',
      kv: 'Чжуан кыв',
      bn: 'চুয়াং ভাষা',
      br: 'Djouangeg',
      ko: '좡어',
      ug: 'جۇاڭزۇ تىلى',
      uk: 'Чжуанська мова',
      bo: 'ཀྲོང་སྐད།',
      mk: 'Џуаншки јазик',
      fa: 'زبان‌های جوانگ',
      ru: 'Чжуанский язык',
      nl: 'Zhuang',
      za: 'Vahcuengh',
      pl: 'Język zhuang',
      lt: 'Džuangų kalba',
      vi: 'Tiếng Tráng',
      ta: 'சுவாங்கு மொழி',
      io: 'Zhuangiana linguo',
      es: 'Idioma chuang',
      fr: 'Zhuang',
      ca: 'Llengües zhuang',
      th: 'ภาษาจ้วง',
      en: 'Zhuang languages',
      eo: 'Ĝuanga lingvo',
      pt: 'Língua zhuang',
      ka: 'ჯუანური ენა',
      nb: 'Zhuang',
      nn: 'Zhuang',
      de: 'Zhuang',
      fi: 'Zhuangin kieli',
      it: 'Lingua zhuang',
      sv: 'Zhuang'
    },
    wikidata: {
      en: 'Q13216'
    }
  },
  'en:lingala-language': {
    name: {
      ja: 'リンガラ語',
      zh: '林加拉语',
      wa: 'Lingala',
      sr: 'Лингала',
      cs: 'Ngalština',
      et: 'Lingala keel',
      kv: 'Лингала',
      rw: 'Ilingala',
      sk: 'Lingalčina',
      ug: 'لىنگالاچە',
      ko: '링갈라어',
      uk: 'Лінґала',
      mk: 'Лингала',
      sw: 'Kingala',
      ru: 'Лингала',
      fa: 'زبان لینگالایی',
      nl: 'Lingala',
      lt: 'Ngalų kalba',
      pl: 'Język lingala',
      hr: 'Lingala jezik',
      ta: 'இலிங்கள மொழி',
      fr: 'Lingala',
      gl: 'Lingua lingala',
      es: 'Idioma lingala',
      ca: 'Lingala',
      kg: 'Dingala',
      ln: 'Lingála',
      en: 'Lingala language',
      eo: 'Lingala lingvo',
      pt: 'Língua lingala',
      eu: 'Lingala',
      nn: 'Lingala',
      nb: 'Lingala',
      am: 'ሊንጋላ',
      de: 'Lingála',
      oc: 'Lingala',
      fi: 'Lingala',
      id: 'Bahasa Lingala',
      qu: 'Lingala simi',
      it: 'Lingala',
      sv: 'Lingala',
      ar: 'لينغالا'
    },
    language_code_3: {
      en: 'lin'
    },
    wikidata: {
      en: 'Q36217'
    },
    language_code_2: {
      en: 'ln'
    }
  },
  'en:bambara': {
    language_code_3: {
      en: 'bam'
    },
    name: {
      nl: 'Bambara',
      da: 'Bambara',
      sw: 'Kibambara',
      mk: 'Бамбарски јазик',
      ru: 'Бамана',
      bm: 'Bamanankan',
      fa: 'زبان بامبارایی',
      ta: 'பம்பாரா மொழி',
      vi: 'Tiếng Bambara',
      lv: 'Bambaru valoda',
      lt: 'Bambarų kalba',
      pl: 'Język bambara',
      hr: 'Bamanankan jezik',
      et: 'Bambara keel',
      kv: 'Бамана',
      sk: 'Bambarčina',
      ja: 'バンバラ語',
      zh: '班巴拉语',
      cs: 'Bambarština',
      se: 'Bambaragiella',
      is: 'Bambara',
      uk: 'Бамбара',
      br: 'Bambareg',
      ko: '밤바라어',
      fi: 'Bambaran kieli',
      eu: 'Banbara hizkuntza',
      nb: 'Bambara',
      nn: 'Bambara',
      de: 'Bambara',
      ur: 'بمبارا',
      sv: 'Bambara',
      ar: 'لغة بمبرية',
      id: 'Bahasa Bambara',
      it: 'Bambara',
      qu: 'Bambara simi',
      fr: 'Bambara',
      gl: 'Lingua bambara',
      es: 'Idioma bambara',
      ca: 'Llengua bamana',
      en: 'Bambara',
      pt: 'Língua bambara',
      eo: 'Bambara lingvo'
    },
    wikidata: {
      en: 'Q33243'
    },
    language_code_2: {
      en: 'bm'
    }
  },
  'en:limburgish-language': {
    wikidata: {
      en: 'Q102172'
    },
    name: {
      fa: 'زبان لیمبورخی',
      ru: 'Лимбургский язык',
      nl: 'Limburgs',
      pl: 'Język limburski',
      lt: 'Limburgiečių kalba',
      hr: 'Limburški jezik',
      af: 'Limburgs',
      ta: 'இலிம்பூர்கு மொழி',
      wa: 'Limbordjwès',
      zh: '林堡语',
      ja: 'リンブルフ語',
      kw: 'Limbourgek',
      cs: 'Limburština',
      ro: 'Limba limburgheză',
      sk: 'Limburčina',
      br: 'Limbourgeg',
      li: 'Limburgs',
      hy: 'Լիմբուրգերեն',
      ko: '림뷔르흐어',
      uk: 'Лімбурзька мова',
      is: 'Limburgíska',
      nb: 'Limburgsk',
      eu: 'Limburgera',
      de: 'Limburgisch',
      am: 'ሊምቡርግኛ',
      fi: 'Limburgin kieli',
      fo: 'Limburgiskt mál',
      bg: 'Лимбургски език',
      it: 'Limburghese',
      id: 'Bahasa Limburgish',
      vo: 'Limburgänapük',
      sv: 'Limburgiska',
      fy: 'Limburchsk',
      ar: 'لغة ليمبورغية',
      gl: 'Lingua limburguesa',
      es: 'Idioma limburgués',
      fr: 'Limbourgeois',
      ca: 'Limburguès',
      el: 'Λιμβουργιανή γλώσσα',
      sq: 'Gjuha limburge',
      he: 'לימבורגית',
      en: 'Limburgish language',
      pt: 'Língua limburguesa',
      eo: 'Limburga lingvo',
      la: 'Lingua limburgica'
    },
    language_code_3: {
      en: 'lim'
    },
    language_code_2: {
      en: 'li'
    }
  },
  'en:nuosu-language': {
    name: {
      ca: 'Yi',
      zh: '彝语北部方言',
      es: 'Idioma nuosu',
      ja: '四川彝語',
      kv: 'Носу',
      ko: '이어',
      bn: 'য়ি ভাষা',
      br: 'Yieg Sichuan',
      uk: 'Носу',
      eo: 'Siĉuana jia lingvo',
      en: 'Nuosu language',
      fa: 'زبان نوسو',
      ru: 'Носу',
      mk: 'Носу',
      fi: 'Sichuanin-yi',
      hr: 'Nuosu jezik',
      it: 'Lingua Nuosu',
      pl: 'Język nuosu',
      vi: 'Tiếng Lô Lô',
      ta: 'நுவோசு மொழி',
      hi: 'नोसू भाषा'
    },
    language_code_3: {
      en: 'iii'
    },
    wikidata: {
      en: 'Q34235'
    },
    language_code_2: {
      en: 'ii'
    }
  },
  'en:kwanyama': {
    wikidata: {
      en: 'Q1405077'
    },
    name: {
      cs: 'Kuanyama',
      ca: 'Kwanyama',
      ja: 'クワニャマ語',
      fr: 'Kuanyama',
      es: 'Idioma kuanyama',
      br: 'Kwanyamaeg',
      pt: 'Oshikwanyama',
      uk: 'Кваньяма',
      eo: 'Kuanjama lingvo',
      en: 'Kwanyama',
      ru: 'Кваньяма',
      fa: 'زبان کوانیاما',
      sw: 'Kikwanyama',
      mk: 'Квањама',
      nl: 'Kwanyama',
      it: 'Kwanyama',
      pl: 'Język kwanyama',
      ta: 'குவான்மய மொழி',
      ms: 'Bahasa Kwanyama',
      hi: 'क्वान्यामा भाषा',
      sv: 'Kwanyama'
    },
    language_code_3: {
      en: 'kua'
    },
    language_code_2: {
      en: 'kj'
    }
  },
  'en:kirundi': {
    wikidata: {
      en: 'Q33583'
    },
    name: {
      pt: 'Língua kirundi',
      ka: 'რუანდა-ბურუნდიული ენა',
      eo: 'Burunda lingvo',
      en: 'Kirundi',
      ca: 'Rundi',
      es: 'Kirundi',
      fr: 'Kirundi',
      he: 'קירונדי',
      el: 'Κιρουντί',
      qu: 'Rundi simi',
      it: 'Kirundi',
      id: 'Bahasa Kirundi',
      ar: 'لغة كيروندية',
      sv: 'Kirundi',
      de: 'Kirundi',
      nn: 'Kirundi',
      nb: 'Kirundi',
      eu: 'Kirundi',
      fi: 'Kirundi',
      bg: 'Рунди',
      hu: 'Kirundi nyelv',
      oc: 'Kirundi',
      uk: 'Кірунді',
      tr: 'Rundice',
      se: 'Kirundigiella',
      cs: 'Kirundi',
      zh: '基隆迪语',
      ja: 'ルンディ語',
      rw: 'Ikirundi',
      kv: 'Рунди',
      az: 'Kirundi',
      et: 'Rundi keel',
      hr: 'Rundi jezik',
      pl: 'Język rundi',
      lt: 'Kirundi',
      lv: 'Kirundi',
      ms: 'Bahasa Kirundi',
      ta: 'கிருண்டி மொழி',
      rn: 'Ikirundi',
      fa: 'زبان روندی',
      ru: 'Рунди',
      mk: 'Рундиски јазик',
      sw: 'Kirundi',
      nl: 'Kirundi'
    },
    language_code_3: {
      en: 'run'
    },
    language_code_2: {
      en: 'rn'
    }
  },
  'en:ewe': {
    wikidata: {
      en: 'Q30005'
    },
    name: {
      mg: 'Fiteny eve',
      uk: 'Еве',
      is: 'Anló',
      br: 'Eweeg',
      et: 'Eve keel',
      rw: 'Icyewe',
      kv: 'Эве',
      zh: '埃维语',
      ja: 'エウェ語',
      cs: 'Eveština',
      ro: 'Limba ewe',
      lv: 'Evu valoda',
      ms: 'Bahasa Ewe',
      ta: 'எவு மொழி',
      pl: 'Język ewe',
      lt: 'Eve kalba',
      hr: 'Éwé jezik',
      nl: 'Ewe',
      mk: 'Еве',
      sw: 'Kiewe',
      yo: 'Èdè efe',
      fa: 'زبان اوه‌ای',
      ru: 'Эве',
      en: 'Ewe',
      pt: 'Língua ewe',
      ka: 'ევე',
      eo: 'Evea lingvo',
      es: 'Idioma ewé',
      ee: 'Eʋegbe',
      fr: 'Ewe',
      ca: 'Ewe',
      pa: 'ਏਬੀ ਭਾਸ਼ਾ',
      hi: 'एबी भाषा',
      sv: 'Ewe',
      qu: 'Ewe simi',
      it: 'Ewe',
      id: 'Bahasa Ewe',
      fi: 'Ewen kieli',
      oc: 'Ewe',
      nb: 'Ewe',
      nn: 'Ewe',
      de: 'Ewe'
    },
    language_code_3: {
      en: 'ewe'
    },
    language_code_2: {
      en: 'ee'
    }
  },
  'en:faroese': {
    language_code_2: {
      en: 'fo'
    },
    wikidata: {
      en: 'Q25258'
    },
    language_code_3: {
      en: 'fao'
    },
    name: {
      wa: 'Faeroyès',
      zh: '法罗语',
      ja: 'フェロー語',
      kw: 'Faroyek',
      sr: 'Ферјарски језик',
      ro: 'Limba feroeză',
      sk: 'Faerčina',
      rw: 'Igifero',
      kv: 'Фарер кыв',
      br: 'Faeroeg',
      hy: 'Ֆարյորերեն',
      li: 'Faeröers',
      ko: '페로어',
      co: 'Lingua faroesa',
      ku: 'Zimanê feroeyî',
      ru: 'Фарерский язык',
      nl: 'Faeröers',
      ms: 'Bahasa Faroe',
      vi: 'Tiếng Faroe',
      ta: 'பரோசு மொழி',
      io: 'Faerana linguo',
      ga: 'An Fharóis',
      kl: 'Savalimmiutut',
      el: 'Φεροϊκή γλώσσα',
      he: 'פארואזית',
      bs: 'Farski jezik',
      pt: 'Língua feroesa',
      uz: 'Farer tili',
      eo: 'Feroa lingvo',
      ka: 'ფარერული ენა',
      cy: 'Ffaröeg',
      la: 'Lingua Faroensis',
      nn: 'Færøysk',
      fi: 'Fäärin kieli',
      be: 'Фарэрская мова',
      oc: 'Feroés',
      tt: 'Фарер теле',
      sl: 'Ferščina',
      id: 'Bahasa Faroe',
      vo: 'Färövuänaspük',
      sv: 'Färöiska',
      hi: 'फ़रोइस भाषा',
      sh: 'Farski jezik',
      ar: 'لغة فاروية',
      se: 'Fearagiella',
      cs: 'Faerština',
      et: 'Fääri keel',
      lo: 'ພາສາແຟໂລ',
      az: 'Farer dili',
      bn: 'ফারোয়েজীয় ভাষা',
      os: 'Фарераг æвзаг',
      ug: 'فائېروس تىل',
      uk: 'Фарерська мова',
      tr: 'Faroece',
      is: 'Færeyska',
      tg: 'Алифбои фароесӣ',
      mk: 'Фарски јазик',
      fa: 'زبان فاروئی',
      an: 'Idioma feroés',
      da: 'Færøsk',
      pl: 'Język farerski',
      lt: 'Farerų kalba',
      hr: 'Ferojski jezik',
      gd: 'Fàrothais',
      af: 'Faroëes',
      lv: 'Fēriešu valoda',
      es: 'Idioma feroés',
      gl: 'Lingua feroesa',
      fr: 'Féroïen',
      ca: 'Feroès',
      th: 'ภาษาแฟโร',
      gv: 'Faaroish',
      sc: 'Limba faroesa',
      en: 'Faroese',
      nb: 'Færøysk',
      eu: 'Faroera',
      am: 'ፋሮኛ',
      de: 'Färöische Sprache',
      fo: 'Føroyskt mál',
      hu: 'Feröeri',
      bg: 'Ферьорски език',
      it: 'Faroese',
      fy: 'Faeröersk'
    }
  },
  'en:sindhi': {
    wikidata: {
      en: 'Q33997'
    },
    name: {
      pl: 'Sindhi',
      lt: 'Sindų kalba',
      af: 'Sindhi',
      lv: 'Sindu valoda',
      mk: 'Синдски јазик',
      fa: 'زبان سندی',
      da: 'Sindhi',
      bn: 'সিন্ধি ভাষা',
      ug: 'سىندى تىلى',
      uk: 'Сіндхі',
      is: 'Sindí',
      tr: 'Sindhî',
      cs: 'Sindhština',
      et: 'Sindhi keel',
      ps: 'سيندي ژبه',
      az: 'Sindhi dili',
      it: 'Sindhi',
      qu: 'Sindi simi',
      nb: 'Sindhi',
      eu: 'Sindhi',
      de: 'Sindhi',
      ur: 'سندھی زبان',
      sd: 'سنڌي ٻولي',
      bg: 'Синдхи',
      en: 'Sindhi',
      mr: 'सिंधी भाषा',
      es: 'Sindhi',
      fr: 'Sindhi',
      ca: 'Sindhi',
      pa: 'ਸਿੰਧੀ ਭਾਸ਼ਾ',
      th: 'ภาษาสินธี',
      ne: 'सिन्धी भाषा',
      vi: 'Tếng Sindh',
      ms: 'Bahasa Sindh',
      ta: 'சிந்தி மொழி',
      sw: 'Kisindhi',
      yo: 'Èdè Sindhi',
      ru: 'Синдхи',
      nl: 'Sindhi',
      za: 'Vahsaenqdaek',
      br: 'Sindeg',
      gu: 'સિંધી ભાષા',
      hy: 'Սինդհի',
      ko: '신디어',
      zh: '信德语',
      ja: 'シンド語',
      sr: 'Синди језик',
      ml: 'സിന്ധി ഭാഷ',
      kv: 'Синдхи',
      or: 'ସିନ୍ଧି ଭାଷା',
      id: 'Bahasa Sindhi',
      sv: 'Sindhi',
      hi: 'सिन्धी भाषा',
      sh: 'Sindi jezik',
      ar: 'لغة سندية',
      nn: 'Sindhi',
      be: 'Сіндхі',
      fi: 'Sindhi',
      oc: 'Sindhi',
      ka: 'სინდჰური ენა',
      uz: 'Sindh',
      pt: 'Língua sindi',
      eo: 'Sinda lingvo',
      cy: 'Sindhi',
      la: 'Lingua sindhuica',
      he: 'סינדהי'
    },
    language_code_3: {
      en: 'snd'
    },
    language_code_2: {
      en: 'sd'
    }
  },
  'en:corsican': {
    language_code_2: {
      en: 'co'
    },
    wikidata: {
      en: 'Q33111'
    },
    name: {
      li: 'Corsicaans',
      ug: 'كورسىكا تىلى',
      ko: '코르시카어',
      br: 'Korseg',
      uk: 'Корсиканська мова',
      tr: 'Korsikaca',
      co: 'Lingua corsa',
      cs: 'Korsičtina',
      sr: 'Корзички језик',
      se: 'Corsicagiella',
      ky: 'Корсика тили',
      kw: 'Corsek',
      ro: 'Limba corsicană',
      zh: '科西嘉语',
      ja: 'コルシカ語',
      kv: 'Корсикан кыв',
      et: 'Korsika keel',
      hr: 'Korzički jezik',
      pl: 'Język korsykański',
      lt: 'Korsikiečių kalba',
      vi: 'Tiếng Corsica',
      lv: 'Korsikāņu valoda',
      ms: 'Bahasa Corsica',
      ta: 'கோர்சு மொழி',
      af: 'Korsikaans',
      ku: 'Zimanê korsîkî',
      fa: 'زبان کرسی',
      ru: 'Корсиканский язык',
      an: 'Idioma corso',
      mk: 'Корзикански јазик',
      da: 'Korsikansk',
      nl: 'Corsicaans',
      tl: 'Wikang Korso',
      pt: 'Língua corsa',
      eo: 'Korsika lingvo',
      la: 'Lingua Corsa',
      cy: 'Corseg',
      en: 'Corsican',
      mr: 'कॉर्सिकन भाषा',
      cv: 'Корсикан чĕлхи',
      ca: 'Cors',
      es: 'Corso',
      gl: 'Lingua corsa',
      fr: 'Corse',
      he: 'קורסיקאית',
      sc: 'Limba corsicana',
      el: 'Κορσικανική γλώσσα',
      sl: 'Korziščina',
      it: 'Corso',
      id: 'Bahasa Korsika',
      ar: 'لغة كورسية',
      sv: 'Korsikanska',
      am: 'ኮርሲካኛ',
      de: 'Korsisch',
      nb: 'Korsikansk',
      eu: 'Korsikera',
      fi: 'Korsikan kieli',
      be: 'Карсіканская мова',
      hu: 'Korzikai nyelv',
      oc: 'Còrs'
    },
    language_code_3: {
      en: 'cos'
    }
  },
  'en:kannada': {
    wikidata: {
      en: 'Q33673'
    },
    language_code_3: {
      en: 'kan'
    },
    name: {
      ru: 'Каннада',
      sw: 'Kikannada',
      nl: 'Kannada',
      ne: 'कन्नड़ भाषा',
      vi: 'Tiếng Kannada',
      ms: 'Bahasa Kannada',
      kn: 'ಕನ್ನಡ',
      ta: 'கன்னடம்',
      sa: 'कन्नडभाषा',
      sr: 'Канада језик',
      ro: 'Limba kannada',
      yi: 'קאנאדא',
      mn: 'Каннада хэл',
      zh: '卡纳达语',
      ja: 'カンナダ語',
      sk: 'Kannadčina',
      kv: 'Каннада',
      ml: 'കന്നഡ',
      hy: 'Կաննադա',
      gu: 'કન્નડ ભાષા',
      ko: '칸나다어',
      br: 'Kannadeg',
      mg: 'Fiteny kannada',
      nn: 'Kannada',
      kk: 'Каннада тілі',
      be: 'Канада',
      fi: 'Kannada',
      oc: 'Kannada',
      or: 'କନ୍ନଡ଼ ଭାଷା',
      id: 'Bahasa Kannada',
      te: 'కన్నడ భాష',
      jv: 'Basa Kannada',
      sh: 'Kanada jezik',
      ar: 'لغة كنادية',
      sv: 'Kannada',
      hi: 'कन्नड़ भाषा',
      io: 'Kanarezo linguo',
      he: 'קאנדה',
      el: 'Κανάντα γλώσσα',
      pt: 'Língua canaresa',
      ka: 'კანადა',
      eo: 'Kanara lingvo',
      la: 'Lingua Cannadica',
      cy: 'Kannada',
      fa: 'زبان کانارا',
      an: 'Idioma canarés',
      mk: 'Канада',
      da: 'Kannada',
      pl: 'Język kannada',
      lt: 'Kanadų kalba',
      lv: 'Kannadu valoda',
      af: 'Kannada',
      cs: 'Kannadština',
      az: 'Kannada dili',
      et: 'Kannada keel',
      ug: 'كانناداچە',
      bn: 'কন্নড় ভাষা',
      uk: 'Каннада',
      tr: 'Kannada dili',
      is: 'Kanaríska',
      ur: 'کنڑا',
      de: 'Kannada',
      am: 'ካነዳኛ',
      nb: 'Kannada',
      eu: 'Kannada',
      dv: 'ކައްނަޑާ',
      hu: 'Kannada',
      qu: 'Kannada simi',
      it: 'Kannada',
      ca: 'Kanara',
      pa: 'ਕੰਨੜ ਭਾਸ਼ਾ',
      gl: 'Lingua canaresa',
      es: 'Canarés',
      fr: 'Kannada',
      th: 'ภาษากันนาดา',
      en: 'Kannada',
      mr: 'कन्नड भाषा'
    },
    language_code_2: {
      en: 'kn'
    }
  },
  'en:norwegian': {
    language_code_2: {
      en: 'no'
    },
    name: {
      sr: 'Норвешки језик',
      kw: 'Norgahek',
      ro: 'Limba norvegiană',
      yi: 'נארוועגיש',
      zh: '挪威语',
      ja: 'ノルウェー語',
      sk: 'Nórčina',
      kv: 'Норск кыв',
      no: 'Norsk',
      hy: 'Նորվեգերեն',
      li: 'Noors',
      gu: 'નૉર્વેજિયન ભાષા',
      ko: '노르웨이어',
      br: 'Norvegeg',
      ku: 'Zimanê norwêcî',
      yo: 'Èdè ará Nọ́rwèy',
      ru: 'Норвежский язык',
      sw: 'Kinorwei',
      nl: 'Noors',
      ne: 'नर्वेजियन भाषा',
      vi: 'Tiếng Na Uy',
      ms: 'Bahasa Norway',
      ta: 'நோர்வே மொழி',
      io: 'Norvegiana linguo',
      ga: 'Ioruais',
      ee: 'Norwegbe',
      he: 'נורבגית',
      el: 'Νορβηγική γλώσσα',
      bs: 'Norveški jezik',
      pt: 'Língua norueguesa',
      uz: 'Norveg tili',
      ka: 'ნორვეგიული ენა',
      eo: 'Norvega lingvo',
      la: 'Lingua Norvegica',
      cy: 'Norwyeg',
      cv: 'Норвег чĕлхи',
      nn: 'Norsk',
      be: 'Нарвежская мова',
      kk: 'Норвег тілі',
      fi: 'Norja',
      oc: 'Norvegian',
      sl: 'Norveščina',
      id: 'Bahasa Norwegia',
      jv: 'Basa Norwegia',
      tt: 'Норвег теле',
      ar: 'لغة نرويجية',
      sh: 'Norveški jezik',
      vo: 'Norgänapük',
      hi: 'नॉर्वेजियाई भाषा',
      sv: 'Norska',
      ky: 'Норвег тили',
      se: 'Dárogiella',
      cs: 'Norština',
      ps: 'ناروېژي ژبه',
      az: 'Norveç dili',
      et: 'Norra keel',
      ug: 'نورۋېگىيە تىلى',
      os: 'Норвегиаг æвзаг',
      bn: 'নরওয়েজীয় ভাষা',
      uk: 'Норвезька мова',
      is: 'Norska',
      tr: 'Norveççe',
      ce: 'Норвегийн мотт',
      fa: 'زبان نروژی',
      an: 'Idioma noruego',
      tg: 'Забони норвегӣ',
      rm: 'Lingua norvegiaisa',
      mk: 'Норвешки јазик',
      da: 'Norsk',
      hr: 'Norveški jezik',
      lb: 'Norwegesch',
      pl: 'Język norweski',
      lt: 'Norvegų kalba',
      lv: 'Norvēģu valoda',
      gd: 'Nirribhis',
      af: 'Noors',
      ca: 'Noruec',
      pa: 'ਨਾਰਵੇਈ ਭਾਸ਼ਾ',
      gl: 'Lingua norueguesa',
      es: 'Noruego',
      fr: 'Norvégien',
      sc: 'Limba norvegesa',
      th: 'ภาษานอร์เวย์',
      sq: 'Gjuha norvegjeze',
      gv: 'Norlynnish',
      tl: 'Wikang Noruwego',
      en: 'Norwegian',
      mr: 'नॉर्वेजियन भाषा',
      de: 'Norwegisch',
      nb: 'Norsk',
      eu: 'Norvegiera',
      fo: 'Norskt mál',
      hu: 'Norvég',
      bg: 'Норвежки език',
      qu: 'Nurwiga simi',
      it: 'Norvegese',
      fy: 'Noarsk'
    },
    language_code_3: {
      en: 'nor'
    },
    wikidata: {
      en: 'Q9043'
    }
  },
  'en:sundanese-language': {
    name: {
      lt: 'Sundų kalba',
      pl: 'Język sundajski',
      hr: 'Sundski jezik',
      af: 'Soendanees',
      ta: 'சுண்டா மொழி',
      vi: 'Tiếng Sunda',
      ms: 'Bahasa Sunda',
      lv: 'Sundu valoda',
      kn: 'ಸುಂಡನೆಸೆ ಭಾಷೆ',
      sw: 'Kisunda',
      mk: 'Сундски јазик',
      an: 'Idioma sundanés',
      ru: 'Сунданский язык',
      fa: 'زبان سوندایی',
      nl: 'Soendanees',
      bn: 'সুন্ডা ভাষা',
      ko: '순다어',
      ug: 'سۇندا تىلى',
      tr: 'Sunda dili',
      uk: 'Сунданська мова',
      ja: 'スンダ語',
      zh: '巽他語',
      se: 'Sundagiella',
      cs: 'Sundština',
      kv: 'Сунда кыв',
      sk: 'Sundčina',
      su: 'Basa Sunda',
      jv: 'Basa Sundha',
      id: 'Bahasa Sunda',
      qu: 'Sunda simi',
      it: 'Sundanese',
      sv: 'Sundanesiska',
      hi: 'सुन्दा भाषा',
      ar: 'لغة سوندية',
      eu: 'Sondanera',
      nn: 'Sunda',
      nb: 'Sundanesisk',
      de: 'Sundanesisch',
      fi: 'Sundan kieli',
      be: 'Сундская мова',
      en: 'Sundanese language',
      mr: 'बासा सुंडा',
      cy: 'Swndaneg',
      uz: 'Sundan tili',
      pt: 'Língua sundanesa',
      ka: 'სუნდური ენა',
      eo: 'Sunda lingvo',
      fr: 'Soundanais',
      es: 'Sondanés',
      gl: 'Lingua sundanesa',
      ca: 'Sondanès',
      th: 'ภาษาซุนดา'
    },
    language_code_3: {
      en: 'sun'
    },
    wikidata: {
      en: 'Q34002'
    },
    language_code_2: {
      en: 'su'
    }
  },
  'en:georgian': {
    language_code_2: {
      en: 'ka'
    },
    wikidata: {
      en: 'Q8108'
    },
    name: {
      bs: 'Gruzijski jezik',
      uz: 'Gruzin tili',
      pt: 'Língua georgiana',
      eo: 'Kartvela lingvo',
      ka: 'ქართული ენა',
      cy: 'Georgeg',
      la: 'Lingua Georgiana',
      cv: 'Грузин чĕлхи',
      io: 'Gruziana linguo',
      ga: 'Seoirsis',
      he: 'גאורגית',
      el: 'Γεωργιανή γλώσσα',
      sl: 'Gruzinščina',
      id: 'Bahasa Georgia',
      jv: 'Basa Georgia',
      tt: 'Гөрҗи теле',
      sh: 'Грузијски језик',
      ar: 'لغة جورجية',
      vo: 'Grusiyänapük',
      sv: 'Georgiska',
      nn: 'Georgisk',
      kk: 'Грузин тілі',
      be: 'Грузінская мова',
      fi: 'Georgian kieli',
      oc: 'Georgian',
      li: 'Georgisch',
      hy: 'Վրացերեն',
      ko: '조지아어',
      br: 'Jorjieg',
      sr: 'Грузијски језик',
      ro: 'Limba georgiană',
      yi: 'גרוזיניש',
      mn: 'Гүрж хэл',
      zh: '格鲁吉亚语',
      ja: 'グルジア語',
      sk: 'Gruzínčina',
      kv: 'Грузин кыв',
      ms: 'Bahasa Georgia',
      vi: 'Tiếng Gruzia',
      ta: 'சியார்சிய மொழி',
      ku: 'Zimanê gurcî',
      ru: 'Грузинский язык',
      sw: 'Kigeorgia',
      nl: 'Georgisch',
      tl: 'Wikang Heorhiyano',
      sm: 'Faasiaosiana',
      en: 'Georgian',
      mr: 'जॉर्जियन भाषा',
      ca: 'Georgià',
      pa: 'ਜਾਰਜੀਆਈ ਭਾਸ਼ਾ',
      es: 'Georgiano',
      gl: 'Lingua xeorxiana',
      fr: 'Géorgien',
      sc: 'Limba georgiana',
      th: 'ภาษาจอร์เจีย',
      qu: 'Kartul simi',
      it: 'Georgiano',
      fy: 'Georgysk',
      ab: 'Ақырҭ бызшәа',
      ur: 'جارجیائی زبان',
      de: 'Georgisch',
      nb: 'Georgisk',
      eu: 'Georgiera',
      fo: 'Georgiskt mál',
      hu: 'Grúz',
      bg: 'Грузински език',
      ug: 'گرۇزىن تىلى',
      os: 'Гуырдзиаг æвзаг',
      bn: 'জর্জীয় ভাষা',
      uk: 'Грузинська мова',
      is: 'Georgíska',
      tr: 'Gürcüce',
      ce: 'Гуьржийн мотт',
      cs: 'Gruzínština',
      ky: 'Грузин тили',
      ps: 'گرجی ژبه',
      az: 'Gürcü dili',
      et: 'Gruusia keel',
      hr: 'Gruzijski jezik',
      pl: 'Język gruziński',
      lt: 'Gruzinų kalba',
      lv: 'Gruzīnu valoda',
      af: 'Georgies',
      fa: 'زبان گرجی',
      an: 'Idioma cheorchián',
      tg: 'Забони гурҷӣ',
      mk: 'Грузиски јазик',
      da: 'Georgisk'
    },
    language_code_3: {
      en: 'geo'
    }
  },
  'en:hausa': {
    language_code_2: {
      en: 'ha'
    },
    wikidata: {
      en: 'Q56475'
    },
    language_code_3: {
      en: 'hau'
    },
    name: {
      pa: 'ਹੌਸਾ ਭਾਸ਼ਾ',
      ca: 'Haussa',
      fr: 'Haoussa',
      gl: 'Lingua hausa',
      es: 'Idioma hausa',
      io: 'Hausa linguo',
      he: 'האוסה',
      el: 'Χάουσα γλώσσα',
      ln: 'Lihausa',
      cy: 'Hausa',
      pt: 'Língua haúça',
      ka: 'ჰაუსა',
      eo: 'Haŭsa lingvo',
      en: 'Hausa',
      mr: 'हौसा भाषा',
      ur: 'ھاوسا زبان',
      de: 'Hausa',
      eu: 'Hausa',
      nb: 'Hausa',
      nn: 'Hausa',
      oc: 'Haussà',
      be: 'Хаўса',
      fi: 'Hausan kieli',
      fo: 'Hausa mál',
      id: 'Bahasa Hausa',
      so: 'Af-Hawsa',
      it: 'Hausa',
      qu: 'Hawsa simi',
      ar: 'لغة هوسية',
      sh: 'Hausa jezik',
      sv: 'Hausa',
      hi: 'हौसा भाषा',
      om: 'Hawussaa',
      sr: 'Хауса језик',
      cs: 'Hauština',
      ja: 'ハウサ語',
      zh: '豪萨语',
      kv: 'Хауса',
      rw: 'Igihawusa',
      sk: 'Hauština',
      ha: 'Hausa',
      lo: 'ພາສາເຮົາຊາ',
      et: 'Hausa keel',
      ko: '하우사어',
      br: 'Haousaeg',
      bn: 'হাউসা ভাষা',
      is: 'Haúsa',
      tr: 'Hausa dili',
      uk: 'Хауса',
      ru: 'Хауса',
      yo: 'Èdè Haúsá',
      fa: 'زبان هوسه',
      mk: 'Хауса',
      sw: 'Kihausa',
      tg: 'Забони хауса',
      nl: 'Hausa',
      hr: 'Hausa jezik',
      lt: 'Hausų kalba',
      pl: 'Język hausa',
      ta: 'ஹவுசா மொழி',
      vi: 'Tiếng Hausa',
      lv: 'Hausu valoda',
      af: 'Hausa'
    }
  },
  'en:tswana': {
    name: {
      br: 'Tswaneg',
      zu: 'IsiTswana',
      ko: '츠와나어',
      uk: 'Сетсвана',
      tr: 'Tsvana dili',
      zh: '茨瓦纳语',
      ja: 'ツワナ語',
      cs: 'Setswana',
      ro: 'Limba tswana',
      et: 'Tsvana keel',
      kv: 'Тсвана',
      tn: 'Setswana',
      az: 'Tsvana dili',
      pl: 'Język tswana',
      lt: 'Tsvanų kalba',
      hr: 'Tswana jezik',
      af: 'Tswana',
      ms: 'Bahasa Setswana',
      vi: 'Tiếng Tswana',
      ta: 'சுவான மொழி',
      sw: 'Kitswana',
      mk: 'Цвана',
      fa: 'زبان تسوانا',
      yo: 'Èdè Tswana',
      ru: 'Тсвана',
      da: 'Setswana',
      nl: 'Tswana',
      en: 'Tswana',
      pt: 'Língua tswana',
      eo: 'Cvana lingvo',
      es: 'Setsuana',
      gl: 'Lingua tswana',
      fr: 'Tswana',
      ca: 'Tswana',
      el: 'Τσουάνα γλώσσα',
      sq: 'Gjuha tsvana',
      he: 'טסואנה',
      tw: 'Tswana',
      it: 'Tswana',
      qu: 'Tswana simi',
      id: 'Bahasa Tswana',
      sv: 'Setswana',
      ar: 'لغة تسوانية',
      nn: 'Setswana',
      nb: 'Tswana',
      eu: 'Tswanera',
      ve: 'SeTswana',
      de: 'Setswana',
      fi: 'Tswanan kieli',
      be: 'Тсвана',
      fo: 'Tswana mál',
      oc: 'Tswana',
      bg: 'Тсвана'
    },
    language_code_3: {
      en: 'tsn'
    },
    wikidata: {
      en: 'Q34137'
    },
    language_code_2: {
      en: 'tn'
    }
  },
  'en:catalan': {
    language_code_2: {
      en: 'ca'
    },
    language_code_3: {
      en: 'cat'
    },
    name: {
      tl: 'Wikang Katalan',
      sm: "Fa'aCatalan",
      en: 'Catalan',
      mr: 'कातालान भाषा',
      ca: 'Català',
      pa: 'ਕੈਟਲਨ ਬੋਲੀ',
      gl: 'Lingua catalá',
      es: 'Catalán',
      fr: 'Catalan',
      sc: 'Limba catalana',
      th: 'ภาษาคาตาลัน',
      sq: 'Gjuha katalane',
      gv: 'Cataloanish',
      it: 'Catalano',
      qu: 'Katalan simi',
      fy: 'Katalaansk',
      am: 'ካታላንኛ',
      de: 'Katalanisch',
      nb: 'Katalansk',
      eu: 'Katalan',
      ty: 'Reo Tatarūnia',
      fo: 'Katalanskt mál',
      bg: 'Каталонски език',
      hu: 'Katalán',
      zu: 'IsiKhathalani',
      ug: 'كاتالان تىلى',
      os: 'Каталайнаг æвзаг',
      bn: 'কাতালান ভাষা',
      uk: 'Каталонська мова',
      tr: 'Katalanca',
      is: 'Katalónska',
      ce: 'Каталанхойн мотт',
      cs: 'Katalánština',
      se: 'Katalánagiella',
      az: 'Katalan dili',
      et: 'Katalaani keel',
      lo: 'ພາສາກາຕາລັງ',
      hr: 'Katalonski jezik',
      pl: 'Język kataloński',
      lt: 'Katalonų kalba',
      lv: 'Katalāņu valoda',
      gd: 'Catalanais',
      af: 'Katalaans',
      fa: 'زبان کاتالان',
      an: 'Idioma catalán',
      rm: 'Lingua catalana',
      mk: 'Каталонски јазик',
      da: 'Catalansk',
      bs: 'Katalonski jezik',
      uz: 'Katalan tili',
      pt: 'Língua catalã',
      ka: 'კატალანური ენა',
      eo: 'Kataluna lingvo',
      la: 'Lingua Catalana',
      cy: 'Catalaneg',
      io: 'Kataluniana linguo',
      ga: 'An Chatalóinis',
      he: 'קטלאנית',
      el: 'Καταλανική γλώσσα',
      sl: 'Katalonščina',
      id: 'Bahasa Katala',
      ba: 'Каталан теле',
      jv: 'Basa Katalan',
      tt: 'Каталан теле',
      ar: 'لغة كتالانية',
      sh: 'Katalonski jezik',
      vo: 'Katalonänapük',
      hi: 'कैटलन भाषा',
      sv: 'Katalanska',
      mt: 'Katalan',
      nn: 'Katalansk',
      kk: 'Каталан тілі',
      be: 'Каталонская мова',
      fi: 'Katalaani',
      oc: 'Catalan',
      hy: 'Կատալաներեն',
      li: 'Catalaans',
      ko: '카탈루냐어',
      br: 'Katalaneg',
      co: 'Lingua catalana',
      sr: 'Каталонски језик',
      kw: 'Katalanek',
      ro: 'Limba catalană',
      yi: 'קאטאלאניש',
      zh: '加泰罗尼亚语',
      mn: 'Каталан хэл',
      wa: 'Catalan',
      gn: "Katalã ñe'ẽ",
      km: 'ភាសាកាតាឡាន',
      ja: 'カタルーニャ語',
      sk: 'Katalánčina',
      rw: 'Igikatalani',
      kv: 'Каталан кыв',
      ml: 'കാറ്റലൻ ഭാഷ',
      tk: 'Katalança',
      vi: 'Tiếng Catala',
      ms: 'Bahasa Catalonia',
      ta: 'காட்டலான் மொழி',
      ia: 'Lingua catalan',
      ku: 'Zimanê katalanî',
      ru: 'Каталанский язык',
      sw: 'Kikatalunya',
      nl: 'Catalaans'
    },
    wikidata: {
      en: 'Q7026'
    }
  },
  'en:ndonga-dialect': {
    wikidata: {
      en: 'Q33900'
    },
    language_code_3: {
      en: 'ndo'
    },
    name: {
      ng: 'Ndonga',
      en: 'Ndonga dialect',
      pt: 'Ndonga',
      uk: 'Ндонга',
      eo: 'Ndonga lingvo',
      ja: 'ンドンガ語',
      fr: 'Ndonga',
      es: 'Ndonga',
      cs: 'Ndonga',
      et: 'Ndonga keel',
      pl: 'Język ndonga',
      it: 'Ndonga',
      sv: 'Ndonga',
      ta: 'இந்தோங்க மொழி',
      ms: 'Bahasa Ndonga',
      eu: 'Ndonga',
      sw: 'Kindonga',
      ru: 'Ндонга',
      fa: 'زبان اندونگا',
      nl: 'Ndonga',
      fi: 'Ndonga',
      be: 'Ндонга'
    },
    language_code_2: {
      en: 'ng'
    }
  },
  'en:igbo-language': {
    language_code_2: {
      en: 'ig'
    },
    language_code_3: {
      en: 'ibo'
    },
    name: {
      ig: 'Asụsụ Igbo',
      la: 'Lingua Igbonica',
      pt: 'Língua igbo',
      eo: 'Igba lingvo',
      mr: 'इग्बो भाषा',
      en: 'Igbo language',
      pa: 'ਇਗਬੋ ਭਾਸ਼ਾ',
      ca: 'Igbo',
      fr: 'Igbo',
      io: 'Ibo-linguo',
      gl: 'Lingua ibo',
      es: 'Idioma igbo',
      id: 'Bahasa Igbo',
      it: 'Igbo',
      qu: 'Igbo simi',
      sh: 'Igbo jezik',
      ar: 'لغة الإيجبو',
      sv: 'Igbo',
      de: 'Igbo',
      eu: 'Igboera',
      nn: 'Igbo',
      nb: 'Igbo',
      oc: 'Igbo',
      fi: 'Igbon kieli',
      be: 'Ігба',
      fo: 'Igbo mál',
      ko: '이그보어',
      hy: 'Իգբո լեզու',
      br: 'Igboeg',
      bn: 'ইগবো ভাষা',
      uk: 'Ігбо',
      ro: 'Limba igbo',
      cs: 'Igboština',
      ja: 'イボ語',
      zh: '伊博語',
      wa: 'Igbo',
      kv: 'Игбо',
      rw: 'Ikigibo',
      sk: 'Igboština',
      hr: 'Igbo jezik',
      lt: 'Igbų kalba',
      pl: 'Język igbo',
      ta: 'இக்போ மொழி',
      lv: 'Igbo valoda',
      af: 'Igbo',
      ru: 'Игбо',
      fa: 'زبان ایگبو',
      yo: 'Èdè Ígbò',
      sw: 'Kiigbo',
      nl: 'Igbo',
      da: 'Igbo'
    },
    wikidata: {
      en: 'Q33578'
    }
  },
  'en:afrikaans': {
    wikidata: {
      en: 'Q14196'
    },
    language_code_3: {
      en: 'afr'
    },
    name: {
      ln: 'Afrikansi',
      sm: 'Faʻa Afelika',
      tl: 'Wikang Afrikaans',
      en: 'Afrikaans',
      mr: 'आफ्रिकान्स भाषा',
      ca: 'Afrikaans',
      pa: 'ਆਫ਼੍ਰੀਕਾਂਸ ਭਾਸ਼ਾ',
      es: 'Afrikáans',
      gl: 'Lingua africáner',
      fr: 'Afrikaans',
      sc: 'Limba afrikaans',
      th: 'ภาษาแอฟริคานส์',
      sq: 'Gjuha afrikane',
      it: 'Afrikaans',
      qu: 'Afrikans simi',
      fy: 'Afrikaansk',
      de: 'Afrikaans',
      ur: 'افریکانز',
      am: 'አፍሪካንስ',
      nb: 'Afrikaans',
      eu: 'Afrikaans',
      fo: 'Afrikaans mál',
      hu: 'Afrikaans',
      bg: 'Африканс',
      zu: 'IsiBhunu',
      ug: 'ئاڧرىقانچە',
      bn: 'আফ্রিকান্স ভাষা',
      uk: 'Африкаанс',
      is: 'Afríkanska',
      tr: 'Afrikaanca',
      se: 'Afrikánsagiella',
      cs: 'Afrikánština',
      az: 'Afrikaans',
      et: 'Afrikaani keel',
      hr: 'Afrikaans',
      pl: 'Język afrikaans',
      lt: 'Afrikanų kalba',
      lv: 'Afrikandu valoda',
      gd: 'Afrikaans',
      af: 'Afrikaans',
      fa: 'زبان آفریکانس',
      an: 'Afrikaans',
      mk: 'Африканс',
      da: 'Afrikaans',
      eo: 'Afrikansa lingvo',
      uz: 'Afrikaans',
      pt: 'Língua africâner',
      ka: 'აფრიკაანსი',
      la: 'Lingua Africana',
      cy: 'Afrikaans',
      cv: 'Африкаанс',
      io: 'Afrikansa linguo',
      ga: 'An Afracáinis',
      he: 'אפריקאנס',
      el: 'Αφρικάανς γλώσσα',
      sl: 'Afrikanščina',
      id: 'Bahasa Afrikaans',
      tt: 'Африкаанс',
      ar: 'لغة أفريقانية',
      sh: 'Afrikaans',
      hi: 'अफ़्रीकाँस भाषा',
      vo: 'Frikopänapük',
      sv: 'Afrikaans',
      ve: 'Afrikaans',
      nn: 'Afrikaans',
      kk: 'Африкаанс',
      be: 'Афрыкаанс',
      fi: 'Afrikaans',
      oc: 'Afrikaans',
      hy: 'Աֆրիկաանս',
      li: 'Afrikaans',
      ko: '아프리칸스어',
      br: 'Afrikaneg',
      mg: 'Fiteny afrikaans',
      kw: 'Afrikaans',
      sr: 'Африканс',
      ro: 'Limba afrikaans',
      zh: '南非語',
      gn: 'Afrikáans',
      ja: 'アフリカーンス語',
      sk: 'Afrikánčina',
      rw: 'Ikinyafurikansi',
      kv: 'Африкаанс',
      ml: 'ആഫ്രികാൻസ്',
      ne: 'अफ्रिकी भाषा',
      ms: 'Bahasa Afrikaans',
      vi: 'Afrikaans',
      ta: 'ஆபிரிக்கான மொழி',
      xh: 'IsiBhulu',
      ia: 'Afrikaans',
      yo: 'Èdè Áfríkáánì',
      ru: 'Африкаанс',
      st: 'Seburu',
      sw: 'Kiafrikaans',
      nl: 'Afrikaans'
    },
    language_code_2: {
      en: 'af'
    }
  },
  'en:polish': {
    name: {
      oc: 'Polonés',
      be: 'Польская мова',
      kk: 'Поляк тілі',
      fi: 'Puola',
      mt: 'Pollak',
      nn: 'Polsk',
      sh: 'Poljski jezik',
      ar: 'لغة بولندية',
      hi: 'पोलिश भाषा',
      vo: 'Polänapük',
      sv: 'Polska',
      id: 'Bahasa Polski',
      ba: 'Поляк теле',
      sl: 'Poljščina',
      tt: 'Поляк теле',
      te: 'పోలిష్',
      jv: 'Basa Polski',
      he: 'פולנית',
      el: 'Πολωνική γλώσσα',
      kl: 'Polenimiutut',
      ee: 'Pɔlisigbe',
      io: 'Poloniana linguo',
      ga: 'An Pholainnis',
      cy: 'Pwyleg',
      la: 'Lingua Polonica',
      ka: 'პოლონური ენა',
      uz: 'Polyak tili',
      pt: 'Língua polaca',
      eo: 'Pola lingvo',
      cv: 'Поляк чĕлхи',
      bs: 'Poljski jezik',
      nl: 'Pools',
      ru: 'Польский язык',
      yo: 'Èdè Pólándì',
      ku: 'Zimanê polonî',
      mi: 'Reo Pōrana',
      sw: 'Kipoland',
      ta: 'போலிய மொழி',
      vi: 'Tiếng Ba Lan',
      ms: 'Bahasa Poland',
      ia: 'Lingua polonese',
      ne: 'पोल्यान्डेली',
      kv: 'Польша кыв',
      sk: 'Poľština',
      ml: 'പോളിഷ് ഭാഷ',
      yi: 'פויליש',
      ro: 'Limba polonă',
      sr: 'Пољски језик',
      kw: 'Polonek',
      gn: "Poloñañe'ẽ",
      km: 'ភាសាប៉ូឡូញ',
      ja: 'ポーランド語',
      zh: '波兰语',
      mn: 'Польш хэл',
      wa: 'Polonès',
      cu: 'Пол҄ьскъ ѩꙁꙑкъ',
      mg: 'Fiteny poloney',
      co: 'Lingua pulacca',
      ko: '폴란드어',
      li: 'Pools',
      hy: 'Լեհերեն',
      br: 'Poloneg',
      dv: 'ލަހިސްތާނީ',
      bg: 'Полски език',
      hu: 'Lengyel',
      fo: 'Pólskt mál',
      de: 'Polnisch',
      ur: 'پولش زبان',
      am: 'ፖልኛ',
      eu: 'Poloniera',
      nb: 'Polsk',
      ab: 'Аполш бызшәа',
      fy: 'Poalsk',
      it: 'Polacco',
      qu: 'Pulaku simi',
      sc: 'Limba polonesa',
      sq: 'Gjuha polake',
      gv: 'Polynnish',
      th: 'ภาษาโปแลนด์',
      pa: 'ਪੋਲਥਾਨੀ ਭਾਸ਼ਾ',
      ca: 'Polonès',
      fr: 'Polonais',
      es: 'Polaco',
      gl: 'Lingua polaca',
      en: 'Polish',
      mr: 'पोलिश भाषा',
      sm: "Fa'apolani",
      tl: 'Wikang Polako',
      da: 'Polsk',
      an: 'Idioma polaco',
      fa: 'زبان لهستانی',
      mk: 'Полски јазик',
      tg: 'Забони полякӣ',
      bo: 'ཕོ་ལན་སྐད།',
      lv: 'Poļu valoda',
      gd: 'Pòlais',
      af: 'Pools',
      hr: 'Poljski jezik',
      lb: 'Polnesch',
      lt: 'Lenkų kalba',
      pl: 'Język polski',
      az: 'Polyak dili',
      et: 'Poola keel',
      cs: 'Polština',
      se: 'Polskkagiella',
      ky: 'Поляк тили',
      na: 'Dorerin Poran',
      tr: 'Lehçe',
      is: 'Pólska',
      uk: 'Польська мова',
      ce: 'Полякийн мотт',
      ug: 'پولەك تىلى',
      os: 'Польшæйаг æвзаг',
      zu: 'IsiPholisi',
      bn: 'পোলীয় ভাষা'
    },
    language_code_3: {
      en: 'pol'
    },
    wikidata: {
      en: 'Q809'
    },
    language_code_2: {
      en: 'pl'
    }
  },
  'en:kashmiri': {
    wikidata: {
      en: 'Q33552'
    },
    language_code_3: {
      en: 'kas'
    },
    name: {
      pt: 'Língua caxemira',
      eo: 'Kaŝmira lingvo',
      la: 'Lingua Casmirica',
      mr: 'काश्मिरी भाषा',
      en: 'Kashmiri',
      ca: 'Caixmiri',
      pa: 'ਕਸ਼ਮੀਰੀ ਭਾਸ਼ਾ',
      es: 'Cachemir',
      fr: 'Cachemiri',
      el: 'Κασμίρι γλώσσα',
      th: 'ภาษาแคชเมียร์',
      it: 'Kashmiri',
      or: 'କାଶ୍ମୀରୀ ଭାଷା',
      id: 'Bahasa Kashmiri',
      te: 'కాశ్మీరీ భాష',
      ar: 'لغة كشميرية',
      sh: 'Kašmiri jezik',
      hi: 'कश्मीरी भाषा',
      sv: 'Kashmiri',
      ur: 'کشمیری زبان',
      de: 'Kashmiri',
      am: 'ካሽሚርኛ',
      nn: 'Kasjmiri',
      nb: 'Kasjmiri',
      eu: 'Kaxmirera',
      be: 'Кашмірская мова',
      fi: 'Kašmirin kieli',
      gu: 'કાશ્મીરી ભાષા',
      ko: '카슈미르어',
      bn: 'কাশ্মীরি ভাষা',
      br: 'Kachmireg',
      uk: 'Кашмірська мова',
      is: 'Kasmíríska',
      tr: 'Keşmirce',
      cs: 'Kašmírština',
      sr: 'Кашмири језик',
      se: 'Kašmirigiella',
      zh: '克什米爾語',
      ja: 'カシミール語',
      sk: 'Kašmírčina',
      az: 'Kəşmir dili',
      kv: 'Кашмир кыв',
      ml: 'കശ്മീരി ഭാഷ',
      et: 'Kašmiiri keel',
      hr: 'Kašmirski jezik',
      pl: 'Język kaszmirski',
      lt: 'Kašmyrų kalba',
      kn: 'ಕಾಶ್ಮೀರಿ',
      vi: 'Tiếng Kashmir',
      ms: 'Bahasa Kashmir',
      lv: 'Kašmiriešu valoda',
      ta: 'காஷ்மீரி மொழி',
      sa: 'कश्मीरी',
      ks: 'کٲشُر زَبان',
      fa: 'زبان کشمیری',
      ru: 'Кашмирский язык',
      an: 'Idioma caixmiri',
      sw: 'Kikashmiri',
      mk: 'Кашмирски јазик',
      da: 'Kashmirisk',
      nl: 'Kasjmiri'
    },
    language_code_2: {
      en: 'ks'
    }
  },
  'en:māori': {
    language_code_2: {
      en: 'mi'
    },
    language_code_3: {
      en: 'mao'
    },
    name: {
      sc: 'Limba māori',
      ca: 'Maori',
      gl: 'Lingua maorí',
      es: 'Maorí',
      fr: 'Maori de Nouvelle-Zélande',
      en: 'Māori',
      tl: 'Wikang Māori',
      sm: 'Gagana Māoli',
      bg: 'Маорски език',
      hu: 'Maori',
      am: 'ማዖሪ ቋንቋ',
      de: 'Maorische Sprache',
      nb: 'Maorisk',
      eu: 'Maoriera',
      it: 'Māori',
      qu: 'Mawri simi',
      az: 'Maori dili',
      et: 'Maoori keel',
      cs: 'Maorština',
      uk: 'Маорі',
      tr: 'Maori dili',
      na: 'Dorerin Maori',
      os: 'Маори',
      da: 'Maori',
      fa: 'زبان مائوری',
      mk: 'Маорски јазик',
      lv: 'Maoru valoda',
      af: 'Maori',
      hr: 'Maorski jezik',
      pl: 'Język maori',
      lt: 'Maorių kalba',
      he: 'מאורית',
      el: 'Μαορί γλώσσα',
      kl: 'Maorimiutut',
      ga: 'Maorais',
      io: 'Maoriana linguo',
      ka: 'მაორი ენა',
      pt: 'Língua maori',
      eo: 'Maoria lingvo',
      la: 'Lingua Maoriana',
      cy: 'Maori',
      kk: 'Маори тілі',
      fi: 'Maorin kieli',
      be: 'Маары',
      oc: 'Maòri',
      nn: 'Maori',
      ar: 'لغة ماورية',
      sh: 'Maorski jezik',
      hi: 'माओरी भाषा',
      sv: 'Maori',
      id: 'Bahasa Maori',
      sk: 'Maorijčina',
      kv: 'Маори кыв',
      ml: 'മാവോറി ഭാഷ',
      sr: 'Маорски језик',
      kw: 'Tavas Māori',
      ro: 'Limba maori',
      wa: 'Mawori',
      zh: '毛利语',
      ja: 'マオリ語',
      ko: '마오리어',
      br: 'Maorieg',
      nl: 'Maori',
      ru: 'Маори',
      sw: 'Kimaori',
      mi: 'Reo Māori',
      vi: 'Tiếng Maori',
      ms: 'Bahasa Māori',
      ta: 'மாவோரி மொழி',
      ne: 'माओरी'
    },
    wikidata: {
      en: 'Q36451'
    }
  },
  'en:hungarian': {
    wikidata: {
      en: 'Q9067'
    },
    name: {
      ga: 'An Ungáiris',
      io: 'Hungariana linguo',
      el: 'Ουγγρική γλώσσα',
      he: 'הונגרית',
      bs: 'Mađarski jezik',
      cv: 'Венгр чĕлхи',
      la: 'Lingua Hungarica',
      cy: 'Hwngareg',
      uz: 'Venger tili',
      eo: 'Hungara lingvo',
      pt: 'Língua húngara',
      ka: 'უნგრული ენა',
      nn: 'Ungarsk',
      oc: 'Ongrés',
      be: 'Венгерская мова',
      kk: 'Мажар тілі',
      fi: 'Unkari',
      tt: 'Маҗар теле',
      jv: 'Basa Hongaria',
      id: 'Bahasa Hongaria',
      sl: 'Madžarščina',
      sv: 'Ungerska',
      vo: 'Macaränapük',
      hi: 'हंगेरियाई भाषा',
      sh: 'Mađarski jezik',
      ar: 'لغة مجرية',
      ja: 'ハンガリー語',
      zh: '匈牙利语',
      wa: 'Hongrwès',
      mn: 'Унгар/Мажар хэл',
      yi: 'אונגעריש',
      ro: 'Limba maghiară',
      kw: 'Hungarek',
      sr: 'Мађарски језик',
      kv: 'Мадьяр кыв',
      rw: 'Igihongiriya',
      sk: 'Maďarčina',
      br: 'Hungareg',
      ko: '헝가리어',
      li: 'Hongaars',
      hy: 'Հունգարերեն',
      mg: 'Fiteny hongariana',
      sw: 'Kihungaria',
      ru: 'Венгерский язык',
      yo: 'Èdè Húngárì',
      nl: 'Hongaars',
      ia: 'Lingua hungare',
      ta: 'அங்கேரிய மொழி',
      vi: 'Tiếng Hungary',
      ms: 'Bahasa Hungary',
      fr: 'Hongrois',
      es: 'Húngaro',
      gl: 'Lingua húngara',
      pa: 'ਮਗਯਾਰ',
      ca: 'Hongarès',
      sq: 'Gjuha hungareze',
      gv: 'Ungaarish',
      th: 'ภาษาฮังการี',
      sc: 'Limba ungheresa',
      sm: 'Gagana Hungary',
      tl: 'Wikang Unggaro',
      en: 'Hungarian',
      mr: 'हंगेरियन भाषा',
      eu: 'Hungariera',
      nb: 'Ungarsk',
      ur: 'مجارستانی زبان',
      am: 'ሀንጋርኛ',
      de: 'Ungarisch',
      bg: 'Унгарски език',
      hu: 'Magyar',
      fo: 'Ungarskt mál',
      it: 'Ungherese',
      qu: 'Unriya simi',
      fy: 'Hongaarsk',
      se: 'Ungárgiella',
      ky: 'Венгр тили',
      cs: 'Maďarština',
      et: 'Ungari keel',
      az: 'Macar dili',
      bn: 'হাঙ্গেরীয় ভাষা',
      os: 'Венгриаг æвзаг',
      ug: 'ماجار تىلى',
      ce: 'Vengarhoyn mott',
      tr: 'Macarca',
      is: 'Ungverska',
      uk: 'Угорська мова',
      mk: 'Унгарски јазик',
      rm: 'Lingua ungaraisa',
      an: 'Idioma hongaro',
      fa: 'زبان مجاری',
      da: 'Ungarsk',
      lt: 'Vengrų kalba',
      pl: 'Język węgierski',
      hr: 'Mađarski jezik',
      af: 'Hongaars',
      gd: 'Ungairis',
      lv: 'Ungāru valoda'
    },
    language_code_3: {
      en: 'hun'
    },
    language_code_2: {
      en: 'hu'
    }
  },
  'en:breton': {
    language_code_2: {
      en: 'br'
    },
    language_code_3: {
      en: 'bre'
    },
    name: {
      ja: 'ブルトン語',
      zh: '布列塔尼语',
      wa: 'Burton',
      ro: 'Limba bretonă',
      kw: 'Bretonek',
      sr: 'Бретонски језик',
      kv: 'Брезон кыв',
      sk: 'Bretónčina',
      br: 'Brezhoneg',
      ko: '브르타뉴어',
      li: 'Bretoens',
      hy: 'Բրետոներեն',
      ru: 'Бретонский язык',
      nl: 'Bretons',
      ne: 'ब्रेटोन',
      ia: 'Lingua britone',
      ta: 'பிரித்தானி மொழி',
      ms: 'Bahasa Breton',
      vi: 'Tiếng Breton',
      io: 'Bretoniana linguo',
      ga: 'An Bhriotáinis',
      el: 'Βρετονική γλώσσα',
      he: 'ברטונית',
      cv: 'Бретон чĕлхи',
      cy: 'Llydaweg',
      la: 'Lingua Britonica',
      eo: 'Bretona lingvo',
      pt: 'Língua bretã',
      nn: 'Bretonsk',
      oc: 'Breton',
      fi: 'Bretoni',
      be: 'Брэтонская мова',
      id: 'Bahasa Breton',
      sl: 'Bretonščina',
      vo: 'Bretänapük',
      sv: 'Bretonska',
      sh: 'Bretonski jezik',
      ar: 'لغة بريتانية',
      se: 'Bretonagiella',
      cs: 'Bretonština',
      et: 'Bretooni keel',
      az: 'Breton dili',
      ug: 'برېتون تىلى',
      tr: 'Bretonca',
      is: 'Bretónska',
      uk: 'Бретонська мова',
      mk: 'Бретонски јазик',
      rm: 'Lingua bretona',
      an: 'Idioma bretón',
      fa: 'زبان برتون',
      da: 'Bretonsk',
      lt: 'Bretonų kalba',
      pl: 'Język bretoński',
      hr: 'Bretonski jezik',
      gd: 'Breatannais',
      af: 'Bretons',
      lv: 'Bretoņu valoda',
      fr: 'Breton',
      gl: 'Lingua bretoa',
      es: 'Bretón',
      ca: 'Bretó',
      sq: 'Gjuha bretoneze',
      gv: 'Britaanish',
      th: 'ภาษาเบรอตง',
      en: 'Breton',
      mr: 'ब्रेतॉन भाषा',
      eu: 'Bretainiera',
      nb: 'Bretonsk',
      de: 'Bretonisch',
      am: 'ብረቶንኛ',
      hu: 'Breton nyelv',
      bg: 'Бретонски език',
      fo: 'Bretonskt mál',
      qu: 'Britun simi',
      it: 'Bretone',
      fy: 'Bretonsk'
    },
    wikidata: {
      en: 'Q12107'
    }
  },
  'en:portuguese': {
    wikidata: {
      en: 'Q5146'
    },
    name: {
      hy: 'Պորտուգալերեն',
      li: 'Portugees',
      ko: '포르투갈어',
      br: 'Portugaleg',
      co: 'Lingua portughese',
      mg: 'Fiteny portogey',
      kw: 'Portyngalek',
      sr: 'Португалски језик',
      ro: 'Limba portugheză',
      yi: 'פארטוגעזיש',
      mn: 'Португал хэл',
      zh: '葡萄牙语',
      wa: 'Portuguès',
      ja: 'ポルトガル語',
      gn: "Poytugañe'ẽ",
      rw: 'Igiporutigali',
      sk: 'Portugalčina',
      kv: 'Португал кыв',
      no: 'Portugisisk',
      ml: 'പോർച്ചുഗീസ് ഭാഷ',
      vi: 'Tiếng Bồ Đào Nha',
      ms: 'Bahasa Portugis',
      kn: 'ಪೋರ್ಚುಗೀಯ ಭಾಷೆ',
      ta: 'போர்த்துக்கீச மொழி',
      sa: 'पुर्तगाली भाषा',
      ia: 'Lingua portugese',
      ku: 'Zimanê portûgalî',
      yo: 'Èdè Pọrtugí',
      ru: 'Португальский язык',
      mi: 'Reo Pōtikī',
      sw: 'Kireno',
      nl: 'Portugees',
      bs: 'Portugalski jezik',
      uz: 'Portugal tili',
      ka: 'პორტუგალიური ენა',
      pt: 'Língua portuguesa',
      eo: 'Portugala lingvo',
      la: 'Lingua Lusitana',
      cy: 'Portiwgaleg',
      cv: 'Португал чĕлхи',
      ga: 'An Phortaingéilis',
      io: 'Portugalana linguo',
      ee: 'Pɔtugisgbe',
      kg: 'Kimputulukesi',
      he: 'פורטוגזית',
      el: 'Πορτογαλική γλώσσα',
      sl: 'Portugalščina',
      id: 'Bahasa Portugis',
      ba: 'Португал теле',
      jv: 'Basa Portugis',
      tt: 'Португал теле',
      ar: 'لغة برتغالية',
      sh: 'Portugalski jezik',
      sv: 'Portugisiska',
      vo: 'Portugänapük',
      hi: 'पुर्तगाली भाषा',
      nn: 'Portugisisk',
      kk: 'Португал тілі',
      fi: 'Portugali',
      be: 'Партугальская мова',
      oc: 'Portugués',
      zu: 'IsiPutukezi',
      os: 'Португайлаг æвзаг',
      ug: 'پورتۇگال تىلى',
      bn: 'পর্তুগিজ ভাষা',
      uk: 'Португальська мова',
      tr: 'Portekizce',
      is: 'Portúgalska',
      na: 'Dorerin Portsiugar',
      ce: 'Португалихойн мотт',
      se: 'Portugalagiella',
      ky: 'Португал тили',
      cs: 'Portugalština',
      az: 'Portuqal dili',
      et: 'Portugali keel',
      lo: 'ພາສາປອກຕຸຍການ',
      lb: 'Portugisesch',
      hr: 'Portugalski jezik',
      pl: 'Język portugalski',
      lt: 'Portugalų kalba',
      lv: 'Portugāļu valoda',
      af: 'Portugees',
      gd: 'Portagailis',
      fa: 'زبان پرتغالی',
      an: 'Idioma portugués',
      tg: 'Забони португалӣ',
      bo: 'པོ་ཅུ་གྷལ་གྱི་སྐད།',
      rm: 'Lingua portugaisa',
      mk: 'Португалски јазик',
      da: 'Portugisisk',
      ln: 'Lipulutugɛ́si',
      sm: "Fa'aPotukale",
      tl: 'Wikang Portuges',
      mr: 'पोर्तुगीज भाषा',
      en: 'Portuguese',
      ca: 'Portuguès',
      pa: 'ਪੁਰਤਗਾਲੀ ਭਾਸ਼ਾ',
      es: 'Portugués',
      gl: 'Lingua portuguesa',
      fr: 'Portugais',
      sc: 'Limba portughesa',
      th: 'ภาษาโปรตุเกส',
      gv: 'Portiugish',
      sq: 'Gjuha portugeze',
      qu: 'Purtuyis simi',
      it: 'Portoghese',
      su: 'Basa Portugis',
      fy: 'Portegeesk',
      de: 'Portugiesisch',
      ur: 'پرتگیزی زبان',
      ht: 'Lang pòtigè',
      am: 'ፖርቱጊዝኛ',
      nb: 'Portugisisk',
      eu: 'Portuges',
      dv: 'ޕޯޗުގީޒު',
      ty: 'Reo Pōtītī',
      fo: 'Portugisiskt mál',
      bg: 'Португалски език',
      hu: 'Portugál'
    },
    language_code_3: {
      en: 'por'
    },
    language_code_2: {
      en: 'pt'
    }
  },
  'en:bulgarian': {
    language_code_3: {
      en: 'bul'
    },
    name: {
      ar: 'لغة بلغارية',
      sh: 'Bugarski jezik',
      vo: 'Bulgaränapük',
      sv: 'Bulgariska',
      hi: 'बुल्गारियाई भाषा',
      sl: 'Bolgarščina',
      id: 'Bahasa Bulgaria',
      ba: 'Болгар теле',
      jv: 'Basa Bulgaria',
      tt: 'Болгар теле',
      kk: 'Болгар тілі',
      fi: 'Bulgaria',
      be: 'Балгарская мова',
      oc: 'Bulgar',
      nn: 'Bulgarsk',
      uz: 'Bolgar tili',
      ka: 'ბულგარული ენა',
      pt: 'Língua búlgara',
      eo: 'Bulgara lingvo',
      cy: 'Bwlgareg',
      la: 'Lingua Bulgarica',
      cv: 'Полхар чĕлхи',
      bs: 'Bugarski jezik',
      he: 'בולגרית',
      el: 'Βουλγαρική γλώσσα',
      io: 'Bulgariana linguo',
      ga: 'An Bhulgáiris',
      ms: 'Bahasa Bulgaria',
      vi: 'Tiếng Bulgaria',
      ta: 'பல்கேரிய மொழி',
      nl: 'Bulgaars',
      yo: 'Èdè Bùlgáríà',
      ku: 'Zimanê bulgarî',
      ru: 'Болгарский язык',
      sw: 'Kibulgaria',
      cu: 'Блъгарьскъ ѩꙁꙑкъ',
      co: 'Lingua bulgara',
      hy: 'Բուլղարերեն',
      li: 'Bölgaars',
      ko: '불가리아어',
      br: 'Bulgareg',
      sk: 'Bulharčina',
      rw: 'Ikibulugariya',
      kv: 'Болгар кыв',
      sr: 'Бугарски језик',
      kw: 'Bulgarek',
      ro: 'Limba bulgară',
      yi: 'בולגאריש',
      mn: 'Болгар хэл',
      zh: '保加利亚语',
      wa: 'Bulgåre',
      gn: "Vugariañe'ẽ",
      ja: 'ブルガリア語',
      fy: 'Bulgaarsk',
      ab: 'Абылгар бызшәа',
      qu: 'Bulgarya simi',
      it: 'Bulgaro',
      fo: 'Bulgarskt mál',
      bg: 'Български език',
      hu: 'Bolgár',
      de: 'Bulgarisch',
      am: 'ቡልጋርኛ',
      nb: 'Bulgarsk',
      eu: 'Bulgariera',
      en: 'Bulgarian',
      mr: 'बल्गेरियन भाषा',
      tl: 'Wikang Bulgaro',
      th: 'ภาษาบัลแกเรีย',
      gv: 'Bulgeyrish',
      sq: 'Gjuha bullgare',
      ca: 'Búlgar',
      pa: 'ਬਲਗਾਰੀ',
      gl: 'Lingua búlgara',
      es: 'Búlgaro',
      fr: 'Bulgare',
      lv: 'Bulgāru valoda',
      af: 'Bulgaars',
      gd: 'Bulgàiris',
      hr: 'Bugarski jezik',
      lb: 'Bulgaresch',
      pl: 'Język bułgarski',
      lt: 'Bulgarų kalba',
      da: 'Bulgarsk',
      fa: 'زبان بلغاری',
      an: 'Idioma bulgaro',
      tg: 'Забони булғорӣ',
      mk: 'Бугарски јазик',
      uk: 'Болгарська мова',
      is: 'Búlgarska',
      tr: 'Bulgarca',
      ce: 'Болгарийн мотт',
      os: 'Болгайраг æвзаг',
      ug: 'بۇلغارىيە تىلى',
      bn: 'বুলগেরীয় ভাষা',
      ps: 'بلغاريايي ژبه',
      az: 'Bolqar dili',
      et: 'Bulgaaria keel',
      ky: 'Болгар тили',
      se: 'Bulgáriagiella',
      cs: 'Bulharština'
    },
    wikidata: {
      en: 'Q7918'
    },
    language_code_2: {
      en: 'bg'
    }
  },
  'en:avestan': {
    language_code_2: {
      en: 'ae'
    },
    language_code_3: {
      en: 'ave'
    },
    name: {
      br: 'Avesteg',
      bn: 'অবেস্তা ভাষা',
      ko: '아베스타어',
      os: 'Авестæйы æвзаг',
      hy: 'Ավեստերեն',
      tr: 'Zentçe',
      uk: 'Авестійська мова',
      ja: 'アヴェスター語',
      zh: '阿維斯陀語',
      ro: 'Limba avestică',
      cs: 'Avestánština',
      sr: 'Авестански језик',
      ky: 'Авеста тили',
      ml: 'അവെസ്താൻ ഭാഷ',
      az: 'Avesta dili',
      kv: 'Авеста кыв',
      lt: 'Avestos kalba',
      pl: 'Język awestyjski',
      tk: 'Awesta dili',
      hr: 'Avestički jezik',
      af: 'Avesties',
      ta: 'அவெஸ்தான் மொழி',
      kn: 'ಅವೇಶ್ಟಿ',
      ms: 'Bahasa Avesta',
      lv: 'Avesta',
      mk: 'Авестански јазик',
      tg: 'Забони авестоӣ',
      an: 'Idioma avestico',
      ru: 'Авестийский язык',
      ku: 'Zimanê avestayî',
      fa: 'زبان اوستایی',
      nl: 'Avestisch',
      da: 'Avestisk',
      tl: 'Wikang Avestan',
      cv: 'Авеста чĕлхи',
      en: 'Avestan',
      la: 'Lingua Avestana',
      eo: 'Avesta lingvo',
      pt: 'Língua avéstica',
      fr: 'Avestique',
      io: 'Avestana linguo',
      es: 'Avéstico',
      ga: 'An Aivéistis',
      pa: 'ਅਵੇਸਤਨ ਭਾਸ਼ਾ',
      ca: 'Avèstic',
      el: 'Αβεστική γλώσσα',
      th: 'ภาษาอเวสตะ',
      he: 'אווסטית',
      id: 'Bahasa Avesta',
      it: 'Avestico',
      hi: 'अवस्ताई भाषा',
      sv: 'Avestiska',
      sh: 'Avestanski jezik',
      ar: 'أفستية',
      eu: 'Avestera',
      nb: 'Avestisk',
      ur: 'اوستائی زبان',
      de: 'Avestische Sprache',
      bg: 'Авестийски език',
      be: 'Авестыйская мова',
      fi: 'Avestan kieli'
    },
    wikidata: {
      en: 'Q29572'
    }
  },
  'en:nepali': {
    wikidata: {
      en: 'Q33823'
    },
    language_code_3: {
      en: 'nep'
    },
    name: {
      gd: 'Nepali',
      af: 'Nepali',
      lv: 'Nepāliešu valoda',
      lt: 'Nepalų kalba',
      pl: 'Język nepalski',
      hr: 'Nepalski jezik',
      da: 'Nepalesisk',
      mk: 'Непалски јазик',
      bo: 'བལ་པོའི་སྐད།',
      tg: 'Забони непалӣ',
      fa: 'زبان نپالی',
      ce: 'Непалийн мотт',
      tr: 'Nepali',
      is: 'Nepalska',
      uk: 'Непальська мова',
      bn: 'নেপাল ভাষা',
      ug: 'نېپال تىلى',
      et: 'Nepali keel',
      az: 'Nepal dili',
      ky: 'Непал тили',
      cs: 'Nepálština',
      it: 'Nepalese',
      qu: 'Nipali simi',
      bg: 'Непалски език',
      eu: 'Nepalera',
      nb: 'Nepali',
      de: 'Nepali',
      en: 'Nepali',
      th: 'ภาษาเนปาล',
      fr: 'Népalais',
      es: 'Nepalí',
      gl: 'Lingua nepalesa',
      pa: 'ਨੇਪਾਲੀ ਭਾਸ਼ਾ',
      ca: 'Nepalès',
      sa: 'नेपाली भाषा',
      ta: 'நேபாளி மொழி',
      bh: 'नेपाली भाषा',
      kn: 'ನೇಪಾಳಿ ಭಾಷೆ',
      vi: 'Tiếng Nepal',
      ne: 'नेपाली भाषा',
      nl: 'Nepalees',
      za: 'Vahnizbozwj',
      sw: 'Kinepali',
      ru: 'Непальский язык',
      br: 'Nepaleg',
      ko: '네팔어',
      gu: 'નેપાળી ભાષા',
      hy: 'Նեպալերեն',
      ml: 'നേപ്പാളി ഭാഷ',
      kv: 'Непали',
      gn: "Nepali ñe'ẽ",
      ja: 'ネパール語',
      zh: '尼泊尔语',
      ro: 'Limba nepaleză',
      yi: 'נעפאליש',
      sr: 'Непалски језик',
      hi: 'नेपाली भाषा',
      sv: 'Nepali',
      sh: 'Nepalski jezik',
      ar: 'لغة نيبالية',
      te: 'నేపాలీ భాష',
      id: 'Bahasa Nepali',
      sl: 'Nepalščina',
      or: 'ନେପାଳୀ ଭାଷା',
      be: 'Непальская мова',
      fi: 'Nepalin kieli',
      nn: 'Nepali',
      cy: 'Nepaleg',
      la: 'Lingua nepalensis',
      eo: 'Nepala lingvo',
      uz: 'Nepal tili',
      pt: 'Língua nepali',
      el: 'Νεπαλεζική γλώσσα',
      io: 'Nepalana linguo',
      ga: 'An Neipeailis'
    },
    language_code_2: {
      en: 'ne'
    }
  },
  'en:twi': {
    language_code_3: {
      en: 'twi'
    },
    name: {
      id: 'Bahasa Twi',
      ak: 'Twi',
      it: 'Twi',
      pl: 'Język twi',
      ta: 'துவி மொழி',
      sh: 'Twi jezik',
      de: 'Twi',
      ru: 'Чви',
      yo: 'Twi',
      nb: 'Twi',
      nl: 'Twi',
      oc: 'Twi',
      da: 'Twi',
      ug: 'تۋىچە',
      br: 'Twieg',
      is: 'Tví',
      uk: 'Чві',
      eo: 'Akana lingvo',
      pt: 'Língua twi',
      en: 'Twi',
      ca: 'Twi',
      fr: 'Twi',
      ja: 'トウィ語',
      es: 'Idioma twi',
      zh: '契維語',
      tw: 'Twi'
    },
    wikidata: {
      en: 'Q36850'
    },
    language_code_2: {
      en: 'tw'
    }
  },
  'en:uzbek': {
    language_code_2: {
      en: 'uz'
    },
    wikidata: {
      en: 'Q9264'
    },
    language_code_3: {
      en: 'uzb'
    },
    name: {
      ky: 'Өзбек тили',
      cs: 'Uzbečtina',
      ps: 'اوزبکي ژبه',
      az: 'Özbək dili',
      et: 'Usbeki keel',
      ug: 'ئۆزبېك تىلى',
      bn: 'উজবেক ভাষা',
      uk: 'Узбецька мова',
      tr: 'Özbekçe',
      is: 'Úsbekíska',
      ce: 'Узбекийн мотт',
      fa: 'زبان ازبکی',
      tg: 'Забони ӯзбакӣ',
      mk: 'Узбечки јазик',
      da: 'Usbekisk',
      hr: 'Uzbečki jezik',
      pl: 'Język uzbecki',
      lt: 'Uzbekų kalba',
      lv: 'Uzbeku valoda',
      af: 'Oesbekies',
      ca: 'Uzbek',
      pa: 'ਉਜ਼ਬੇਕ ਭਾਸ਼ਾ',
      es: 'Uzbeko',
      gl: 'Lingua usbeca',
      fr: 'Ouzbek',
      th: 'ภาษาอุซเบก',
      mr: 'उझबेक भाषा',
      en: 'Uzbek',
      ur: 'ازبک زبان',
      am: 'ዑዝበክኛ',
      de: 'Usbekische Sprache',
      nb: 'Usbekisk',
      eu: 'Uzbekera',
      hu: 'Üzbég nyelv',
      bg: 'Узбекски език',
      it: 'Uzbeco',
      qu: 'Usbik simi',
      sr: 'Узбечки језик',
      ro: 'Limba uzbecă',
      zh: '乌兹别克语',
      mn: 'Узбек хэл',
      ja: 'ウズベク語',
      sk: 'Uzbečtina',
      kv: 'Узбек кыв',
      ml: 'ഉസ്ബെക്ക് ഭാഷ',
      hy: 'Ուզբեկերեն',
      ko: '우즈베크어',
      br: 'Ouzbekeg',
      ru: 'Узбекский язык',
      sw: 'Kiuzbeki',
      za: 'Vahvuhswhbiedkez',
      nl: 'Oezbeeks',
      ms: 'Bahasa Uzbek',
      vi: 'Tiếng Uzbek',
      ta: 'உசுபேகிய மொழி',
      ga: 'An Úisbéiceastáinis',
      he: 'אוזבקית',
      el: 'Ουζμπεκική γλώσσα',
      bs: 'Uzbečki jezik',
      uz: 'Oʻzbek tili',
      ka: 'უზბეკური ენა',
      pt: 'Língua uzbeque',
      eo: 'Uzbeka lingvo',
      la: 'Lingua Uzbecica',
      cv: 'Узбек чĕлхи',
      nn: 'Usbekisk',
      fi: 'Uzbekin kieli',
      kk: 'Өзбек тілі',
      be: 'Узбекская мова',
      oc: 'Uzbek',
      id: 'Bahasa Uzbek',
      tt: 'Үзбәк теле',
      ar: 'لغة أوزبكية',
      sh: 'Uzbečki jezik',
      sv: 'Uzbekiska',
      vo: 'Lusbekänapük',
      hi: 'उज़्बेक भाषा'
    }
  },
  'en:chamorro': {
    language_code_2: {
      en: 'ch'
    },
    wikidata: {
      en: 'Q33262'
    },
    name: {
      en: 'Chamorro',
      mr: 'चामोरो भाषा',
      la: 'Lingua Chamoruana',
      pt: 'Língua chamorro',
      ka: 'ჩამორო',
      eo: 'Ĉamora lingvo',
      sm: 'Gagana Chamorro',
      he: "צ'מורו",
      fr: 'Chamorro',
      gl: 'Lingua chamorra',
      es: 'Idioma chamorro',
      ca: 'Chamorro',
      sv: 'Chamorro',
      sh: 'Chamorro jezik',
      id: 'Bahasa Chamorro',
      it: 'Chamorro',
      oc: 'Chamorro',
      fi: 'Tšamorron kieli',
      eu: 'Chamorro',
      nn: 'Chamorro',
      nb: 'Chamorro',
      de: 'Chamorro',
      uk: 'Чаморро',
      br: 'Tchamoroueg',
      ko: '차모로어',
      lo: 'ພາສາຈາໂມໂລ',
      et: 'Tšamorro keel',
      kv: 'Чаморро',
      ch: "Fino' Chamoru",
      az: 'Çamorro',
      ja: 'チャモロ語',
      zh: '查莫罗语',
      sr: 'Чаморо језик',
      cs: 'Čamoro',
      ta: 'சமோரோ மொழி',
      ms: 'Bahasa Chamorro',
      vi: 'Tiếng Chamorro',
      lv: 'Čamorru valoda',
      lt: 'Čamorų kalba',
      pl: 'Język czamorro',
      ne: 'चामोर्रो',
      hr: 'Chamorro jezik',
      nl: 'Chamorro',
      mk: 'Чаморски јазик',
      ru: 'Чаморро',
      fa: 'زبان چامورو',
      yo: 'Èdè Tsàmórò'
    },
    language_code_3: {
      en: 'cha'
    }
  },
  'en:guarani': {
    wikidata: {
      en: 'Q35876'
    },
    language_code_3: {
      en: 'grn'
    },
    name: {
      ru: 'Гуарани',
      nl: 'Guaraní',
      ia: 'Lingua guarani',
      ms: 'Bahasa Guaraní',
      vi: 'Tiếng Guaraní',
      ta: 'குவாரனி மொழி',
      zh: '瓜拉尼語',
      wa: 'Gwarani',
      ja: 'グアラニー語',
      gn: "Avañe'ẽ",
      sr: 'Гварани',
      ro: 'Limba guarani',
      sk: 'Guaraníjčina',
      kv: 'Гуарани',
      br: 'Gwaranieg',
      ko: '과라니어',
      nn: 'Guaraní',
      kk: 'Гуарани тілі',
      fi: 'Guaranin kieli',
      be: 'Гуарані',
      oc: 'Guaraní',
      sl: 'Gvaranščina',
      sv: 'Guaraní',
      hi: 'गुआरानी भाषा',
      ar: 'غوارانية',
      ga: 'An Ghuaráinis',
      io: 'Guarani linguo',
      el: 'Γκουαρανί γλώσσα',
      he: 'גוארני',
      uz: 'Guarani tili',
      pt: 'Língua guarani',
      ka: 'გუარანის ენა',
      eo: 'Gvarania lingvo',
      la: 'Lingua Guaranica',
      cy: 'Guaraní',
      mk: 'Гварански јазик',
      fa: 'زبان گوارانی',
      da: 'Guaraní',
      pl: 'Język guarani',
      lt: 'Gvaranių kalba',
      hr: 'Guarani',
      gd: 'Guaranaidh',
      af: 'Guaraní',
      lv: 'Gvaranu valoda',
      cs: 'Guaraní',
      et: 'Guaranii keel',
      az: 'Quarani dili',
      bn: 'গুয়ারানি ভাষা',
      ug: 'گۇئارانچە',
      uk: 'Гуарані',
      is: 'Gvaraní',
      tr: 'Guaraní dili',
      nb: 'Guaraní',
      eu: 'Guaraniera',
      de: 'Guaraní',
      bg: 'Гуарански език',
      it: 'Guaraní',
      qu: 'Waraniyi simi',
      gl: 'Lingua guaraní',
      es: 'Guaraní',
      ay: 'Waraniyi aru',
      fr: 'Guarani',
      ca: 'Guaraní',
      sq: 'Gjuha guarani',
      en: 'Guarani',
      mr: 'ग्वारानी भाषा'
    },
    language_code_2: {
      en: 'gn'
    }
  },
  'en:nynorsk': {
    language_code_2: {
      en: 'nn'
    },
    language_code_3: {
      en: 'nno'
    },
    name: {
      en: 'Nynorsk',
      cy: 'Nynorsk',
      pt: 'Novo norueguês',
      ka: 'ნიუნორსკი',
      eo: 'Nynorsk',
      he: 'נורבגית חדשה',
      fr: 'Nynorsk',
      es: 'Nynorsk',
      gl: 'Nynorsk',
      ca: 'Nynorsk',
      sv: 'Nynorska',
      vo: 'Nula-Norgänapük',
      ar: 'ني نوشك',
      fy: 'Nynorsk',
      id: 'Nynorsk',
      it: 'Nynorsk',
      oc: 'Nynorsk',
      bg: 'Нюношк',
      be: 'Нованарвежская мова',
      fi: 'Uusnorja',
      eu: 'Nynorsk',
      nn: 'Nynorsk',
      nb: 'Nynorsk',
      de: 'Nynorsk',
      is: 'Nýnorska',
      uk: 'Нюношк',
      br: 'Nynorsk',
      ug: 'يېڭى نورۋېگىيە تىلى',
      os: 'Нюнорск',
      ko: '뉘노르스크',
      hy: 'Նորվեգերեն',
      li: 'Nynorsk',
      et: 'Uusnorra keel',
      az: 'Nyunorsk',
      sk: 'Nynorsk',
      ps: 'نينورشک',
      ja: 'ニーノシュク',
      zh: '新挪威語',
      ro: 'Nynorsk',
      cs: 'Nynorsk',
      se: 'Ođđadárogiella',
      af: 'Nynorsk',
      ta: 'நீநொர்ஸ்க் மொழி',
      ms: 'Nynorsk',
      vi: 'Nynorsk',
      lv: 'Jaunnorvēģu valoda',
      lt: 'Naujoji norvegų kalba',
      pl: 'Nynorsk',
      hr: 'Nynorsk',
      nl: 'Nynorsk',
      da: 'Nynorsk',
      ru: 'Нюнорск',
      ku: 'Nynorsk',
      fa: 'نروژی نو'
    },
    wikidata: {
      en: 'Q25164'
    }
  },
  'en:azerbaijani': {
    language_code_2: {
      en: 'az'
    },
    name: {
      en: 'Azerbaijani',
      mr: 'अझरबैजानी भाषा',
      sq: 'Gjuha azerbajxhane',
      gv: 'Asserbajaanish',
      th: 'ภาษาอาเซอร์ไบจาน',
      pa: 'ਅਜ਼ੇਰੀ ਭਾਸ਼ਾ',
      ca: 'Àzeri',
      fr: 'Azéri',
      es: 'Azerí',
      gl: 'Lingua azerí',
      fy: 'Azerbeidzjaansk',
      qu: 'Asar simi',
      it: 'Azero',
      su: 'Basa Azerbaijan',
      bg: 'Азербайджански език',
      hu: 'Azeri',
      ur: 'آذربائیجانی',
      am: 'አዘርኛ',
      de: 'Aserbaidschanische Sprache',
      eu: 'Azerbaijanera',
      nb: 'Aserbajdsjansk',
      is: 'Aserbaídsjanska',
      tr: 'Azerice',
      uk: 'Азербайджанська мова',
      ce: 'Азербайджанийн мотт',
      os: 'Азербайджайнаг æвзаг',
      ug: 'ئەزەربەيجان تىلى',
      bn: 'আজারবাইজানি ভাষা',
      az: 'Azərbaycan dili',
      ps: 'آزربايجاني',
      et: 'Aserbaidžaani keel',
      se: 'Aserbaižanagiella',
      cs: 'Ázerbájdžánština',
      ky: 'Азербайжан тили',
      av: 'Къажар мацI',
      lv: 'Azerbaidžāņu valoda',
      af: 'Azerbeidjans',
      lb: 'Aserbaidschanesch',
      hr: 'Azerski jezik',
      lt: 'Azerbaidžaniečių kalba',
      pl: 'Język azerski',
      da: 'Aserbajdsjansk',
      fa: 'زبان ترکی آذربایجانی',
      mk: 'Азербејџански јазик',
      tg: 'Забони озарбойҷонӣ',
      rm: 'Lingua aserbeidschanica',
      cy: 'Aserbaijaneg',
      la: 'Lingua Atropatenica',
      uz: 'Ozarbayjon tili',
      ka: 'აზერბაიჯანული ენა',
      pt: 'Língua azeri',
      eo: 'Azerbajĝana lingvo',
      cv: 'Азербайджан чĕлхи',
      bs: 'Azerbejdžanski jezik',
      he: 'אזרית',
      el: 'Αζερική γλώσσα',
      kl: 'Aserbajdsjaniskisut',
      io: 'Azerbaijana linguo',
      ga: 'An Asarbaiseáinis',
      sh: 'Azerski jezik',
      ar: 'لغة أذرية',
      vo: 'Lasärbäcänapük',
      sv: 'Azerbajdzjanska',
      hi: 'अज़ेरी भाषा',
      id: 'Bahasa Azeri',
      or: 'ଆଜାରବାଇଜାନୀ',
      sl: 'Azerbajdžanščina',
      tt: 'Азәрбайҗан теле',
      oc: 'Azeri',
      kk: 'Әзірбайжан тілі',
      fi: 'Azeri',
      be: 'Азербайджанская мова',
      nn: 'Aserbajdsjansk',
      ko: '아제르바이잔어',
      li: 'Azerbaidzjaans',
      hy: 'Ադրբեջաներեն',
      br: 'Azereg',
      kv: 'Азербайджан кыв',
      sk: 'Azerbajdžančina',
      rw: 'Ikinyazeribayijani',
      ml: 'അസർബൈജാനി ഭാഷ',
      ro: 'Limba azeră',
      sr: 'Азерски језик',
      kw: 'Azerek',
      ja: 'アゼルバイジャン語',
      mn: 'Азербайжан хэл',
      zh: '阿塞拜疆语',
      ta: 'அசர்பைஜான் மொழி',
      vi: 'Tiếng Azerbaijan',
      ms: 'Bahasa Azeri',
      tk: 'Azerbaýjan dili',
      nl: 'Azerbeidzjaans',
      ru: 'Азербайджанский язык',
      yo: 'Èdè Azerbaijani',
      ku: 'Zimanê azerbaycanî',
      sw: 'Kiazeri'
    },
    language_code_3: {
      en: 'aze'
    },
    wikidata: {
      en: 'Q9292'
    }
  },
  'en:czech': {
    language_code_2: {
      en: 'cs'
    },
    language_code_3: {
      en: 'ces'
    },
    name: {
      cy: 'Tsieceg',
      la: 'Lingua Bohemica',
      uz: 'Chex tili',
      ka: 'ჩეხური ენა',
      pt: 'Língua checa',
      eo: 'Ĉeĥa lingvo',
      cv: 'Чех чĕлхи',
      bs: 'Češki jezik',
      he: "צ'כית",
      el: 'Τσεχική γλώσσα',
      io: 'Chekiana linguo',
      ga: 'An tSeicis',
      ar: 'لغة تشيكية',
      sh: 'Češki jezik',
      vo: 'Tsyegänapük',
      hi: 'चॅक भाषा',
      sv: 'Tjeckiska',
      id: 'Bahasa Ceska',
      sl: 'Češčina',
      jv: 'Basa Céko',
      oc: 'Chèc',
      fi: 'Tšekki',
      kk: 'Чех тілі',
      be: 'Чэшская мова',
      nn: 'Tsjekkisk',
      cu: 'Чєшьскъ ѩꙁꙑкъ',
      co: 'Lingua ceca',
      ko: '체코어',
      hy: 'Չեխերեն',
      li: 'Tsjechisch',
      br: 'Tchekeg',
      kv: 'Чех кыв',
      sk: 'Čeština',
      rw: 'Igiceke',
      yi: 'טשעכיש',
      ro: 'Limba cehă',
      kw: 'Chekek',
      sr: 'Чешки језик',
      ja: 'チェコ語',
      wa: 'Tcheke',
      zh: '捷克语',
      mn: 'Чех хэл',
      ta: 'செக் மொழி',
      vi: 'Tiếng Séc',
      ms: 'Bahasa Czech',
      kn: 'ಚೆಕ್ ಭಾಷೆ',
      ia: 'Lingua chec',
      ne: 'चेक भाषा',
      nl: 'Tsjechisch',
      ru: 'Чешский язык',
      yo: 'Èdè Tsẹ́kì',
      ku: 'Zimanê çekî',
      sw: 'Kicheki',
      en: 'Czech',
      mr: 'चेक भाषा',
      tl: 'Wikang Tseko',
      sm: "Fa'aSieki",
      sc: 'Limba ceca',
      gv: 'Sheckish',
      sq: 'Gjuha çeke',
      th: 'ภาษาเช็ก',
      pa: 'ਚੇਕ ਭਾਸ਼ਾ',
      ca: 'Txec',
      fr: 'Tchèque',
      es: 'Checo',
      gl: 'Lingua checa',
      ab: 'Ачех бызшәа',
      fy: 'Tsjechysk',
      it: 'Ceco',
      qu: 'Chiku simi',
      bg: 'Чешки език',
      hu: 'Cseh',
      fo: 'Kekkiskt mál',
      am: 'ቼክኛ',
      ur: 'چیکی زبان',
      de: 'Tschechisch',
      eu: 'Txekiera',
      nb: 'Tsjekkisk',
      tr: 'Çekçe',
      is: 'Tékkneska',
      uk: 'Чеська мова',
      ce: 'Чехийн мотт',
      os: 'Чехаг æвзаг',
      ug: 'چېخ تىلى',
      zu: 'IsiTsheki',
      bn: 'চেক ভাষা',
      az: 'Çex dili',
      ps: 'چيکی ژبه',
      et: 'Tšehhi keel',
      cs: 'Čeština',
      se: 'Čeahkagiella',
      lv: 'Čehu valoda',
      gd: 'Seacais',
      af: 'Tsjeggies',
      lb: 'Tschechesch',
      hr: 'Češki jezik',
      lt: 'Čekų kalba',
      pl: 'Język czeski',
      da: 'Tjekkisk',
      an: 'Idioma checo',
      fa: 'زبان چکی',
      mk: 'Чешки јазик',
      rm: 'Lingua tscheca',
      tg: 'Забони чехӣ'
    },
    wikidata: {
      en: 'Q9056'
    }
  },
  'en:navajo': {
    language_code_2: {
      en: 'nv'
    },
    name: {
      gd: 'Navajo',
      ta: 'நாவஹோ மொழி',
      lv: 'Navahu valoda',
      lt: 'Navahų kalba',
      pl: 'Język nawaho',
      nl: 'Navajo',
      mk: 'Навахо',
      ru: 'Навахо',
      ku: 'Zimanê navaho',
      fa: 'زبان ناواهو',
      tr: 'Navahoca',
      is: 'Navajóíska',
      uk: 'Навахо',
      br: "Navac'hoeg",
      ko: '나바호어',
      li: 'Navajo',
      et: 'Navaho keel',
      kv: 'Навахо',
      nv: 'Diné bizaad',
      ja: 'ナバホ語',
      zh: '納瓦荷語',
      ro: 'Limba navajo',
      cs: 'Navažština',
      sr: 'Навахо језик',
      sv: 'Navajo',
      jv: 'Basa Navajo',
      id: 'Bahasa Navajo',
      it: 'Lingua navajo',
      qu: 'Diné simi',
      oc: 'Navajo',
      fo: 'Navaho',
      fi: 'Navajo',
      eu: 'Navajoera',
      nb: 'Navajo',
      nn: 'Navaho',
      de: 'Navajo',
      en: 'Navajo',
      cy: 'Nafacho',
      pt: 'Língua navaja',
      eo: 'Navaha lingvo',
      ka: 'ნავახო',
      gv: 'Navajo',
      he: 'נאוואחו',
      fr: 'Navajo',
      es: 'Navajo',
      ga: 'Navachóis',
      gl: 'Lingua navajo',
      ca: 'Navajo'
    },
    language_code_3: {
      en: 'nav'
    },
    wikidata: {
      en: 'Q13310'
    }
  },
  'en:finnish': {
    language_code_2: {
      en: 'fi'
    },
    wikidata: {
      en: 'Q1412'
    },
    name: {
      bs: 'Finski jezik',
      la: 'Lingua Finnica',
      cy: 'Ffinneg',
      pt: 'Língua finlandesa',
      uz: 'Fin tili',
      ka: 'ფინური ენა',
      eo: 'Finna lingvo',
      cv: 'Финн чĕлхи',
      kl: 'Finlandimiutut',
      ee: 'Finngbe',
      io: 'Finlandana linguo',
      ga: 'An Fhionlainnis',
      he: 'פינית',
      el: 'Φινλανδική γλώσσα',
      id: 'Bahasa Suomi',
      ba: 'Фин теле',
      sl: 'Finščina',
      tt: 'Фин теле',
      jv: 'Basa Suomi',
      sh: 'Finski jezik',
      ar: 'لغة فنلندية',
      vo: 'Suomiyänapük',
      hi: 'फ़िनिश भाषा',
      sv: 'Finska',
      my: 'ဖင်လန်ဘာသာစကား',
      nn: 'Finsk',
      oc: 'Finés',
      be: 'Фінская мова',
      fi: 'Suomi',
      ko: '핀란드어',
      hy: 'Ֆիններեն',
      li: 'Fins',
      br: 'Finneg',
      yi: 'פיניש',
      ro: 'Limba finlandeză',
      kw: 'Finnek',
      sr: 'Фински језик',
      ja: 'フィンランド語',
      zh: '芬兰语',
      kv: 'Суоми кыв',
      no: 'Finsk',
      sk: 'Fínčina',
      rw: 'Igifinilande',
      ml: 'ഫിന്നിഷ് ഭാഷ',
      ta: 'பின்னிய மொழி',
      ms: 'Bahasa Finland',
      vi: 'Tiếng Phần Lan',
      ia: 'Lingua finnese',
      ru: 'Финский язык',
      yo: 'Èdè Fínlándì',
      ku: 'Zimanê fînî',
      mi: 'Reo Hinerangi',
      sw: 'Kifini',
      nl: 'Fins',
      tl: 'Wikang Pinlandes',
      sm: "Fa'afinelani",
      en: 'Finnish',
      mr: 'फिनिश भाषा',
      pa: 'ਫ਼ਿਨੀ ਭਾਸ਼ਾ',
      ca: 'Finès',
      fr: 'Finnois',
      es: 'Finés',
      gl: 'Lingua finesa',
      sc: 'Limba finlandesa',
      gv: 'Finnlynnish',
      sq: 'Gjuha finlandeze',
      th: 'ภาษาฟินแลนด์',
      it: 'Finlandese',
      qu: 'Phinis simi',
      ab: 'Суоми бызшәа',
      fy: 'Finsk',
      de: 'Finnisch',
      am: 'ፊንኛ',
      eu: 'Finlandiera',
      nb: 'Finsk',
      dv: 'ސުއޯމީ',
      hu: 'Finn',
      bg: 'Фински език',
      fo: 'Finskt mál',
      ug: 'فىنلاندىيە تىلى',
      os: 'Финнаг æвзаг',
      bn: 'ফিনীয় ভাষা',
      tr: 'Fince',
      is: 'Finnska',
      uk: 'Фінська мова',
      ce: 'Finhoyn mott',
      se: 'Suomagiella',
      ky: 'Саам тили',
      cs: 'Finština',
      az: 'Fin dili',
      et: 'Soome keel',
      hr: 'Finski jezik',
      lt: 'Suomių kalba',
      pl: 'Język fiński',
      lv: 'Somu valoda',
      af: 'Fins',
      gd: 'Fionnlannais',
      an: 'Idioma finés',
      fa: 'زبان فنلاندی',
      mk: 'Фински јазик',
      rm: 'Lingua finlandaisa',
      tg: 'Забони финӣ',
      da: 'Finsk'
    },
    language_code_3: {
      en: 'fin'
    }
  },
  'en:luxembourgish': {
    wikidata: {
      en: 'Q9051'
    },
    language_code_3: {
      en: 'ltz'
    },
    name: {
      sl: 'Luksemburščina',
      id: 'Bahasa Luksemburg',
      sv: 'Luxemburgiska',
      vo: 'Luxämburgänapük',
      sh: 'Luksemburški jezik',
      ar: 'لغة لوكسمبورغية',
      nn: 'Luxemburgsk',
      be: 'Люксембургская мова',
      fi: 'Luxemburgin kieli',
      oc: 'Luxemborgués',
      ka: 'ლუქსემბურგული ენა',
      pt: 'Língua luxemburguesa',
      eo: 'Luksemburga lingvo',
      la: 'Lingua Luxemburgica',
      cy: 'Lwcsembwrgeg',
      io: 'Luxemburgiana linguo',
      ga: 'An Lucsambuirgis',
      el: 'Λουξεμβουργιανή γλώσσα',
      he: 'לוקסמבורגית',
      ms: 'Bahasa Luxembourg',
      vi: 'Tiếng Luxembourg',
      ta: 'இலுகுசெம்பூர்கிய மொழி',
      ru: 'Люксембургский язык',
      nl: 'Luxemburgs',
      br: 'Luksembourgeg',
      li: 'Luxemburgs',
      hy: 'Լյուքսեմբուրգերեն',
      ko: '룩셈부르크어',
      co: 'Lingua lussemburghese',
      zh: '卢森堡语',
      wa: 'Lussimbordjwès',
      ja: 'ルクセンブルク語',
      sr: 'Луксембуршки језик',
      kw: 'Lushaborgek',
      yi: 'לוקסעמבורגיש',
      ro: 'Limba luxemburgheză',
      sk: 'Luxemburčina',
      kv: 'Люксембург кыв',
      it: 'Lussemburghese',
      fy: 'Lúksemboarchsk',
      nb: 'Luxembourgsk',
      eu: 'Luxenburgera',
      de: 'Luxemburgisch',
      ur: 'لکسمبرگی زبان',
      hu: 'Luxemburgi',
      bg: 'Люксембургски език',
      mr: 'लक्झेंबर्गिश भाषा',
      en: 'Luxembourgish',
      gl: 'Lingua luxemburguesa',
      es: 'Luxemburgués',
      fr: 'Luxembourgeois',
      ca: 'Luxemburguès',
      pa: 'ਲਕਸਮਬਰਗੀ ਭਾਸ਼ਾ',
      sq: 'Gjuha luksemburgeze',
      pl: 'Język luksemburski',
      lt: 'Liuksemburgiečių kalba',
      lb: 'Lëtzebuergesch',
      hr: 'Luksemburški jezik',
      gd: 'Lucsamburgais',
      af: 'Luxemburgs',
      lv: 'Luksemburgiešu valoda',
      mk: 'Луксембуршки јазик',
      fa: 'زبان لوکزامبورگی',
      an: 'Idioma luxemburgués',
      da: 'Luxembourgsk',
      ug: 'ليۇكسېمبۇرگ تىلى',
      ce: 'Люксембургхойн мотт',
      uk: 'Люксембурзька мова',
      tr: 'Lüksemburgca',
      is: 'Lúxemborgíska',
      na: 'Dorerin Ruketemburg',
      cs: 'Lucemburština',
      se: 'Luxemburggagiella',
      et: 'Letseburgi keel',
      az: 'Lüksemburq dili'
    },
    language_code_2: {
      en: 'lb'
    }
  },
  'en:swedish': {
    language_code_2: {
      en: 'sv'
    },
    name: {
      nl: 'Zweeds',
      ru: 'Шведский язык',
      ku: 'Zimanê swêdî',
      yo: 'Èdè Sweden',
      sw: 'Kiswidi',
      mi: 'Reo Huitene',
      ta: 'சுவீடிய மொழி',
      ms: 'Bahasa Sweden',
      vi: 'Tiếng Thụy Điển',
      kn: 'ಸ್ವೀಡಿಶ್',
      ia: 'Lingua svedese',
      no: 'Svensk',
      kv: 'Швед кыв',
      sk: 'Švédčina',
      ml: 'സ്വീഡിഷ് ഭാഷ',
      ro: 'Limba suedeză',
      yi: 'שוועדיש',
      sr: 'Шведски језик',
      kw: 'Swedek',
      ja: 'スウェーデン語',
      mn: 'Швед хэл',
      zh: '瑞典語',
      wa: 'Suwedwès',
      cu: 'Свєньскъ ѩꙁꙑкъ',
      mg: 'Fiteny soedoa',
      co: 'Lingua svedese',
      ko: '스웨덴어',
      li: 'Zweeds',
      hy: 'Շվեդերեն',
      br: 'Svedeg',
      oc: 'Suedés',
      kk: 'Швед тілі',
      be: 'Шведская мова',
      fi: 'Ruotsi',
      nn: 'Svensk',
      ar: 'لغة سويدية',
      sh: 'Švedski jezik',
      hi: 'स्वीडिश भाषा',
      vo: 'Svedänapük',
      sv: 'Svenska',
      id: 'Bahasa Swedia',
      ba: 'Швед теле',
      sl: 'Švedščina',
      tt: 'Швед теле',
      jv: 'Basa Swédia',
      he: 'שוודית',
      el: 'Σουηδική γλώσσα',
      kl: 'Svenskisuut',
      ee: 'Swedgbe',
      ga: 'An tSualainnis',
      io: 'Suediana linguo',
      cy: 'Swedeg',
      la: 'Lingua Suecica',
      uz: 'Shved tili',
      pt: 'Língua sueca',
      eo: 'Sveda lingvo',
      ka: 'შვედური ენა',
      cv: 'Швед чĕлхи',
      bs: 'Švedski jezik',
      da: 'Svensk',
      iu: 'ᔅᕗᓐᔅᑭᑐᑦ',
      an: 'Idioma sueco',
      fa: 'زبان سوئدی',
      mk: 'Шведски јазик',
      tg: 'Забони шведӣ',
      rm: 'Lingua svedaisa',
      lv: 'Zviedru valoda',
      af: 'Sweeds',
      gd: 'Suainis',
      hr: 'Švedski jezik',
      lt: 'Švedų kalba',
      pl: 'Język szwedzki',
      az: 'İsveç dili',
      et: 'Rootsi keel',
      cs: 'Švédština',
      se: 'Ruoŧagiella',
      ky: 'Швед тили',
      is: 'Sænska',
      tr: 'İsveççe',
      uk: 'Шведська мова',
      ce: 'Şvedhoyn mott',
      ug: 'شۋېتسىيە تىلى',
      os: 'Шведаг æвзаг',
      zu: 'IsiSwidishi',
      bn: 'সুয়েডীয় ভাষা',
      dv: 'ސުވެޑިޝް',
      bg: 'Шведски език',
      hu: 'Svéd',
      fo: 'Svenskt mál',
      ur: 'سونسکا',
      am: 'ስዊድንኛ',
      de: 'Schwedisch',
      eu: 'Suediera',
      nb: 'Svensk',
      fy: 'Sweedsk',
      so: 'Af-Iswiidhan',
      qu: 'Suwiri simi',
      it: 'Svedese',
      su: 'Basa Swédia',
      sc: 'Limba isvedesa',
      gv: 'Soolynnish',
      sq: 'Gjuha suedeze',
      th: 'ภาษาสวีเดน',
      pa: 'ਸਵੀਡਿਸ਼ ਭਾਸ਼ਾ',
      ca: 'Suec',
      fr: 'Suédois',
      es: 'Sueco',
      gl: 'Lingua sueca',
      mr: 'स्वीडिश भाषा',
      en: 'Swedish',
      sm: "Fa'asuetena",
      tl: 'Wikang Suweko'
    },
    language_code_3: {
      en: 'swe'
    },
    wikidata: {
      en: 'Q9027'
    }
  },
  'en:yiddish': {
    wikidata: {
      en: 'Q8641'
    },
    language_code_3: {
      en: 'yid'
    },
    name: {
      ie: 'Yiddic',
      it: 'Yiddish',
      fy: 'Jiddysk',
      nb: 'Jiddisch',
      eu: 'Yiddish',
      ur: 'یدیش زبان',
      de: 'Jiddisch',
      am: 'ዪዲሽኛ',
      fo: 'Jiddiskt mál',
      hu: 'Jiddis',
      bg: 'Идиш',
      tl: 'Wikang Yidis',
      mr: 'यिडिश भाषा',
      en: 'Yiddish',
      gl: 'Lingua yiddish',
      es: 'Yidis',
      fr: 'Yiddish',
      ca: 'Jiddisch',
      th: 'ภาษายิดดิช',
      gv: 'Ewdish',
      pl: 'Jidysz',
      lt: 'Jidiš',
      hr: 'Jidiš',
      af: 'Jiddisj',
      gd: 'Iùdais',
      lv: 'Jidišs',
      mk: 'Јидиш',
      fa: 'زبان ییدیش',
      an: 'Yídix',
      da: 'Jiddisch',
      uk: 'Їдиш',
      tr: 'Yidiş',
      is: 'Jiddíska',
      cs: 'Jidiš',
      et: 'Jidiši keel',
      az: 'İdiş dili',
      jv: 'Basa Yiddish',
      tt: 'Идиш теле',
      sl: 'Jidiš',
      ba: 'Идиш',
      id: 'Bahasa Yiddi',
      vo: 'Yuda-Deutänapük',
      hi: 'यिद्दी भाषा',
      sv: 'Jiddisch',
      sh: 'Jidiš',
      ar: 'لغة يديشية',
      nn: 'Jiddisch',
      kk: 'Идиш',
      be: 'Ідыш',
      fi: 'Jiddiš',
      oc: 'Yiddish',
      cv: 'Идиш',
      uz: 'Idish',
      eo: 'Jida lingvo',
      pt: 'Língua iídiche',
      ka: 'იდიში',
      la: 'Lingua Iudaeogermanica',
      cy: 'Iddew-Almaeneg',
      io: 'Yiddisha linguo',
      ga: 'An Ghiúdais',
      el: 'Γίντις',
      he: 'יידיש',
      ms: 'Bahasa Yiddish',
      vi: 'Tiếng Yiddish',
      ta: 'இத்திய மொழி',
      ku: 'Zimanê yîdîş',
      ru: 'Идиш',
      nl: 'Jiddisch',
      br: 'Yideg',
      hy: 'Իդիշ',
      li: 'Jiddisch',
      ko: '이디시어',
      zh: '意第緒語',
      ja: 'イディッシュ語',
      kw: 'Yedhowek',
      sr: 'Јидиш',
      ro: 'Limba idiș',
      yi: 'יידיש',
      ml: 'യിദ്ദിഷ്',
      sk: 'Jidiš',
      kv: 'Идиш'
    },
    language_code_2: {
      en: 'yi'
    }
  },
  'en:inuktitut': {
    language_code_2: {
      en: 'iu'
    },
    wikidata: {
      en: 'Q29921'
    },
    language_code_3: {
      en: 'iku'
    },
    name: {
      mg: 'Fiteny inuktitut',
      na: 'Inuktitut',
      tr: 'Doğu Kanada İnuitçesi',
      is: 'Inúktitút',
      uk: 'Інуктитут',
      br: 'Inuktitut',
      ko: '이누크티투트어',
      li: 'Inuktitut',
      et: 'Inuktituti keel',
      kv: 'Инуктитут',
      ja: 'イヌクティトゥット語',
      gn: "Inutytu Ñe'ẽ",
      zh: '因纽特语',
      cs: 'Inuktitutština',
      se: 'Inuktitut',
      gd: 'Inuktitut',
      ta: 'இனுக்ரிருற் மொழி',
      lt: 'Inuktitutas',
      pl: 'Inuktitut',
      nl: 'Inuktitut',
      da: 'Inuktitut',
      mk: 'Инуктитут',
      ru: 'Инуктитут',
      iu: 'ᐃᓄᒃᑎᑐᑦ',
      an: 'Idioma inuktitut',
      fa: 'اینوکتیتوت',
      en: 'Inuktitut',
      mr: 'इनुक्टिटुट भाषा',
      cy: 'Inuktitut',
      ka: 'ინუკტიტუტი',
      pt: 'Língua inuktitut',
      gv: 'Inuktitut',
      he: 'אינוקטיטוט',
      fr: 'Inuktitut',
      ga: 'Inuktitut',
      es: 'Lenguas inuit',
      gl: 'Lingua inuktitut',
      ca: 'Inuktitut',
      sv: 'Inuktitut',
      ar: 'لغة إنكتيتوتية',
      id: 'Bahasa Inuktitut',
      qu: 'Inuyt simi',
      it: 'Inuktitut',
      hu: 'Inuktitut nyelv',
      oc: 'Inuktitut',
      bg: 'Инуктитут',
      fi: 'Inuktitut',
      eu: 'Inuktitutera',
      nb: 'Inuktitut',
      nn: 'Inuktitut',
      de: 'Inuktitut'
    }
  },
  'en:lao': {
    language_code_2: {
      en: 'lo'
    },
    wikidata: {
      en: 'Q9211'
    },
    name: {
      mr: 'लाओ भाषा',
      en: 'Lao',
      cy: 'Lao',
      pt: 'Língua laociana',
      uz: 'Laos tili',
      eo: 'Laosa lingvo',
      th: 'ภาษาลาว',
      el: 'Λαοτινή γλώσσα',
      he: 'לאו',
      fr: 'Lao',
      es: 'Lao',
      gl: 'Lingua laosiana',
      ca: 'Laosià',
      hi: 'लाओ भाषा',
      vo: 'Laosänapük',
      sv: 'Lao',
      ar: 'لاو',
      sh: 'Laoski jezik',
      id: 'Bahasa Laos',
      qu: 'Law simi',
      it: 'Lao',
      bg: 'Лаоски език',
      hu: 'Lao',
      be: 'Лаоская мова',
      fi: 'Laon kieli',
      kk: 'Лаос тілі',
      eu: 'Laosera',
      nn: 'Laotisk',
      nb: 'Laotisk',
      de: 'Laotische Sprache',
      tr: 'Laoca',
      uk: 'Лаоська мова',
      bn: 'লাও ভাষা',
      ko: '라오어',
      ug: 'لائوس تىلى',
      lo: 'ພາສາລາວ',
      et: 'Lao keel',
      az: 'Laos dili',
      kv: 'Лаос кыв',
      sk: 'Laoština',
      ja: 'ラーオ語',
      zh: '老挝语',
      mn: 'Лаос хэл',
      ro: 'Limba laoțiană',
      sr: 'Лаоски језик',
      cs: 'Laoština',
      af: 'Laosiaans',
      ta: 'இலவோத்திய மொழி',
      vi: 'Tiếng Lào',
      ms: 'Bahasa Lao',
      lv: 'Laosiešu valoda',
      lt: 'Lao',
      pl: 'Język laotański',
      ne: 'लाओ',
      hr: 'Laoski jezik',
      nl: 'Laotiaans',
      mk: 'Лаошки јазик',
      ru: 'Лаосский язык',
      fa: 'زبان لائو'
    },
    language_code_3: {
      en: 'lao'
    }
  },
  'en:chuvash': {
    name: {
      vi: 'Tiếng Chuvash',
      ms: 'Bahasa Chuvash',
      ta: 'சுவாசு மொழி',
      tk: 'Çuwaş dili',
      nl: 'Tsjoevasjisch',
      ru: 'Чувашский язык',
      cu: 'Чьвашьскъ ѩꙁꙑкъ',
      hy: 'Չուվաշերեն',
      ko: '추바시어',
      br: 'Tchouvacheg',
      rw: 'Igicuvashi',
      sk: 'Čuvaština',
      kv: 'Чуваш кыв',
      sr: 'Чувашки језик',
      ro: 'Limba ciuvașă',
      zh: '楚瓦什語',
      ja: 'チュヴァシ語',
      ar: 'لغة تشوفاش',
      hi: 'चुवाश भाषा',
      sv: 'Tjuvasjiska',
      ba: 'Сыуаш теле',
      id: 'Bahasa Chuvash',
      tt: 'Çuaş tele',
      be: 'Чувашская мова',
      fi: 'Tšuvassin kieli',
      kk: 'Шуаш тілі',
      nn: 'Tsjuvasjisk',
      eo: 'Ĉuvaŝa lingvo',
      uz: 'Chuvash tili',
      pt: 'Língua tchuvache',
      ka: 'ჩუვაშური ენა',
      la: 'Chuvassica',
      cv: 'Чăваш чĕлхи',
      he: "צ'ובשית",
      ga: 'An tSuvaisis',
      lv: 'Čuvašu valoda',
      hr: 'Čuvaški jezik',
      pl: 'Język czuwaski',
      lt: 'Čiuvašų kalba',
      fa: 'زبان چوواشی',
      mk: 'Чувашки јазик',
      uk: 'Чуваська мова',
      tr: 'Çuvaşça',
      ce: 'Чувашийн мотт',
      ug: 'چۇۋاش تىلى',
      az: 'Çuvaş dili',
      et: 'Tšuvaši keel',
      ky: 'Чуваш тили',
      cs: 'Čuvaština',
      it: 'Ciuvascio',
      qu: 'Chuwash simi',
      hu: 'Csuvas nyelv',
      bg: 'Чувашки език',
      de: 'Tschuwaschische Sprache',
      ur: 'چوواش زبان',
      nb: 'Tsjuvasjisk',
      eu: 'Txuvaxera',
      en: 'Chuvash',
      mr: 'चुवाश भाषा',
      th: 'ภาษาชูวัช',
      ca: 'Txuvaix',
      es: 'Idioma chuvasio',
      gl: 'Lingua chuvash',
      fr: 'Tchouvache'
    },
    language_code_3: {
      en: 'chv'
    },
    wikidata: {
      en: 'Q33348'
    },
    language_code_2: {
      en: 'cv'
    }
  },
  'en:maltese': {
    language_code_2: {
      en: 'mt'
    },
    wikidata: {
      en: 'Q9166'
    },
    name: {
      tl: 'Wikang Maltes',
      en: 'Maltese',
      mr: 'माल्टी भाषा',
      fr: 'Maltais',
      gl: 'Lingua maltesa',
      es: 'Maltés',
      pa: 'ਮਾਲਟਾਈ ਭਾਸ਼ਾ',
      ca: 'Maltès',
      sq: 'Gjuha malteze',
      gv: 'Maltish',
      th: 'ภาษามอลตา',
      sc: 'Limba maltesa',
      qu: 'Malta simi',
      it: 'Maltese',
      fy: 'Malteesk',
      eu: 'Maltera',
      nb: 'Maltesisk',
      ur: 'مالٹی زبان',
      am: 'ማልትኛ',
      de: 'Maltesisch',
      hu: 'Máltai',
      bg: 'Малтийски език',
      fo: 'Maltiskt mál',
      ug: 'مالتا تىلى',
      is: 'Maltneska',
      tr: 'Maltaca',
      uk: 'Мальтійська мова',
      se: 'Maltagiella',
      cs: 'Maltština',
      et: 'Malta keel',
      az: 'Malta dili',
      lt: 'Maltiečių kalba',
      pl: 'Język maltański',
      hr: 'Malteški jezik',
      gd: 'Maltais',
      af: 'Maltees',
      lv: 'Maltiešu valoda',
      mk: 'Малтешки јазик',
      an: 'Idioma maltés',
      fa: 'زبان مالتی',
      da: 'Maltesisk',
      bs: 'Malteški jezik',
      cy: 'Malteg',
      la: 'Lingua Melitensis',
      ka: 'მალტური ენა',
      pt: 'Maltês',
      eo: 'Malta lingvo',
      io: 'Maltana linguo',
      ga: 'An Mháltais',
      el: 'Μαλτεζική γλώσσα',
      he: 'מלטזית',
      jv: 'Basa Malta',
      id: 'Bahasa Malta',
      sl: 'Malteščina',
      sv: 'Maltesiska',
      vo: 'Malteänapük',
      sh: 'Malteški jezik',
      ar: 'لغة مالطية',
      nn: 'Maltesisk',
      mt: 'Malti',
      oc: 'Maltés',
      fi: 'Malta',
      kk: 'Мальта тілі',
      be: 'Мальтыйская мова',
      br: 'Malteg',
      ko: '몰타어',
      li: 'Maltees',
      hy: 'Մալթայերեն',
      co: 'Lingua maltese',
      ja: 'マルタ語',
      wa: 'Maltès',
      zh: '马耳他语',
      yi: 'מאלטעזיש',
      ro: 'Limba malteză',
      sr: 'Малтешки језик',
      kv: 'Мальта кыв',
      sk: 'Maltčina',
      ia: 'Lingua maltese',
      ta: 'மால்திய மொழி',
      ms: 'Bahasa Malta',
      vi: 'Tiếng Malta',
      sw: 'Kimalta',
      ru: 'Мальтийский язык',
      nl: 'Maltees'
    },
    language_code_3: {
      en: 'mlt'
    }
  },
  'en:maldivian-language': {
    language_code_3: {
      en: 'div'
    },
    name: {
      kv: 'Дивехи',
      az: 'Maldiv dili',
      sk: 'Maldivčina',
      et: 'Maldiivi keel',
      ml: 'ദിവെഹി ഭാഷ',
      cs: 'Divehi',
      ja: 'ディベヒ語',
      zh: '迪维希语',
      tr: 'Maldivce',
      uk: 'Мальдівська мова',
      ko: '디베히어',
      hy: 'Մալդիվերեն',
      br: 'Diveheg',
      nl: 'Divehi',
      da: 'Dhivehi',
      ru: 'Мальдивский язык',
      an: 'Dhivehi',
      fa: 'زبان دیوهی',
      mk: 'Малдивски јазик',
      sw: 'Kidhivehi',
      si: 'දිවෙහි භාෂාව',
      ta: 'திவெயி மொழி',
      lv: 'Maldīviešu valoda',
      vi: 'Tiếng Dhivehi',
      ms: 'Bahasa Divehi',
      af: 'Dhivehi',
      lt: 'Maldyvų kalba',
      pl: 'Język malediwski',
      el: 'Ντιβέχι γλώσσα',
      th: 'ภาษาดิเวฮิ',
      pa: 'ਦਿਵੇਹੀ ਭਾਸ਼ਾ',
      ca: 'Divehi',
      fr: 'Maldivien',
      es: 'Dhivehi',
      la: 'Lingua Dhivehi',
      cy: 'Divehi',
      pt: 'Língua divehi',
      eo: 'Mahla lingvo',
      mr: 'दिवॅही',
      en: 'Maldivian language',
      dv: 'ދިވެހި',
      bg: 'Дивехи',
      oc: 'Divehi',
      hu: 'Maldív',
      fi: 'Divehin kieli',
      be: 'Дывехі',
      de: 'Dhivehi',
      ur: 'دیویہی',
      eu: 'Maldivera',
      nn: 'Dhivehi',
      nb: 'Dhivehi',
      ar: 'ديفهي',
      sv: 'Divehi',
      hi: 'महल्',
      id: 'Bahasa Divehi',
      it: 'Maldiviano'
    },
    wikidata: {
      en: 'Q32656'
    },
    language_code_2: {
      en: 'dv'
    }
  },
  'en:interlingue': {
    language_code_2: {
      en: 'ie'
    },
    language_code_3: {
      en: 'ile'
    },
    name: {
      pl: 'Occidental',
      ta: 'கிழக்கிய மொழி',
      af: 'Interlingue',
      ia: 'Interlingue',
      ru: 'Окциденталь',
      fa: 'زبان غربی',
      mk: 'Интерлингве',
      nl: 'Interlingue',
      ko: '옥시덴탈어',
      uk: 'Окциденталь',
      cs: 'Occidental',
      ja: 'インターリング',
      kv: 'Оксиденталь',
      et: 'Interlingue',
      it: 'Occidental',
      ie: 'Interlingue',
      sv: 'Occidental',
      vo: 'Interlingue',
      de: 'Interlingue',
      am: 'ኢንተርሊንግዌ',
      nb: 'Interlingue',
      hu: 'Occidental nyelv',
      oc: 'Interlingue',
      fi: 'Interlingue',
      be: 'Акцыдэнталь',
      la: 'Interlingue',
      pt: 'Interlingue',
      eo: 'Okcidentalo',
      cv: 'Окциденталь',
      en: 'Interlingue',
      ca: 'Interlingue',
      fr: 'Occidental',
      ga: 'An Interlingua',
      es: 'Occidental',
      gl: 'Lingua occidental',
      io: 'Occidental',
      el: 'Οξιντένταλ'
    },
    wikidata: {
      en: 'Q35850'
    }
  },
  'en:ossetian': {
    name: {
      fi: 'Osseetin kieli',
      be: 'Асецінская мова',
      nn: 'Ossetisk',
      sh: 'Osetinski jezik',
      ar: 'لغة أوسيتية',
      sv: 'Ossetiska',
      hi: 'ओसेती भाषा',
      sl: 'Osetinščina',
      ba: 'Осетин теле',
      id: 'Bahasa Ossetia',
      tt: 'Осетин теле',
      ga: 'Oiséitis',
      io: 'Osetiana linguo',
      uz: 'Osetin tili',
      ka: 'ოსური ენა',
      pt: 'Língua osseta',
      eo: 'Oseta lingvo',
      la: 'Lingua Ossetica',
      cv: 'Осетин чĕлхи',
      nl: 'Ossetisch',
      ku: 'Zimanê osetî',
      ru: 'Осетинский язык',
      vi: 'Tiếng Ossetia',
      ta: 'ஒசேத்திய மொழி',
      sk: 'Osetčina',
      rw: 'Icyosetiya',
      kv: 'Осет кыв',
      ml: 'ഒസ്സെഷ്യൻ ഭാഷ',
      sr: 'Осетински језик',
      ro: 'Limba osetă',
      zh: '奧塞梯語',
      ja: 'オセット語',
      cu: 'Осєтьскъ ѩꙁꙑкъ',
      hy: 'Օսերեն',
      ko: '오세트어',
      br: 'Oseteg',
      fo: 'Ossetiskt mál',
      bg: 'Осетински език',
      hu: 'Oszét nyelv',
      de: 'Ossetische Sprache',
      am: 'ኢሮንኛ',
      nb: 'Ossetisk',
      eu: 'Osetiera',
      qu: 'Usit simi',
      it: 'Osseto',
      th: 'ภาษาออสซีเชีย',
      gv: 'Ossetish',
      ca: 'Osseta',
      es: 'Osetio',
      gl: 'Lingua ossetia',
      fr: 'Ossète',
      en: 'Ossetian',
      da: 'Ossetisk',
      fa: 'زبان آسی',
      an: 'Idioma oseta',
      mk: 'Осетиски јазик',
      lv: 'Osetīnu valoda',
      af: 'Osseties',
      hr: 'Osetski jezik',
      pl: 'Język osetyjski',
      lt: 'Osetinų kalba',
      az: 'Osetin dili',
      ky: 'Осетин тили',
      cs: 'Osetština',
      uk: 'Осетинська мова',
      tr: 'Osetçe',
      ce: 'ХӀирийн мотт',
      os: 'Ирон æвзаг',
      bn: 'অসেটীয় ভাষা'
    },
    language_code_3: {
      en: 'oss'
    },
    wikidata: {
      en: 'Q33968'
    },
    language_code_2: {
      en: 'os'
    }
  },
  'en:bashkir': {
    language_code_3: {
      en: 'bak'
    },
    name: {
      ca: 'Baixkir',
      pa: 'ਬਸ਼ਕੀਰ ਭਾਸ਼ਾ',
      es: 'Bashkir',
      gl: 'Lingua baskir',
      ga: 'An Bhaiscíris',
      fr: 'Bachkir',
      he: 'בשקירית',
      th: 'ภาษาแบชเคียร์',
      pt: 'Língua bashkir',
      uz: 'Boshqird tili',
      eo: 'Baŝkira lingvo',
      en: 'Bashkir',
      mr: 'बाश्किर भाषा',
      cv: 'Пушкăрт чĕлхи',
      de: 'Baschkirische Sprache',
      nb: 'Basjkirsk',
      nn: 'Basjkirsk',
      eu: 'Baxkirera',
      be: 'Башкірская мова',
      fi: 'Baškiirin kieli',
      kk: 'Башқұрт тілі',
      hu: 'Baskír nyelv',
      bg: 'Башкирски език',
      sl: 'Baškirščina',
      it: 'Baschiro',
      ba: 'Башҡорт теле',
      id: 'Bahasa Bashkir',
      tt: 'Başqort tele',
      ar: 'لغة باشقير',
      sh: 'Baškirski jezik',
      hi: 'बाश्किर भाषा',
      sv: 'Basjkiriska',
      sr: 'Башкирски језик',
      cs: 'Baškirština',
      ky: 'Башкырт тили',
      ro: 'Limba bașchiră',
      zh: '巴什基尔语',
      ja: 'バシキール語',
      sk: 'Baškirčina',
      az: 'Başqırd dili',
      kv: 'Башкир кыв',
      et: 'Baškiiri keel',
      os: 'Башкираг æвзаг',
      ug: 'باشقۇرتچە',
      ko: '바시키르어',
      br: 'Bachkireg',
      cu: 'Башкортьскъ ѩꙁꙑкъ',
      uk: 'Башкирська мова',
      tr: 'Başkurtça',
      ce: 'Башкирийн мотт',
      fa: 'زبان باشقیری',
      ru: 'Башкирский язык',
      mk: 'Башкирски јазик',
      da: 'Basjkirsk',
      nl: 'Basjkiers',
      hr: 'Baškirski jezik',
      pl: 'Język baszkirski',
      lt: 'Baškirų kalba',
      vi: 'Tiếng Bashkir',
      lv: 'Baškīru valoda',
      ms: 'Bahasa Bashkir',
      ta: 'பசுகிர மொழி',
      af: 'Basjkiries'
    },
    wikidata: {
      en: 'Q13389'
    },
    language_code_2: {
      en: 'ba'
    }
  },
  'en:ojibwe': {
    name: {
      br: 'Ojibweg',
      en: 'Ojibwe',
      pt: 'Língua ojíbua',
      uk: 'Оджибве',
      eo: 'Aniŝinabeka lingvo',
      la: 'Lingua Ojibwayensis',
      cy: 'Ojibwe',
      zh: '奥杰布瓦语',
      gl: 'Lingua ojibwa',
      ga: 'Óisibis',
      es: 'Idioma ojibwa',
      ja: 'オジブウェー語',
      fr: 'Ojibwé',
      pa: 'ਓਜੀਬਵੇ ਭਾਸ਼ਾ',
      th: 'ภาษาโอจิบเว',
      kv: 'Анишинаб кыв',
      pl: 'Język odżibwe',
      lt: 'Anišinabų kalba',
      it: 'Lingua ojibwe',
      qu: 'Anishinapay simi',
      id: 'Bahasa Ojibwe',
      sv: 'Ojibwa',
      nn: 'Ojibwe-språket',
      nb: 'Ojibwa',
      de: 'Ojibwe',
      ru: 'Оджибве',
      fi: 'Odžibwan kieli',
      cr: 'ᐊᓂᔑᓈᐯᒧᐎᓐ',
      nl: 'Ojibwe'
    },
    language_code_3: {
      en: 'oji'
    },
    wikidata: {
      en: 'Q33875'
    },
    language_code_2: {
      en: 'oj'
    }
  },
  'en:kanuri': {
    language_code_2: {
      en: 'kr'
    },
    wikidata: {
      en: 'Q36094'
    },
    language_code_3: {
      en: 'kau'
    },
    name: {
      de: 'Kanuri',
      eu: 'Kanuri hizkuntza',
      nn: 'Kanuri-språket',
      oc: 'Kanuri',
      fi: 'Kanurin kieli',
      id: 'Bahasa Kanuri',
      it: 'Kanuri',
      ar: 'لغة كانورية',
      hi: 'कनूरी भाषा',
      pa: 'ਕਨੂਰੀ ਭਾਸ਼ਾ',
      ca: 'Kanuri',
      fr: 'Kanouri',
      gl: 'Lingua canuri',
      es: 'Kanuri',
      eo: 'Kanura lingvo',
      pt: 'Língua kanuri',
      en: 'Kanuri',
      ru: 'Канури',
      fa: 'زبان کنوری',
      yo: 'Èdè Kánúrí',
      sw: 'Kikanuri',
      mk: 'Канурски јазик',
      nl: 'Kanuri',
      pl: 'Język kanuri',
      ta: 'கனுரி மொழி',
      cs: 'Kanurijština',
      ja: 'カヌリ語',
      zh: '卡努里語',
      kv: 'Канури',
      az: 'Kanuri dili',
      br: 'Kanoureg',
      bn: 'কানুরি ভাষা',
      tr: 'Kanuri dili',
      uk: 'Канурі'
    }
  },
  'en:indonesian': {
    language_code_3: {
      en: 'ind'
    },
    name: {
      kl: 'Indonesiamiutut',
      ga: 'An Indinéisis',
      he: 'אינדונזית',
      bs: 'Bahasa Indonezijski jezik',
      ka: 'ინდონეზიური ენა',
      uz: 'Indonez tili',
      pt: 'Língua indonésia',
      eo: 'Indonezia lingvo',
      cy: 'Indoneseg',
      la: 'Lingua Indonesia',
      nn: 'Indonesisk',
      be: 'Інданезійская мова',
      fi: 'Indonesian kieli',
      oc: 'Bahasa Indonesia',
      sl: 'Indonezijščina',
      id: 'Bahasa Indonesia',
      jv: 'Basa Indonésia',
      ar: 'لغة إندونيسية',
      sh: 'Indonežanski jezik',
      sv: 'Indonesiska',
      vo: 'Lindäna-Seänuänaspük',
      hi: 'इंडोनेशियाई भाषा',
      kw: 'Indonesek',
      sr: 'Индонежански језик',
      ro: 'Limba indoneziană',
      wa: 'Indonezyin',
      zh: '印尼语',
      km: 'ភាសាឥណ្ឌូនេស៊ី',
      ja: 'インドネシア語',
      sk: 'Indonézština',
      kv: 'Индонезия кыв',
      ml: 'ഇന്തോനേഷ്യൻ ഭാഷ',
      hy: 'Ինդոնեզերեն',
      ko: '인도네시아어',
      br: 'Indonezeg',
      mg: 'Fiteny indoneziana',
      ru: 'Индонезийский язык',
      mi: 'Reo Initonīhia',
      sw: 'Kiindonesia',
      za: 'Vah Yindunizsihya',
      nl: 'Indonesisch',
      tk: 'Indoneziýa dilleri',
      ne: 'इण्डोनेशियाई भाषा',
      ms: 'Bahasa Indonesia',
      vi: 'Tiếng Indonesia',
      ta: 'இந்தோனேசிய மொழி',
      bh: 'इंडोनेशियाई भाषा',
      sa: 'बहासा इंडोनेशिया',
      ia: 'Lingua indonesian',
      ca: 'Indonesi',
      pa: 'ਇੰਡੋਨੇਸ਼ੀ ਬੋਲੀ',
      gl: 'Lingua indonesia',
      es: 'Indonesio',
      fr: 'Indonésien',
      th: 'ภาษาอินโดนีเซีย',
      sq: 'Gjuha indoneziane',
      tl: 'Wikang Indones',
      en: 'Indonesian',
      mr: 'बहासा इंडोनेशिया',
      ur: 'انڈونیشیائی زبان',
      de: 'Indonesisch',
      nb: 'Indonesisk',
      eu: 'Indonesiera',
      bg: 'Индонезийски език',
      hu: 'Indonéz',
      qu: 'Indunisya simi',
      it: 'Indonesiano',
      su: 'Basa Indonésia',
      cs: 'Indonéština',
      az: 'İndoneziya dili',
      et: 'Indoneesia keel',
      zu: 'Isi-Indonesia',
      ug: 'ھىندونېزىيە تىلى',
      bn: 'ইন্দোনেশীয় ভাষা',
      uk: 'Індонезійська мова',
      is: 'Indónesíska',
      tr: 'Endonezce',
      ce: 'Индонезихойн мотт',
      fa: 'زبان اندونزیایی',
      an: 'Idioma indonesio',
      tg: 'Забони индонезӣ',
      mk: 'Индонезиски јазик',
      da: 'Indonesisk',
      hr: 'Indonezijski jezik',
      pl: 'Język indonezyjski',
      lt: 'Indoneziečių kalba',
      lv: 'Indonēziešu valoda',
      af: 'Indonesies'
    },
    wikidata: {
      en: 'Q9240'
    },
    language_code_2: {
      en: 'id'
    }
  },
  'en:sardinian-language': {
    name: {
      uk: 'Сардська мова',
      tr: 'Sarduca',
      is: 'Sardiníska',
      os: 'Сардаг æвзаг',
      ug: 'ساردىنىيە تىلى',
      az: 'Sardiniya dili',
      cs: 'Sardinština',
      se: 'Sardigiella',
      lv: 'Sardu valoda',
      af: 'Sardies',
      gd: 'Sàrdais',
      hr: 'Sardinski jezici',
      pl: 'Język sardyński',
      da: 'Sardisk',
      fa: 'زبان ساردنیایی',
      an: 'Idioma sardo',
      mk: 'Сардински јазик',
      en: 'Sardinian language',
      mr: 'सार्दिनियन भाषा',
      tl: 'Wikang Sardinia',
      sc: 'Limba sarda',
      ca: 'Sard',
      es: 'Sardo',
      gl: 'Lingua sarda',
      fr: 'Sarde',
      it: 'Sardo',
      hu: 'Szárd nyelv',
      bg: 'Сардински език',
      de: 'Sardische Sprache',
      nb: 'Sardisk',
      eu: 'Sardiniera',
      co: 'Lingua sarda',
      li: 'Sardiens',
      ko: '사르데냐어',
      br: 'Sardeg',
      sk: 'Sardínčina',
      kv: 'Сардин кыв',
      kw: 'Sardaynek',
      sr: 'Сардински језик',
      ro: 'Limba sardă',
      zh: '薩丁尼亞語',
      wa: 'Sarde',
      ja: 'サルデーニャ語',
      vi: 'Tiếng Sardinia',
      ta: 'சார்தீனியம்',
      ia: 'Lingua sarde',
      nl: 'Sardijns',
      ku: 'Zimanê sardî',
      ru: 'Сардинский язык',
      sw: 'Kisardinia',
      eo: 'Sarda lingvo',
      pt: 'Língua sarda',
      la: 'Lingua Sarda',
      bs: 'Sardinski jezik',
      he: 'סרדו',
      ga: 'An tSairdínis',
      io: 'Sardiniana linguo',
      ar: 'لغة سردينية',
      sh: 'Sardinski',
      sv: 'Sardiska',
      id: 'Bahasa Sardinia',
      tt: 'Сардин теле',
      fi: 'Sardi',
      oc: 'Sarde',
      nn: 'Sardisk'
    },
    language_code_3: {
      en: 'srd'
    },
    wikidata: {
      en: 'Q33976'
    },
    language_code_2: {
      en: 'sc'
    }
  },
  'en:akan': {
    language_code_2: {
      en: 'ak'
    },
    language_code_3: {
      en: 'aka'
    },
    name: {
      hr: 'Akan jezik',
      pl: 'Język akan',
      lt: 'Akanų kalba',
      lv: 'Akanu valodas',
      ta: 'அகன மொழி',
      fa: 'زبان اکانی',
      ru: 'Акан',
      mk: 'Акански јазик',
      sw: 'Kiakan',
      nl: 'Akan',
      hy: 'Աքաներեն',
      ko: '아칸어군',
      bn: 'আকান ভাষাসমূহ',
      br: 'Akaneg',
      uk: 'Акан',
      is: 'Akanmál',
      cs: 'Akanština',
      zh: '阿坎语',
      ja: 'アカン語',
      kv: 'Акан',
      et: 'Akani keel',
      qu: 'Akan simi',
      it: 'Akan',
      ak: 'Akan',
      ar: 'لغة أكانية',
      sv: 'Akan',
      de: 'Akan-Sprachen',
      ur: 'اکان زبان',
      nb: 'Akan',
      nn: 'Akan',
      fi: 'Akanin kieli',
      oc: 'Àkan',
      pt: 'Língua akan',
      eo: 'Akanaj lingvoj',
      en: 'Akan',
      ca: 'Àkan',
      gl: 'Lingua akán',
      es: 'Lenguas akánicas',
      fr: 'Akan',
      tw: 'Akan',
      gv: 'Akanish'
    },
    wikidata: {
      en: 'Q28026'
    }
  },
  'en:manx': {
    name: {
      cv: 'Мэн чĕлхи',
      la: 'Lingua Monensis',
      cy: 'Manaweg',
      eo: 'Manksa lingvo',
      pt: 'Língua manesa',
      ka: 'მენური ენა',
      el: 'Μανξ γλώσσα',
      he: 'מאנית',
      io: 'Manxana linguo',
      ga: 'Gaeilge Mhanann',
      sv: 'Manx',
      ar: 'لغة مانكس',
      sh: 'Manski jezik',
      id: 'Bahasa Manx',
      sl: 'Manska gelščina',
      oc: 'Manés',
      fi: 'Manksin kieli',
      be: 'Мэнская мова',
      nn: 'Manx',
      br: 'Manaveg',
      ko: '맨어',
      li: 'Manx Gaelic',
      kv: 'Манинса кыв',
      sk: 'Mančina',
      ja: 'マン島語',
      zh: '曼島語',
      ro: 'Limba manx',
      sr: 'Манкс језик',
      kw: 'Manowek',
      ia: 'Lingua mannese',
      ta: 'மான்சு மொழி',
      vi: 'Tiếng Manx',
      nl: 'Manx-Gaelisch',
      ru: 'Мэнский язык',
      mr: 'मांक्स भाषा',
      en: 'Manx',
      tl: 'Wikang Manx',
      gv: 'Gaelg',
      fr: 'Mannois',
      es: 'Manés',
      gl: 'Lingua manx',
      ca: 'Gaèlic manx',
      it: 'Mannese',
      hu: 'Manx nyelv',
      fo: 'Manskt mál',
      eu: 'Manxera',
      nb: 'Mansk',
      de: 'Manx',
      tr: 'Man dili',
      is: 'Manska',
      uk: 'Менська мова',
      os: 'Мэнаг æвзаг',
      et: 'Mänksi keel',
      az: 'Men dili',
      cs: 'Manština',
      se: 'Manksgiella',
      af: 'Manx-Gaelies',
      gd: 'Gàidhlig Mhanainn',
      lv: 'Meniešu valoda',
      lt: 'Menksiečių kalba',
      pl: 'Język manx',
      hr: 'Manski jezik',
      da: 'Manx',
      mk: 'Мански јазик',
      an: 'Idioma manés',
      fa: 'زبان مانکس'
    },
    language_code_3: {
      en: 'glv'
    },
    wikidata: {
      en: 'Q12175'
    },
    language_code_2: {
      en: 'gv'
    }
  },
  'en:turkish': {
    language_code_3: {
      en: 'tur'
    },
    name: {
      oc: 'Turc',
      kk: 'Түрік тілі',
      be: 'Турэцкая мова',
      fi: 'Turkki',
      nn: 'Tyrkisk',
      ar: 'لغة تركية',
      sh: 'Turski jezik',
      sv: 'Turkiska',
      vo: 'Türkänapük',
      hi: 'तुर्कीयाई भाषा',
      ba: 'Төрөк теле',
      id: 'Bahasa Turki',
      sl: 'Turščina',
      or: 'ତୁର୍କୀ ଭାଷା',
      tt: 'Törek tele',
      jv: 'Basa Turki',
      he: 'טורקית',
      el: 'Τουρκική γλώσσα',
      kl: 'Tyrkiskisut',
      ga: 'An Tuircis',
      io: 'Turkiana linguo',
      cy: 'Tyrceg',
      la: 'Lingua turcica',
      pt: 'Língua turca',
      uz: 'Turk tili',
      ka: 'თურქული ენა',
      eo: 'Turka lingvo',
      cv: 'Турк чĕлхи',
      bs: 'Turski jezik',
      nl: 'Turks',
      ru: 'Турецкий язык',
      ku: 'Zimanê tirkî',
      yo: 'Èdè Túrkì',
      sw: 'Kituruki',
      mi: 'Reo Tākei',
      ta: 'துருக்கிய மொழி',
      ms: 'Bahasa Turki',
      vi: 'Tiếng Thổ Nhĩ Kỳ',
      tk: 'Türk dili',
      ne: 'तुर्की',
      kv: 'Туреч кыв',
      no: 'Tyrkisk',
      rw: 'Igiturukiya',
      sk: 'Turečtina',
      nv: 'Tʼóok bizaad',
      ml: 'തുർക്കിഷ് ഭാഷ',
      yi: 'טערקיש',
      ro: 'Limba turcă',
      kw: 'Turkek',
      sr: 'Турски језик',
      gn: "Tuykiañe'ẽ",
      ja: 'トルコ語',
      zh: '土耳其语',
      wa: 'Trouk',
      mn: 'Түрк хэл бичиг',
      mg: 'Fiteny tiorka',
      ko: '터키어',
      li: 'Turks',
      gu: 'તુર્કિશ ભાષા',
      hy: 'Թուրքերեն',
      br: 'Turkeg',
      hu: 'Török',
      bg: 'Турски език',
      fo: 'Turkiskt',
      ur: 'ترک زبان',
      am: 'ቱርክኛ',
      de: 'Türkisch',
      eu: 'Turkiera',
      nb: 'Tyrkisk',
      ab: 'Аҭырқ бызшәа',
      fy: 'Turksk',
      so: 'Af-Turki',
      it: 'Turco',
      qu: 'Turku simi',
      su: 'Basa Turki',
      sc: 'Lìngua turca',
      gv: 'Turkish',
      sq: 'Gjuha turke',
      th: 'ภาษาตุรกี',
      pa: 'ਤੁਰਕ ਭਾਸ਼ਾ',
      ca: 'Turc',
      fr: 'Turc',
      es: 'Turco',
      gl: 'Lingua turca',
      mr: 'तुर्की भाषा',
      en: 'Turkish',
      tl: 'Wikang Turko',
      da: 'Tyrkisk',
      an: 'Idioma turco',
      fa: 'زبان ترکی استانبولی',
      mk: 'Турски јазик',
      rm: 'Lingua tirca',
      tg: 'Забони туркӣ',
      lv: 'Turku valoda',
      af: 'Turks',
      gd: 'Turcais',
      lb: 'Tierkesch',
      hr: 'Turski jezik',
      lt: 'Turkų kalba',
      pl: 'Język turecki',
      az: 'Türk dili',
      ps: 'ترکي ژبه',
      et: 'Türgi keel',
      se: 'Durkkagiella',
      ky: 'Анадолу Түркчөсү',
      cs: 'Turečtina',
      av: 'Турк мацӀ',
      na: 'Dorerin Terki',
      tr: 'Türkçe',
      is: 'Tyrkneska',
      uk: 'Турецька мова',
      ce: 'Туркойн мотт',
      os: 'Туркаг æвзаг',
      ug: 'تۈرك تىلى',
      bn: 'তুর্কি ভাষা'
    },
    wikidata: {
      en: 'Q256'
    },
    language_code_2: {
      en: 'tr'
    }
  },
  'en:esperanto': {
    language_code_3: {
      en: 'epo'
    },
    name: {
      cu: 'Єспєранто',
      mg: 'Esperanto',
      li: 'Esperanto',
      hy: 'Էսպերանտո',
      ko: '에스페란토',
      br: 'Esperanteg',
      sk: 'Esperanto',
      rw: 'Icyesiperanto',
      no: 'Esperanto',
      kv: 'Эсперанто',
      ml: 'എസ്പെരാന്തോ',
      sr: 'Есперанто',
      ro: 'Esperanto',
      yi: 'עספעראנטא',
      wa: 'Esperanto',
      zh: '世界语',
      mn: 'Эсперанто',
      gn: 'Esperanto',
      ja: 'エスペラント',
      ff: 'Esperanto',
      kn: 'ಎಸ್ಪೆರಾಂಟೊ',
      vi: 'Esperanto',
      ms: 'Bahasa Esperanto',
      rn: 'Kiseperanto',
      ta: 'எஸ்பெராண்டோ',
      sa: 'एस्पेरान्तो',
      ia: 'Esperanto',
      ne: 'एस्पेरान्तो',
      ki: 'Esperanto',
      za: 'Esperanto',
      nl: 'Esperanto',
      yo: 'Esperanto',
      ku: 'Esperanto',
      bm: 'Esperanto',
      ru: 'Эсперанто',
      sw: 'Kiesperanto',
      uz: 'Esperanto',
      eo: 'Esperanto',
      pt: 'Esperanto',
      ka: 'ესპერანტო',
      cy: 'Esperanto',
      la: 'Lingua Esperantica',
      cv: 'Эсперанто',
      bs: 'Esperanto',
      he: 'אספרנטו',
      el: 'Εσπεράντο',
      kl: 'Esperanto',
      io: 'Esperanto',
      ga: 'Esperanto',
      ar: 'إسبرانتو',
      sh: 'Esperanto',
      my: 'အက်စပါရန်တို',
      vo: 'Sperantapük',
      hi: 'एस्पेरान्तो',
      sv: 'Esperanto',
      sl: 'Esperanto',
      ba: 'Эсперанто теле',
      id: 'Bahasa Esperanto',
      te: 'ఎస్పెరాన్టొ',
      tt: 'Эсперанто',
      be: 'Эсперанта',
      kk: 'Эсперанто',
      fi: 'Esperanto',
      oc: 'Esperanto',
      nn: 'Esperanto',
      uk: 'Есперанто',
      tr: 'Esperanto',
      na: 'Esperanto',
      is: 'Esperanto',
      ce: 'Эсперанто',
      zu: 'Isi-Esperanto',
      os: 'Эсперанто',
      ug: 'ئېسپېرانتو',
      bn: 'এস্পেরান্তো',
      ha: 'Esperanto',
      az: 'Esperanto',
      et: 'Esperanto',
      cs: 'Esperanto',
      se: 'Esperantogiella',
      wo: 'Esperanto',
      lv: 'Esperanto',
      af: 'Esperanto',
      gd: 'Esperanto',
      hr: 'Esperanto',
      lb: 'Esperanto',
      pl: 'Esperanto',
      lt: 'Esperanto',
      da: 'Esperanto',
      fa: 'زبان اسپرانتو',
      iu: 'ᐃᓯᐱᕋᓐᑐ',
      an: 'Esperanto',
      rm: 'Esperanto',
      tg: 'Забони эсперанто',
      mk: 'Есперанто',
      si: 'එස්පෙරාන්තො',
      en: 'Esperanto',
      mr: 'एस्पेरांतो',
      tl: 'Esperanto',
      sc: 'Esperanto',
      th: 'ภาษาเอสเปรันโต',
      sq: 'Gjuha esperanto',
      ca: 'Esperanto',
      ss: 'Sí-Speranto',
      pa: 'ਏਸਪੇਰਾਨਤੋ',
      es: 'Esperanto',
      gl: 'Esperanto',
      ay: 'Ispirantu aru',
      fr: 'Espéranto',
      fy: 'Esperanto',
      ab: 'Есперанто',
      qu: 'Esperanto simi',
      it: 'Esperanto',
      so: 'Esberanto',
      ie: 'Esperanto',
      fo: 'Esperanto',
      hu: 'Eszperantó',
      bg: 'Есперанто',
      ur: 'اسپرانٹو',
      de: 'Esperanto',
      am: 'ኤስፔራንቶ',
      nb: 'Esperanto',
      eu: 'Esperanto'
    },
    wikidata: {
      en: 'Q143'
    },
    language_code_2: {
      en: 'eo'
    }
  },
  'en:pashto': {
    name: {
      en: 'Pashto',
      mr: 'पश्तो भाषा',
      th: 'ภาษาพัชโต',
      fr: 'Pachto',
      gl: 'Lingua paxtúa',
      es: 'Pastún',
      pa: 'ਪਸ਼ਤੋ',
      ca: 'Paixtu',
      su: 'Basa Pastun',
      it: 'Pashtu',
      qu: 'Pashtu simi',
      bg: 'Пущунски език',
      eu: 'Paxtuera',
      nb: 'Pashto',
      ur: 'پشتو',
      de: 'Paschtunische Sprache',
      am: 'ፐሽቶ',
      tr: 'Peştuca',
      is: 'Pashto',
      uk: 'Пушту',
      bn: 'পশতু ভাষা',
      os: 'Пушту',
      ug: 'پاشتوچە',
      et: 'Puštu keel',
      az: 'Puştu dili',
      ps: 'پښتو ژبه',
      cs: 'Paštština',
      af: 'Pasjtoe',
      lv: 'Puštu',
      lt: 'Puštūnų kalba',
      pl: 'Język paszto',
      hr: 'Paštunski jezici',
      da: 'Pashto',
      mk: 'Паштунски јазик',
      tg: 'Забони пушту',
      fa: 'زبان پشتو',
      cv: 'Пушту',
      la: 'Lingua Afganica',
      cy: 'Pashto',
      uz: 'Pushtu tili',
      pt: 'Língua pachto',
      eo: 'Paŝtoa lingvo',
      ka: 'პუშტუ',
      bs: 'Paštu jezik',
      he: 'פשטו',
      ga: 'An Phaistis',
      io: 'Pashto-linguo',
      kl: 'Pashtomiutut',
      hi: 'पश्तो भाषा',
      vo: 'Pajtunapük',
      sv: 'Pashto',
      sh: 'Paštunski jezik',
      ar: 'لغة بشتوية',
      id: 'Bahasa Pashtun',
      oc: 'Pashto',
      be: 'Пушту',
      fi: 'Paštun kieli',
      nn: 'Pasjto',
      br: 'Pachtoueg',
      ko: '파슈토어',
      hy: 'Փաշթո',
      gu: 'અફઘાની ભાષા',
      ml: 'പഷ്തു',
      kv: 'Пушту',
      sk: 'Paštčina',
      ja: 'パシュトー語',
      km: 'ភាសាប៉ាស្តូ',
      zh: '普什图语',
      ro: 'Limba paștună',
      sr: 'Паштунски језик',
      ta: 'பஷ்தூ மொழி',
      ms: 'Bahasa Pashto',
      vi: 'Tiếng Pashtun',
      nl: 'Pasjtoe',
      mi: 'Reo Pashto',
      ru: 'Пушту',
      ku: 'Zimanê peşto',
      yo: 'Èdè Pashtó'
    },
    language_code_3: {
      en: 'pus'
    },
    wikidata: {
      en: 'Q58680'
    },
    language_code_2: {
      en: 'ps'
    }
  },
  'en:kyrgyz': {
    language_code_2: {
      en: 'ky'
    },
    wikidata: {
      en: 'Q9255'
    },
    name: {
      bs: 'Kirgiški jezik',
      pt: 'Língua quirguiz',
      uz: 'Qirgʻiz tili',
      ka: 'ყირგიზული ენა',
      eo: 'Kirgiza lingvo',
      la: 'Lingua Chirgisica',
      cv: 'Кăркăс чĕлхи',
      ga: 'Cirgisis',
      he: 'קירגיזית',
      el: 'Κιργιζική γλώσσα',
      ba: 'Ҡырғыҙ теле',
      id: 'Bahasa Kirgiz',
      tt: 'Кыргыз теле',
      ar: 'لغة قرغيزية',
      sh: 'Kirgiski jezik',
      hi: 'किर्गिज़ भाषा',
      sv: 'Kirgiziska',
      nn: 'Kirgisisk',
      fi: 'Kirgiisin kieli',
      kk: 'Қырғыз тілі',
      be: 'Rыргызская мова',
      oc: 'Quirguiz',
      hy: 'Ղրղզերեն',
      ko: '키르기스어',
      br: 'Kirgizeg',
      sr: 'Киргиски језик',
      ro: 'Limba kirghiză',
      mn: 'Хиргис хэл',
      zh: '吉尔吉斯语',
      ja: 'キルギス語',
      sk: 'Kirgizština',
      rw: 'Igikirigizi',
      kv: 'Кыргыз кыв',
      vi: 'Tiếng Kyrgyz',
      ta: 'கிருகிசு மொழி',
      ru: 'Киргизский язык',
      nl: 'Kirgizisch',
      mr: 'किर्गिझ भाषा',
      en: 'Kyrgyz',
      ca: 'Kirguís',
      pa: 'ਕਿਰਗੀਜ਼ ਭਾਸ਼ਾ',
      gl: 'Lingua kirguiz',
      es: 'Kirguís',
      fr: 'Kirghize',
      th: 'ภาษาคีร์กีซ',
      it: 'Chirghiso',
      qu: 'Kirkis simi',
      fy: 'Kirgizysk',
      de: 'Kirgisische Sprache',
      nb: 'Kirgisisk',
      eu: 'Kirgizera',
      hu: 'Kirgiz nyelv',
      bg: 'Киргизки език',
      ug: 'قىرغىز تىلى',
      bn: 'কিরগিজ ভাষা',
      uk: 'Киргизька мова',
      tr: 'Kırgızca',
      ce: 'ГӀиргӀизойн мотт',
      ky: 'Кыргыз тили',
      cs: 'Kyrgyzština',
      az: 'Qırğız dili',
      et: 'Kirgiisi keel',
      hr: 'Kirgiski jezik',
      pl: 'Język kirgiski',
      lt: 'Kirgizų kalba',
      lv: 'Kirgīzu valoda',
      af: 'Kirgisies',
      fa: 'زبان قرقیزی',
      tg: 'Забони қирғизӣ',
      mk: 'Киргиски јазик',
      da: 'Kirgisisk'
    },
    language_code_3: {
      en: 'kir'
    }
  },
  'en:volapuk': {
    language_code_3: {
      en: 'vol'
    },
    name: {
      oc: 'Volapük',
      kk: 'Волапюк',
      fi: 'Volapük',
      be: 'Валапюк',
      nn: 'Volapük',
      ar: 'فولابوك',
      sh: 'Volapik',
      vo: 'Volapük nulik',
      sv: 'Volapük',
      hi: 'वोलापूक भाषा',
      id: 'Bahasa Volapük',
      sl: 'Volapik',
      he: 'וולאפיק',
      el: 'Βολαπιούκ',
      io: 'Volapük',
      ga: 'Volapük',
      cy: 'Volapük',
      la: 'Volapük',
      ka: 'ვოლაპიუკი',
      pt: 'Volapuque',
      eo: 'Volapuko',
      nl: 'Volapük',
      ru: 'Волапюк',
      ku: 'Volapük',
      ta: 'வோலாபுக்கு மொழி',
      ms: 'Volapük',
      vi: 'Volapük',
      ia: 'Volapuk',
      kv: 'Воляпук',
      sk: 'Volapük',
      yi: 'וואלאפוק',
      ro: 'Volapük',
      sr: 'Волапик',
      ja: 'ヴォラピュク',
      zh: '沃拉普克语',
      cu: 'Волапюкъ',
      mg: 'Fiteny volapoky',
      ko: '볼라퓌크',
      hy: 'Վոլափյուք',
      li: 'Volapük',
      hu: 'Volapük nyelv',
      bg: 'Волапюк',
      de: 'Volapük',
      am: 'ቮላፒውክ',
      eu: 'Volapük',
      nb: 'Volapük',
      fy: 'Folapúk',
      it: 'Volapük',
      ie: 'Volapük',
      sq: 'Gjuha volapük',
      th: 'ภาษาโวลาปุก',
      ca: 'Volapük',
      fr: 'Volapük',
      gl: 'Volapük',
      es: 'Volapük',
      mr: 'व्होलाप्युक भाषा',
      en: 'Volapük',
      da: 'Volapük',
      an: 'Volapük',
      fa: 'زبان ولاپوک',
      mk: 'Волапик',
      rm: 'Volapük',
      lv: 'Volapiks',
      af: 'Volapük',
      hr: 'Volapük',
      lt: 'Volapiukas',
      pl: 'Volapük',
      az: 'Volapük',
      et: 'Volapük',
      cs: 'Volapük',
      tr: 'Volapük',
      na: 'Volapük',
      is: 'Volapük',
      uk: 'Волапюк',
      ce: 'Волапйукхойн мотт'
    },
    wikidata: {
      en: 'Q36986'
    },
    language_code_2: {
      en: 'vo'
    }
  },
  'en:avar': {
    name: {
      nl: 'Avaars',
      yo: 'Èdè Avar',
      fa: 'زبان آواری',
      ru: 'Аварский язык',
      mk: 'Аварски јазик',
      ms: 'Bahasa Avar',
      vi: 'Tiếng Avar',
      lv: 'Avāru valoda',
      ta: 'அவர மொழி',
      af: 'Awaars',
      hr: 'Avarski jezik',
      tk: 'Awar dili',
      pl: 'Język awarski',
      lt: 'Avarų kalba',
      az: 'Avar dili',
      kv: 'Авар кыв',
      ky: 'Авар тили',
      cs: 'Avarština',
      av: 'МагIарул мацI',
      zh: '阿瓦尔语',
      ja: 'アヴァル語',
      uk: 'Аварська мова',
      cu: 'Аварьскъ ѩꙁꙑкъ',
      is: 'Avarska',
      tr: 'Avarca',
      ce: 'Суьйлийн мотт',
      hy: 'Ավարերեն',
      os: 'Солиаг æвзаг',
      ko: '아바르어',
      br: 'Avareg',
      fi: 'Avaarin kieli',
      kk: 'Авар тілі',
      be: 'Аварская мова',
      bg: 'Аварски език',
      oc: 'Àvar',
      de: 'Awarisch',
      nb: 'Avarisk',
      nn: 'Avarisk',
      ar: 'لغة أوارية',
      sv: 'Avariska',
      sl: 'Avarščina',
      it: 'Avaro',
      ba: 'Ауар теле',
      tt: 'Авар теле',
      ca: 'Àvar',
      pa: 'ਅਵਾਰ ਭਾਸ਼ਾ',
      es: 'Ávaro',
      gl: 'Lingua avar',
      fr: 'Avar',
      ka: 'ხუნძური ენა',
      uz: 'Avar tili',
      pt: 'Língua avar',
      eo: 'Avara lingvo',
      en: 'Avar',
      cv: 'Авар чĕлхи'
    },
    language_code_3: {
      en: 'ava'
    },
    wikidata: {
      en: 'Q29561'
    },
    language_code_2: {
      en: 'av'
    }
  },
  'en:sango': {
    language_code_2: {
      en: 'sg'
    },
    wikidata: {
      en: 'Q33954'
    },
    language_code_3: {
      en: 'sag'
    },
    name: {
      sg: 'Sängö',
      fi: 'Sango',
      oc: 'Sango',
      bg: 'Санго',
      nl: 'Sango',
      fa: 'زبان سانگو',
      de: 'Sango',
      ru: 'Санго',
      nb: 'Sango',
      eu: 'Sango',
      ta: 'சாங்கோ மொழி',
      ar: 'السانغوية',
      it: 'Sango',
      id: 'Bahasa Sango',
      pl: 'Język sango',
      lt: 'Sangų kalba',
      kv: 'Санго',
      et: 'Sango keel',
      ca: 'Sango',
      zh: '桑戈语',
      es: 'Idioma sango',
      ja: 'サンゴ語',
      fr: 'Sango',
      uk: 'Санго',
      eo: 'Sangoa lingvo',
      pt: 'Língua sango',
      tr: 'Sango',
      en: 'Sango',
      ce: 'Санго мотт',
      ko: '상고어',
      br: 'Sangoeg'
    }
  },
  'en:venda': {
    wikidata: {
      en: 'Q32704'
    },
    language_code_3: {
      en: 'ven'
    },
    name: {
      sr: 'Венда',
      zh: '文達語',
      ja: 'ヴェンダ語',
      rw: 'Ikivenda',
      kv: 'Венда',
      az: 'Venda dili',
      zu: 'TshiVenda',
      ko: '벤다어',
      br: 'Vendeg',
      uk: 'Венда',
      tr: 'Venda dili',
      yo: 'Èdè Fenda',
      fa: 'زبان وندا',
      ru: 'Венда',
      sw: 'Kivenda',
      mk: 'Венда',
      st: 'Sevenda',
      nl: 'Venda',
      hr: 'Venda jezik',
      pl: 'Język venda',
      vi: 'Tiếng Venda',
      ta: 'வேந்த மொழி',
      af: 'Venda',
      ca: 'Venda',
      es: 'Venda',
      fr: 'Venda',
      he: 'ונדה',
      sq: 'Gjuha venda',
      eo: 'Venda lingvo',
      pt: 'Língua venda',
      en: 'Venda',
      ve: 'TshiVenḓa',
      de: 'Tshivenda',
      nb: 'Venda',
      nn: 'Tshivenda',
      eu: 'Vendera',
      fi: 'Vendan kieli',
      fo: 'Venda mál',
      bg: 'Венда',
      oc: 'Venda',
      it: 'Venda',
      id: 'Bahasa Venda',
      ar: 'لغة فيندية',
      sh: 'Venda jezik',
      sv: 'Venda'
    },
    language_code_2: {
      en: 've'
    }
  },
  'en:albanian': {
    wikidata: {
      en: 'Q8748'
    },
    language_code_3: {
      en: 'alb'
    },
    name: {
      io: 'Albaniana linguo',
      ga: 'An Albáinis',
      el: 'Αλβανική γλώσσα',
      he: 'אלבנית',
      bs: 'Albanski jezik',
      cv: 'Албан чĕлхи',
      cy: 'Albaneg',
      la: 'Lingua Albanica',
      eo: 'Albana lingvo',
      uz: 'Alban tili',
      pt: 'Língua albanesa',
      ka: 'ალბანური ენა',
      nn: 'Albansk',
      oc: 'Albanés',
      kk: 'Албан тілі',
      be: 'Албанская мова',
      fi: 'Albanian kieli',
      jv: 'Basa Albania',
      id: 'Bahasa Albania',
      sl: 'Albanščina',
      sv: 'Albanska',
      vo: 'Lalbanänapük',
      hi: 'अल्बेनियन भाषा',
      ar: 'لغة ألبانية',
      sh: 'Albanski jezik',
      ja: 'アルバニア語',
      zh: '阿尔巴尼亚语',
      ro: 'Limba albaneză',
      sr: 'Албански језик',
      kw: 'Albaynek',
      ml: 'അൽബേനിയൻ ഭാഷ',
      kv: 'Штип кыв',
      sk: 'Albánčina',
      rw: 'Icyalubaniya',
      br: 'Albaneg',
      ko: '알바니아어',
      li: 'Albanees',
      hy: 'Ալբաներեն',
      co: 'Lingua albanese',
      sw: 'Kialbania',
      ru: 'Албанский язык',
      ku: 'Zimanê albanî',
      yo: 'Èdè Albáníà',
      nl: 'Albanees',
      ia: 'Lingua albanese',
      ta: 'அல்பானிய மொழி',
      vi: 'Tiếng Albania',
      ms: 'Bahasa Albania',
      kn: 'ಆಲ್ಬೇನಿಯಾ ಭಾಷೆ ಮತ್ತು ಸಾಹಿತ್ಯ',
      fr: 'Albanais',
      es: 'Albanés',
      gl: 'Lingua albanesa',
      pa: 'ਅਲਬੇਨਿਆਈ ਬੋਲੀ',
      ca: 'Albanès',
      sq: 'Gjuha shqipe',
      gv: 'Albaanish',
      th: 'ภาษาแอลเบเนีย',
      sc: 'Limba albanesa',
      sm: 'Gagana Kosovo',
      en: 'Albanian',
      mr: 'आल्बेनियन भाषा',
      eu: 'Albaniera',
      nb: 'Albansk',
      de: 'Albanisch',
      ur: 'البانوی زبان',
      hu: 'Albán',
      bg: 'Албански език',
      fo: 'Albanskt mál',
      it: 'Albanese',
      qu: 'Albanya simi',
      cs: 'Albánština',
      se: 'Albánagiella',
      et: 'Albaania keel',
      az: 'Alban dili',
      bn: 'আলবেনীয় ভাষা',
      ug: 'ئالبان تىلى',
      ce: 'Албанийн мотт',
      tr: 'Arnavutça',
      is: 'Albanska',
      uk: 'Албанська мова',
      mk: 'Албански јазик',
      tg: 'Забони албанӣ',
      an: 'Idioma albanés',
      fa: 'زبان آلبانیایی',
      da: 'Albansk',
      lt: 'Albanų kalba',
      pl: 'Język albański',
      hr: 'Albanski jezik',
      gd: 'Albàinis',
      af: 'Albanees',
      lv: 'Albāņu valoda'
    },
    language_code_2: {
      en: 'sq'
    }
  },
  'en:basque': {
    language_code_2: {
      en: 'eu'
    },
    name: {
      sh: 'Baskijski jezik',
      ar: 'لغة بشكنشية',
      sv: 'Baskiska',
      hi: 'बास्क भाषा',
      id: 'Bahasa Basque',
      sl: 'Baskovščina',
      tt: 'Баск теле',
      jv: 'Basa Basque',
      oc: 'Basc',
      be: 'Баскская мова',
      fi: 'Baskin kieli',
      nn: 'Baskisk',
      cy: 'Basgeg',
      la: 'Lingua Vasconica',
      pt: 'Língua basca',
      ka: 'ბასკური ენა',
      eo: 'Eŭska lingvo',
      bs: 'Baskijski jezik',
      he: 'בסקית',
      el: 'Βασκική γλώσσα',
      io: 'Baskiana linguo',
      ga: 'An Bhascais',
      ta: 'பாஸ்க் மொழி',
      ms: 'Bahasa Basque',
      vi: 'Tiếng Basque',
      nl: 'Baskisch',
      ru: 'Баскский язык',
      ku: 'Zimanê baskî',
      co: 'Lingua basca',
      ko: '바스크어',
      li: 'Baskisch',
      hy: 'Բասկերեն',
      br: 'Euskareg',
      kv: 'Эускара кыв',
      sk: 'Baskičtina',
      ro: 'Limba bască',
      yi: 'באסקיש',
      kw: 'Baskek',
      sr: 'Баскијски језик',
      ja: 'バスク語',
      zh: '巴斯克語',
      wa: 'Basse',
      fy: 'Baskysk',
      qu: 'Yuskara simi',
      it: 'Basco',
      hu: 'Baszk',
      bg: 'Баски език',
      ur: 'باسک زبانیں',
      de: 'Baskische Sprache',
      eu: 'Euskara',
      nb: 'Baskisk',
      mr: 'बास्क भाषा',
      en: 'Basque',
      sc: 'Limba basca',
      sq: 'Gjuha baske',
      gv: 'Bascish',
      th: 'ภาษาบาสก์',
      ca: 'Basc',
      fr: 'Basque',
      gl: 'Lingua éuscara',
      es: 'Euskera',
      lv: 'Basku valoda',
      af: 'Baskies',
      gd: 'Basgais',
      hr: 'Baskijski jezik',
      lt: 'Baskų kalba',
      pl: 'Język baskijski',
      da: 'Baskisk',
      an: 'Idioma vasco',
      fa: 'زبان باسکی',
      mk: 'Баскиски јазик',
      tg: 'Забони баскӣ',
      is: 'Baskneska',
      tr: 'Baskça',
      uk: 'Баскійська мова',
      ug: 'باسكى تىلى',
      bn: 'বাস্ক ভাষা',
      az: 'Bask dili',
      et: 'Baski keel',
      cs: 'Baskičtina',
      se: 'Baskagiella'
    },
    language_code_3: {
      en: 'baq'
    },
    wikidata: {
      en: 'Q8752'
    }
  },
  'en:fula-language': {
    language_code_2: {
      en: 'ff'
    },
    language_code_3: {
      en: 'ful'
    },
    name: {
      en: 'Fula language',
      ka: 'ფულანი',
      eo: 'Fula lingvo',
      pt: 'Língua fula',
      sq: 'Gjuha fula',
      fr: 'Peul',
      io: 'Fula-linguo',
      es: 'Idioma fula',
      ga: 'Fuláinis',
      gl: 'Lingua fula',
      ca: 'Ful',
      sv: 'Fula',
      ar: 'لغة فولانية',
      it: 'Fula',
      qu: 'Fula simi',
      bg: 'Фула',
      fi: 'Fulani',
      eu: 'Fula',
      nn: 'Fulfulde',
      nb: 'Fulfulde',
      de: 'Fulfulde',
      ur: 'فولا زبان',
      is: 'Fúlaní',
      uk: 'Фульфульде',
      br: 'Fouleg',
      bn: 'ফুলা ভাষা',
      kv: 'Фула',
      ff: 'Fulfulde',
      ja: 'フラニ語',
      wa: 'Peul',
      zh: '富拉語',
      wo: 'Pullaar',
      cs: 'Fulbština',
      ta: 'ஃபுலா மொழி',
      lv: 'Fulu valoda',
      lt: 'Fulų kalba',
      pl: 'Język ful',
      hr: 'Fula jezici',
      nl: 'Fula',
      sw: 'Kifulfulde-Borgu',
      mk: 'Фулански јазик',
      ru: 'Фула',
      fa: 'زبان فولا',
      yo: 'Èdè Fúlàní'
    },
    wikidata: {
      en: 'Q33454'
    }
  },
  'en:german': {
    language_code_2: {
      en: 'de'
    },
    wikidata: {
      en: 'Q188'
    },
    name: {
      sq: 'Gjuha gjermane',
      sc: 'Limba tedesca',
      ay: 'Aliman aru',
      fr: 'Allemand',
      gl: 'Lingua alemá',
      es: 'Alemán',
      ss: 'SíJalimáne',
      ab: 'Алман бызшәа',
      fy: 'Dútsk',
      ie: 'German',
      qu: 'Aliman simi',
      it: 'Tedesco',
      bg: 'Немски език',
      hu: 'Német',
      dv: 'އަލްމާނީ',
      eu: 'Aleman',
      nb: 'Tysk',
      de: 'Deutsch',
      am: 'ጀርመንኛ',
      ht: 'Alman',
      tr: 'Almanca',
      bn: 'জার্মান ভাষা',
      os: 'Немыцаг æвзаг',
      ug: 'نېمىس تىلى',
      cs: 'Němčina',
      se: 'Duiskkagiella',
      af: 'Duits',
      pl: 'Język niemiecki',
      hr: 'Njemački jezik',
      mk: 'Германски јазик',
      la: 'Lingua Theodisca',
      pt: 'Língua alemã',
      eo: 'Germana lingvo',
      uz: 'Nemis tili',
      bs: 'Njemački jezik',
      el: 'Γερμανική γλώσσα',
      he: 'גרמנית',
      io: 'Germaniana linguo',
      ga: 'An Ghearmáinis',
      kl: 'Tyskisut',
      vo: 'Deutänapük',
      jv: 'Basa Jerman',
      te: 'జర్మన్ భాష',
      id: 'Bahasa Jerman',
      fi: 'Saksa',
      co: 'Lingua tedesca',
      br: 'Alamaneg',
      no: 'Tysk',
      kv: 'Немеч кыв',
      rw: 'Ikidage',
      sk: 'Nemčina',
      gn: "Alemañañe'ẽ",
      km: 'ភាសាអាល្លឺម៉ង់',
      mn: 'Герман хэл',
      zh: '德语',
      ro: 'Limba germană',
      sr: 'Немачки језик',
      kw: 'Almaynek',
      sa: 'जर्मन् भाषा',
      ta: 'இடாய்ச்சு மொழி',
      kn: 'ಜರ್ಮನ್ ಭಾಷೆ',
      ne: 'जर्मन भाषा',
      tk: 'Nemes dili',
      za: 'Vah Dwzgoz',
      st: 'Se-jeremane',
      ru: 'Немецкий язык',
      ku: 'Zimanê almanî',
      yo: 'Èdè Jẹ́mánì',
      en: 'German',
      mr: 'जर्मन भाषा',
      sm: "Fa'asiamani",
      tl: 'Wikang Aleman',
      ln: 'Lialémani',
      gv: 'Germaanish',
      th: 'ภาษาเยอรมัน',
      pa: 'ਜਰਮਨ ਭਾਸ਼ਾ',
      ca: 'Alemany',
      su: 'Basa Jérman',
      so: 'Af-Jarmal',
      fo: 'Týskt mál',
      ty: 'Reo Heremani',
      ur: 'جرمن زبان',
      ce: 'Германхойн мотт',
      is: 'Þýska',
      na: 'Dorerin Djermani',
      uk: 'Німецька мова',
      zu: 'IsiJalimani',
      et: 'Saksa keel',
      az: 'Alman dili',
      ps: 'آلماني ژبه',
      ky: 'Немис тили',
      gd: 'Gearmailtis',
      lv: 'Vācu valoda',
      lt: 'Vokiečių kalba',
      lb: 'Däitsch',
      da: 'Tysk',
      tg: 'Забони олмонӣ',
      rm: 'Lingua tudestga',
      bo: 'འཇར་མན་སྐད།',
      an: 'Idioma alemán',
      fa: 'زبان آلمانی',
      cv: 'Нимĕç чĕлхи',
      cy: 'Almaeneg',
      ka: 'გერმანული ენა',
      kg: 'Kidoitce',
      hi: 'जर्मन भाषा',
      sv: 'Tyska',
      my: 'ဂျာမန်ဘာသာစကား',
      ar: 'لغة ألمانية',
      sh: 'Nemački jezik',
      tt: 'Alman tele',
      ba: 'Немец теле',
      sl: 'Nemščina',
      oc: 'Alemand',
      kk: 'Неміс тілі',
      be: 'Нямецкая мова',
      nn: 'Tysk',
      mg: 'Fiteny alemana',
      cu: 'Нѣмьчьскъ ѩꙁꙑкъ',
      ko: '독일어',
      li: 'Duits',
      gu: 'જર્મન ભાષા',
      hy: 'Գերմաներեն',
      ml: 'ജർമ്മൻ ഭാഷ',
      nv: 'Bééshbichʼahii bizaad',
      ja: 'ドイツ語',
      ff: 'Almankoore',
      wa: 'Almand',
      yi: 'דייטשיש',
      ia: 'Lingua german',
      xh: 'IsiJamani',
      bh: 'जर्मन भाषा',
      ms: 'Bahasa Jerman',
      vi: 'Tiếng Đức',
      nl: 'Duits',
      sw: 'Kijerumani',
      mi: 'Reo Tiamana'
    },
    language_code_3: {
      en: 'deu'
    }
  },
  'en:latvian': {
    language_code_2: {
      en: 'lv'
    },
    wikidata: {
      en: 'Q9078'
    },
    language_code_3: {
      en: 'lav'
    },
    name: {
      en: 'Latvian',
      mr: 'लात्व्हियन भाषा',
      sc: 'Limba lètone',
      gv: 'Latvish',
      sq: 'Gjuha letoneze',
      th: 'ภาษาลัตเวีย',
      ca: 'Letó',
      fr: 'Letton',
      es: 'Letón',
      gl: 'Lingua letona',
      fy: 'Letsk',
      qu: 'Litunya simi',
      it: 'Lettone',
      bg: 'Латвийски език',
      hu: 'Lett',
      fo: 'Lettiskt mál',
      de: 'Lettische Sprache',
      eu: 'Letoniera',
      nb: 'Latvisk',
      tr: 'Letonca',
      na: 'Dorerin Ratebiya',
      is: 'Lettneska',
      uk: 'Латиська мова',
      ce: 'Латышийн мотт',
      ug: 'لاتۋىيە تىلى',
      zu: 'IsiLatviya',
      az: 'Latış dili',
      et: 'Läti keel',
      se: 'Látviagiella',
      ky: 'Латыш тили',
      cs: 'Lotyština',
      lv: 'Latviešu valoda',
      gd: 'Latbhais',
      af: 'Lets',
      hr: 'Latvijski jezik',
      lt: 'Latvių kalba',
      pl: 'Język łotewski',
      da: 'Lettisk',
      an: 'Idioma letón',
      fa: 'زبان لتونیایی',
      mk: 'Латвиски јазик',
      la: 'Lingua Lettonica',
      cy: 'Latfieg',
      pt: 'Língua letã',
      uz: 'Latish tili',
      ka: 'ლატვიური ენა',
      eo: 'Latva lingvo',
      bs: 'Latvijski jezik',
      he: 'לטבית',
      el: 'Λετονική γλώσσα',
      kl: 'Letlandimiutut',
      io: 'Latviana linguo',
      ga: 'An Laitvis',
      sh: 'Letonski jezik',
      ar: 'لغة لاتفية',
      vo: 'Latviyänapük',
      hi: 'लातवियाई भाषा',
      sv: 'Lettiska',
      id: 'Bahasa Latvi',
      sl: 'Latvijščina',
      tt: 'Латыш теле',
      oc: 'Leton',
      fi: 'Latvia',
      kk: 'Латыш тілі',
      be: 'Латышская мова',
      nn: 'Latvisk',
      ko: '라트비아어',
      li: 'Lets',
      hy: 'Լատվիերեն',
      br: 'Latveg',
      kv: 'Латыш кыв',
      sk: 'Lotyština',
      rw: 'Ikilativiya',
      ro: 'Limba letonă',
      yi: 'לעטיש',
      sr: 'Летонски језик',
      kw: 'Latviek',
      ja: 'ラトビア語',
      zh: '拉脱维亚语',
      ta: 'இலத்துவிய மொழி',
      vi: 'Tiếng Latvia',
      ms: 'Bahasa Latvia',
      nl: 'Lets',
      ru: 'Латышский язык',
      ku: 'Zimanê letonî'
    }
  },
  'en:cornish': {
    language_code_2: {
      en: 'kw'
    },
    wikidata: {
      en: 'Q25289'
    },
    name: {
      lv: 'Korniešu valoda',
      af: 'Kornies',
      gd: 'Còrnais',
      hr: 'Kornijski jezik',
      lt: 'Kornų kalba',
      pl: 'Język kornijski',
      da: 'Kornisk',
      an: 'Idioma cornuallés',
      fa: 'زبان کورنی',
      mk: 'Корнски јазик',
      rm: 'Lingua cornica',
      tr: 'Kernevekçe',
      is: 'Kornbreska',
      uk: 'Корнська мова',
      os: 'Корнаг æвзаг',
      bn: 'কর্নিশ ভাষা',
      et: 'Korni keel',
      cs: 'Kornština',
      fy: 'Kornysk',
      it: 'Cornico',
      bg: 'Корнуолски език',
      hu: 'Korni nyelv',
      fo: 'Corniskt mál',
      am: 'ኮርንኛ',
      de: 'Kornische Sprache',
      eu: 'Kornubiera',
      nb: 'Kornisk',
      mr: 'कॉर्निश भाषा',
      en: 'Cornish',
      gv: 'Cornish',
      th: 'ภาษาคอร์นวอลล์',
      ca: 'Còrnic',
      fr: 'Cornique',
      gl: 'Lingua córnica',
      es: 'Córnico',
      ta: 'கோர்னீசு மொழி',
      vi: 'Tiếng Cornwall',
      ms: 'Bahasa Cornish',
      ia: 'Lingua cornic',
      nl: 'Cornisch',
      ru: 'Корнский язык',
      ko: '콘월어',
      li: 'Cornish',
      hy: 'Կոռներէն',
      br: 'Kerneveureg',
      kv: 'Керновса кыв',
      sk: 'Kornčina',
      yi: 'קארניש',
      ro: 'Limba cornică',
      sr: 'Корнишки језик',
      kw: 'Kernowek',
      ja: 'コーンウォール語',
      zh: '康瓦爾語',
      ar: 'لغة كورنية',
      sh: 'Kornijski jezik',
      vo: 'Kornänapük',
      sv: 'Korniska',
      id: 'Bahasa Kernowek',
      sl: 'Kornijščina',
      oc: 'Cornic',
      fi: 'Korni',
      be: 'Корнская мова',
      nn: 'Kornisk',
      cy: 'Cernyweg',
      la: 'Lingua Cornubica',
      pt: 'Língua córnica',
      eo: 'Kornvala lingvo',
      cv: 'Корн чĕлхи',
      he: 'קורנית',
      el: 'Κορνουαλική γλώσσα',
      io: 'Kornwalana linguo',
      ga: 'An Choirnis'
    },
    language_code_3: {
      en: 'cor'
    }
  },
  'en:pali': {
    wikidata: {
      en: 'Q36727'
    },
    language_code_3: {
      en: 'pli'
    },
    name: {
      fa: 'زبان پالی',
      si: 'පාලි',
      da: 'Pali',
      pl: 'Język pali',
      lt: 'Pali',
      lv: 'Pāli valoda',
      gd: 'Pāli',
      af: 'Pali',
      cs: 'Páli',
      et: 'Paali keel',
      bn: 'পালি ভাষা',
      uk: 'Палі',
      tr: 'Pali',
      ur: 'پالی',
      de: 'Pali',
      nb: 'Pali',
      eu: 'Paliera',
      bg: 'Пали',
      hu: 'Páli nyelv',
      it: 'Pāli',
      ca: 'Pali',
      pa: 'ਪਾਲੀ ਭਾਸ਼ਾ',
      es: 'Pali',
      gl: 'Lingua pali',
      fr: 'Pali',
      th: 'ภาษาบาลี',
      mr: 'पाली भाषा',
      en: 'Pali',
      ru: 'Пали',
      sw: 'Kipali',
      nl: 'Pali',
      ne: 'पाली भाषा',
      vi: 'Tiếng Pali',
      ms: 'Bahasa Pali',
      ta: 'பாளி',
      sa: 'पालिभाषा',
      sr: 'Пали',
      ro: 'Limba pali',
      zh: '巴利语',
      km: 'ភាសាបាលី',
      ja: 'パーリ語',
      sk: 'Pálí',
      kv: 'Пали',
      ml: 'പാലി',
      gu: 'પાલિ ભાષા',
      ko: '팔리어',
      nn: 'Pali',
      fi: 'Paalin kieli',
      be: 'Палі',
      oc: 'Pali',
      or: 'ପାଳି ଭାଷା',
      id: 'Bahasa Pali',
      jv: 'Basa Pali',
      te: 'పాళీ భాష',
      sh: 'Pali',
      ar: 'بالي',
      hi: 'पालि भाषा',
      sv: 'Pali',
      ga: 'An Pháilis',
      he: 'פאלי',
      eo: 'Palia lingvo',
      pt: 'Páli',
      cy: 'Pali',
      pi: 'पालि Pāli',
      la: 'Lingua Palica'
    },
    language_code_2: {
      en: 'pi'
    }
  },
  'en:tatar': {
    language_code_2: {
      en: 'tt'
    },
    wikidata: {
      en: 'Q25285'
    },
    language_code_3: {
      en: 'tat'
    },
    name: {
      ru: 'Татарский язык',
      nl: 'Tataars',
      ta: 'தடர மொழி',
      vi: 'Tiếng Tatar',
      ms: 'Bahasa Tatar',
      ro: 'Limba tătară',
      sr: 'Татарски језик',
      ja: 'タタール語',
      zh: '鞑靼语',
      wa: 'Tatår',
      mn: 'Татар хэл',
      kv: 'Тотара кыв',
      sk: 'Tatárčina',
      ml: 'താത്താർ ഭാഷ',
      ko: '타타르어',
      br: 'Tatareg',
      cu: 'Татарьскъ ѩꙁꙑкъ',
      nn: 'Tatarisk',
      oc: 'Tatar',
      kk: 'Татар тілі',
      be: 'Татарская мова',
      fi: 'Tataarin kieli',
      ba: 'Татар теле',
      id: 'Bahasa Tatar',
      tt: 'Татар теле',
      ar: 'لغة تترية',
      sv: 'Tatariska',
      hi: 'तातार भाषा',
      he: 'טטרית',
      la: 'Lingua Tatarica',
      ka: 'თათრული ენა',
      uz: 'Tatar tili',
      pt: 'Língua tártara',
      eo: 'Tatara lingvo',
      cv: 'Тутар чĕлхи',
      fa: 'زبان تاتاری',
      mk: 'Татарски јазик',
      da: 'Tatarisk',
      hr: 'Tatarski jezik',
      lt: 'Totorių kalba',
      pl: 'Język tatarski',
      lv: 'Tatāru valoda',
      af: 'Tartaars',
      cs: 'Tatarština',
      az: 'Tatar dili',
      et: 'Tatari keel',
      ug: 'تاتار تىلى',
      os: 'Тæтæйраг æвзаг',
      tr: 'Tatarca',
      uk: 'Татарська мова',
      ce: 'ГӀезалойн мотт',
      de: 'Tatarisch',
      am: 'ታታርኛ',
      eu: 'Tatarera',
      nb: 'Tatarisk',
      hu: 'Tatár nyelv',
      bg: 'Татарски език',
      it: 'Tataro',
      fy: 'Tataarsk',
      pa: 'ਤਤਾਰ ਭਾਸ਼ਾ',
      ca: 'Tàtar',
      fr: 'Tatar',
      gl: 'Lingua tártara',
      es: 'Tártaro',
      th: 'ภาษาตาตาร์',
      en: 'Tatar',
      mr: 'तातर भाषा'
    }
  },
  'en:romanian': {
    language_code_2: {
      en: 'ro'
    },
    wikidata: {
      en: 'Q7913'
    },
    name: {
      rm: 'Lingua rumena',
      tg: 'Забони руминӣ',
      mk: 'Романски јазик',
      fa: 'زبان رومانیایی',
      an: 'Idioma rumán',
      da: 'Rumænsk',
      pl: 'Język rumuński',
      lt: 'Rumunų kalba',
      hr: 'Rumunjski jezik',
      af: 'Roemeens',
      gd: 'Ròmainis',
      lv: 'Rumāņu valoda',
      se: 'Romániagiella',
      cs: 'Rumunština',
      et: 'Rumeenia keel',
      lo: 'ພາສາລູມານີ',
      ps: 'رومانيايي ژبه',
      az: 'Rumın dili',
      bn: 'রুমানীয় ভাষা',
      ug: 'رۇمىنىيە تىلى',
      ce: 'Румынийн мотт',
      uk: 'Румунська мова',
      tr: 'Rumence',
      is: 'Rúmenska',
      nb: 'Rumensk',
      eu: 'Errumaniera',
      ur: 'رومانیانی زبان',
      am: 'ሩማንኛ',
      de: 'Rumänisch',
      fo: 'Rumenskt mál',
      bg: 'Румънски език',
      hu: 'Román',
      it: 'Romeno',
      qu: 'Rumanya simi',
      so: 'Af-Romani',
      fy: 'Roemeensk',
      gl: 'Lingua romanesa',
      es: 'Rumano',
      fr: 'Roumain',
      ca: 'Romanès',
      th: 'ภาษาโรมาเนีย',
      sq: 'Gjuha rumune',
      gv: 'Romaanish',
      sc: 'Limba romuna',
      sm: "Fa'aRomania",
      tl: 'Wikang Rumano',
      mr: 'रोमेनियन भाषा',
      en: 'Romanian',
      sw: 'Kiromania',
      ku: 'Zimanê rûmenî',
      yo: 'Èdè Romaníà',
      ru: 'Румынский язык',
      nl: 'Roemeens',
      ia: 'Lingua romanian',
      ms: 'Bahasa Romania',
      vi: 'Tiếng Romania',
      ta: 'உருமானிய மொழி',
      wa: 'Roumin',
      zh: '羅馬尼亞語',
      ja: 'ルーマニア語',
      kw: 'Roumanek',
      sr: 'Румунски језик',
      yi: 'רומעניש',
      ro: 'Limba română',
      mo: 'Лимба ромынэ',
      sk: 'Rumunčina',
      kv: 'Румын кыв',
      br: 'Roumaneg',
      li: 'Roemeens',
      hy: 'Ռումիներեն',
      ko: '루마니아어',
      co: 'Lingua rumena',
      mg: 'Fiteny romainina',
      cu: 'Роумꙑньскъ ѩꙁꙑкъ',
      nn: 'Rumensk',
      be: 'Румынская мова',
      fi: 'Romania',
      oc: 'Romanés',
      jv: 'Basa Rumania',
      tt: 'Румын теле',
      sl: 'Romunščina',
      id: 'Bahasa Rumania',
      sv: 'Rumänska',
      vo: 'Rumänapük',
      hi: 'रोमानियाई भाषा',
      ar: 'لغة رومانية',
      sh: 'Rumunski jezik',
      ga: 'An Rómáinis',
      io: 'Rumaniana linguo',
      el: 'Ρουμανική γλώσσα',
      he: 'רומנית',
      bs: 'Rumunski jezik',
      uz: 'Rumin tili',
      pt: 'Língua romena',
      ka: 'რუმინული ენა',
      eo: 'Rumana lingvo',
      la: 'Lingua Dacoromanica',
      cy: 'Rwmaneg'
    },
    language_code_3: {
      en: 'rum'
    }
  },
  'en:gikuyu': {
    name: {
      lt: 'Kikujų kalba',
      pl: 'Język kikuju',
      ta: 'கிகுயு மொழி',
      ru: 'Кикуйю',
      fa: 'زبان کیکویو',
      sw: 'Kikikuyu',
      mk: 'Кикујски јазик',
      ki: 'Gĩgĩkũyũ',
      nl: 'Gikuyu',
      br: 'Gikouyoueg',
      bn: 'গিকুয়ু ভাষা',
      is: 'Kíkújú',
      uk: 'Кікуйю',
      cs: 'Kikujština',
      ja: 'キクユ語',
      kv: 'Кикуйю',
      sk: 'Kikujčina',
      rw: 'Igikuyu',
      et: 'Kikuju keel',
      qu: 'Kikuyu simi',
      it: 'Gikuyu',
      sv: 'Kikuyu',
      de: 'Kikuyu',
      nb: 'Kikuyu',
      oc: 'Gikuyu',
      fi: 'Kikujun kieli',
      eo: 'Kikuja lingvo',
      pt: 'Língua kikuyu',
      en: 'Gikuyu',
      pa: 'ਕਿਕਿਊ ਭਾਸ਼ਾ',
      ca: 'Gikuyu',
      fr: 'Kikuyu',
      gl: 'Lingua kikuyu',
      ga: 'An Chiocúis',
      es: 'Idioma kikuyu'
    },
    language_code_3: {
      en: 'kik'
    },
    wikidata: {
      en: 'Q33587'
    },
    language_code_2: {
      en: 'ki'
    }
  },
  'en:tigrinya': {
    language_code_2: {
      en: 'ti'
    },
    language_code_3: {
      en: 'tir'
    },
    name: {
      nn: 'Tigrinja',
      nb: 'Tigrinja',
      eu: 'Tigrinyera',
      de: 'Tigrinya',
      am: 'ትግርኛ',
      fi: 'Tigrinja',
      be: 'Тыгрынья',
      hu: 'Tigrinya nyelv',
      bg: 'Тигриня',
      it: 'Tigrino',
      sl: 'Tigrajščina',
      id: 'Bahasa Tigrinya',
      so: 'Af-Tigrinya',
      sv: 'Tigrinska',
      hi: 'तिग्रीन्या भाषा',
      ar: 'لغة تغرينية',
      gl: 'Lingua tigriña',
      es: 'Idioma tigriña',
      fr: 'Tigrigna',
      ca: 'Tigrinya',
      pa: 'ਟਿਗਰੀਨਿਆ ਬੋਲੀ',
      th: 'ภาษาตึกรึญญา',
      he: 'תיגרינית',
      en: 'Tigrinya',
      mr: 'तिग्रिन्या भाषा',
      pt: 'Língua tigrínia',
      eo: 'Tigraja lingvo',
      la: 'Lingua Tigrinya',
      mk: 'Тигриња',
      sw: 'Kitigrinya',
      fa: 'زبان تیگرینیا',
      ku: 'Zimanê tigrînya',
      ru: 'Тигринья',
      an: 'Idioma tigrinya',
      nl: 'Tigrinya',
      ti: 'ትግርኛ ፊደል',
      pl: 'Język tigrinia',
      lt: 'Tigrajų kalba',
      hr: 'Tigrinja jezik',
      lv: 'Tigrinja',
      vi: 'Tiếng Tigrinya',
      ta: 'திகுரிஞா மொழி',
      zh: '提格利尼亞語',
      ja: 'ティグリニャ語',
      ro: 'Limba tigrinya',
      et: 'Tigrinja keel',
      ml: 'ടിഗ്രീന്യ ഭാഷ',
      sk: 'Tigriňa',
      kv: 'Тигринья',
      bn: 'তিগ্রিনিয়া ভাষা',
      br: 'Tigrigneg',
      ug: 'تىگرىنىيەچە',
      ko: '티그리냐어',
      uk: 'Тигринья',
      cu: 'Тигрин҄ьскъ ѩꙁꙑкъ',
      tr: 'Tigrinya dili',
      is: 'Tígrinja'
    },
    wikidata: {
      en: 'Q34124'
    }
  },
  'en:galician': {
    language_code_2: {
      en: 'gl'
    },
    name: {
      ro: 'Limba galiciană',
      kw: 'Galijek',
      sr: 'Галицијски језик',
      ja: 'ガリシア語',
      gn: "Gallégo ñe'ẽ",
      zh: '加利西亞語',
      kv: 'Галис кыв',
      rw: 'Ikigalisiya',
      sk: 'Galícijčina',
      ko: '갈리시아어',
      hy: 'Գալիսերեն',
      li: 'Galicisch',
      br: 'Galizeg',
      co: 'Lingua galiziana',
      ru: 'Галисийский язык',
      ku: 'Zimanê galîsyayî',
      mi: 'Reo Galicia',
      sw: 'Kigalicia',
      nl: 'Galicisch',
      ne: 'गालिसियाली',
      ta: 'கலீசிய மொழி',
      ms: 'Bahasa Galicia',
      vi: 'Tiếng Galicia',
      ia: 'Lingua gallecian',
      io: 'Galisiana linguo',
      ga: 'An Ghailísis',
      he: 'גליסית',
      el: 'Γαλικιανή γλώσσα',
      cy: 'Galisieg',
      la: 'Lingua gallaica',
      uz: 'Galisiyaliklar tili',
      pt: 'Língua galega',
      ka: 'გალისიური ენა',
      eo: 'Galega lingvo',
      nn: 'Galisisk',
      oc: 'Galèc',
      be: 'Галісійская мова',
      fi: 'Galego',
      id: 'Bahasa Galisia',
      sl: 'Galicijščina',
      sh: 'Galicijski jezik',
      ar: 'لغة غاليسية',
      vo: 'Galiziänapük',
      hi: 'गैलिशियन भाषा',
      sv: 'Galiciska',
      cs: 'Galicijština',
      se: 'Galegogiella',
      az: 'Qalisiya dili',
      et: 'Galeegi keel',
      ug: 'گالىسىيەچە',
      os: 'Галисиаг æвзаг',
      bn: 'গালিসিয়ান ভাষা',
      tr: 'Galiçyaca',
      uk: 'Галісійська мова',
      ce: 'Галисийн мотт',
      an: 'Idioma gallego',
      fa: 'زبان گالیسی',
      mk: 'Галициски јазик',
      da: 'Galicisk',
      hr: 'Galješki jezik',
      lt: 'Galisų kalba',
      pl: 'Język galicyjski',
      lv: 'Galisiešu valoda',
      af: 'Galicies',
      ca: 'Gallec',
      fr: 'Galicien',
      es: 'Gallego',
      gl: 'Lingua galega',
      sc: 'Limba galega',
      sq: 'Gjuha galiciane',
      gv: 'Galeeshish',
      th: 'ภาษากาลิเซีย',
      tl: 'Wikang Galisyano',
      mr: 'गालेगो भाषा',
      en: 'Galician',
      de: 'Galicische Sprache',
      eu: 'Galiziera',
      nb: 'Galisisk',
      hu: 'Galiciai nyelv',
      bg: 'Галисийски език',
      qu: 'Galligu simi',
      it: 'Gallego'
    },
    language_code_3: {
      en: 'glg'
    },
    wikidata: {
      en: 'Q9307'
    }
  },
  'en:telugu': {
    name: {
      fa: 'زبان تلگو',
      mk: 'Телугу',
      da: 'Telugu',
      hr: 'Telugu jezik',
      lt: 'Telugų kalba',
      pl: 'Język telugu',
      lv: 'Telugu valoda',
      af: 'Telugu',
      se: 'Telugugiella',
      cs: 'Telugština',
      az: 'Teluqu dili',
      et: 'Telugu keel',
      ug: 'تېلۇگۇ تىلى',
      bn: 'তেলুগু ভাষা',
      tr: 'Telugu dili',
      is: 'Telúgú',
      uk: 'Телуґу',
      ur: 'تیلگو زبان',
      de: 'Telugu',
      eu: 'Telugu',
      nb: 'Telugu',
      dv: 'ތެލުގޫ',
      hu: 'Telugu',
      it: 'Telugu',
      qu: 'Telugu simi',
      pa: 'ਤੇਲੁਗੂ ਭਾਸ਼ਾ',
      ca: 'Telugu',
      fr: 'Télougou',
      gl: 'Lingua telugúa',
      es: 'Telugu',
      th: 'ภาษาเตลูกู',
      mr: 'तेलुगू भाषा',
      en: 'Telugu',
      ru: 'Телугу',
      sw: 'Kitelugu',
      nl: 'Telugu',
      ne: 'तेलगु भाषा',
      ta: 'தெலுங்கு',
      bh: 'तेलुगू',
      kn: 'ತೆಲುಗು',
      vi: 'Tiếng Telugu',
      ms: 'Bahasa Telugu',
      sa: 'तेलुगु',
      yi: 'טעלוגו',
      ro: 'Limba telugu',
      sr: 'Телугу језик',
      ja: 'テルグ語',
      wa: 'Telougou',
      zh: '泰卢固语',
      mn: 'Телугу хэл',
      kv: 'Телугу',
      sk: 'Telugčina',
      ml: 'തെലുഗു',
      ko: '텔루구어',
      gu: 'તેલુગુ ભાષા',
      br: 'Telougoueg',
      mg: 'Fiteny telogo',
      nn: 'Telugu',
      oc: 'Telugu',
      be: 'Тэлугу',
      fi: 'Telugu',
      id: 'Bahasa Telugu',
      or: 'ତେଲୁଗୁ ଭାଷା',
      te: 'తెలుగు',
      sh: 'Telugu jezik',
      ar: 'لغة تيلوغوية',
      sv: 'Telugu',
      hi: 'तेलुगू भाषा',
      io: 'Telugu linguo',
      he: 'טלוגו',
      el: 'Τελούγκου',
      cy: 'Telugu',
      la: 'Lingua Telingana',
      uz: 'Telugu tili',
      pt: 'Língua telugu',
      ka: 'ტელუგუ',
      eo: 'Telugua lingvo'
    },
    language_code_3: {
      en: 'tel'
    },
    wikidata: {
      en: 'Q8097'
    },
    language_code_2: {
      en: 'te'
    }
  },
  'en:hindi': {
    language_code_2: {
      en: 'hi'
    },
    wikidata: {
      en: 'Q1568'
    },
    name: {
      ug: 'ھىندى تىلى',
      bn: 'হিন্দি ভাষা',
      tr: 'Hintçe',
      is: 'Hindí',
      uk: 'Гінді',
      ce: 'Хинди',
      se: 'Hindigiella',
      cs: 'Hindština',
      az: 'Hind dili',
      ps: 'هندي',
      et: 'Hindi keel',
      hr: 'Hindski jezik',
      lt: 'Hindi',
      pl: 'Język hindi',
      lv: 'Hindi',
      gd: 'Hindi',
      af: 'Hindi',
      bi: 'Hindi',
      an: 'Hindi',
      iu: 'ᐦᐃᓐᑏ',
      fa: 'زبان هندی',
      mk: 'Хиндиски јазик',
      tg: 'Забони ҳиндӣ',
      bo: 'རྒྱ་གར་སྐད།',
      da: 'Hindi',
      tl: 'Wikang Hindi',
      mr: 'हिंदी भाषा',
      en: 'Hindi',
      pa: 'ਹਿੰਦੀ ਭਾਸ਼ਾ',
      ca: 'Hindi',
      fr: 'Hindi',
      gl: 'Lingua hindi',
      es: 'Hindi',
      sq: 'Hindi',
      gv: 'Hindish',
      th: 'ภาษาฮินดี',
      qu: 'Hindi simi',
      it: 'Hindi',
      ie: 'Hindi',
      fy: 'Hindy',
      de: 'Hindi',
      ur: 'ہندی',
      am: 'ህንዲ',
      eu: 'Hindi',
      nb: 'Hindi',
      dv: 'ހިންދީ',
      bg: 'Хинди',
      hu: 'Hindi',
      ko: '힌디어',
      gu: 'હિંદી ભાષા',
      li: 'Hindi',
      hy: 'Հինդի',
      br: 'Hindeg',
      mg: 'Fiteny hindi',
      ro: 'Limba hindi',
      sr: 'Хинди',
      ja: 'ヒンディー語',
      gn: "Hindi ñe'ẽ",
      mn: 'Хинди хэл',
      zh: '印地语',
      kv: 'Хинди',
      sk: 'Hindčina',
      ml: 'ഹിന്ദി',
      ne: 'हिन्दी',
      bh: 'हिन्दी',
      ta: 'இந்தி',
      kn: 'ಹಿಂದಿ',
      vi: 'Tiếng Hindi',
      ms: 'Bahasa Hindi',
      sa: 'हिन्दीभाषा',
      ru: 'Хинди',
      yo: 'Èdè Híndì',
      ku: 'Zimanê hindî',
      mi: 'Reo Hindi',
      sw: 'Kihindi',
      za: 'Vahyindi',
      nl: 'Hindi',
      bs: 'Hindu',
      la: 'Lingua hindica',
      cy: 'Hindi',
      pt: 'Língua hindi',
      uz: 'Hind tili',
      ka: 'ჰინდი ენა',
      eo: 'Hindia lingvo',
      cv: 'Хинди',
      kl: 'Hindimiutut',
      ee: 'Axa do Ŋgɔ',
      io: 'Hindi linguo',
      ga: 'An Hiondúis',
      he: 'הינדי',
      kg: 'Kihindi',
      as: 'হিন্দী ভাষা',
      el: 'Χίντι',
      id: 'Bahasa Hindi',
      or: 'ହିନ୍ଦୀ ଭାଷା',
      sl: 'Hindijščina',
      tt: 'Һинд теле',
      te: 'హిందీ భాష',
      jv: 'Basa Hindi',
      ar: 'لغة هندية',
      sh: 'Hindi',
      sv: 'Hindi',
      vo: 'Hindiy',
      hi: 'हिन्दी',
      nn: 'Hindi',
      oc: 'Indi',
      fi: 'Hindi',
      kk: 'Хинди тілі',
      be: 'Хіндзі'
    },
    language_code_3: {
      en: 'hin'
    }
  },
  'en:kongo-language': {
    language_code_2: {
      en: 'kg'
    },
    name: {
      it: 'Kongo',
      qu: 'Kongo simi',
      id: 'Bahasa Kongo',
      sv: 'Kikongo',
      ur: 'کانگو زبان',
      de: 'Kikongo',
      nn: 'Kikongo',
      nb: 'Kongo',
      eu: 'Kikongo',
      fi: 'Kongon kieli',
      ln: 'Kikɔ́ngɔ',
      eo: 'Konga lingvo',
      pt: 'Kikongo',
      en: 'Kongo language',
      ca: 'Llengües kongo',
      gl: 'Lingua kikongo',
      es: 'Kikongo',
      fr: 'Kikongo',
      kg: 'Kisikongo',
      pl: 'Język kongo',
      lt: 'Kongo kalba',
      ta: 'கோங்கோ மொழி',
      fa: 'زبان کنگوئی',
      yo: 'Èdè Kóngò',
      ru: 'Конго',
      mk: 'Конгоански јазик',
      sw: 'Kikoongo',
      da: 'Kikongo',
      nl: 'Kongo',
      ko: '콩고어',
      bn: 'কঙ্গো ভাষা',
      br: 'Kongoeg',
      uk: 'Конго',
      tr: 'Kongo Dili',
      cs: 'Konžština',
      zh: '刚果语',
      wa: 'Kicongo',
      ja: 'コンゴ語',
      sk: 'Konžština',
      kv: 'Конго',
      az: 'Konqo dili'
    },
    language_code_3: {
      en: 'kon'
    },
    wikidata: {
      en: 'Q33702'
    }
  },
  'en:xhosa': {
    name: {
      xh: 'IsiXhosa',
      vi: 'Tiếng Xhosa',
      ta: 'சோசா மொழி',
      ne: 'खोसा भाषा',
      nl: 'Xhosa',
      sw: 'Kixhosa',
      yo: 'Èdè Xhosa',
      ru: 'Коса',
      mg: 'Fiteny Xhosa',
      br: 'Xhosaeg',
      li: 'Xhosa',
      ko: '코사어',
      rw: 'Ikigisosa',
      kv: 'Коса',
      zh: '科萨语',
      ja: 'コサ語',
      sr: 'Коса',
      hi: 'ख़ोसा भाषा',
      sv: 'Xhosa',
      sh: 'Xhosa jezik',
      ar: 'اللغة الخوسية',
      id: 'Bahasa Xhosa',
      be: 'Коса',
      fi: 'Xhosan kieli',
      oc: 'Xhosa',
      nn: 'Xhosa',
      ve: 'IsiXhoza',
      cv: 'Коса чĕлхи',
      pt: 'Língua xhosa',
      eo: 'Kosa lingvo',
      cy: 'Xhosa',
      kg: 'Kixhosa',
      he: 'קוסה',
      ga: 'Xhosa',
      io: 'Xosa-linguo',
      af: 'Xhosa',
      lv: 'Isiķhosa',
      pl: 'Język xhosa',
      lt: 'Kosų kalba',
      hr: 'Xhosa jezik',
      mk: 'Коса',
      fa: 'زبان خوسایی',
      uk: 'Коса',
      tr: 'Xhosa dili',
      bn: 'খোসা ভাষা',
      zu: 'IsiXhosa',
      et: 'Koosa keel',
      az: 'Xosa dili',
      cs: 'Xhoština',
      it: 'Xhosa',
      qu: 'Xhosa simi',
      fo: 'Xhosa mál',
      bg: 'Кхоса',
      nb: 'Xhosa',
      eu: 'Xhosera',
      de: 'IsiXhosa',
      mr: 'कौसा भाषा',
      en: 'Xhosa',
      sq: 'Gjuha xhosa',
      gl: 'Lingua xhosa',
      es: 'Xhosa',
      fr: 'Xhosa',
      ca: 'Xosa',
      pa: 'ਕੋਸਾ ਭਾਸ਼ਾ'
    },
    language_code_3: {
      en: 'xho'
    },
    wikidata: {
      en: 'Q13218'
    },
    language_code_2: {
      en: 'xh'
    }
  },
  'en:swazi': {
    language_code_2: {
      en: 'ss'
    },
    name: {
      sv: 'SiSwati',
      ar: 'لغة سوازي',
      it: 'Swati',
      qu: 'Swasi simi',
      id: 'Bahasa Swati',
      fi: 'Swazin kieli',
      bg: 'Свази',
      nb: 'Swati',
      nn: 'Swazi',
      eu: 'Swaziera',
      ve: 'IsiSwati',
      de: 'Siswati',
      en: 'Swazi',
      mr: 'स्वाती भाषा',
      eo: 'Svazia lingvo',
      pt: 'Língua suázi',
      he: 'סווזי',
      es: 'Suazi',
      fr: 'Swati',
      ca: 'Swazi',
      ss: 'SiSwati',
      af: 'Swazi',
      vi: 'Tiếng Swazi',
      ms: 'Bahasa Swati',
      ta: 'சுவாசி மொழி',
      pl: 'Język suazi',
      lt: 'Svazių kalba',
      hr: 'Swati jezik',
      nl: 'Swazi',
      sw: 'Kiswati',
      mk: 'Свази',
      yo: 'Èdè Swazi',
      fa: 'زبان سوازی',
      ru: 'Свати',
      uk: 'Сваті',
      tr: 'Swati dili',
      br: 'Swatieg',
      zu: 'IsiSwazi',
      ko: '스와티어',
      kv: 'Свати',
      az: 'Svati dili',
      zh: '史瓦濟語',
      gn: "Suasi ñe'ẽ",
      ja: 'スワジ語',
      cs: 'Siswatština',
      yi: 'סוואזי'
    },
    language_code_3: {
      en: 'ssw'
    },
    wikidata: {
      en: 'Q34014'
    }
  },
  'en:luganda': {
    wikidata: {
      en: 'Q33368'
    },
    language_code_3: {
      en: 'lug'
    },
    name: {
      lg: 'Luganda',
      ta: 'ஒலுகாந்த மொழி',
      pl: 'Język luganda',
      lt: 'Luganda',
      nl: 'Luganda',
      fa: 'زبان لوگاندا',
      ku: 'Luganda',
      ru: 'Луганда',
      sw: 'Kiganda',
      uk: 'Луганда',
      br: 'Gandaeg',
      sk: 'Gandčina',
      kv: 'Луганда',
      ml: 'ഒലുഗാൻഡ ഭാഷ',
      et: 'Ganda keel',
      cs: 'Gandština',
      sr: 'Луганда језик',
      ro: 'Ganda',
      wa: 'Louganda',
      zh: '卢干达语',
      ja: 'ルガンダ語',
      sv: 'Luganda',
      it: 'Luganda',
      qu: 'Ganda simi',
      id: 'Bahasa Luganda',
      be: 'Луганда',
      fi: 'Gandan kieli',
      oc: 'Luganda',
      de: 'Luganda',
      nb: 'Luganda',
      nn: 'Luganda',
      pt: 'Língua luganda',
      eo: 'Ganda lingvo',
      en: 'Luganda',
      he: 'לוגנדה',
      ca: 'Ganda',
      es: 'Idioma luganda',
      gl: 'Lingua luganda',
      fr: 'Luganda'
    },
    language_code_2: {
      en: 'lg'
    }
  },
  'en:urdu': {
    wikidata: {
      en: 'Q1617'
    },
    language_code_3: {
      en: 'urd'
    },
    name: {
      mg: 'Fiteny urdu',
      ko: '우르두어',
      gu: 'ઉર્દુ ભાષા',
      li: 'Urdu',
      br: 'Ourdoueg',
      kv: 'Урду',
      rw: 'Inyeyurudu',
      sk: 'Urdčina',
      ml: 'ഉർദു',
      yi: 'אורדו',
      ro: 'Limba urdu',
      sr: 'Урду',
      gn: "Urdu ñe'ẽ",
      ja: 'ウルドゥー語',
      zh: '乌尔都语',
      mn: 'Урдү хэл',
      ta: 'உருது',
      bh: 'उर्दू',
      ms: 'Bahasa Urdu',
      vi: 'Tiếng Urdu',
      kn: 'ಉರ್ದೂ',
      ne: 'उर्दू',
      za: 'Vah Vuhwjduh',
      nl: 'Urdu',
      ru: 'Урду',
      ku: 'Zimanê urdû',
      mi: 'Reo Urdu',
      sw: 'Kiurdu',
      cy: 'Wrdw',
      pi: 'उर्दू भाषा',
      la: 'Lingua Urdu',
      ka: 'ურდუ ენა',
      pt: 'Língua urdu',
      eo: 'Urduo',
      cv: 'Урду',
      bs: 'Urdu jezik',
      he: 'אורדו',
      el: 'Ουρντού',
      kl: 'Urdumiutut',
      ga: 'An Urdais',
      sh: 'Urdu',
      ar: 'لغة أردوية',
      sv: 'Urdu',
      vo: 'Lurduapük',
      hi: 'उर्दू भाषा',
      id: 'Bahasa Urdu',
      sl: 'Urdujščina',
      or: 'ଉର୍ଦ୍ଦୁ ଭାଷା',
      tt: 'Урду',
      te: 'ఉర్దూ భాష',
      jv: 'Basa Urdu',
      oc: 'Ordo',
      kk: 'Урду',
      be: 'Урду',
      fi: 'Urdu',
      nn: 'Urdu',
      tr: 'Urduca',
      is: 'Úrdú',
      uk: 'Урду',
      ce: 'Урду',
      ug: 'ئوردو تىلى',
      bn: 'উর্দু ভাষা',
      az: 'Urdu dili',
      ps: 'اوردو ژبه',
      et: 'Urdu keel',
      se: 'Urdugiella',
      cs: 'Urdština',
      lv: 'Urdu',
      af: 'Oerdoe',
      ks: 'اُردُو زَبان',
      hr: 'Urdu jezik',
      lt: 'Urdu',
      pl: 'Język urdu',
      da: 'Urdu',
      an: 'Urdu',
      fa: 'زبان اردو',
      mk: 'Урду',
      tg: 'Забони урду',
      en: 'Urdu',
      mr: 'उर्दू भाषा',
      th: 'ภาษาอูรดู',
      pa: 'ਉਰਦੂ ਭਾਸ਼ਾ',
      ca: 'Urdú',
      fr: 'Ourdou',
      es: 'Urdu',
      gl: 'Lingua urdú',
      fy: 'Oerdoe',
      it: 'Urdu',
      qu: 'Urdu simi',
      ie: 'Urdu',
      dv: 'އުރުދޫ',
      bg: 'Урду',
      hu: 'Urdu',
      sd: 'اردو',
      ur: 'اردو',
      de: 'Urdu',
      eu: 'Urdu',
      nb: 'Urdu'
    },
    language_code_2: {
      en: 'ur'
    }
  },
  'en:northern-ndebele-language': {
    language_code_2: {
      en: 'nd'
    },
    wikidata: {
      en: 'Q35613'
    },
    name: {
      io: 'Norda-Ndebele-linguo',
      es: 'Idioma ndebele del norte',
      ja: '北ンデベレ語',
      fr: 'Sindebele',
      az: 'Şimali Ndebele dili',
      kv: 'Войвыв ндебеле',
      he: 'נדבלית צפונית',
      en: 'Northern Ndebele language',
      uk: 'Північна ндебеле',
      pt: 'Língua ndebele do norte',
      eo: 'Norda Ndebela lingvo',
      nb: 'Nordndebele',
      eu: 'Iparraldeko ndebelera',
      mk: 'Северен ндебеле',
      de: 'Nord-Ndebele',
      ru: 'Северный ндебеле',
      be: 'Паўночная ндэбеле',
      fi: 'Pohjois-ndebelen kieli',
      nl: 'Noord-Ndebele',
      pl: 'Język ndebele północny',
      lt: 'Ndebelių kalba',
      it: 'Ndebele del nord',
      af: 'Noord-Ndebele',
      sv: 'Nordndebele'
    },
    language_code_3: {
      en: 'nde'
    }
  },
  'en:yoruba': {
    language_code_3: {
      en: 'yor'
    },
    name: {
      nl: 'Yoruba',
      mk: 'Јоруба',
      sw: 'Kiyoruba',
      yo: 'Èdè Yorùbá',
      fa: 'زبان یوروبایی',
      ru: 'Йоруба',
      af: 'Yoruba',
      vi: 'Tiếng Yoruba',
      lv: 'Jorubu valoda',
      ta: 'யொரூபா மொழி',
      pl: 'Język joruba',
      lt: 'Jorubų kalba',
      et: 'Joruba keel',
      kv: 'Йоруба',
      az: 'Yoruba dili',
      wa: 'Yorouba',
      zh: '约鲁巴语',
      ja: 'ヨルバ語',
      ro: 'Limba yoruba',
      uk: 'Йоруба',
      bn: 'ইয়োরুবা ভাষা',
      br: 'Yoroubeg',
      ko: '요루바어',
      ug: 'يورۇباچە',
      be: 'Ёруба',
      fi: 'Joruban kieli',
      oc: 'Ioruba',
      nn: 'Joruba',
      nb: 'Joruba',
      eu: 'Jorubera',
      de: 'Yoruba',
      sv: 'Yoruba',
      ar: 'لغة يوربا',
      it: 'Yoruba',
      qu: 'Yoruba simi',
      id: 'Bahasa Yoruba',
      el: 'Γιορούμπα γλώσσα',
      es: 'Idioma yoruba',
      gl: 'Lingua ioruba',
      fr: 'Yoruba',
      ca: 'Ioruba',
      mr: 'योरुबा भाषा',
      en: 'Yoruba',
      cv: 'Йоруба',
      uz: 'Yoruba tili',
      ka: 'იორუბა',
      pt: 'Língua iorubá',
      eo: 'Joruba lingvo',
      cy: 'Iorwba'
    },
    wikidata: {
      en: 'Q34311'
    },
    language_code_2: {
      en: 'yo'
    }
  }
}
